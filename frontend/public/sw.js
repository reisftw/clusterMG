const CACHE_NAME = "adm-pwa-v65-facilities-delete-assets-icon-fix-20260917";
const APP_SHELL = [
	"/index.html",
	"/painel",
	"/painel/relatorios",
...
function fetchSameOriginAsset(request) {
	return fetchAndCacheAsset(request).catch(() => caches.match(request));
}

function isCacheableAssetPath(pathname) {
	return /\.(?:js|css|png|jpe?g|svg|webp|ico|woff2?|json|webmanifest)$/i.test(
		pathname,
	);
}
...
function handleGetRequest(event) {
	...
	} else if (isSameOrigin && isCacheableAssetPath(url.pathname)) {
...
self.addEventListener("push", (event) => {
	...
	const notification =
		payload?.notification || payload?.data?.notification || {};
	const data = payload?.data || {};
	const title =
		notification.title || data.title || "Atualizacao no Painel de Retiradas";
	const body =
		notification.body || data.body || "Novos dados estao disponiveis.";
