	...
	return vi.fn(async (sql, params = []) => {
		...
		if (text.includes("insert into app_sessions")) {
			sessions.push({
				...
				refresh_expires_at: params[4] || "",
				revoked: false,
			});
			return { rows: [] };
		}
		if (text.includes("update app_sessions") && text.includes("revoked_at")) {
			const session = sessions.find((item) => item.jti === params[0]);
		...
		if (
			text.includes("from app_sessions") &&
			text.includes("refresh_token_hash")
		) {
			const session = sessions.find(
				(item) => item.refresh_token_hash === params[0] && !item.revoked,
			);
			return {
				rows:
		...
		if (text.includes("from app_sessions")) {
			const session = sessions.find((item) => item.jti === params[0]);
			return {
				rows: sessionActive && session && !session.revoked ? [{ jti: params[0] }] : [],
			};
		}
		return { rows: [] };
	});
...
describe("auth/RBAC isolated security tests", () => {
	...
	it("login accepts valid credentials and rejects invalid credentials", async () => {
		...
		await expect(
			...
			refreshExpiresIn: auth.REFRESH_TOKEN_TTL_SECONDS,
	...
	it("issues short access sessions and rotates refresh tokens", async () => {
		let auth = loadAuthWithDb(vi.fn(async () => ({ rows: [] })));
		const passwordData = await auth.hashPassword("SenhaForte123!");
		const user = {
			uid: "user-1",
