...
import {
	limparArmazenamentoContratos,
	obterStatusArmazenamentoContratos,
	urlDownloadZipContratos,
} from "../../imoveisAdministrativos/services/imoveisAdministrativosService";
...
function formatStorageBytes(value = 0) {
	...
	return `${size.toLocaleString("pt-BR", { maximumFractionDigits: unit >= 3 ? 2 : 1 })} ${units[unit]}`;
}
...
export default function ConfiguracoesGeraisPage() {
	...
	const [contractsStorage, setContractsStorage] = useState(null);
	...
	useEffect(() => {
		...
		obterStatusArmazenamentoContratos()
			.then((storage) => {
				if (active) setContractsStorage(storage);
			})
			.catch(() => {});
		return () => {
			active = false;
	...
	const refreshContractsStorage = async () => {
		setContractsStorage(await obterStatusArmazenamentoContratos());
	};

	const clearContractsStorage = async () => {
		if (!canManage) return;
		if (!window.confirm("Tem certeza que deseja zerar a pasta local de contratos de imóveis? Baixe o ZIP antes se precisar manter cópia.")) return;
		setSaving(true);
		setMessage("");
		try {
			const storage = await limparArmazenamentoContratos();
			setContractsStorage(storage);
			setMessage("Pasta local de contratos zerada.");
		} catch (error) {
			setMessage(error?.message || "Não foi possível zerar a pasta de contratos.");
		} finally {
			setSaving(false);
		}
	};
						...
						<div className="flex items-start gap-4">
							...
							<div>
								<h2 className="text-lg font-black text-slate-950">
									Contratos de imóveis na VPS
								</h2>
								<p className="mt-1 max-w-2xl text-sm font-semibold leading-relaxed text-slate-500">
									Pasta local usada pelos PDFs anexados na ficha do imóvel. Limite de 2GB.
								</p>
								<div className="mt-4 grid gap-3 sm:grid-cols-3">
									<div className="rounded-2xl bg-slate-50 p-3">
										<p className="text-xs font-black uppercase text-slate-500">Usado</p>
										<p className="mt-1 text-lg font-black text-slate-950">{formatStorageBytes(contractsStorage?.usedBytes)}</p>
									</div>
									<div className="rounded-2xl bg-slate-50 p-3">
										<p className="text-xs font-black uppercase text-slate-500">Livre</p>
										<p className="mt-1 text-lg font-black text-slate-950">{formatStorageBytes(contractsStorage?.freeBytes)}</p>
									</div>
									<div className="rounded-2xl bg-slate-50 p-3">
										<p className="text-xs font-black uppercase text-slate-500">Arquivos</p>
										<p className="mt-1 text-lg font-black text-slate-950">{contractsStorage?.filesCount ?? 0}</p>
									</div>
								</div>
								<div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-100">
									<div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(Number(contractsStorage?.percentUsed || 0), 100)}%` }} />
								</div>
								<p className="mt-2 text-xs font-bold text-slate-500">{contractsStorage?.percentUsed ?? 0}% utilizado de {formatStorageBytes(contractsStorage?.limitBytes || 2 * 1024 * 1024 * 1024)}</p>
							</div>
						</div>
						<div className="flex flex-col gap-2 sm:flex-row lg:flex-col xl:flex-row">
							<a href={`${getApiBaseUrl()}${urlDownloadZipContratos()}`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-blue-700">
								<Download size={17} /> Baixar ZIP
							</a>
							<button type="button" disabled={saving || !canManage} onClick={refreshContractsStorage} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50 disabled:opacity-60">
								Atualizar
							</button>
							<button type="button" disabled={saving || !canManage} onClick={clearContractsStorage} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-100 bg-red-50 px-5 py-3 text-sm font-black text-red-700 transition hover:bg-red-100 disabled:opacity-60">
								<Trash2 size={17} /> Zerar
							</button>
						</div>
					</div>
				</section>
