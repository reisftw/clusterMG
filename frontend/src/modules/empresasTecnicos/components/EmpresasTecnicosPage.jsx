...
function countTecnicosByArea(tecnicos = [], area) {
	return (Array.isArray(tecnicos) ? tecnicos : []).filter(
		(tecnico) => {
			const scopes = Array.isArray(tecnico.operationScopes)
				? tecnico.operationScopes.map((item) => String(item).toUpperCase())
				: [];
			if (area === "delivery" && scopes.includes("DELIVERY")) return true;
			if (area === "field_service" && scopes.includes("FIELD")) return true;
			return normalizeOperationalArea(tecnico.areaOperacional) === area;
		},
	).length;
}
...
function EmpresaForm({ initialValue, regionais, onCancel, onSave, saving }) {
			...
			<div className="flex flex-wrap items-start justify-between gap-4">
				<div>
					...
					<p className="text-sm text-slate-500">
						Dados usados para alertas, perfil, técnicos e histórico de acerto de
						estoque.
					</p>
				</div>
				<button
				...
				<div className="grid gap-4 xl:col-span-9 xl:grid-cols-2">
					...
					<div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3">
						...
						<p className="mt-1 text-sm font-semibold leading-relaxed text-blue-900">
							Escolha Delivery ou Field Service em cada técnico. O supervisor
							vem das Regionais preenchidas.
						</p>
					</div>
					<label>
						<span className="mb-1.5 block text-xs font-black uppercase tracking-wide text-slate-500">
...
function CriarLiderEmpresaModal({ empresa, onClose, onCreated }) {
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			setMessage(result?.welcomeEmailSent
				? "Usuário criado. Enviamos o link seguro de primeiro acesso por e-mail."
				: "Usuário criado, mas o e-mail de primeiro acesso não foi enviado. Verifique o SMTP.");
			onCreated?.();
		} catch (error) {
			...
			<form onSubmit={handleSubmit} className="p-6">
				...
				<div className="mt-5 space-y-4">
					...
					<div className="rounded-2xl border border-blue-100 bg-blue-50 p-3 text-sm font-semibold text-blue-800">
						O primeiro acesso será enviado por link seguro ao e-mail informado. O sistema não exibe mais senhas temporárias.
					</div>
					{message ? (
		...
		<div className="space-y-5">
			...
			<section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
				...
				<EmpresasMetricCard
					icon={Users}
					label="Técnicos"
					value={visibleEmpresas.reduce(
						(sum, item) => sum + item.tecnicos.length,
						0,
					)}
								...
								<div className="mt-4 grid grid-cols-3 gap-2">
									...
									<div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
										...
										<p className="text-xl font-black text-slate-950">
											{countTecnicosByArea(empresa.tecnicos, "delivery")}
										</p>
									</div>
									<div
									...
									>
										...
										<p
											className={`text-xl font-black ${empresaStatusClass(empresa.status).value}`}
										>
											{countTecnicosByArea(empresa.tecnicos, "field_service")}
										</p>
									</div>
								</div>
