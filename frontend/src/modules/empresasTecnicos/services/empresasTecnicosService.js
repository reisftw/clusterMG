...
import { listarUsuariosAdmin } from "../../auth/services/authService";
...
export function normalizeEmpresaTecnicos(id, data = {}) {
	...
	return {
		...
		supervisor: {
			...
			telefone: text(supervisor.telefone || supervisor.phone || data.supervisorTelefone),
		},
...
export function buildEmpresaTecnicosPayload(form = {}) {
	...
	return {
		...
		responsavel: {
			...
			telefone: text(form.responsavel?.telefone),
		},
		gruposOperacionais,
		regional,
		...
		supervisor: {
			...
			telefone: text(
				legacySupervisor.telefone || deliverySupervisor.telefone,
			),
		},
...
export function normalizeUsuarioEmpresa(id, data = {}) {
	return {
		id,
		uid: text(data.uid || data.id || id),
		nome: text(data.nome || data.display_name),
		email: text(data.email),
		telefone: text(data.telefone || data.phone || data.celular),
...
export async function buscarUsuariosAdmCadastrados() {
	const response = await listarUsuariosAdmin();
	return (response?.items || [])
		.map((item) => normalizeUsuarioEmpresa(item.uid || item.id, item))
		.filter((item) => item.nome || item.email)
		.sort((a, b) => a.nome.localeCompare(b.nome, "pt-BR"));
}
