...
// carregar/trocar de mes): antes desses indices, `buildAccountRows` fazia
// `matrix.filter(item => item.costCenterId === row.costCenterId)` DENTRO do
// `.map()` que ja itera `matrix` — O(matrix.length^2) por chamada — e essa
// funcao e chamada de novo pra CADA um dos 12 meses da evolucao mensal
// (buildMonthlyEvolutionRows), multiplicando o custo por 13x. Com poucos
// milhares de linhas de matriz orcamentaria isso ja passa de centenas de
// milhoes de comparacoes por recalculo. Os indices abaixo (Map por id/por
// centro de custo) sao construidos uma unica vez em getBudgetInsights e
// reaproveitados nas 13 chamadas, trocando O(n^2) por O(n).
...
function groupMatrixByCenterId(matrix = []) {
	const map = new Map();
	matrix.forEach((row) => {
...
function sumMatrixRows(rows = [], centerById = new Map(), rowTotal = () => 0, companyId = "") {
	return sumBy(rows, (row) => {
		const center = centerById.get(row.costCenterId);
		if (
			companyId &&
...
// Velocidade de consumo do orcamento (roteiro Finan #19): compara o %
// ja consumido com o % ideal para o dia do mes atual (insights.idealPercent,
// ja calculado em buildMonthInsights a partir de dayOfMonth/daysInMonth).
// Diferente de budgetStatusColor (que so olha o valor absoluto consumido),
// aqui o que importa e o quanto o consumo esta a frente do ritmo esperado.
function budgetPaceStatusColor(usedPercent = 0, idealPercent = 0) {
	const gap = Number(usedPercent || 0) - Number(idealPercent || 0);
	if (gap > 15) return "rose";
	if (gap > 5) return "amber";
	return "emerald";
}

function budgetPaceHelper(usedPercent = 0, idealPercent = 0) {
	const gap = Number(usedPercent || 0) - Number(idealPercent || 0);
	const usedLabel = `${decimal.format(Number(usedPercent || 0))}% utilizado`;
	if (gap > 5) {
		return `${usedLabel} · consumo acima do ritmo esperado (+${decimal.format(gap)} p.p.)`;
	}
	return `${usedLabel} · dentro do ritmo esperado para o período`;
}
...
function buildAccountRows({
	...
	centers = [],
	matrix = [],
	accountById = indexById(accounts),
	centerById = indexById(centers),
	matrixByCenterId = groupMatrixByCenterId(matrix),
	periodKeys = new Set(),
	...
	const matrixRows = matrix
		.map((row) => {
			...
			const centerTotalPlanned = sumBy(
				matrixByCenterId.get(row.costCenterId) || [],
				rowPeriodTotal,
			);
	...
	const breakdownRows = centers.flatMap((center) => {
		...
		return centerBreakdowns(center)
			...
			.map((breakdown) => {
				const account = accountById.get(breakdown.accountId);
...
function buildCenterRows({
	...
	return centers.map((center) => {
		...
		const planned =
			plannedFromCenter ||
			sumBy(matrixByCenterId.get(center.id) || [], rowPeriodTotal);
		return {
			center,
...
export function getBudgetInsights(config = {}, selectedPeriod = {}) {
	...
	// Indices construidos uma unica vez e reaproveitados em todas as
	// chamadas de sumMatrixRows/buildAccountRows/buildCenterRows abaixo
	// (inclusive as 12 chamadas por mes em buildMonthlyEvolutionRows) —
	// ver comentario em indexById/groupMatrixByCenterId.
	const accountById = indexById(accounts);
	const centerById = indexById(centers);
	const matrixByCenterId = groupMatrixByCenterId(matrix);
	const now = new Date();
	...
	const computedApprovals = centers
		...
		.map((center) => {
			const centerPlanned =
				configuredBudgetForCenter(center) ||
				(matrixByCenterId.get(center.id) || []).reduce(
					(sum, row) => sum + rowPeriodTotal(row),
					0,
				);
	...
	(config.approvals || []).forEach((approval) => {
		const center = centerById.get(approval.centerId);
		const used =
...
export function buildBudgetOperationalKpis(insights = {}, config = {}) {
	...
	return [
		...
		},
		{
			...
			value: usedPercent,
			type: "percent",
			helper: budgetPaceHelper(usedPercent, insights.idealPercent),
			icon: "CalendarClock",
			statusColor: budgetPaceStatusColor(usedPercent, insights.idealPercent),
			progress: {
				value: usedPercent,
				label: `${decimal.format(insights.idealPercent || 0)}% do período decorrido`,
			},
		},
		{
