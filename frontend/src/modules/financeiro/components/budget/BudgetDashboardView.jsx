...
export default function BudgetDashboardView({
	...
	// cada KPI abre o painel de detalhe mais correspondente ao que ele
	// mede — "movimentacoes" e o mais literal ("navegação até o
	// lançamento de origem", texto do roteiro), os demais abrem o detalhe
	// mais proximo do que o numero representa.
	const KPI_DETAIL_MAP = {
		orcado: "movimentacoes",
		realizado: "movimentacoes",
		saldo: "movimentacoes",
		"desvio-percentual": "cascata",
		"ritmo-orcamento": "ritmo",
		"centros-positivos": "centros",
		"centros-negativos": "centros",
		"diretoria-positiva": "diretorias",
		"diretoria-negativa": "diretorias",
	};
	const withDrillDown = (item) => {
		const detailKey = KPI_DETAIL_MAP[item.id];
		if (!detailKey || !onShowDashboardDetail) return item;
		return { ...item, onClick: () => onShowDashboardDetail(detailKey) };
	};
	const primaryKpis = kpis.slice(0, 3).map(withDrillDown);
	const secondaryKpis = kpis.slice(3).map(withDrillDown);
	return (
				...
				<div className="overflow-auto">
					<table className="min-w-[980px] divide-y divide-slate-200 text-left text-xs font-bold">
						<thead className="bg-slate-50 text-slate-500">
							<tr>
								<th scope="col" className="px-3 py-2">Fornecedor</th>
								<th scope="col" className="px-3 py-2">Conta financeira</th>
								<th scope="col" className="px-3 py-2">Centro de custo</th>
								<th scope="col" className="px-3 py-2">Matriz / filial</th>
