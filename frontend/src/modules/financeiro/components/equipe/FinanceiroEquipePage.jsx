		...
		<ModalShell title={colaborador?.id ? "Editar usuário" : "Novo usuário"} onClose={onClose} size="xl">
			...
			>
				...
				<div className="flex flex-wrap justify-between gap-2">
					{colaborador?.id ? (
						<button type="button" onClick={() => onDelete(colaborador)} className="inline-flex items-center gap-2 rounded-xl border border-red-200 px-4 py-2 text-sm font-bold text-red-600 hover:bg-red-50">
...
export default function FinanceiroEquipePage({ canManage = false }) {
	...
	const [confirmDelete, setConfirmDelete] = useState(null); // { kind: "setor"|"cargo"|"colaborador", id, label }
	...
	function deleteSetor(setor) {
		setConfirmDelete({ kind: "setor", id: setor.id, label: setor.nome || setor.name });
	}

	function deleteCargo(cargo) {
		setConfirmDelete({ kind: "cargo", id: cargo.id, label: cargo.nome || cargo.name });
	}

	function deleteColaborador(colaborador) {
		setConfirmDelete({ kind: "colaborador", id: colaborador.id, label: colaborador.nome || colaborador.name });
	}
	...
	async function confirmDeleteEntity() {
		...
		setConfirmError("");
		try {
			if (confirmDelete.kind === "setor") {
				await removerSetorEquipeFinanceiro(confirmDelete.id);
			} else if (confirmDelete.kind === "cargo") {
				await removerCargoEquipeFinanceiro(confirmDelete.id);
			} else {
				await removerColaboradorEquipeFinanceiro(confirmDelete.id);
				setColaboradorModal(null);
			}
			closeConfirmDelete();
			await loadEquipe();
			...
			const fallback =
				confirmDelete.kind === "setor"
					? "Não foi possível excluir o setor."
					: confirmDelete.kind === "cargo"
						? "Não foi possível excluir o cargo."
						: "Não foi possível remover o usuário do organograma.";
			setConfirmError(error?.message || fallback);
		} finally {
			setConfirming(false);
		}
	}
		...
		<section className="space-y-4">
			<div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
				...
				<div className="flex flex-wrap gap-2">
					...
					{canManage ? (
						<>
							<label className="flex min-w-64 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black uppercase text-slate-500 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-100">
								Responsável geral
								<select
								...
								<div key={setor.id} className="rounded-xl border border-slate-200 p-3">
									...
									{canManage ? (
										<div className="mt-3 flex gap-2">
											...
											<button type="button" onClick={() => deleteSetor(setor)} className="rounded-lg border border-red-200 p-2 text-red-500 hover:bg-red-50" title="Excluir setor" aria-label={`Excluir setor ${setor.nome || setor.name}`}><Trash2 size={14} /></button>
										</div>
									) : null}
								</div>
							))}
								...
								<div key={cargo.id} className="rounded-xl border border-slate-200 p-3">
									...
									{canManage ? (
										<div className="mt-3 flex gap-2">
											...
											<button type="button" onClick={() => deleteCargo(cargo)} className="rounded-lg border border-red-200 p-2 text-red-500 hover:bg-red-50" title="Excluir cargo" aria-label={`Excluir cargo ${cargo.nome || cargo.name}`}><Trash2 size={14} /></button>
										</div>
									) : null}
								</div>
							))}
								...
								<div key={colaborador.id} className="rounded-xl border border-slate-200 p-3">
									...
									{canManage ? (
										<div className="mt-3 flex gap-2">
											...
											<button type="button" onClick={() => deleteColaborador(colaborador)} className="rounded-lg border border-red-200 p-2 text-red-500 hover:bg-red-50" title="Excluir usuário" aria-label={`Remover ${colaborador.nome || colaborador.name} do organograma`}><Trash2 size={14} /></button>
										</div>
									) : null}
								</div>
							))}
							...
							<div className="rounded-2xl border border-slate-300 bg-slate-50 p-5">
								...
								<div className="relative">
									...
									{equipeTree.setores.map((setor) => (
										// Drag-and-drop é um atalho de mouse; a API nativa de DnD não tem
										// equivalente de teclado real (limitação conhecida, não um bug
										// corrigível com role/tabIndex/onKeyDown fake aqui). A mesma ação
										// (mover colaborador de setor) já é 100% operável por teclado via
										// "Editar usuário" (Pencil, linha ~876) -> campo "Setor" -> Salvar,
										// que passa pelo mesmo saveColaborador/moveColaboradorToSetor.
			...
			<ConfirmDialog
				...
				title={
					confirmDelete?.kind === "setor"
						...
						: confirmDelete?.kind === "cargo"
							? "Excluir este cargo?"
							: "Remover este usuário do organograma?"
				}
				description="Essa ação não pode ser desfeita."
				items={confirmDelete ? [{ label: "Item", value: confirmDelete.label || "-" }] : []}
