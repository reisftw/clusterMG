...
function useAcertoEstoqueController() {
	...
	const handleEditAcerto = (acerto) => {
		if (!acerto) return;
		const agenda = agendasOrdenadas.find(
			(item) =>
				item.cidade === acerto.cidade &&
				(!acerto.turno || item.turno === acerto.turno),
		);
		const lancamentos = (
			acerto.tecnicoLancamentos?.length
									...
									<table className="w-full text-sm">
										...
										<tbody>
											{metrics.latestAcertos.map((item) => (
												<tr
													key={item.id}
													onClick={() => openAcerto(item.id)}
													className="cursor-pointer border-b border-gray-50 last:border-0 hover:bg-orange-50"
												>
...
function AcertoLancamentoTab(props) {
	...
	return (
				<div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
					<SectionCard
						title={isEditing ? "Editar acerto" : "Novo acerto"}
						...
						<form className="space-y-5" onSubmit={handleLancamentoSubmit}>
							...
							<div className="flex flex-wrap items-center gap-3">
								...
								>
									...
									{isEditing ? "Salvar alteracoes" : "Registrar acerto"}
								</button>
								{isEditing ? (
									<button
										type="button"
										onClick={() => setLancamentoForm(EMPTY_LANCAMENTO)}
						...
						action={
							selectedAcerto ? (
								<div className="flex flex-wrap items-center gap-2">
									...
									<button
										type="button"
										onClick={() => handleEditAcerto(selectedAcerto)}
										className="inline-flex items-center gap-2 rounded-xl border border-blue-200 px-3 py-2 text-xs font-semibold text-blue-600 hover:bg-blue-50"
									>
										<Pencil size={14} />
										Editar
									</button>
