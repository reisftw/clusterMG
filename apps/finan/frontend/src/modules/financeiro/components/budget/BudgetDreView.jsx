...
export default function BudgetDreView({ canManage, setFeedback }) {
	...
	// UX_AUDIT.md, Fase 1: a tabela `dre_lancamentos` nunca foi criada no
	// banco do Finan (CLAUDE.md, seção 11 — "DRE não implementado"), então
	// GET /gestao-orcamento/dre sempre devolve 500 em produção hoje. Sem
	// essa distinção, a tela renderizava normalmente com filtros/upload
	// ativos e uma tabela "vazia" — o usuário lia isso como "não há
	// lançamentos neste mês", quando na verdade a funcionalidade inteira
	// ainda não existe no backend. Agora um 500 na primeira carga vira um
	// aviso explícito de "em construção", em vez de uma tela falsamente
	// funcional.
	...
	const loadDre = useCallback(async () => {
		...
		try {
			...
			// Só trata como "funcionalidade em construção" quando o servidor
			// respondeu 500 de verdade (o caso conhecido da tabela ausente) —
			// uma falha de rede (sem `.status`, fetch nem chegou a responder)
			// continua caindo no banner de erro genérico, não neste aviso.
			if (error?.status === 500) setUnavailable(true);
			setFeedback?.({
				type: "error",
	...
	if (unavailable && !loading) {
		return (
			<section className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-amber-300 bg-amber-50 p-10 text-center">
				<span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-700">
					<Construction size={28} />
				</span>
				<div>
					<h2 className="text-lg font-black text-slate-950">DRE ainda em construção</h2>
					<p className="mx-auto mt-1 max-w-md text-sm font-bold text-slate-600">
						Essa demonstração de resultado ainda não está disponível para consulta — a base de lançamentos DRE
						deste sistema está sendo preparada. Nenhum dado seu foi perdido; assim que a funcionalidade for
						liberada, ela aparece aqui automaticamente.
					</p>
				</div>
				<button
					type="button"
					onClick={loadDre}
					className="inline-flex h-10 items-center gap-2 rounded-xl border border-amber-300 bg-white px-4 text-sm font-black text-amber-800 hover:bg-amber-100"
				>
				...
				<div className="overflow-x-auto">
					<table className="min-w-full table-fixed text-left text-sm">
						<thead className="bg-slate-50 text-xs font-black uppercase text-slate-500">
							<tr>
								<th scope="col" className="w-20 px-4 py-3">Ordem</th>
								<th scope="col" className="px-4 py-3">Linha DRE</th>
								<th scope="col" className="w-32 px-4 py-3">Tipo</th>
								<th scope="col" className="w-48 px-4 py-3 text-right">Valor</th>
							</tr>
						</thead>
