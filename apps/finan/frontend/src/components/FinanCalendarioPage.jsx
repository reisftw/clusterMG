...
function DayModal({
	...
	return (
		<div className="fixed inset-0 z-layout-modal flex items-center justify-center bg-slate-950/50 px-4 backdrop-blur-sm">
			...
			<ConfirmDialog
				open={Boolean(confirmRuleId)}
				tone="danger"
				title="Excluir a rotina recorrente inteira?"
				description="Isso remove a rotina de TODOS os meses, não só esta ocorrência. Essa ação não pode ser desfeita."
				confirmLabel="Excluir rotina inteira"
				cancelLabel="Voltar"
				onConfirm={() => {
					onDeleteRule(confirmRuleId);
	...
	return (
		<div className="flex flex-col gap-2">
			...
			<ConfirmDialog
				...
				tone="danger"
				title="Excluir esta regra recorrente?"
				description="Isso remove a regra de TODOS os meses futuros. Essa ação não pode ser desfeita."
				items={confirmRule ? [{ label: "Regra", value: confirmRule.title }] : []}
				confirmLabel="Excluir regra"
				cancelLabel="Voltar"
				onConfirm={() => {
					onDelete(confirmRule.id);
					setConfirmRule(null);
				}}
