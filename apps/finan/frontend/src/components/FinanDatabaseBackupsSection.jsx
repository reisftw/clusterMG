...
// src/modules/databaseBackups/components/DatabaseBackupsPage.jsx do
// Retiradas, ligada as rotas de backup/restore do Postgres do Finan
// (/admin/database/backups* em compat/routes.js).
import {
	AlertTriangle,
	Archive,
	CheckCircle2,
...
import {
	createFinanDatabaseBackup,
	fetchFinanDatabaseBackups,
	restoreFinanDatabaseBackup,
} from "../api/finanApi";
...
export default function FinanDatabaseBackupsSection() {
	...
	const loadStatus = useCallback(async () => {
		setError("");
		const data = await fetchFinanDatabaseBackups();
		setStatus(data);
		return data;
	}, []);
	...
	if (loading) {
		return <p className="text-sm font-semibold text-slate-500">Carregando backups...</p>;
	}
		...
		<div className="space-y-5">
			<section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
				<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
					<div>
						...
						<h2 className="mt-2 text-2xl font-bold text-slate-900">Backups do PostgreSQL</h2>
						<p className="mt-1 max-w-3xl text-sm text-slate-500">
							Backup automático diário, retenção configurável e limite total de espaço. O rollback cria um backup de
							segurança antes de restaurar o arquivo escolhido.
						</p>
					</div>
					...
					<div className="flex flex-wrap gap-2">
						...
						<button
							type="button"
							onClick={() => runAction(createFinanDatabaseBackup, "Backup criado com sucesso.")}
							disabled={working}
			...
			<section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-1 text-xs text-slate-500">Tamanho real informado pelo PostgreSQL.</p>
				</div>
				...
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-3 text-2xl font-bold text-slate-900">{formatBytes(status?.storage?.usedBytes)}</p>
				...
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-1 text-xs text-slate-500">Ao passar do limite, os mais antigos são apagados.</p>
				</div>
				...
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-1 text-xs text-slate-500">
						Pasta: {status?.storage?.backupDir || "/opt/retiradas/backups/finan"}
						{" · "}
			...
			<section className="rounded-xl border border-slate-200 bg-white shadow-sm">
				<div className="flex flex-col gap-2 border-b border-slate-100 p-4 md:flex-row md:items-center md:justify-between">
					<div>
						<h3 className="text-base font-bold text-slate-900">Backups Disponíveis</h3>
						<p className="text-xs text-slate-500">Escolha um arquivo apenas quando precisar voltar o banco para aquele momento.</p>
					</div>
				</div>
						...
						<tbody className="divide-y divide-slate-100">
							{(status?.backups || []).map((backup) => (
								<tr key={backup.fileName} className="hover:bg-slate-50">
									<td className="px-4 py-3 font-semibold text-slate-800">{formatDateTime(backup.createdAt)}</td>
									<td className="px-4 py-3 text-slate-600">{reasonLabel[backup.reason] || backup.reason || "Manual"}</td>
									<td className="px-4 py-3 text-slate-600">{formatBytes(backup.sizeBytes)}</td>
									<td className="px-4 py-3 font-mono text-xs text-slate-500">{backup.fileName}</td>
					...
					<div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
						<div className="max-w-3xl">
							...
							<p className="mt-2 text-sm text-amber-800">
								O banco será restaurado para o backup <strong>{selectedBackup.fileName}</strong>. Digite{" "}
						...
						<div className="flex flex-wrap gap-2">
							...
							<button
								...
								onClick={() =>
									runAction(
										() => restoreFinanDatabaseBackup(selectedBackup.fileName, confirmation),
