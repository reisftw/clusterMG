...
const pool = new Pool({
	...
	//
	// max=10 (o default da lib) era o teto real de escala do Finan — achado
	// via teste de carga k6 em producao (2026-09-10): com ~70 usuarios
	// simultaneos, TODAS as rotas (ate as triviais, tipo contador do menu)
	// comecavam a falhar quase igual, porque toda rota espera uma conexao
	// do pool e so 10 existiam. Confirmado no Postgres real: max_connections
	// = 100, com so ~20 em uso no total (finan + retiradas + homolog) — tinha
	// bastante folga sem usar. Subido pra 30 (3x), deixando ~50 conexoes de
	// margem pro app principal (que roda no mesmo servidor Postgres, banco
	// separado) sem chegar perto do teto de 100.
	max: Number(process.env.FINAN_PG_POOL_MAX || 30),
...
module.exports = {
	...
	closePool: () => pool.end(),
	// Usado pelo painel de Observabilidade (Roteiro #27) pra mostrar o
	// tamanho da fila de conexoes do pool, sem expor host/senha.
	getPoolStats: () => ({
