...
// Um "nome" de matriz/filial é placeholder (não um nome de verdade) quando
// está vazio, é puramente numérico, ou é idêntico ao próprio código — sinal
// de que a planilha só trouxe o código e o nome acabou sendo preenchido com
// o próprio número (ver correção de companyName/branchName acima).
function isPlaceholderBudgetName(nome, codigo) {
	const text = cleanText(nome);
	if (!text) return true;
	if (/^\d+$/.test(text)) return true;
	if (codigo && text === cleanText(codigo)) return true;
	return false;
}
...
// Replica, linha a linha, a mesma chave de deduplicacao usada na criacao
// real do cadastro de fornecedores (mais abaixo: partnerId =
// row.codFornecedor ? slug(codFornecedor) : slug(nomeFornecedor)) — ao
// contrario de uniqueCount(rows, "fornecedor"), que conta o texto bruto
// "codigo-nome" da planilha e inflava o card de resumo do importador (o
// mesmo fornecedor aparece com o nome formatado de jeitos diferentes em
// linhas diferentes, ex.: "260131-PGTO CEMIG DISTRIBUIÇÃO" vs "260131-
// CEMIG DISTRIBUICAO SA", mesmo codigo 260131).
function uniqueSupplierCount(rows) {
	const keys = new Set();
	rows.forEach((row) => {
		const codigo = cleanText(row.codFornecedor);
		if (codigo) {
			keys.add(`cod:${codigo}`);
			return;
		}
		const nome = cleanText(row.fornecedor);
		if (nome) keys.add(`nome:${slug(nome, "")}`);
	});
	return keys.size;
}
...
function summarizeBudgetDataRows(rows = []) {
	...
	return {
		...
		uniqueSuppliers: uniqueSupplierCount(rows),
		uniqueCompanies:
...
function mergeBudgetConfigFromRows(existingConfig = {}, rows = [], user = {}) {
	...
	rows.forEach((row, index) => {
		...
		const branchId = branchCode
			...
			: "";
		// Correção (2026-09-10): quando a coluna "Filial"/"Empresa" da
		// planilha traz só um código numérico sem "código - nome" (o caso
		// comum de exportações operacionais), splitCodeName não tem
		// separador pra dividir e devolve o número inteiro como `nome`
		// (ex.: "1" vira {codigo: "", nome: "1"}). Isso fazia os chips de
		// "Matrizes e filiais vinculadas" mostrarem "1 - 1"/"14 - 14" em vez
		// de um nome legível — o fallback abaixo (que já existia) nunca era
		// alcançado porque `companyParts.nome`/`branchParts.nome` truthy
		// sempre vencia primeiro. Agora só aceitamos `nome` vindo do split
		// quando ele não é puramente numérico (ou seja, é mesmo um nome, não
		// só o código repetido).
		const companyName = cleanText(
			(companyParts.nome && !/^\d+$/.test(companyParts.nome)
				? companyParts.nome
				: "") ||
		...
		const branchName = cleanText(
			(branchParts.nome && !/^\d+$/.test(branchParts.nome)
				? branchParts.nome
				: "") ||
		...
		if (companyId) {
			...
			if (!currentCompany) {
				...
				(branchId && !(currentCompany.filiais || []).includes(branchId)) ||
				// Cura nomes numéricos herdados de importações antigas (antes
				// da correção acima): uma reimportação com nome legível agora
				// disponível substitui o placeholder "1"/"14" sem precisar de
				// edição manual.
				(isPlaceholderBudgetName(currentCompany.nome, currentCompany.codigo) &&
					companyName &&
					!isPlaceholderBudgetName(companyName, companyCode))
			) {
				const nextBranches = [
					...new Set(
				...
				companiesMap.set(companyId, {
					...currentCompany,
					nome: isPlaceholderBudgetName(currentCompany.nome, currentCompany.codigo)
						? companyName || currentCompany.nome
						: currentCompany.nome,
		...
		if (branchId) {
			...
			if (!currentBranch) {
				...
				(isPlaceholderBudgetName(currentBranch.nome, currentBranch.codigo) &&
					branchName &&
					!isPlaceholderBudgetName(branchName, branchCode))
			) {
				branchesMap.set(branchId, {
					...currentBranch,
					nome: isPlaceholderBudgetName(currentBranch.nome, currentBranch.codigo)
						? branchName || currentBranch.nome
						: currentBranch.nome,
					empresas: [
						...new Set([
...
async function clearBudgetData(user = {}) {
	...
	// Ordem importa: finan_orcamento_lancamentos tem FK RESTRICT pra
	// finan_centros_custo/finan_contas (migration 010) — limpar os
	// lancamentos ANTES de resetar a estrutura evita
	// "update or delete on table finan_centros_custo violates foreign
	// key constraint fk_finan_orcamento_lancamentos_centro" (erro real
	// visto em producao ao clicar em "Zerar dados teste" com lancamentos
	// ja importados). saveBudgetCostCenters faz DELETE+reinsert completo
	// de centros/contas (ver financeiroBudgetConfigRepository.js) —
	// se ainda houver lancamento referenciando o centro antigo, a
	// exclusao falha.
	await financeiroStatement.saveBudgetDataStatement(data, user);
