...
async function ensureSetorExists(setorNome) {
	...
	const existing = await db.query(
		"select id, nome from finan_equipe_setores where lower(nome) = lower($1)",
		[nome],
	);
	...
	const [configResult, setoresResult, cargosResult, colaboradoresResult] = await Promise.all([
		db.query(
			`select cfg.*, c.nome as responsavel_geral_nome
			   from finan_equipe_config cfg
		  left join finan_equipe_colaboradores c on c.id = cfg.responsavel_geral_id
			  where cfg.id = 'global'`,
		),
		db.query(
			`select s.*, c.nome as responsavel_nome
			   from finan_equipe_setores s
		  left join finan_equipe_colaboradores c on c.id = s.responsavel_id
			  order by s.ordem asc, s.nome asc`,
		),
		db.query(
			`select cg.*
			   from finan_equipe_cargos cg
		...
		db.query(
			`select c.*,
			   ...
			   from finan_equipe_colaboradores c
		  left join finan_equipe_cargos cg on cg.id = c.cargo_id
			  where c.ativo = true
...
async function updateConfig(payload = {}, user = {}) {
	...
	try {
		const result = await db.query(
			`insert into finan_equipe_config
...
async function createSetor(payload = {}, user = {}) {
	...
	try {
		const result = await db.query(
			`insert into finan_equipe_setores
...
async function updateSetor(id, payload = {}, user = {}) {
	...
	const current = await db.query(
		"select * from finan_equipe_setores where id = $1",
		[setorId],
	);
	...
	try {
		...
		await db.query(
			`update finan_equipe_colaboradores
			    set setor = $1,
			        updated_by = $2
...
async function deleteSetor(id) {
	...
	const inUse = await db.query(
		`select count(*)::int as total
		   from finan_equipe_colaboradores
		  where ativo = true
		    and lower(setor) = lower($1)`,
		[
			(
				await db.query("select nome from finan_equipe_setores where id = $1", [
					setorId,
				])
			).rows[0]?.nome || "",
		],
	...
	const result = await db.query(
		"delete from finan_equipe_setores where id = $1 returning id",
		[setorId],
	);
...
async function createCargo(payload = {}, user = {}) {
	...
	try {
		const result = await db.query(
			`insert into finan_equipe_cargos
...
async function deleteCargo(id) {
	...
	try {
		result = await db.query(
			"delete from finan_equipe_cargos where id = $1 returning id",
			[cargoId],
		);
	} catch (error) {
		if (error?.code === "23503") {
...
async function createColaborador(payload = {}, user = {}) {
	...
	try {
		const result = await db.query(
			`insert into finan_equipe_colaboradores
...
async function updateColaborador(id, payload = {}, user = {}) {
	...
	const current = await db.query(
		"select * from finan_equipe_colaboradores where id = $1 and ativo = true",
		[colaboradorId],
	);
	if (!current.rows.length) {
