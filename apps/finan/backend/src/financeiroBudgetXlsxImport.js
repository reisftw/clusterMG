...
// Regra de sinal do Access (query Cria_APagar):
// IIf(Valor<0 And Tipo="DEV", Valor, IIf(Valor<0, -Valor, Valor))
// — valor negativo vira positivo, EXCETO quando o tipo e "DEV"
// (devolucao), que fica negativo mesmo. Confirmado com a planilha real:
// 105 linhas negativas nao-DEV somando R$ 1.359.951,78 — sem essa
// correcao, o total de Contas a Pagar diverge do Access no dobro desse
// valor (R$ 2.719.903,56), porque a diferenca entre "-X" e "+X" e "2X".
function applyAccessSignRule(rawValue, tipo) {
	if (rawValue < 0 && cleanText(tipo).toUpperCase() !== "DEV") return -rawValue;
	return rawValue;
}
	...
	return withSource(
		{
			...
			codCc,
			nomeCc,
			realizado: applyAccessSignRule(parseCurrency(row[15]), tipo),
...
function parseBudgetWorkbookBuffer(buffer, fileName = "orcamento.xlsx") {
	...
	for (const sheetName of workbook.SheetNames) {
		...
		files.push({
			...
			rows: parsed.length,
			// Assinatura crua da 1a linha da planilha (cabecalho), normalizada —
			// usada em financeiroBudgetImportJobs.js pra reconhecer se essa
			// "forma" de arquivo ja apareceu antes (roteiro Finan #17).
			headerSignature: (sheetRows[0] || []).map(normalizeKey).join("|"),
		});
		rows.push(...parsed);
	}
	return {
