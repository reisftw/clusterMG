...
async function saveMeta(client, reportId, data = {}) {
	await client.query(
		`insert into finan_reports_meta (
...
function normalizeSerasaMovement(item = {}) {
	const legacyPath = "finan_reports/serasa";
	const payload = { ...item };
	return {
...
function normalizeSerasaClientBase(item = {}) {
	return {
		...
		legacy_path: "finan_reports/serasa",
		legacy_document_id: SERASA_REPORT_ID,
		source_payload: { ...item },
	};
}
...
async function saveSerasaReport(data = {}) {
	...
	try {
		...
		await client.query("delete from finan_serasa_movimentacoes");
		await client.query("delete from finan_serasa_clientes_base");
		await upsertMany(
			client,
...
async function getSerasaReport() {
	const [movements, clients, meta] = await Promise.all([
		db.query(
			`select * from finan_serasa_movimentacoes
			  order by data desc nulls last, id`,
		),
		db.query(
			`select * from finan_serasa_clientes_base
			  order by ano, mes`,
		),
...
function normalizeTariffPeriodItem(kind, item = {}, mapper = () => ({})) {
	const legacyPath = "finan_reports/tarifas";
	const payload = { ...item };
	return {
...
async function saveTariffsReport(data = {}) {
	...
	try {
		await client.query("begin");
		await client.query("delete from finan_tarifas_faturas");
		await client.query("delete from finan_tarifas_formas_pagamento");
		await client.query("delete from finan_tarifas_receita_cliente");
		await client.query("delete from finan_tarifas_mensais");
		await client.query("delete from finan_tarifas_cobranca_clientes");
		await client.query("delete from finan_tarifas_boletos");
		await upsertMany(client, "finan_tarifas_faturas", normalized.faturas, ["source_hash"]);
		await upsertMany(client, "finan_tarifas_formas_pagamento", normalized.pagamento, ["source_hash"]);
		await upsertMany(client, "finan_tarifas_receita_cliente", normalized.receitaCliente, ["source_hash"]);
		await upsertMany(client, "finan_tarifas_mensais", normalized.mensais, ["source_hash"]);
		await upsertMany(client, "finan_tarifas_cobranca_clientes", normalized.cobrancaClientes, ["source_hash"]);
		await upsertMany(client, "finan_tarifas_boletos", normalized.boletos, ["source_hash"]);
...
async function getTariffsReport() {
	const [faturas, pagamento, receitaCliente, mensais, cobrancaClientes, boletos, meta] =
		await Promise.all([
			db.query("select * from finan_tarifas_faturas order by ano, mes, metric"),
			db.query("select * from finan_tarifas_formas_pagamento order by ano, mes, forma"),
			db.query("select * from finan_tarifas_receita_cliente order by ano, mes, cliente_nome"),
			db.query("select * from finan_tarifas_mensais order by ano, mes, banco"),
			db.query("select * from finan_tarifas_cobranca_clientes order by ano, mes, forma_cobranca"),
			db.query("select * from finan_tarifas_boletos order by tarifa"),
...
async function listImportLogs(limit = 20) {
	...
	const result = await db.query(
		`select * from finan_import_logs order by created_at desc limit $1`,
		[safeLimit],
	);
	return result.rows.map((row) => ({
		path: row.legacy_path || `finan_import_logs/${row.id}`,
		collectionPath: "finan_import_logs",
