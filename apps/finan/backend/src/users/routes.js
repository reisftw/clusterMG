...
router.delete("/roles/:id", validate({ params: IdParamDTO }), async (req, res, next) => {
	try {
		const { id } = req.validated.params;
		const role = await db.query(
			"select id, system_role from finan_roles where id = $1",
			[id],
		);
		if (!role.rows.length) {
			res.status(404).json({ ok: false, error: "Cargo/perfil não encontrado." });
		...
		const inUse = await db.query(
			"select count(*)::int as total from finan_users where role_id = $1",
			[id],
		);
		if (inUse.rows[0].total > 0) {
			res.status(400).json({
				ok: false,
				error: `${inUse.rows[0].total} usuário(s) ainda usam este cargo. Mova-os para outro cargo antes de excluir.`,
			});
			return;
		}
		await db.query("delete from finan_roles where id = $1", [id]);
		res.json({ ok: true });
	} catch (error) {
		next(error);
	}
...
router.get("/migration-snapshots", async (_req, res, next) => {
	try {
		const { rows } = await db.query(
