...
function createEmailAdminController({ emailService, notificationsService }) {
	async function getConfig(req, res, next) {
		try {
			res.json({
				ok: true,
				config: emailService.sanitizeConfig(await emailService.getConfig()),
			});
		} catch (error) {
			notificationsService
				.createNotification({
					...
					message:
						error?.message || "Não foi possível gerar o backup do PostgreSQL.",
				...
				.catch((notifyError) =>
					console.warn(
						"[notifications] Falha ao avisar erro de backup:",
	...
	async function saveConfig(req, res, next) {
		try {
			res.json({
				ok: true,
				config: await emailService.saveConfig(req.body || {}),
			});
		} catch (error) {
			notificationsService
				.createNotification({
				...
				.catch((notifyError) =>
					console.warn(
						"[notifications] Falha ao avisar erro de restore:",
						notifyError?.message || notifyError,
					),
				);
			next(error);
	...
	async function sendTestEmail(req, res, next) {
		try {
			res.json(
				await emailService.sendTestEmail(req.body?.to || req.user?.email),
			);
		} catch (error) {
			next(error);
		}
