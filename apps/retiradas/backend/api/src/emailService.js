...
const DEFAULT_CONFIG = {
	...
	mfaEmailEnabled:
		String(process.env.MFA_EMAIL_ENABLED || "false").toLowerCase() === "true",
...
function createEmailTemplate({
	...
	const resolvedLogoUrl =
		logoUrl ||
		`${String(appUrl || DEFAULT_CONFIG.appUrl).replace(/\/+$/, "")}/cluster-mg.png`;
...
async function sendInsumosLowStockEmail({ to, produto = {}, recipients = [] }) {
	const config = await getConfig();
	const appUrl = String(
		config.appUrl || DEFAULT_CONFIG.appUrl || "https://retiradas.tech",
	).replace(/\/+$/, "");
