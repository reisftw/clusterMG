...
function getResponsavel(data = {}) {
	...
	return {
		nome: text(
			responsavel.nome || data.nomeResponsavel || data.responsavelNome,
		),
		email: text(
			responsavel.email ||
...
function buildEmpresaPayload(id, data = {}) {
	const nome = text(
		data.nome || data.empresa || data.empresaNome || data.razaoSocial || id,
	);
	...
	return {
		...
		tecnicos: Array.isArray(data.tecnicos)
			? data.tecnicos.map(normalizeTecnico).filter((item) => item.nome)
			: [],
		observacoes: text(data.observacoes),
	...
	await db.query(
     ...
     on conflict (path) do update
		...
		[
			...
			TARGET_COLLECTION,
			id,
			JSON.stringify({ ...data, atualizado_em: now }),
		],
	);
}
...
function findEmpresaIdForTecnico(tecnico, empresasByLegacyId, empresasByName) {
	...
	const empresaNome = text(
		tecnico.empresaNome || tecnico.empresa || tecnico.nomeEmpresa,
	);
...
function mergeLegacyEmpresas(legacyEmpresas, empresasById, empresasByName, empresasByLegacyId) {
	...
	for (const legacy of legacyEmpresas) {
		...
		const targetId =
			empresasByName.get(nameKey) ||
			text(legacy.id) ||
			slugify(normalizedLegacy.nome);
