...
export async function exportTecnicosXlsx(month, monthData) {
	...
	sheet.columns = [{ width: 12 }, { width: 34 }, { width: 14 }, { width: 18 }];
	await saveWorkbook(
		workbook,
		monthFileName("relatorio-tecnicos", month, "xlsx"),
	);
}
...
export function exportRegionaisPdf(month, monthData) {
	...
	imprimirHtml(
		...
		{
			areaId: "relatorio-regionais-print",
			styleId: "relatorio-regionais-style",
		},
	);
}
...
export async function exportRegionaisXlsx(month, monthData) {
	...
	await saveWorkbook(
		workbook,
		monthFileName("relatorio-regionais", month, "xlsx"),
	);
}
...
export function exportEntregaMesPdf(month, monthData) {
	const resumoRows = [
		...
		[
			"Total realizado",
			Number(monthData?.totalOS || 0).toLocaleString("pt-BR"),
		],
		[
			"Técnico retirada",
			Number(
				monthData?.technicians?.reduce(
		...
		[
			"Agente autorizado",
			Number(monthData?.agenteTotal || 0).toLocaleString("pt-BR"),
		],
		[
			"Entregue em loja",
			Number(monthData?.lojaTotal || 0).toLocaleString("pt-BR"),
		],
		[
			"Regionais",
			Number(
				monthData?.regionais?.reduce(
					(sum, item) => sum + Number(item.total || 0),
					0,
				) || 0,
			).toLocaleString("pt-BR"),
...
export async function exportEntregaMesXlsx(month, monthData) {
	...
	await saveWorkbook(
		workbook,
		monthFileName("relatorio-entregas", month, "xlsx"),
	);
}
...
export async function exportAgentesXlsx(month, agentesData) {
	...
	await saveWorkbook(
		workbook,
		monthFileName("relatorio-agentes", month, "xlsx"),
	);
}
      ...
      <div style="font-family:Arial,sans-serif;background:#fff;color:#0f172a;padding:24px">
        ...
        ${buildMetaHtmlTable(
					...
					[
						[
							"Regionais",
							matchData.resumo.totalRegionais,
							matchData.regionais.reduce(
						...
						[
							"Agentes autorizados",
							matchData.resumo.totalAgentes,
							matchData.agentes.reduce(
								(sum, item) => sum + item.totalMatches,
								0,
							),
						],
						[
							"Geral",
							matchData.resumo.totalCidades,
							matchData.resumo.totalMatches,
						],
					],
				)}
...
export async function exportMatchXlsx(ordens = []) {
	...
	[
		[
			"Regionais",
			matchData.resumo.totalRegionais,
			matchData.regionais.reduce((sum, item) => sum + item.totalMatches, 0),
		],
		[
			"Agentes autorizados",
			matchData.resumo.totalAgentes,
			matchData.agentes.reduce((sum, item) => sum + item.totalMatches, 0),
		],
