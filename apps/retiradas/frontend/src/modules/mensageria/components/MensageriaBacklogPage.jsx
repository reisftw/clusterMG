...
const MensageriaBacklogPage = () => {
	...
	const regionais = useMemo(
		() =>
			[...new Set(ordens.map((item) => item.regional).filter(Boolean))].sort(
	...
	const cidades = useMemo(() => {
		...
		return [...new Set(base.map((item) => item.cidade).filter(Boolean))].sort(
			(a, b) => String(a).localeCompare(String(b), "pt-BR"),
		);
	...
	const filteredOrdens = useMemo(() => {
		...
		return ordens.filter((item) => {
			if (filters.regional !== ALL && item.regional !== filters.regional)
				return false;
			if (filters.cidade !== ALL && item.cidade !== filters.cidade)
				return false;
			if (filters.idade !== ALL && getAgeBucket(item) !== filters.idade)
				return false;
			if (
				filters.tentativa === "sem_tentativa" &&
				Number(item.tentativas || 0) > 0
			)
				return false;
			if (
				filters.tentativa === "com_tentativa" &&
				Number(item.tentativas || 0) === 0
			)
				return false;
			if (filters.tentativa === "na_fila" && !item.ja_esta_na_fila)
				return false;
	...
	const stats = useMemo(
		() => ({
			...
			semTentativa: filteredOrdens.filter(
				(item) => Number(item.tentativas || 0) === 0,
			).length,
	...
	const previewButtonMessage = previewItem
		...
		: "";
	const centralWhatsappPhone = String(CENTRAL_WHATSAPP_PHONE || "").replace(
		/\D/g,
		"",
	);
	...
	const loadData = async (force = false) => {
		...
		try {
			...
			setAutoCities(
				Array.isArray(nextConfig.autoEnqueueCities)
					? nextConfig.autoEnqueueCities
					: [],
			);
	...
	const handleSendToQueue = async () => {
		...
		try {
			...
			setFeedback(
				error?.message || "Não foi possível enviar o grupo para a fila.",
			);
		} finally {
			setSending(false);
		}
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							O.S. abertas importadas pelo XLSX do Mapa, agrupadas por regional
							e cidade para envio em massa.
						</p>
					</div>
				</div>
				...
				<div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-6">
					...
					<label className="block">
						...
						<select
							...
							onChange={(event) =>
								updateFilter("tentativa", event.target.value)
							}
							className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
						>
				...
				<div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
					<div>
						<h2 className="text-lg font-bold text-slate-900">
							Automação por cidades no próximo Mapa
						</h2>
						<p className="mt-1 text-sm text-slate-500">
							Quando subir um novo mapa, clientes novos das cidades marcadas
							entram automaticamente na fila. Sem cidade marcada, entram todas.
						</p>
					</div>
					<button
						type="button"
				...
				<div className="mt-4 flex flex-wrap gap-2">
					{cidades.length ? (
						...
						<span className="text-sm text-slate-500">
							Nenhuma cidade carregada no backlog atual.
						</span>
					)}
				</div>
			</section>
			...
			<section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
				<div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
					<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						<div>
							<h2 className="text-lg font-bold text-slate-900">
								O.S. por regional e cidade
							</h2>
							<p className="mt-1 text-sm text-slate-500">
								Selecione cidades inteiras ou clientes específicos para preparar
								o envio.
							</p>
						</div>
						<div className="flex flex-wrap gap-2">
					...
					<div className="mt-4 flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
						<p className="text-sm font-semibold text-slate-600">
							Mostrando {formatNumber(paginatedOrdens.length)} de{" "}
							{formatNumber(filteredOrdens.length)} O.S. filtradas
						</p>
					...
					<div className="mt-4 space-y-4">
						{groupedOrdens.length ? (
							groupedOrdens.map((group) => (
								<div
									key={`${group.regional}-${group.cidade}`}
						...
						<div className="mt-4 space-y-3 text-sm text-slate-600">
							<p>
								<strong className="text-slate-900">
									{formatNumber(selectedOrdens.length)}
								</strong>{" "}
								cliente(s) selecionados.
							</p>
							<p>
								O envio em massa será registrado na fila da Mensageria para
								aprovação, retentativa e histórico.
							</p>
							<p>
								Quando a API chegar, este Backlog pode trocar a origem XLSX pela
								consulta automática sem mudar o fluxo da operação.
							</p>
						</div>
					</div>
				</aside>
