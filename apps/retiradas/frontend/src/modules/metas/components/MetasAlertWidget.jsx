...
const MetasAlertWidget = () => {
				...
				<div className="flex items-start gap-3">
					...
					<div className="min-w-0 flex-1">
						...
						<p className="text-xs text-amber-700">
							A aba de {metaMes.mes.toLowerCase()} ja foi identificada, mas
							ainda nao ha lancamentos para exibir.
						</p>
					</div>
				</div>
			</div>
	...
	const pct = Number.parseFloat(metaMes.percentAchieved);
			...
			<div className="flex items-start gap-3">
				...
				<div className="min-w-0 flex-1">
					...
					<p className="mb-1 text-xs text-red-700">
						Faltam {falta.toLocaleString("pt-BR")} O.S -{" "}
						{metaMes.percentAchieved}% concluido
					</p>

					<p className="text-xs font-semibold text-red-700">
						Ritmo necessario:{" "}
						<span className="text-lg">{Math.round(ritmoNecessario)}</span>{" "}
