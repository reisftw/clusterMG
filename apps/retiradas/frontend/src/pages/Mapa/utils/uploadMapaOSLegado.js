...
function samePayload(a, b) {
	return (
		JSON.stringify(comparablePayload(a)) ===
		JSON.stringify(comparablePayload(b))
	);
}

async function carregarMapaCidades() {
	const regionais = await listAllVpsDocuments("ferramentas_regionais", {
		pageSize: 500,
	});
	const mapa = {};
...
async function carregarOrdensLegadasAtuais() {
	...
	const atuais = await listAllVpsDocuments(LEGACY_COLLECTION, {
		pageSize: 1000,
	});
	atuais.forEach((item) => {
...
export async function processarUploadMapaLegado(file, onProgress, periodo) {
	...
	const ordensNovas = {};
	const ignoradasSemData = 0;
	const ignoradasForaDoCorte = 0;
	...
	for (let index = 0; index < atualizar.length; index += batchSize) {
		...
		await Promise.all(
			lote.map((id) =>
				setVpsDocument(`${LEGACY_COLLECTION}/${id}`, ordensNovas[id]),
			),
		);
	...
	await refreshLegacyMapaSnapshot({
		ordens: Object.entries(ordensNovas).map(([id, ordem]) => ({
			id,
			...ordem,
		})),
		periodo,
