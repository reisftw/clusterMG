...
const { requireRotAuth, requireRotPermission, scopeRegionalFilter, getUserRegionalIds } = require("../auth/middleware");
...
router.get("/", readGuard(READ_PERMISSIONS.shifts), noStore, async (req, res, next) => {
	try {
		...
		const regionalIds = regionalScope ? getUserRegionalIds(req.rotUser) : null;
		const params = regionalIds ? [regionalIds] : [];
		const [{ rows }, { rows: users }] = await Promise.all([
			db.query(
				regionalIds ? `select * from rot_shifts where regional_id = any($1::text[]) order by start_at desc` : `select * from rot_shifts order by start_at desc`,
				params,
			),
			db.query(
				regionalIds ? `select id, name, regional_id from rot_users where regional_id = any($1::text[]) and status = 'ativo' order by name` : `select id, name, regional_id from rot_users where status = 'ativo' order by name`,
				params,
			),
		]);
...
// So global pode lancar plantao fora das proprias regionais (regional
// principal + extras em rot_user_regionais_extras). Sem essa checagem,
// qualquer usuario com rot.shifts.manage conseguia escolher regionalId
// livremente no payload, mesmo sendo de outra regional.
function assertShiftRegionalAllowed(req, regionalId) {
	if (req.rotUser.is_global) return;
	const allowed = getUserRegionalIds(req.rotUser);
	if (!allowed.includes(regionalId)) {
		const error = new Error("Você só pode lançar plantão nas regionais que você é responsável.");
		error.status = 403;
		throw error;
	}
}
...
router.post("/", requireRotPermission("rot.shifts.manage"), async (req, res, next) => {
	try {
		...
		assertShiftRegionalAllowed(req, regionalId);
		const id = randomId();
...
router.put("/:id", requireRotPermission("rot.shifts.manage"), async (req, res, next) => {
	try {
		...
		assertShiftRegionalAllowed(req, before[0].regional_id);
		if (regionalId) assertShiftRegionalAllowed(req, regionalId);
		const { rows } = await db.query(
...
router.delete("/:id", requireRotPermission("rot.shifts.manage"), async (req, res, next) => {
	try {
		const { rows: before } = await db.query(`select regional_id from rot_shifts where id = $1`, [req.params.id]);
		if (!before[0]) {
			res.json({ ok: true });
			return;
		}
