...
function canSeeCompany(user, empresa) {
	...
	if (
		...
		return Boolean(
			regional &&
				(regional === normalizeText(empresa.regional) ||
					regional === normalizeText(empresa.regionalId)),
		);
	}
...
function normalizeEmpresa(doc) {
	...
	return {
		...
		regionalId: cleanText(data.regionalId || data.regional_id),
...
function getAnnualRange(year) {
	...
	return { start, end, year: safeYear };
}

function getYearToDateRefreshRange(end) {
	const targetEnd = end instanceof Date && !Number.isNaN(end.getTime()) ? end : new Date();
	const start = new Date(targetEnd.getFullYear(), 0, 1);
	start.setHours(0, 0, 0, 0);
	return { start, end: targetEnd };
}

function splitDateRange(start, end, chunkDays = 15) {
	const chunks = [];
	...
	while (currentStart <= end) {
		...
		chunks.push({
			...
			end: currentEnd > end ? new Date(end) : currentEnd,
		});
...
async function refreshMovementCacheInChunks({ user = {}, start, end, onChunk } = {}) {
	const chunks = splitDateRange(start, end, 15);
	let movements = 0;
	let saved = 0;
	let cachedAt = "";
	for (let index = 0; index < chunks.length; index += 1) {
		const chunk = chunks[index];
		await onChunk?.({ chunk, index, total: chunks.length });
		const result = await refreshMovementCache({
			user,
			start: chunk.start,
			end: chunk.end,
		});
		...
		cachedAt = result.cachedAt || cachedAt;
	}
	const currentConfig = await readConfig({ sanitized: false });
	await saveConfig(
		{
			...currentConfig,
			lastReportRefreshAt: cachedAt || nowIso(),
			lastReportRefreshStart: start ? start.toISOString() : "",
			lastReportRefreshEnd: end ? end.toISOString() : "",
			lastReportRefreshTotal: movements,
		},
		user,
	);
	return {
...
async function refreshReportMovements({ user = {}, query = {} } = {}) {
	...
	const requested = getPeriodRange(
		filters.periodo,
		filters.dataInicio,
		filters.dataFim,
	);
	const { start, end } = getYearToDateRefreshRange(requested.end);
	const result = await refreshMovementCacheInChunks({ user, start, end });
	return {
		...result,
		filters,
		periodo: { inicio: start.toISOString(), fim: end.toISOString() },
		periodoSolicitado: {
			inicio: requested.start.toISOString(),
			fim: requested.end.toISOString(),
		},
	};
}
...
async function runReportRefreshJob(jobId, user = {}, query = {}) {
	...
	try {
		...
		const chunks = splitDateRange(start, end, 15);
		await updateRefreshJob(jobId, {
			stage: "Lendo notas e movimentações desde o começo do ano",
			percent: 30,
			filters,
			total: chunks.length,
			periodo: { inicio: start.toISOString(), fim: end.toISOString() },
		});
		const result = await refreshMovementCacheInChunks({
			user,
			start,
			end,
			onChunk: async ({ chunk, index, total }) => {
				await updateRefreshJob(jobId, {
					stage: `Lendo bloco ${index + 1}/${total} (${getBrazilDateKey(chunk.start)} a ${getBrazilDateKey(chunk.end)})`,
					percent: Math.min(95, 30 + Math.round(((index + 1) / total) * 60)),
					processed: index + 1,
					total,
				});
			},
...
function mergeEstimatedDayMaps(maps = []) {
	const merged = new Map();
	for (const map of maps) {
		for (const [date, values] of map.entries()) {
			const current = merged.get(date) || { unidades: 0, metros: 0 };
			current.unidades += toNumber(values.unidades);
			current.metros += toNumber(values.metros);
...
async function getHistory({
	...
	const normalizedPeriod = ["diario", "mensal", "anual"].includes(normalizeText(periodo))
		? normalizeText(periodo)
		: "mensal";
	const range = dataInicio || dataFim
		? getPeriodRange(normalizedPeriod, dataInicio, dataFim)
		: normalizedPeriod === "anual"
			? getAnnualRange(year)
			: getPeriodRange(normalizedPeriod);
	const { start, end } = range;
	const safeYear = start.getFullYear();
	const selectedEmpresas =
		normalizedType === "empresa"
			? empresas.filter((empresa) => empresa.id === targetId)
			: empresas
					.map((empresa) => ({
						...empresa,
						tecnicos: empresa.tecnicos.filter(
							(tecnico) =>
								tecnico.id === targetId || tecnico.snapshotId === targetId,
						),
					}))
					.filter((empresa) => empresa.tecnicos.length);
	const selectedTecnicos = selectedEmpresas.flatMap((empresa) => empresa.tecnicos);
	if (!selectedTecnicos.length) {
		const error = new Error("Técnico ou empresa não encontrado para auditoria.");
		error.statusCode = 404;
		throw error;
	}
	const tecnicoByEmail = buildTecnicoByEmail(selectedEmpresas);
	const movements = await loadCachedMovements({
		start,
		end,
		tecnicoByEmail,
	...
	const logs = await loadSuccessfulRefreshLogs(start, end);
	const estimatedBySnapshotId = groupEstimatedBySnapshotId(logs, selectedTecnicos);
	const estimatedByDay =
		normalizedType === "empresa"
			? mergeEstimatedDayMaps([...estimatedBySnapshotId.values()])
			: estimatedBySnapshotId.get(selectedTecnicos[0].snapshotId) || new Map();
	return {
		ok: true,
		type: normalizedType === "empresa" ? "empresa" : "tecnico",
		...
		target:
			normalizedType === "empresa"
				? {
						id: selectedEmpresas[0].id,
						nome: selectedEmpresas[0].nome,
						regional: selectedEmpresas[0].regional,
						tecnicos: selectedTecnicos.length,
					}
				: {
						id: selectedTecnicos[0].id,
						snapshotId: selectedTecnicos[0].snapshotId,
						nome: selectedTecnicos[0].nome,
						empresaNome: selectedTecnicos[0].empresaNome,
						regional: selectedTecnicos[0].regional,
					},
		history: summarizeMovementHistory(movements, { start, end, estimatedByDay }),
	};
}
