...
const DEFAULT_EMAIL_TEMPLATES = {
	...
	sst_protocol_opened: {
		label: "SST — Novo protocolo aberto (Gestor SST)",
		subject: "Novo protocolo SST aberto — {protocolNumber}",
		title: "Novo protocolo aguardando triagem",
		preview: "Um novo protocolo de Segurança do Trabalho foi aberto.",
		actionLabel: "Ver protocolo",
		body: "Olá, {nome}.\n\nUm novo protocolo foi aberto na Segurança do Trabalho e ainda não tem responsável.\n\nProtocolo: {protocolNumber}\nTipo: {tipo}\nAssunto: {assunto}\nPrioridade: {prioridade}\nColaborador: {colaborador}\n\nClique abaixo para assumir ou acompanhar.",
	},
	sst_protocol_assigned: {
		label: "SST — Protocolo atribuído a você",
		subject: "Protocolo SST atribuído a você — {protocolNumber}",
		title: "Protocolo atribuído a você",
		preview: "Um protocolo de Segurança do Trabalho foi atribuído a você.",
		actionLabel: "Ver protocolo",
		body: "Olá, {nome}.\n\nO protocolo abaixo foi atribuído a você na Segurança do Trabalho.\n\nProtocolo: {protocolNumber}\nAssunto: {assunto}\n\nClique abaixo para acompanhar.",
	},
};
...
const DEFAULT_CONFIG = {
	...
	appUrl: process.env.ROT_PUBLIC_URL || "https://operacao.retiradas.tech",
	...
	// Ao abrir um protocolo SST sem responsavel, avisa por e-mail todo
	// mundo com o cargo Gestor de Seguranca do Trabalho. A partir do
	// momento que alguem assume, os avisos passam a ir so pra essa
	// pessoa (nao mais broadcast). Administrador liga/desliga aqui.
	sstProtocolEmailEnabled: String(process.env.ROT_SST_PROTOCOL_EMAIL_ENABLED || "true").toLowerCase() !== "false",
...
async function getConfig() {
	...
	return {
		...
		sstProtocolEmailEnabled: Boolean(data.sstProtocolEmailEnabled ?? DEFAULT_CONFIG.sstProtocolEmailEnabled),
...
async function saveConfig(patch = {}) {
	...
	const next = {
		...
		sstProtocolEmailEnabled: Boolean(patch.sstProtocolEmailEnabled ?? current.sstProtocolEmailEnabled),
...
function renderConfiguredEmail(config, templateKey, variables = {}) {
	...
	const actionUrlVariable = { password_reset: "{resetUrl}", welcome_first_access: "{firstAccessUrl}", smtp_test: "{appUrl}", sst_protocol_opened: "{protocolUrl}", sst_protocol_assigned: "{protocolUrl}" }[templateKey];
...
// Sem responsavel ainda: avisa todo mundo com o cargo Gestor SST. Cada
// falha de envio (SMTP fora do ar, e-mail invalido etc.) e isolada —
// um destinatario com problema nao deve travar os outros.
async function sendSstProtocolOpenedEmail({ recipients, protocolNumber, subject, type, priority, employeeName, protocolUrl }) {
	const config = await getConfig();
	if (!config.sstProtocolEmailEnabled) return { sent: false, reason: "sst_protocol_email_disabled" };
	const results = [];
	for (const recipient of recipients || []) {
		if (!recipient?.email) continue;
		try {
			const rendered = renderConfiguredEmail(config, "sst_protocol_opened", {
			...
			results.push(await sendMail({ to: recipient.email, ...rendered, meta: { type: "sst_protocol_opened", protocolNumber } }));
		} catch (error) {
			console.warn("[rot-email-sst]", error?.message || error);
		}
	}
	return { sent: results.length, results };
}

async function sendSstProtocolAssignedEmail({ to, name, protocolNumber, subject, protocolUrl }) {
	const config = await getConfig();
	if (!config.sstProtocolEmailEnabled || !to) return { sent: false, reason: "sst_protocol_email_disabled" };
	try {
		const rendered = renderConfiguredEmail(config, "sst_protocol_assigned", {
			nome: name || to,
			email: to,
			protocolNumber,
			assunto: subject,
		...
		return await sendMail({ to, ...rendered, meta: { type: "sst_protocol_assigned", protocolNumber } });
	} catch (error) {
		console.warn("[rot-email-sst]", error?.message || error);
