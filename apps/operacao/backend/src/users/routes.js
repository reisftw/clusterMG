...
// Ator global gerencia qualquer usuario. Ator regional so gerencia
// usuario da PROPRIA regional e de cargo estritamente abaixo do
// proprio (nunca igual nem acima) — usado em GET/PUT/DELETE/reset-password
// por :id, pra nao depender so da listagem filtrada (SEC: IDOR real
// encontrado em producao — editar/desativar/resetar senha por id nao
// checava posse nenhuma antes desta correcao).
function assertUserManageable(req, targetUser) {
	if (!targetUser) fail(404, "Usuário não encontrado.");
	if (req.rotUser.is_global) return;
	if (targetUser.regional_id !== req.rotUser.regional_id) fail(403, "Você só pode gerenciar usuários da sua regional.");
	if (Number(targetUser.role_level) >= req.rotUser.role_level) fail(403, "Você não pode gerenciar um usuário de cargo igual ou superior ao seu.");
}

async function loadUserWithRoleLevel(client, id) {
	const { rows } = await client.query(
		`select u.*, r.level as role_level, r.is_global as role_is_global
		   from rot_users u
...
function publicAdminUser(row) {
	return {
		...
		extraRegionalIds: Array.isArray(row.extra_regional_ids) ? row.extra_regional_ids : [],
		cityId: row.city_id,
		phone: row.phone,
...
// Regionais adicionais (supervisor/lider responsavel por mais de uma
// regional) — cada id precisa resolver pra uma regional valida
// (mesma logica de resolveLegacyRegionalId), senao rejeita.
async function replaceUserExtraRegionais(client, userId, regionalIds, actorId = null) {
	const rawIds = Array.isArray(regionalIds) ? regionalIds : [];
	const resolved = [];
	for (const rawId of rawIds) {
		const id = await resolveLegacyRegionalId(client, rawId);
		if (!id) fail(400, "Regional adicional inválida.");
		if (!resolved.includes(id)) resolved.push(id);
	}
	await client.query(`delete from rot_user_regionais_extras where user_id = $1`, [userId]);
	if (resolved.length) {
		await client.query(
			`insert into rot_user_regionais_extras (user_id, regional_id, created_by)
			select $1, regional_id, $3 from unnest($2::text[]) as regional_id
			on conflict (user_id, regional_id) do nothing`,
...
// `regionais` (pagina admin Regionais) e a fonte unica de regional desde a
// migracao 038 — rot_users.regional_id guarda o id de `regionais`
// diretamente, sem traducao. So valida que o id existe e esta ativo.
async function resolveLegacyRegionalId(client, regionalId) {
	if (!regionalId) return null;
	const id = String(regionalId);
	const { rows } = await client.query(
		`select id from regionais where id = $1 or legacy_document_id = $1 limit 1`,
		[id],
	);
...
// So cargo global ve todos os usuarios/regionais. Cargo regional so
// enxerga usuarios da PROPRIA regional e de cargo estritamente abaixo
// do proprio (nunca o mesmo nivel nem acima) — antes so filtrava por
// regional, entao um supervisor/lider via colegas e superiores da
// mesma regional (SEC: relatado em producao, Adao via usuarios que
// nao deveria).
router.get("/", requireRotPermission("rot.users.manage"), noStore, async (req, res, next) => {
	try {
		...
		if (!req.rotUser.is_global) {
			...
			conditions.push(`u.regional_id = $${params.length}`);
			params.push(req.rotUser.role_level);
...
router.post("/", requireRotPermission("rot.users.manage"), validateBody(["name", "username", "email", "role", "regionalId", "cityId", "phone", "operationScopes", "extraRegionalIds", "generatePassword"], { allowEmpty: false }), async (req, res, next) => {
	...
	try {
		...
		if (error?.constraint === "rot_users_email_key") {
			res.status(409).json({ ok: false, error: "Já existe um usuário com esse e-mail." });
			return;
		}
		next(error);
	} finally {
...
router.put("/:id", requireRotPermission("rot.users.manage"), validateBody(["name", "email", "role", "regionalId", "cityId", "phone", "status", "operationScopes", "extraRegionalIds"], { allowEmpty: false }), async (req, res, next) => {
	try {
		...
		let roleId = null;
		const { rows: currentRoleRows } = await db.query(`select level, is_global from rot_roles where id = $1`, [
			beforeRows[0].role_id,
		]);
		// IDOR (SEC): antes so validava regional/cargo quando esses campos
		// vinham no payload — editar so nome/telefone/status de um usuario de
		// outra regional ou de cargo igual/superior passava direto, mesmo sem
		// esse usuario aparecer na listagem pro ator. Trava aqui, antes de
		// qualquer alteracao.
		if (!req.rotUser.is_global) {
			if (beforeRows[0].regional_id !== req.rotUser.regional_id) {
				res.status(403).json({ ok: false, error: "Você só pode gerenciar usuários da sua regional." });
				return;
			}
			if (Number(currentRoleRows[0]?.level) >= req.rotUser.role_level) {
				res.status(403).json({ ok: false, error: "Você não pode gerenciar um usuário de cargo igual ou superior ao seu." });
				return;
			}
		}
		...
		if (payload.regionalId !== undefined) {
			...
			// Mesma regra do POST: cargo regional (supervisor pra baixo, ate
			// nivel 73) so move usuario dentro da propria regional. Sem essa
			// checagem, um supervisor regional podia editar um usuario
			// existente e transferi-lo pra outra regional — a mesma acao que
			// ja era bloqueada na criacao.
			if (!req.rotUser.is_global && regionalId && regionalId !== req.rotUser.regional_id) {
				res.status(403).json({ ok: false, error: "Você só pode gerenciar usuários da sua regional." });
				return;
			}
		...
		if (payload.extraRegionalIds !== undefined) {
			await replaceUserExtraRegionais(db, req.params.id, payload.extraRegionalIds, req.rotUser.id);
		}
		const { rows: withRole } = await db.query(
			`${ADMIN_USER_SELECT} where u.id = $1`,
			[req.params.id],
...
router.delete("/:id", requireRotPermission("rot.users.manage"), async (req, res, next) => {
	try {
		...
		const target = await loadUserWithRoleLevel(db, req.params.id);
		assertUserManageable(req, target);
...
router.get("/roles", noStore, async (req, res, next) => {
	try {
		...
		// UsuarioModal.jsx le role.isGlobal (camelCase) pra decidir se o campo
		// Regional e obrigatorio — devolver so is_global (snake_case) deixava
		// esse campo sempre undefined, entao "needsRegional" ficava sempre
		// true mesmo pra cargos globais (bug real: campo Regional aparecia
		// como obrigatorio mesmo pra site_admin).
		res.json({
			ok: true,
			items: visible.map((role) => ({ ...role, isGlobal: Boolean(role.is_global) })),
		});
	} catch (error) {
		next(error);
	}
