...
// Aceita: "YYYY-MM-DD" (input type=date), datetime ISO completo
// ("...T00:00:00.000Z", devolvido por colunas `date`/`timestamp` do
// Postgres depois de passar por JSON.stringify), objeto Date, timestamp
// numerico ou timestamp estilo Firestore ({seconds}/{_seconds}). Uma
// string "YYYY-MM-DD" pura ganha "T00:00:00" (horario local) pra nao
// desviar um dia em fusos negativos; qualquer outro formato e' deixado
// pro parser nativo do Date, sem concatenar nada.
function toSafeDate(value) {
	...
	const isPlainDate = /^\d{4}-\d{2}-\d{2}$/.test(text);
	const date = new Date(isPlainDate ? `${text}T00:00:00` : text);
	return Number.isNaN(date.getTime()) ? null : date;
}
