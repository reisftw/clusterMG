...
const buildOperationalGroups = (regional) => {
	...
	return {
		rot: buildOperationalGroup(groups.rot),
...
const RegionalModalContent = ({ regional, onSalvar, onClose }) => {
	...
	const addCidade = () =>
		setCidades((c) => [...c, { nome: "", tipo: "Comum", code: "" }]);
								...
								{cidades.map((cidade, i) => (
									<div key={i} className="flex gap-2 items-center">
										...
										<input
											...
											value={cidade.code || ""}
											onChange={(e) =>
												setCidadeField(i, "code", e.target.value.toUpperCase().slice(0, 8))
											}
											placeholder="Código"
											title="Código curto da base (ex: BMO), usado no ID do ativo e na distância de rompimentos"
											className="input-field w-24 shrink-0 uppercase"
										/>
										<select
											value={cidade.tipo}
											onChange={(e) =>
