...
const inputClass =
	"w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100";
...
function getVisualCategoryId(item = {}) {
	...
	if (CAMERA_TERMS.some((term) => name.includes(normalizeCategoryText(term))))
		return "cameras";
...
function monthKeyLocal(date = new Date()) {
	return dateKeyLocal(date).slice(0, 7);
}

function lastDayOfMonth(monthKey) {
	const [year, month] = String(monthKey || monthKeyLocal()).split("-").map(Number);
...
function initialHistoryFilter() {
	...
	return {
		periodo: "mensal",
		day: dateKeyLocal(today),
		month: monthKeyLocal(today),
		year: String(today.getFullYear()),
	};
}

function buildHistoryQuery(filter = {}) {
	const periodo = filter.periodo || "mensal";
	if (periodo === "diario") {
		const day = filter.day || dateKeyLocal();
		return { periodo, dataInicio: day, dataFim: day, ano: day.slice(0, 4) };
	}
	if (periodo === "anual") {
		const year = String(filter.year || new Date().getFullYear());
		return {
			periodo,
	...
	const month = filter.month || monthKeyLocal();
	return {
		periodo: "mensal",
		dataInicio: `${month}-01`,
		dataFim: `${month}-${String(lastDayOfMonth(month)).padStart(2, "0")}`,
		ano: month.slice(0, 4),
	};
}

function periodLabel(filter = {}) {
	if (filter.periodo === "diario") return `Dia ${formatDate(filter.day).split(",")[0]}`;
	if (filter.periodo === "anual") return `Ano ${filter.year}`;
	return `Mês ${String(filter.month || "").split("-").reverse().join("/")}`;
}

function findDefaultHistoryDay(days = []) {
	return (
		[...days]
			.reverse()
			.find((day) => day.movimentacoes || day.estimadoUnidades || day.estimadoMetros)?.date ||
		days[0]?.date ||
		""
	);
}
...
function formatConsumptionAmount(row = {}) {
	...
	if (unidades > 0 && metros > 0)
		return `${metricValue(unidades)} un · ${metricValue(metros)} m`;
	return "0";
}
...
function groupItemsByCategory(items = []) {
	const groups = new Map(
		CATEGORY_ORDER.map((category) => [
			category.id,
			{ ...category, items: [], unidades: 0, metros: 0 },
		]),
	);
	items.forEach((item) => {
...
async function pollRefreshJob(jobId, onProgress) {
	...
	while (Date.now() - startedAt < JOB_TIMEOUT_MS) {
		...
		if (job.status === "failed")
			throw new Error(job.error || "Falha na atualização da API.");
...
async function exportTecnicoPdf(tecnico) {
	...
	pdf.save(
		`bolsa-tecnico-${tecnico.nome || "tecnico"}.pdf`
			.replace(/\s+/g, "-")
			.toLowerCase(),
	);
}
...
function MetricCard({ label, value, helper, tone = "blue" }) {
	...
	return (
		<div
			className={`rounded-2xl border p-4 shadow-sm ${styles[tone] || styles.blue}`}
		>
			<p className="text-xs font-black uppercase tracking-wide opacity-75">
				{label}
			</p>
			<p className="mt-2 whitespace-nowrap text-2xl font-black leading-none">
				{value}
			</p>
			{helper ? (
...
function PaginationControls({
	...
	return (
		<div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3 text-xs font-bold text-slate-500">
			<span>
				Mostrando {Math.min(total, (page - 1) * pageSize + 1)} a{" "}
	...
	if (!items.length) {
		return (
			<p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
				Nenhum item com saldo positivo encontrado.
			</p>
		);
	}
				...
				return (
					...
					>
						<button
							type="button"
							onClick={() =>
								setOpen((current) => ({ ...current, [group.id]: !isOpen }))
							}
									...
									group.items.map((item) => (
										...
										>
											<div className="flex flex-wrap items-center justify-between gap-2">
												<p className="min-w-0 flex-1 break-words font-black text-slate-900">
													{item.nome}
												</p>
												<span className="whitespace-nowrap rounded-full bg-slate-50 px-3 py-1 text-xs font-black text-slate-600">
													{formatQuantity(item)}
												</span>
											</div>
											<div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200">
												<div
													...
													style={{
														width: `${Math.min(100, (Number(item.quantidade || 0) / max) * 100)}%`,
													}}
												/>
											</div>
											<p className="mt-2 text-xs font-bold text-slate-500">
												Diferença desde a leitura anterior:{" "}
												{metricValue(item.diferenca)} {item.unidade}
											</p>
										</div>
									))
								) : (
...
function MonthlyHistoryTable({ rows = [] }) {
	...
	if (!rows.length) {
		return (
			<p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
				Nenhuma movimentação salva para este período.
			</p>
		);
	}
					...
					<tbody className="divide-y divide-slate-100">
						{visibleRows.map((row) => (
							<tr key={row.id || `${row.numero}-${row.nome}-${row.data}`}>
								...
								<td className="px-3 py-3 font-bold text-slate-700">
									{row.tipoMovimentacaoLabel || row.tipoMovimentacao || "-"}
								</td>
								<td className="px-3 py-3 font-bold text-slate-700">
									{row.nome || "-"}
								</td>
								<td className="whitespace-nowrap px-3 py-3 font-black text-slate-900">
									{metricValue(row.quantidade)} {row.unidade || ""}
								</td>
								<td className="px-3 py-3 font-bold text-slate-700">
									{row.clienteNome || "-"}
								</td>
		...
		<section className="rounded-2xl border border-slate-200 bg-white p-4">
			...
			<div className="mt-3 space-y-2">
				{rows.length ? (
								...
								{showClients && isOpen ? (
									<div className="mt-3 space-y-2 border-t border-slate-200 pt-3">
										{clients.length ? (
											...
											<p className="rounded-lg bg-white p-3 text-xs font-bold text-slate-500">
					...
					<p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
						Nenhum gasto confirmado para este período.
					</p>
				)}
			</div>
			<PaginationControls
...
function HistoryFilterControls({ filter, onChange, loading }) {
	const update = (patch) => onChange({ ...filter, ...patch });
	return (
		<section className="rounded-2xl border border-slate-200 bg-white p-4">
			<div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
				<div>
					<p className="text-xs font-black uppercase tracking-wide text-blue-600">Filtro</p>
					<h3 className="text-lg font-black text-slate-950">Período da auditoria</h3>
					<p className="text-xs font-bold text-slate-500">{periodLabel(filter)}</p>
				</div>
				<div className="flex flex-col gap-2 sm:flex-row sm:items-end">
					...
					{filter.periodo === "diario" ? (
						<input
							type="date"
							value={filter.day || dateKeyLocal()}
							onChange={(event) => update({ day: event.target.value })}
							className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
						/>
					) : null}
					{filter.periodo === "mensal" ? (
						<input
							type="month"
							value={filter.month || monthKeyLocal()}
					...
					{filter.periodo === "anual" ? (
						<input
							...
							className="h-11 w-28 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
						/>
					) : null}
					{loading ? (
						<span className="inline-flex h-11 items-center rounded-xl bg-blue-50 px-3 text-xs font-black text-blue-700">
							Carregando...
						</span>
					) : null}
				</div>
...
function DailyHistoryCalendar({ days = [], selectedDay, onSelectDay, visibleMonth, onVisibleMonthChange }) {
	if (!days.length) return null;
	const safeVisibleMonth = visibleMonth || selectedDay?.slice(0, 7) || days[0]?.date?.slice(0, 7) || monthKeyLocal();
	const visibleDays = days.filter((day) => String(day.date || "").startsWith(safeVisibleMonth));
	return (
		<section className="rounded-2xl border border-slate-200 bg-white p-4">
			<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				<div>
					<h3 className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-wide text-slate-500">
						<CalendarDays size={16} /> Calendário de gastos
					</h3>
					<p className="mt-1 text-xs font-bold capitalize text-slate-500">{monthLabel(safeVisibleMonth)}</p>
				</div>
				<input
					type="month"
					value={safeVisibleMonth}
					onChange={(event) => onVisibleMonthChange?.(event.target.value)}
					className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
				/>
			</div>
			<div className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-7 md:grid-cols-10 lg:grid-cols-[repeat(14,minmax(0,1fr))]">
				{visibleDays.map((day) => {
					const hasMovement =
						Number(day.movimentacoes || 0) > 0 ||
						Number(day.estimadoUnidades || 0) > 0 ||
						Number(day.estimadoMetros || 0) > 0;
					...
					return (
						...
						>
							...
							<span className="mt-1 block text-[10px]">
								{metricValue(day.unidades)} un · {metricValue(day.metros)} m
							</span>
						</button>
					);
				})}
			...
			{!visibleDays.length ? (
				<p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-400">
					Nenhum dia carregado para este mês.
				</p>
			) : null}
		</section>
	);
...
function TecnicoModal({ tecnico, onClose, onRefresh, refreshing }) {
	...
	const monthlyHistory = historyResponse?.history || snapshot.monthlyHistory || {};
	const days = monthlyHistory.dias || [];
	const [visibleMonth, setVisibleMonth] = useState(() => historyFilter.month || monthKeyLocal());
	const [selectedDay, setSelectedDay] = useState(
		() =>
			findDefaultHistoryDay(days),
	);
	...
	useEffect(() => {
		...
		buscarHistoricoAuditoriaBolsa({
			tipo: "tecnico",
			id: tecnico.snapshotId || tecnico.id,
			...query,
		})
			.then((response) => {
				if (!alive) return;
				const responseDays = response?.history?.dias || [];
				const defaultDay = findDefaultHistoryDay(responseDays);
				setHistoryResponse(response);
				setSelectedDay(defaultDay);
				setVisibleMonth(defaultDay?.slice(0, 7) || query.dataInicio?.slice(0, 7) || monthKeyLocal());
			})
			.catch((error) => {
				if (!alive) return;
	...
	}, [tecnico.id, tecnico.snapshotId, historyFilter]);
		...
		>
			<div className="overflow-hidden">
				<header className="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
					<div className="min-w-0">
						...
						<p className="mt-1 text-sm font-bold text-slate-500">
							{tecnico.empresaNome} ·{" "}
							{tecnico.emailHubsoft || "sem e-mail Hubsoft"}
						</p>
						<p className="mt-1 text-xs font-black uppercase tracking-wide text-blue-600">
							{historyLoading
								? "Carregando histórico..."
								: periodLabel(historyFilter)}
						</p>
					</div>
					<div className="flex flex-wrap items-center gap-2">
						<button
							type="button"
							onClick={() => exportTecnicoPdf(tecnico)}
						...
						<button
							type="button"
							disabled={refreshing}
							onClick={() => onRefresh(tecnico)}
							className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-black text-white hover:bg-blue-700 disabled:opacity-60"
						>
							<RefreshCw
								size={16}
				...
				<div className="grid gap-4 p-5 md:grid-cols-4">
					...
					<MetricCard
						label="Unidades"
						value={metricValue(snapshot.totals?.unidades)}
						tone="green"
					/>
					<MetricCard
						label="Metragem"
						value={`${metricValue(snapshot.totals?.metros)} m`}
						tone="amber"
					/>
					<MetricCard
						label="Alertas"
						value={snapshot.alerts?.length || 0}
						tone={snapshot.alerts?.length ? "red" : "blue"}
					/>
					<MetricCard
						label="Gasto no período"
						value={metricValue(monthlyHistory.consumos)}
						helper="Movimentações"
						tone="green"
					/>
					<MetricCard
						label="Itens período"
						value={metricValue(monthlyHistory.unidades)}
						helper="Confirmados"
						tone="blue"
					/>
					<MetricCard
						label="Metros período"
						value={`${metricValue(monthlyHistory.metros)} m`}
						helper="Confirmados"
						tone="amber"
					/>
					<MetricCard
						label="Histórico"
						value={metricValue(monthlyHistory.movimentacoes)}
						helper="Mov. salvas"
					/>
				</div>
				...
				{historyError ? (
					<div className="mx-5 mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold text-amber-800">
						{historyError} Exibindo o período mensal carregado no dashboard.
					</div>
				) : null}
					...
					<section className="rounded-2xl border border-slate-200 bg-white p-4">
						...
						<dl className="mt-4 space-y-3 text-sm">
							<div className="flex justify-between gap-3">
								<dt className="font-bold text-slate-500">Última atualização</dt>
								<dd className="font-black text-slate-900">
									{formatDate(snapshot.lastUpdatedAt)}
								</dd>
							</div>
							<div className="flex justify-between gap-3">
								<dt className="font-bold text-slate-500">ID Sênior</dt>
								<dd className="font-black text-slate-900">
									{snapshot.seniorId || "-"}
								</dd>
							</div>
							<div className="flex justify-between gap-3">
								...
								<dd className="font-black text-slate-900">
									{snapshot.diagnostics?.rawItemCount ?? "-"}
								</dd>
							</div>
							<div className="flex justify-between gap-3">
								<dt className="font-bold text-slate-500">
								...
								<dd className="font-black text-slate-900">
									{snapshot.diagnostics?.parsedItemCount ?? "-"}
								</dd>
							</div>
							<div className="flex justify-between gap-3">
								<dt className="font-bold text-slate-500">
									Saldo positivo bruto
								</dt>
								<dd className="font-black text-slate-900">
									{snapshot.diagnostics?.rawPositiveItemCount ?? "-"}
								</dd>
							</div>
							<div className="flex justify-between gap-3">
								<dt className="font-bold text-slate-500">Cidade</dt>
								<dd className="font-black text-slate-900">
									{tecnico.cidade || "-"}
								</dd>
							</div>
				...
				<div className="grid gap-5 p-5 pt-0 lg:grid-cols-2">
					<MonthlyConsumptionList
						title="Gastos do período por categoria"
						rows={monthlyHistory.categorias || []}
						showClients
					/>
					<MonthlyConsumptionList
						title="Gastos do período por item"
						rows={monthlyHistory.itens || []}
					/>
				</div>
				...
				{selectedDayData ? (
					<div className="grid gap-5 p-5 pt-0 lg:grid-cols-[0.85fr_1.15fr]">
						<section className="rounded-2xl border border-slate-200 bg-white p-4">
							<h3 className="text-sm font-black uppercase tracking-wide text-slate-500">
								Match do dia {formatDayLabel(selectedDayData.date)}
							</h3>
							<dl className="mt-4 space-y-3 text-sm">
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">Playground itens</dt>
									<dd className="font-black text-slate-900">
										{metricValue(selectedDayData.unidades)}
									</dd>
								</div>
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">
									...
									<dd className="font-black text-slate-900">
										{metricValue(selectedDayData.metros)} m
									</dd>
								</div>
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">
										Diferença bolsa itens
									</dt>
									<dd className="font-black text-slate-900">
										{metricValue(selectedDayData.estimadoUnidades)}
									</dd>
								</div>
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">
										Diferença bolsa metros
									</dt>
									<dd className="font-black text-slate-900">
										{metricValue(selectedDayData.estimadoMetros)} m
									</dd>
								</div>
							</dl>
							<div
								className={`mt-4 rounded-2xl p-4 text-sm font-black ${selectedDayData.bate ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}
							>
								{selectedDayData.bate
									? "Bate com a diferença da bolsa."
									: "Não bate com a diferença da bolsa. Verifique as movimentações."}
							</div>
						</section>
						<section className="rounded-2xl border border-slate-200 bg-white p-4">
					...
					<div className="rounded-2xl border border-slate-200 bg-white p-4">
						<div className="mb-4 flex flex-wrap items-center justify-between gap-2">
							...
							<span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700">
								{formatDate(monthlyHistory.periodoInicio)} até{" "}
								{formatDate(monthlyHistory.periodoFim)}
							</span>
						</div>
						<MonthlyHistoryTable
							rows={monthlyHistory.ultimasMovimentacoes || []}
						/>
					</div>
				</section>
			</div>
...
function EmpresaHistoryModal({ empresa, onClose }) {
	const [historyFilter, setHistoryFilter] = useState(() => initialHistoryFilter());
	const [response, setResponse] = useState(null);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState("");
	const history = response?.history || {};
	const days = history.dias || [];
	const [visibleMonth, setVisibleMonth] = useState(() => historyFilter.month || monthKeyLocal());
	const [selectedDay, setSelectedDay] = useState("");
	const selectedDayData = days.find((day) => day.date === selectedDay) || null;
	...
	useEffect(() => {
		...
		buscarHistoricoAuditoriaBolsa({
			...
			.then((result) => {
				if (!alive) return;
				const resultDays = result?.history?.dias || [];
				const defaultDay = findDefaultHistoryDay(resultDays);
				setResponse(result);
				setSelectedDay(defaultDay);
				setVisibleMonth(defaultDay?.slice(0, 7) || query.dataInicio?.slice(0, 7) || monthKeyLocal());
			})
			.catch((err) => {
				if (alive) setError(err?.message || "Não foi possível carregar o histórico da empresa.");
			})
			.finally(() => {
		...
		<ModalShell onClose={onClose} showClose={false} size="6xl" bodyClassName="p-0">
			<div className="overflow-hidden">
				<header className="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
					<div className="min-w-0">
						<p className="text-xs font-black uppercase tracking-[0.22em] text-blue-600">
							Bolsa Técnico por empresa
						</p>
						<h2 className="mt-1 truncate text-2xl font-black text-slate-950">
							{empresa.nome}
						</h2>
						<p className="mt-1 text-sm font-bold text-slate-500">
							{empresa.regional || "-"} · {periodLabel(historyFilter)}
						</p>
					</div>
					<button
						type="button"
				...
				<div className="grid gap-4 p-5 md:grid-cols-4">
					<MetricCard label="Técnicos" value={response?.target?.tecnicos || empresa.tecnicos?.length || 0} />
					<MetricCard label="Consumos" value={metricValue(history.consumos)} tone="green" />
					<MetricCard label="Unidades" value={metricValue(history.unidades)} tone="blue" />
					<MetricCard label="Metros" value={`${metricValue(history.metros)} m`} tone="amber" />
				</div>
				{loading ? (
					<div className="mx-5 mb-5 rounded-2xl border border-blue-100 bg-blue-50 p-5 text-sm font-black text-blue-700">
						Carregando histórico anual da empresa...
					</div>
				) : null}
				<div className="grid gap-5 p-5 pt-0 lg:grid-cols-2">
					<MonthlyConsumptionList title="Gastos do período por categoria" rows={history.categorias || []} showClients />
					<MonthlyConsumptionList title="Gastos do período por item" rows={history.itens || []} />
				</div>
				<div className="p-5 pt-0">
					<DailyHistoryCalendar
						days={days}
					...
					<div className="grid gap-5 p-5 pt-0 lg:grid-cols-[0.85fr_1.15fr]">
						<section className="rounded-2xl border border-slate-200 bg-white p-4">
							...
							<dl className="mt-4 space-y-3 text-sm">
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">Playground itens</dt>
									<dd className="font-black text-slate-900">{metricValue(selectedDayData.unidades)}</dd>
								</div>
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">Playground metros</dt>
									<dd className="font-black text-slate-900">{metricValue(selectedDayData.metros)} m</dd>
								</div>
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">Diferença bolsa itens</dt>
									<dd className="font-black text-slate-900">{metricValue(selectedDayData.estimadoUnidades)}</dd>
								</div>
								<div className="flex justify-between gap-3">
									<dt className="font-bold text-slate-500">Diferença bolsa metros</dt>
									<dd className="font-black text-slate-900">{metricValue(selectedDayData.estimadoMetros)} m</dd>
								</div>
							</dl>
				...
				<section className="p-5 pt-0">
					<div className="rounded-2xl border border-slate-200 bg-white p-4">
						...
						<MonthlyHistoryTable rows={history.ultimasMovimentacoes || []} />
					</div>
				</section>
			</div>
		</ModalShell>
...
function ConfigModal({
	...
	const addThreshold = () => {
		...
		setForm((current) => ({
			...current,
			itemThresholds: [...current.itemThresholds, threshold],
		}));
					...
					<label>
						...
						<input
							...
							onChange={(event) =>
								setForm((current) => ({
									...current,
									dailyRunTime: event.target.value,
								}))
							}
					...
					<div className="mt-3 grid gap-3 md:grid-cols-2">
						{form.categoryThresholds.map((category, index) => (
							...
							>
								<span className="mb-1.5 block text-xs font-black uppercase tracking-wide text-slate-500">
									{category.label} ·{" "}
									{category.tipo === "metragem" ? "metros" : "unidades"}
								</span>
								<input
									type="number"
									min="0"
									...
									onChange={(event) =>
										setForm((current) => ({
											...current,
											categoryThresholds: current.categoryThresholds.map(
												(item, itemIndex) =>
													itemIndex === index
														? { ...item, limite: event.target.value }
														: item,
											),
										}))
									}
				...
				<div className="border-t border-slate-100 p-5">
					...
					<div className="mt-3 space-y-2">
						{form.itemThresholds.map((item, index) => (
							...
							>
								<span>
									{item.nome} · {item.quantidade} · {item.tipo}
								</span>
								<button
									type="button"
									onClick={() =>
										setForm((current) => ({
											...current,
											itemThresholds: current.itemThresholds.filter(
												(_, i) => i !== index,
											),
										}))
									}
					...
					<div className="mx-5 mb-5 rounded-2xl border border-blue-200 bg-blue-50 p-4">
						...
						<div className="mt-3 h-2 overflow-hidden rounded-full bg-blue-100">
							<div
								...
								style={{
									width: `${Math.min(100, Math.max(0, Number(job.percent || 0)))}%`,
								}}
							/>
						</div>
						<p className="mt-2 text-xs font-bold text-blue-700">
							{Number(job.processed || 0)} de {Number(job.total || 0)}{" "}
							técnico(s) processado(s).
						</p>
					</div>
				) : null}
				<footer className="flex flex-col gap-3 border-t border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
					...
					<div className="flex justify-end gap-3">
						...
						<button
							...
							disabled={saving}
							onClick={() => onSave(form)}
							className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-black text-white disabled:opacity-60"
						>
				...
				<div className="max-h-[70vh] overflow-y-auto p-5">
							...
							>
								<div className="flex flex-wrap justify-between gap-2">
									<p className="font-black text-slate-950">
										{log.tecnicoNome || log.type}
									</p>
									<span
										className={`rounded-full px-3 py-1 text-xs font-black ${log.status === "erro" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}
									>
										{log.status || "-"}
									</span>
								</div>
								<p className="mt-1 font-bold text-slate-500">
									{formatDate(log.createdAt)} · {log.empresaNome || "-"} ·{" "}
									{log.regional || "-"}
								</p>
								{log.error ? (
									<p className="mt-2 font-bold text-red-600">{log.error}</p>
								) : null}
								{log.alerts?.length ? (
									<p className="mt-2 font-bold text-amber-700">
										Alertas: {log.alerts.join(" | ")}
									</p>
								) : null}
								{log.alertReason ? (
									<p className="mt-2 font-bold text-slate-500">
										Envio: {log.alertReason}
									</p>
								) : null}
							</article>
						))}
					...
					<div className="mt-5 flex items-center justify-between gap-3 text-sm font-black text-slate-600">
						<button
							...
							onClick={() => {
								setLoading(true);
								setLogs((current) => ({ ...current, page: current.page - 1 }));
							}}
						...
						<span>
							Página {logs.page} de {logs.totalPages}
						</span>
						<button
							type="button"
...
export default function TecnicosBolsaAuditoriaPage() {
	...
	const [selectedEmpresaHistory, setSelectedEmpresaHistory] = useState(null);
	...
	const load = async () => {
		...
		try {
			...
			setSelectedEmpresaId(
				(current) => current || dashboard.empresas?.[0]?.id || "",
			);
		} finally {
			setLoading(false);
		}
	...
	useEffect(() => {
		load().catch((error) =>
			setMessage(error?.message || "Não foi possível carregar a auditoria."),
		);
	}, []);

	const selectedEmpresa = useMemo(
		() =>
			data.empresas.find((empresa) => empresa.id === selectedEmpresaId) ||
			data.empresas[0] ||
			null,
	...
	const handleRefreshAll = async () => {
		...
		try {
			...
			setMessage(
				result?.errors
					? `Leitura finalizada com ${result.errors} erro(s) e ${result.success || 0} técnico(s) atualizado(s).`
					: "Leitura geral finalizada.",
			);
			setRefreshJob((current) =>
				current
					? {
							...
							status: "completed",
							stage: result?.errors
								? `Atualização finalizada com ${result.errors} erro(s)`
	...
	return (
		<main className="space-y-5">
			...
			{selectedEmpresaHistory ? (
				<EmpresaHistoryModal
					empresa={selectedEmpresaHistory}
					onClose={() => setSelectedEmpresaHistory(null)}
				/>
			) : null}
			{showConfig ? (
				<ConfigModal
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<h1 className="text-2xl font-black text-slate-950">
								Auditoria de Bolsa Técnico
							</h1>
							<p className="text-sm font-semibold text-slate-500">
								Consulta materiais por e-mail Hubsoft na API Sempre e grava
				...
				<section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 shadow-sm">
					<div className="flex flex-wrap items-center justify-between gap-3">
						<div>
							<p className="text-sm font-black text-blue-950">
								{refreshJob.stage || "Atualizando API Sempre"}
							</p>
							<p className="text-xs font-bold text-blue-700">
								{Number(refreshJob.processed || 0)} de{" "}
								{Number(refreshJob.total || 0)} técnico(s) processado(s)
								{refreshJob.current?.tecnicoNome
									? ` · ${refreshJob.current.tecnicoNome}`
									: ""}
							</p>
							{refreshJob.success || refreshJob.errors ? (
								<p className="mt-1 text-xs font-bold text-blue-700">
									{Number(refreshJob.success || 0)} atualizado(s),{" "}
									{Number(refreshJob.errors || 0)} com erro.
								</p>
							) : null}
							{refreshJob.error ? (
					...
					<div className="mt-3 h-2 overflow-hidden rounded-full bg-blue-100">
						<div
							className="h-full rounded-full bg-blue-600 transition-all"
							style={{
								width: `${Math.min(100, Math.max(0, Number(refreshJob.percent || 0)))}%`,
							}}
						/>
					</div>
				</section>
			...
			<section className="grid gap-4 md:grid-cols-4">
				<MetricCard
					label="Empresas"
					value={data.summary?.empresas || 0}
					helper="Com técnicos visíveis"
				/>
				<MetricCard
					label="Técnicos"
					value={data.summary?.tecnicos || 0}
					helper="Cadastrados para auditoria"
					tone="green"
				/>
				<MetricCard
					label="Atualizados"
					value={data.summary?.atualizados || 0}
					helper="Com leitura salva"
					tone="amber"
				/>
				<MetricCard
					label="Alertas"
					value={data.summary?.alertas || 0}
					helper="Gerados na última leitura"
					tone={data.summary?.alertas ? "red" : "blue"}
				/>
			</section>
			...
			<section className="grid gap-5 lg:grid-cols-[320px_1fr]">
							...
							>
								<div className="flex items-center gap-3">
									...
									<div className="min-w-0">
										...
										<p className="text-xs font-bold text-slate-500">
											{empresa.regional || "-"} · {empresa.tecnicos.length}{" "}
											técnico(s)
										</p>
									</div>
								</div>
				...
				<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
					<div className="flex flex-wrap items-center justify-between gap-3">
						<div>
							<h2 className="text-xl font-black text-slate-950">
								{selectedEmpresa?.nome || "Técnicos"}
							</h2>
							<p className="text-sm font-bold text-slate-500">
								{selectedEmpresa?.regional || "-"}
							</p>
						</div>
						{loading ? (
						...
						{selectedEmpresa ? (
							<button
								type="button"
								onClick={() => setSelectedEmpresaHistory(selectedEmpresa)}
								className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700 hover:bg-slate-50"
							>
								<BarChart3 size={16} /> Ver empresa no ano
							</button>
						) : null}
					</div>

					<div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
						{selectedEmpresa?.tecnicos?.length ? (
							selectedEmpresa.tecnicos.map((tecnico) => {
								const snapshot = tecnico.snapshot || {};
								return (
									<article
										key={tecnico.snapshotId}
										className="rounded-2xl border border-slate-200 bg-slate-50 p-4"
									>
										...
										<div className="mt-4 grid grid-cols-3 gap-2">
											...
											<MetricCard
												label="M"
												value={metricValue(snapshot.totals?.metros)}
												tone="amber"
											/>
										</div>
										...
										{snapshot.diagnostics ? (
											<p className="mt-1 text-xs font-bold text-slate-500">
												API: {snapshot.diagnostics.rawItemCount || 0}{" "}
												recebido(s), {snapshot.diagnostics.parsedItemCount || 0}{" "}
												com saldo.
												{snapshot.diagnostics.rawPositiveItemCount !== undefined
													? ` Bruto: ${snapshot.diagnostics.rawPositiveItemCount}.`
													: ""}
											</p>
										) : null}
										{snapshot.lastError ? (
										...
										<div className="mt-4 flex gap-2">
											<button
												...
												className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm font-black text-slate-700 hover:bg-slate-50"
											>
											...
											{manage ? (
												<button
													type="button"
													disabled={refreshing === tecnico.snapshotId}
													onClick={() => handleRefreshTecnico(tecnico)}
													className="inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-600 px-3 text-white hover:bg-blue-700 disabled:opacity-60"
												>
													<RefreshCw
														size={16}
														className={
															refreshing === tecnico.snapshotId
																? "animate-spin"
																: ""
														}
													/>
