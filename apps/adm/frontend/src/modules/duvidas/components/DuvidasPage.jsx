...
export default function DuvidasPage() {
	...
	const handleSaveDuvida = async (form) => {
		const nextDuvidas = form.id
			? conteudo.duvidas.map((item) =>
					item.id === form.id ? { ...item, ...form } : item,
				)
			: [{ ...form, id: `duvida-${Date.now()}` }, ...conteudo.duvidas];
	...
	const handleSaveContato = async (form) => {
		...
		if (form.id && contatoModal?.grupo && contatoModal.grupo !== grupo) {
			nextEquipe[contatoModal.grupo] = (
				conteudo.equipe?.[contatoModal.grupo] || []
	...
	const handleDeleteContato = async (grupo, id) => {
		await replaceContent({
			equipe: {
				...conteudo.equipe,
				[grupo]: (conteudo.equipe?.[grupo] || []).filter(
					(item) => item.id !== id,
				),
			},
		});
															...
															{podeEditar ? (
																<div className="flex shrink-0 items-center gap-1">
																	...
																	<button
																		type="button"
																		onClick={() =>
																			handleDeleteContato(groupKey, item.id)
																		}
																		className="rounded-xl p-2 text-gray-400 hover:bg-red-50 hover:text-red-600"
																	>
																		<Trash2 size={15} />
