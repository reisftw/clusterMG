...
const AgendamentoModal = ({
	...
	useEffect(() => {
		...
		const timer = window.setTimeout(async () => {
			...
			try {
				...
				setForm((prev) => {
					if (String(prev.codigo_cliente || "").replace(/\D/g, "") !== codigo)
						return prev;
					return {
						...prev,
						...
						observacao:
							prev.observacao ||
							[
								cliente.num_os ? `O.S ${cliente.num_os}` : "",
								cliente.regional || "",
							]
								.filter(Boolean)
								.join(" - "),
	...
	const handleSalvar = async () => {
		...
		if (
			!form.tecnico_nome.trim() ||
			!form.codigo_cliente.trim() ||
			!form.data
		) {
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4 backdrop-blur-sm">
			<div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl border border-gray-100 bg-white shadow-2xl">
				...
				<div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
					<div>
						<label
							htmlFor="agendamento-tecnico-nome"
							className="mb-1.5 block text-xs font-semibold text-gray-600"
						>
							Tecnico *
						</label>
						<input
							id="agendamento-tecnico-nome"
					...
					<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
						<div>
							<label
								htmlFor="agendamento-codigo-cliente"
								className="mb-1.5 block text-xs font-semibold text-gray-600"
							>
								Codigo do cliente *
							</label>
							<input
								id="agendamento-codigo-cliente"
								className={inputClass}
								inputMode="numeric"
						...
						<div>
							<label
								htmlFor="agendamento-cliente-nome"
								className="mb-1.5 block text-xs font-semibold text-gray-600"
							>
								Nome do cliente
							</label>
							<input
								id="agendamento-cliente-nome"
								className={inputClass}
					...
					<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
						<div>
							<label
								htmlFor="agendamento-cidade"
								className="mb-1.5 block text-xs font-semibold text-gray-600"
							>
								Cidade
							</label>
							<input
								id="agendamento-cidade"
								className={inputClass}
								placeholder="Cidade"
								value={form.cidade}
					...
					<div>
						<label
							htmlFor="agendamento-observacao"
							className="mb-1.5 block text-xs font-semibold text-gray-600"
						>
							Observacao
						</label>
