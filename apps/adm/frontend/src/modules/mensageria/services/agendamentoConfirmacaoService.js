...
export async function executarConfirmacaoAgendamentos({
	forceMorning = false,
} = {}) {
...
export async function preencherResponsavelConfirmacaoAgendamentos(
	trackId,
	payload = {},
) {
	return requestVpsApi(
		`/agendamentos/confirmacao/envios/${encodeURIComponent(trackId)}/responsavel`,
		{
			method: "POST",
			body: JSON.stringify(payload),
		},
...
export async function conectarEvolutionConfirmacaoAgendamentos() {
	return requestVpsApi("/agendamentos/confirmacao/evolution/connect", {
		method: "POST",
	});
}

export async function desconectarEvolutionConfirmacaoAgendamentos() {
	return requestVpsApi("/agendamentos/confirmacao/evolution/disconnect", {
		method: "POST",
	});
}

export async function configurarWebhookEvolutionConfirmacaoAgendamentos(
	webhookUrl,
) {
...
export async function buscarEnviosConfirmacaoAgendamentos({
	limit = 20,
	offset = 0,
} = {}) {
	...
	const response = await requestVpsApi(
		`/agendamentos/confirmacao/envios?${params.toString()}`,
	);
	return response?.items || [];
}

export async function buscarLogsConfirmacaoAgendamentos({
	limit = 20,
	offset = 0,
} = {}) {
	const params = new URLSearchParams({
	...
	const response = await requestVpsApi(
		`/agendamentos/confirmacao/logs?${params.toString()}`,
	);
	return response?.items || [];
}

export async function buscarPreviaConfirmacaoAgendamentos({
	dateKey = "",
	daysAhead = 1,
} = {}) {
	const params = new URLSearchParams();
	...
	return requestVpsApi(
		`/agendamentos/confirmacao/preview?${params.toString()}`,
	);
}
