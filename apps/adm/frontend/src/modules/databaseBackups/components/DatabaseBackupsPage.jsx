...
const DatabaseBackupsPage = () => {
	...
	const canRestore =
		confirmation === CONFIRMATION_TEXT && selectedBackup?.fileName;
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
					<div>
						...
						<h1 className="mt-2 text-2xl font-bold text-slate-900">
							Backups do PostgreSQL Administrativo
						</h1>
						<p className="mt-1 max-w-3xl text-sm text-slate-500">
							Backup automático diário às 00:01, retenção de 30 arquivos e
							limite total de 10GB. O rollback cria um backup de segurança antes
							de restaurar o arquivo escolhido no banco dedicado do ADM.
						</p>
					</div>
			...
			<section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-3 text-2xl font-bold text-slate-900">
						{status?.database?.pretty ||
							formatBytes(status?.database?.sizeBytes)}
					</p>
					<p className="mt-1 text-xs text-slate-500">
						Banco: {status?.database?.name || "adm"}.
					</p>
				</div>
				...
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-3 text-2xl font-bold text-slate-900">
						{status?.retention?.currentBackups || 0}/
						{status?.retention?.maxBackups || 30}
					</p>
					<p className="mt-1 text-xs text-slate-500">
				...
				<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
					...
					<p className="mt-1 text-xs text-slate-500">
						Pasta:{" "}
						{status?.storage?.backupDir || "/opt/retiradas/backups/adm-postgres"}
						{" · "}
						{status?.storage?.encryptionEnabled
							? "Criptografado"
							: "Sem criptografia"}
					</p>
				</div>
				...
				<div className="flex flex-col gap-2 border-b border-slate-100 p-4 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<p className="text-xs text-slate-500">
							Escolha um arquivo apenas quando precisar voltar o banco para
							aquele momento.
						</p>
					</div>
				</div>
