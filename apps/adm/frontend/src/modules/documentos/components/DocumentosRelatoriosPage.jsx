...
function hasRetrabalho(submission) {
	return (
		String(submission.status || "").toLowerCase() === "reprovado" ||
...
function buildMetrics(submissions) {
	...
	submissions.forEach((submission) => {
		...
		const submissionReviewed = isReviewedSubmission(submission);
		const rework = hasRetrabalho(submission);
		const pendingSupervisor =
			String(submission.status || "").toLowerCase() === "pendente";
		...
		const fileReviewed = documentFiles.some(
			(file) => isApprovedFile(file) || isRejectedFile(file),
		);
		const reviewed = submissionReviewed || fileReviewed;
		const approved =
			String(submission.status || "").toLowerCase() === "aprovado" ||
			(Boolean(documentFiles.length) &&
				documentFiles.every(isApprovedFile) &&
				!documentFiles.some(isRejectedFile));
		...
		if (pendingSupervisor) {
			...
			if (submittedAt && !Number.isNaN(submittedAt.getTime())) {
				base.tempoPendenteSupervisorTotalMs += Math.max(
					0,
		...
		invoiceFiles.forEach((file) => {
			...
			if (mk) {
				const monthInvoice = invoiceMonths.get(mk) || {
					key: mk,
					label: monthLabel(mk),
					notas: 0,
					valor: 0,
				...
				const yearInvoice = invoiceYears.get(year) || {
					key: year,
					label: year,
					notas: 0,
					valor: 0,
		...
		documentFiles.forEach((file) => {
			if (isApprovedFile(file) || isRejectedFile(file))
				base.documentosAnalisados += 1;
			if (isRejectedFile(file)) {
				...
				const field = fields.get(fieldKey) || {
					id: fieldKey,
					nome:
						file.fieldNome || file.tipo || file.nome || "Documento sem campo",
					reprovacoes: 0,
				};
				field.reprovacoes += 1;
	...
	const companyRows = [...companies.values()].map((company) => ({
		...company,
		taxaRetrabalho: company.analisados
			? company.retrabalho / company.analisados
			: 0,
		taxaAcerto: company.analisados
			? (company.analisados - company.retrabalho) / company.analisados
			: 0,
	}));
	const supervisorRows = [...supervisors.values()]
		.map((supervisor) => ({
			...supervisor,
			mediaMs: supervisor.avaliacoes
				? supervisor.totalMs / supervisor.avaliacoes
				: 0,
		}))
	...
	return {
		...
		taxaAcerto: base.analisados
			? (base.analisados - base.retrabalho) / base.analisados
			: 0,
		...
		fields: [...fields.values()]
			...
			.slice(0, 8),
		invoiceCompanies: [...invoiceCompanies.values()].sort(
			(a, b) => b.valor - a.valor,
		),
		invoiceMonths: [...invoiceMonths.values()]
			.sort((a, b) => a.key.localeCompare(b.key))
			.slice(-12),
		invoiceYears: [...invoiceYears.values()].sort((a, b) =>
			a.key.localeCompare(b.key),
		),
		supervisorRows,
		tempoMedioSupervisorMs: base.tempoSupervisorCount
			? base.tempoSupervisorTotalMs / base.tempoSupervisorCount
			: 0,
		tempoMedioPendenteSupervisorMs: base.aguardandoSupervisor
			? base.tempoPendenteSupervisorTotalMs / base.aguardandoSupervisor
			: 0,
	};
}
			...
			<div className="flex items-center justify-between gap-3">
				<div className="min-w-0">
					...
					<p className="text-xs font-semibold text-slate-500">
						{item.regional} · {item.avaliacoes} avaliação(ões)
					</p>
				</div>
				<span className="text-lg font-black text-slate-950">
					{durationLabel(item.mediaMs)}
				</span>
			</div>
			<div className="mt-3 h-2 overflow-hidden rounded-full bg-white">
				<div
...
export default function DocumentosRelatoriosPage() {
	...
	const load = async () => {
		...
		try {
			...
			setMessage(
				error?.message ||
					"Não foi possível carregar os relatórios administrativos.",
			);
		} finally {
			setLoading(false);
		}
	...
	const periods = useMemo(() => {
		const keys = [
			...new Set(
				submissions
					.map((item) => monthKey(item.mesReferencia || item.submittedAt))
					.filter(Boolean),
			),
		]
	...
	const filtered = useMemo(() => {
		...
		return submissions.filter((submission) => {
			if (
				period !== "todos" &&
				monthKey(submission.mesReferencia || submission.submittedAt) !== period
			)
				return false;
			if (!term) return true;
			return normalize(
				`${submission.empresaNome} ${submission.regional} ${submission.supervisorNome}`,
			).includes(term);
		});
	}, [period, search, submissions]);

	const metrics = useMemo(() => buildMetrics(filtered), [filtered]);
	const maxRetrabalho = Math.max(
		1,
		...metrics.companies.map((company) => company.retrabalho),
	);
	const maxAcertos = Math.max(
		1,
		...metrics.companies.map((company) => company.aprovados),
	);
	const maxMonth = Math.max(1, ...metrics.months.map((item) => item.envios));
	const maxField = Math.max(
		1,
		...metrics.fields.map((field) => field.reprovacoes),
	);
	const fastestSupervisors = [...metrics.supervisorRows]
		.sort((a, b) => a.mediaMs - b.mediaMs)
		.slice(0, 6);
	const slowestSupervisors = [...metrics.supervisorRows]
		.sort((a, b) => b.mediaMs - a.mediaMs)
		.slice(0, 6);
	const maxSupervisorDuration = Math.max(
		1,
		...metrics.supervisorRows.map((item) => item.mediaMs),
	);
	...
	const pageItems = filteredCompanies.slice(
		(page - 1) * pageSize,
		page * pageSize,
	);
	...
	return (
		<div className="space-y-6">
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="mt-1 text-sm font-semibold text-slate-500">
								Analise produtividade, retrabalho, empresas com maior acerto e
								pontos de atenção.
							</p>
						</div>
					</div>
			...
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
				<KpiCard
					icon={FileText}
					label="Envios analisados"
					value={metrics.analisados}
					description={`${metrics.envios} envio(s) no período`}
				/>
				<KpiCard
					icon={FileCheck2}
					label="Documentos analisados"
					value={metrics.documentosAnalisados}
					description={`${metrics.documentos} arquivo(s) enviados`}
					tone="green"
				/>
				<KpiCard
					icon={AlertTriangle}
					label="Retrabalho"
					value={metrics.retrabalho}
					description={`${percent(metrics.retrabalho, metrics.analisados)} dos envios analisados`}
					tone="red"
				/>
				<KpiCard
					icon={TrendingUp}
					...
					value={percent(
						metrics.analisados - metrics.retrabalho,
						metrics.analisados,
					)}
					description={`${metrics.aprovados} envio(s) aprovados`}
					tone="green"
				/>
				<KpiCard
					icon={Clock3}
					label="Aguardando supervisor"
					value={metrics.aguardandoSupervisor}
					description={`Média parada: ${durationLabel(metrics.tempoMedioPendenteSupervisorMs)}`}
					tone="amber"
				/>
				<KpiCard
					icon={Clock3}
					label="Tempo médio operacional"
					value={durationLabel(metrics.tempoMedioSupervisorMs)}
					description={`${metrics.tempoSupervisorCount} avaliação(ões) com tempo medido`}
					tone="blue"
				/>
				<KpiCard
					icon={ReceiptText}
					label="Notas fiscais"
					value={metrics.notasFiscais}
					description={`${formatCurrency(metrics.valorNotasFiscais)} em notas`}
					tone="amber"
				/>
			</section>
			...
			<section className="grid items-start gap-5 xl:grid-cols-[1.2fr_0.8fr]">
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<div className="mb-5 flex items-center justify-between gap-3">
						<div>
							...
							<p className="text-sm font-semibold text-slate-500">
								Envios, aprovações e retrabalho nos últimos meses.
							</p>
						</div>
					...
					<div className="space-y-4">
						{metrics.months.length ? (
							metrics.months.map((item) => (
								<div
									key={item.key}
									className="grid gap-2 sm:grid-cols-[120px_1fr_110px] sm:items-center"
								>
									...
									<div className="h-4 overflow-hidden rounded-full bg-slate-100">
										<div
											...
											style={{
												width: `${Math.max(4, (item.envios / maxMonth) * 100)}%`,
											}}
										/>
									</div>
									...
									<div className="sm:col-start-2 sm:col-end-4 flex flex-wrap gap-2 text-xs font-black">
										...
										<span className="rounded-full bg-red-50 px-2.5 py-1 text-red-700">
											{item.retrabalho} com retrabalho
										</span>
									</div>
								</div>
							))
				...
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Documentos com maior retrabalho
					</h2>
					<p className="mb-5 text-sm font-semibold text-slate-500">
						Tipos de documento mais reprovados.
					</p>
					<div className="space-y-3">
						{metrics.fields.length ? (
			...
			<section className="grid items-start gap-5 xl:grid-cols-[1.1fr_0.9fr]">
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<div className="mb-5 flex items-center justify-between gap-3">
						<div>
							...
							<p className="text-sm font-semibold text-slate-500">
								Quantidade e valor total anexados aos envios aprovados.
							</p>
						</div>
						<ReceiptText className="text-orange-600" size={22} />
					</div>
					<div className="space-y-3">
						{metrics.invoiceMonths.length ? (
							metrics.invoiceMonths.map((item) => (
								<div
									key={item.key}
									className="grid gap-2 rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:grid-cols-[130px_1fr_140px] sm:items-center"
								>
				...
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Notas fiscais por ano
					</h2>
					<p className="mb-5 text-sm font-semibold text-slate-500">
						Resumo anual dos valores anexados.
					</p>
					<div className="space-y-3">
						{metrics.invoiceYears.length ? (
							metrics.invoiceYears.map((item) => (
								<div
									key={item.key}
			...
			<section className="grid items-start gap-5 xl:grid-cols-2">
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Empresas com mais retrabalho
					</h2>
					<p className="mb-5 text-sm font-semibold text-slate-500">
						Onde a validação mais voltou para correção.
					</p>
					<div className="space-y-3">
						{[...metrics.companies]
				...
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Empresas que mais enviam certo
					</h2>
					<p className="mb-5 text-sm font-semibold text-slate-500">
						Ranking por aprovações e menor retrabalho.
					</p>
					<div className="space-y-3">
						{[...metrics.companies]
							...
							.sort(
								(a, b) =>
									b.taxaAcerto - a.taxaAcerto || b.aprovados - a.aprovados,
							)
							.slice(0, 6)
							.map((company) => (
								<RankingBar
			...
			<section className="grid items-start gap-5 xl:grid-cols-2">
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Supervisores mais rápidos
					</h2>
					<p className="mb-5 text-sm font-semibold text-slate-500">
						Menor tempo médio entre envio e avaliação operacional.
					</p>
					<div className="space-y-3">
						{fastestSupervisors.length ? (
							fastestSupervisors.map((supervisor) => (
				...
				<div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Supervisores que mais demoram
					</h2>
					<p className="mb-5 text-sm font-semibold text-slate-500">
						Maior tempo médio parado antes da avaliação operacional.
					</p>
					<div className="space-y-3">
						{slowestSupervisors.length ? (
							slowestSupervisors.map((supervisor) => (
								<SupervisorDurationBar
									key={supervisor.id}
									item={supervisor}
									max={maxSupervisorDuration}
			...
			<section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
				<div className="flex flex-col gap-2 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-sm font-semibold text-slate-500">
							Visão auditável dos envios, aprovações e correções.
						</p>
					</div>
						...
						<tbody className="divide-y divide-slate-100">
							{pageItems.map((company) => (
								<tr key={company.id} className="hover:bg-slate-50/70">
									{(() => {
										const invoiceCompany = metrics.invoiceCompanies.find(
										...
										return (
											<>
												...
												<td className="px-5 py-4 font-black text-slate-800">
													{invoiceCompany.notas}
												</td>
												<td className="px-5 py-4 font-black text-slate-800">
													{formatCurrency(invoiceCompany.valor)}
												</td>
												<td className="px-5 py-4 font-black text-slate-900">
													{percent(
														company.analisados - company.retrabalho,
														company.analisados,
													)}
												</td>
											</>
