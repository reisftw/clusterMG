...
function normalizeConfig(config = {}) {
	return {
		...
		valorPorEntrega: Number(
			config.valorPorEntrega ?? DEFAULT_CONFIG.valorPorEntrega,
		),
		pagamentoAteDia: Number(
			config.pagamentoAteDia ?? DEFAULT_CONFIG.pagamentoAteDia,
		),
...
function resolveValor(item, config) {
	return item.status === STATUS.VALIDA
		? Number(config.valorPorEntrega || 0)
		: 0;
}
...
function buildRanking(entregas, config) {
	...
	return [...map.values()].sort(
		(a, b) => b.validas - a.validas || b.valor - a.valor,
	);
}
...
function buildReportRows(entregas, config, report) {
	return entregas
		.filter((item) => {
			const tecnicoOk =
				report.tecnico === "Todos" || item.tecnico === report.tecnico;
			const empresaOk =
				report.empresa === "Todas" || item.empresa === report.empresa;
...
async function exportPagamentosRegionaisPdf(
	rows,
	regionais,
	config,
	filters,
			...
			<div className="space-y-4 p-5">
				<div className="rounded-lg border border-blue-100 bg-blue-50 p-4">
					...
					<p className="mt-1 text-sm text-blue-700">
						A competência considera entregas do dia 1 ao último dia do mês. O
						pagamento fica previsto até o dia {draft.pagamentoAteDia} do mês
						seguinte.
					</p>
				</div>
				...
				<div className="grid gap-3 md:grid-cols-2">
					...
					<label className="block">
						...
						<input
							...
							onChange={(event) =>
								handleDraft("statusOsValido", event.target.value)
							}
							className="input-field"
						/>
						<span className="mt-1 block text-xs text-gray-400">
			...
			<div className="flex items-start justify-between gap-4 border-b border-gray-100 p-5">
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-sm text-gray-500">
							Acompanhe valores por regional, técnico, empresa, competência e
							status.
						</p>
					</div>
				</div>
				...
				<div className="grid gap-3 lg:grid-cols-5">
					<label className="block">
						...
						<select
							...
							onChange={(event) =>
								onFilterChange("regional", event.target.value)
							}
							className="input-field"
						>
					...
					<label className="block">
						...
						<select
							...
							onChange={(event) =>
								onFilterChange("tecnico", event.target.value)
							}
							className="input-field"
						>
					...
					<label className="block">
						...
						<select
							...
							onChange={(event) =>
								onFilterChange("empresa", event.target.value)
							}
							className="input-field"
						>
...
export default function EntregasTecnicosPage() {
	...
	const entregasBase = useMemo(
		() =>
			entregasDemo.map((item, index) => ({
				...item,
				regional:
					regionaisNomes[index % regionaisNomes.length] || item.regional,
			})),
		[regionaisNomes],
	);

	const tecnicos = useMemo(
		() => unique(entregasBase.map((item) => item.tecnico)),
		[entregasBase],
	);
	const empresas = useMemo(
		() => unique(entregasBase.map((item) => item.empresa)),
		[entregasBase],
	);
	const meses = useMemo(
		() => unique(entregasBase.map((item) => item.mes)),
		[entregasBase],
	);
	const ranking = useMemo(
		() => buildRanking(entregasBase, config),
		[config, entregasBase],
	);
	const tecnicoAtual =
		ranking.find((item) => item.tecnico === selectedTecnico) || ranking[0];
	...
	const entregasFiltradas = useMemo(() => {
		...
		return entregasBase.filter((item) => {
			const tecnicoOk =
				selectedTecnico === "Todos" || item.tecnico === selectedTecnico;
	...
	const reportSummary = useMemo(
		() => ({
			...
			validas: reportRows.filter((item) => item.status === STATUS.VALIDA)
				.length,
			pendentes: reportRows.filter((item) => item.status === STATUS.PENDENTE)
				.length,
			invalidas: reportRows.filter((item) => item.status === STATUS.INVALIDA)
				.length,
	...
	const resumo = useMemo(
		() => ({
			...
			validas: entregasBase.filter((item) => item.status === STATUS.VALIDA)
				.length,
			pendentes: entregasBase.filter((item) => item.status === STATUS.PENDENTE)
				.length,
			valor: entregasBase.reduce(
				(sum, item) => sum + resolveValor(item, config),
				0,
			),
		}),
		[config, entregasBase],
		...
		<div className="space-y-5">
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-sm text-gray-500">
								Ranking, validações, configuração de pagamento e relatórios por
								técnico ou empresa.
							</p>
						</div>
					</div>
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-sm text-gray-500">
								Contagem do dia 1 ao último dia do mês. Pagamento previsto até o
								dia {config.pagamentoAteDia} do mês seguinte.
							</p>
						</div>
					</div>
				...
				<div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
					...
					<div className="mt-5 grid gap-3 md:grid-cols-4">
						...
						<div className="rounded-lg border border-green-100 bg-green-50 px-3 py-2">
							<p className="text-xs text-green-600">Válidas</p>
							<p className="text-xl font-black text-green-800">
								{tecnicoAtual?.validas}
							</p>
						</div>
						<div className="rounded-lg border border-amber-100 bg-amber-50 px-3 py-2">
							<p className="text-xs text-amber-600">Pendentes</p>
							<p className="text-xl font-black text-amber-700">
								{tecnicoAtual?.pendentes}
							</p>
						</div>
						<div className="rounded-lg border border-red-100 bg-red-50 px-3 py-2">
							<p className="text-xs text-red-600">Inválidas</p>
							<p className="text-xl font-black text-red-700">
								{tecnicoAtual?.invalidas}
							</p>
						</div>
					</div>
				</div>
			...
			<section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
				<div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
					...
					<div className="flex flex-wrap gap-2">
						...
						<button
							type="button"
							onClick={() =>
								exportPdf(reportRows, config, report, reportSummary)
							}
							className="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-bold text-white hover:bg-gray-800"
						>
							<Download size={16} /> Baixar PDF
				...
				<div className="mt-4 grid gap-3 md:grid-cols-5">
					...
					<div className="rounded-lg border border-red-100 bg-red-50 px-3 py-2">
						<p className="text-xs text-red-600">Inválidas</p>
						<p className="text-xl font-black text-red-700">
							{reportSummary.invalidas}
						</p>
					</div>
					<div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-2">
						<p className="text-xs text-blue-600">Total a pagar</p>
						<p className="text-xl font-black text-blue-900">
							{money(reportSummary.valor)}
						</p>
					</div>
				</div>
			</section>
			...
			<section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
				<div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
					<div>
						<h2 className="text-base font-bold text-gray-900">
							Validações de entregas
						</h2>
						<p className="text-sm text-gray-500">
							Modelo visual da conciliação entre O.S. fechada e equipamento
							entregue.
						</p>
					</div>
									...
									return (
										<tr key={item.id}>
											...
											<td className="px-4 py-3 text-xs text-gray-500">
												...
												<p>
													Janela:{" "}
													{daysBetween(item.fechamentoOs, item.entrega) ?? "-"}{" "}
													dia(s)
												</p>
											</td>
											<td className="px-4 py-3">
