						...
						{visibleRows.length ? (
							visibleRows.map((row, index) => (
								<tr key={row.id || row.nome || `${title}-${safePage}-${index}`}>
									{columns.map((column) => (
										<td
											key={column.key}
											className="px-3 py-3 font-bold text-slate-700"
										>
							...
							<tr>
								<td
									...
									className="px-3 py-6 text-center font-bold text-slate-400"
								>
...
async function buildReportPdf(report, filters) {
	...
	pdf.setFontSize(10);
	pdf.text(
		`${formatPeriod(filters.periodo)} · Gerado em ${new Date().toLocaleString("pt-BR")}`,
		14,
		24,
	);
	...
	autoTable(pdf, {
		...
		body: [
			...
			[
				"Consumos confirmados em cliente",
				metricValue(report.summary?.consumosConfirmados),
			],
			[
				"Unidades confirmadas",
				metricValue(report.summary?.unidadesMovimentadas),
			],
			[
				"Metragem confirmada",
				`${metricValue(report.summary?.metrosMovimentados)} m`,
			],
		],
		theme: "grid",
				...
				<header className="flex items-center justify-between gap-4 border-b border-slate-100 p-5">
					<div>
						...
						<h2 className="text-xl font-black text-slate-950">
							Rotina dos relatórios
						</h2>
						<p className="mt-1 text-sm font-bold text-slate-500">
							A leitura do Playground será salva para consulta pelos filtros.
						</p>
					</div>
					<button
						type="button"
				...
				<div className="space-y-4 p-5">
					...
					<div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm font-bold text-blue-900">
						<p>
							...
							{formatDate(config?.lastReportRefreshAt)}
						</p>
						<p className="mt-1">
							Movimentações salvas:{" "}
							{metricValue(config?.lastReportRefreshTotal)}
						</p>
					</div>
				</div>
...
export default function TecnicosAuditoriaRelatoriosPage() {
	...
	const empresasOptions = useMemo(() => {
		...
		return all.filter(
			(empresa) => !filters.regional || empresa.regional === filters.regional,
		);
			...
			if (result?.jobId) {
				...
				for (let attempt = 0; attempt < 1200; attempt += 1) {
					...
					setRefreshProgress({
						...
						total: Number(
							job?.total || job?.processed || job?.result?.movements || 0,
						),
						error: job?.error || "",
					});
					...
					if (job?.status === "failed") {
						throw new Error(
							job.error || "Falha ao atualizar movimentações do Playground.",
						);
					}
				}
				if (!completed) {
					throw new Error(
						"Tempo esgotado ao acompanhar a atualização. A rotina pode continuar em segundo plano; recarregue em alguns minutos.",
					);
				}
			} else {
				setRefreshProgress({
					stage: "Salvando movimentações no banco",
					percent: 80,
					total: result?.movements || 0,
				});
			}
			await load();
			const totalMovements =
				finalResult?.movements ||
				finalResult?.saved ||
				finalResult?.processed ||
				0;
			setMessage(
				`${metricValue(totalMovements)} movimentação(ões) atualizada(s) para consulta.`,
			);
			setRefreshProgress({
				stage: "Atualização finalizada",
				percent: 100,
				total: totalMovements,
			});
		} catch (error) {
			const message = String(
				error?.message || "Falha ao atualizar dados da auditoria.",
			);
	...
	return (
		<main className="space-y-5">
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<h1 className="text-2xl font-black text-slate-950">
								Relatórios de Bolsa Técnico
							</h1>
							<p className="text-sm font-semibold text-slate-500">
								Consumo confirmado pelo Playground por empresa, técnico,
								categoria e item.
							</p>
						</div>
					</div>
					<div className="flex flex-col gap-2 lg:items-end">
						<p className="text-xs font-black uppercase tracking-wide text-slate-500">
							...
							<span className="text-slate-900">
								{formatDate(
									config?.lastReportRefreshAt ||
										report?.cache?.lastReportRefreshAt,
								)}
							</span>
						</p>
						<div className="flex flex-wrap justify-end gap-2">
							...
							<button
								...
								onClick={load}
								disabled={loading}
								className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-black text-slate-700 hover:bg-slate-50 disabled:opacity-60"
							>
								<RefreshCw
									size={16}
							...
							{manage ? (
								<>
									<button
										...
										className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 text-sm font-black text-emerald-700 hover:bg-emerald-100 disabled:opacity-60"
									>
										<RefreshCw
											size={16}
					...
					<div className="flex flex-col gap-4 sm:flex-row sm:items-center">
						...
						<div className="min-w-0">
							...
							<p className="mt-1 text-sm font-bold text-blue-700">
								Consultando o histórico salvo no banco. Use “Forçar atualização”
								para buscar uma nova leitura no Playground.
							</p>
						</div>
					</div>
				</section>
				...
				<section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
					<div className="flex flex-wrap items-center justify-between gap-3">
						<div>
							<p className="text-sm font-black text-emerald-950">
								{refreshProgress.stage ||
									"Atualizando movimentações do Playground"}
							</p>
							<p className="text-xs font-bold text-emerald-700">
								Essa atualização lê apenas as movimentações/notas e salva o
								histórico para os filtros.
							</p>
							{refreshProgress.total ? (
								<p className="mt-1 text-xs font-bold text-emerald-700">
									{metricValue(refreshProgress.total)} movimentação(ões)
									encontrada(s).
								</p>
							) : null}
						...
						<span className="rounded-full bg-white px-3 py-1 text-sm font-black text-emerald-700">
							{Number(refreshProgress.percent || 0)}%
						</span>
					</div>
					<div className="mt-3 h-2 overflow-hidden rounded-full bg-emerald-100">
						<div
							...
							style={{
								width: `${Math.min(100, Math.max(0, Number(refreshProgress.percent || 0)))}%`,
							}}
						/>
					</div>
				</section>
			...
			<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
				<div className="grid gap-3 md:grid-cols-4">
					...
					<label>
						...
						>
							...
							{(report?.options?.regionais || []).map((regional) => (
				...
				<div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
					<button
						...
						onClick={load}
						disabled={loading}
						className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 text-sm font-black text-white disabled:opacity-60"
					>
					...
					<div className="flex flex-col gap-2 sm:flex-row">
						<input
							...
							className={inputClass}
							placeholder="E-mail para enviar PDF"
						/>
						<button
							type="button"
							onClick={handleSendEmail}
							disabled={sending || loading || !report || !email}
							className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-5 text-sm font-black text-emerald-700 disabled:opacity-60"
						>
							{sending ? (
			...
			<section className="grid gap-4 md:grid-cols-4 xl:grid-cols-8">
				<MetricCard
					label="Leituras"
					value={metricValue(report?.summary?.leituras)}
					helper={formatPeriod(filters.periodo)}
				/>
				<MetricCard
					label="Empresas"
					value={metricValue(report?.summary?.empresas)}
					helper="Com consumo"
					tone="green"
				/>
				<MetricCard
					label="Técnicos"
					value={metricValue(report?.summary?.tecnicos)}
					helper="Com consumo"
					tone="amber"
				/>
				<MetricCard
					label="Itens"
					value={metricValue(report?.summary?.unidades)}
					helper="Confirmados"
					tone="violet"
				/>
				<MetricCard
					label="Metragem"
					value={`${metricValue(report?.summary?.metros)} m`}
					helper="Confirmada"
				/>
				<MetricCard
					label="Movimentações"
					value={metricValue(report?.summary?.movimentacoes)}
					helper="Playground"
					tone="green"
				/>
				<MetricCard
					label="Consumo real"
					value={metricValue(report?.summary?.consumosConfirmados)}
					helper="Cliente"
					tone="amber"
				/>
				<MetricCard
					label="Metros reais"
					value={`${metricValue(report?.summary?.metrosMovimentados)} m`}
					helper="Playground"
					tone="violet"
				/>
			</section>
			...
			<div className="grid gap-5 xl:grid-cols-2">
				<ReportTable
					...
					columns={[
						...
						{ key: "regional", label: "Regional" },
						{
							key: "unidades",
							label: "Itens",
							render: (row) => metricValue(row.unidades),
						},
						{
							key: "metros",
							label: "Metros",
							render: (row) => metricValue(row.metros),
						},
					]}
				/>
				<ReportTable
					title="Consumo por técnico"
					rows={report?.porTecnico || []}
					columns={[
				...
				/>
				<ReportTable
					title="Consumo confirmado por empresa"
					rows={report?.movimentosPorEmpresa || []}
					columns={[
						{ key: "nome", label: "Empresa" },
						{ key: "regional", label: "Regional" },
						{
				...
				/>
				<ReportTable
					title="Consumo confirmado por técnico"
					rows={report?.movimentosPorTecnico || []}
					columns={[
						{ key: "nome", label: "Técnico" },
						{ key: "empresaNome", label: "Empresa" },
						{
			...
			<ReportTable
				title="Últimas movimentações do Playground"
				rows={report?.movimentosConsumo || []}
				columns={[
			...
			<div className="grid gap-5 xl:grid-cols-2">
				<ReportTable
					title="Categorias confirmadas pelo Playground"
					rows={report?.movimentosPorCategoria || []}
					columns={[
						{ key: "nome", label: "Categoria" },
						{
							key: "unidades",
				...
				/>
				<ReportTable
					title="Itens confirmados pelo Playground"
					rows={report?.movimentosItens || []}
					columns={[
						{ key: "nome", label: "Item" },
						{ key: "categoria", label: "Categoria" },
						{
