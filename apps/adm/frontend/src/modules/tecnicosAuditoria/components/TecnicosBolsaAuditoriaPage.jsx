...
function MonthlyHistoryTable({ rows = [] }) {
	...
	if (!rows.length) {
		return (
			<p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
				Nenhuma movimentação mensal salva para este técnico.
			</p>
		);
	}
		...
		<section className="rounded-2xl border border-slate-200 bg-white p-4">
			...
			<div className="mt-3 space-y-2">
				{rows.length ? (
					...
					<p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
						Nenhum gasto mensal confirmado.
					</p>
				)}
			</div>
			<PaginationControls
	...
	const [selectedDay, setSelectedDay] = useState(
		() =>
			[...days]
				...
				.find(
					(day) =>
						day.movimentacoes || day.estimadoUnidades || day.estimadoMetros,
				)?.date ||
			days[0]?.date ||
			"",
	);
