...
function PropertyExecutiveDashboard({ dashboard, imoveis = [], onOpenImovel }) {
	...
	const distribuicao = dashboard?.distribuicao || {};
	const maioresCustos = dashboard?.maioresCustos || [];
	const requerAtencao = dashboard?.requerAtencao || [];
	const proximosEventos = dashboard?.proximosEventos || [];
	const totalFallback = imoveis.length;
	const alugadosFallback = imoveis.filter((item) => item.tipoContrato === "alugado").length;
	const propriosFallback = imoveis.filter((item) => item.tipoContrato !== "alugado").length;
	...
	return (
		<div className="space-y-5">
			<section className="overflow-hidden rounded-3xl border border-blue-100 bg-[linear-gradient(135deg,#0f3f91_0%,#1d7bf2_58%,#ff8a00_130%)] p-6 text-white shadow-sm">
				<div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
					<div>
						...
						<p className="mt-2 max-w-2xl text-sm font-semibold text-blue-50">
							Acompanhe contratos, custos, vencimentos e situação operacional
							dos imóveis administrativos com dados reais do ADM.
						</p>
					</div>
					<div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
						<div className="rounded-2xl bg-white/12 p-3 backdrop-blur">
							<p className="font-black">{formatPercent(kpis.percentualAlugados)}</p>
							<p className="text-xs font-semibold text-blue-50">alugados</p>
						</div>
						<div className="rounded-2xl bg-white/12 p-3 backdrop-blur">
							<p className="font-black">{formatPercent(kpis.percentualProprios)}</p>
							<p className="text-xs font-semibold text-blue-50">próprios</p>
						</div>
						<div className="rounded-2xl bg-white/12 p-3 backdrop-blur">
							<p className="font-black">{dashboard?.metadados?.reajustesRegistrados || 0}</p>
							<p className="text-xs font-semibold text-blue-50">reajustes</p>
						</div>
					</div>
				</div>
			</section>
			...
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
				...
				<StatCard icon={Building2} label="Alugados" value={alugados} hint={`${formatPercent(kpis.percentualAlugados)} da base ativa`} tone="orange" />
				<StatCard icon={Building2} label="Próprios" value={proprios} hint={`${formatPercent(kpis.percentualProprios)} da base ativa`} tone="green" />
			...
			<section className="grid items-start gap-4 xl:grid-cols-2">
				<DashboardBarList
					title="Custo mensal por regional"
					icon={BarChart3}
					items={dashboard?.custosPorRegional || []}
					valueFormatter={formatCurrency}
				/>
				<DashboardBarList
					title="Custo por empresa"
					icon={Building2}
					items={dashboard?.custosPorEmpresa || []}
					valueFormatter={formatCurrency}
				/>
			</section>
			...
			<section className="grid items-start gap-4 xl:grid-cols-2">
				<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
					...
					<div className="mt-4 space-y-2">
						{maioresCustos.length ? maioresCustos.slice(0, 5).map((item, index) => (
							<button
								type="button"
								key={item.imovelId}
								onClick={() => onOpenImovel?.(item.imovelId)}
								className="flex w-full items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2 text-left hover:bg-blue-50"
							>
								<span className="font-black text-slate-700">
									{index + 1}. {item.imovelNome}
								</span>
				...
				<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
					...
					<div className="mt-4 space-y-2">
						{proximosEventos.length ? proximosEventos.slice(0, 6).map((item, index) => (
							<button
								type="button"
								key={`${item.tipo}-${item.imovelId}-${index}`}
								onClick={() => item.imovelId && onOpenImovel?.(item.imovelId)}
								className="flex w-full items-center gap-3 rounded-xl bg-slate-50 px-3 py-2 text-left hover:bg-blue-50"
							>
								<span className="w-16 shrink-0 rounded-lg bg-white px-2 py-1 text-center text-xs font-black text-blue-700">
									{formatDate(item.data)}
								</span>
								<span>
									<p className="text-sm font-black text-slate-900">{item.tipo}</p>
									<p className="text-xs font-semibold text-slate-500">{item.imovelNome}</p>
								</span>
							</button>
						)) : (
							<p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-500">
