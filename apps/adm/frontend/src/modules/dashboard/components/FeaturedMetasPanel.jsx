...
const FeaturedMetasPanel = () => {
	...
	const ultimasEntregas = useMemo(() => {
		...
		const maxIndex =
			currentIndex >= 0 ? currentIndex : MESES_DASHBOARD.length - 1;

		return MESES_DASHBOARD.slice(0, maxIndex + 1)
			.map((mes) => {
	...
	return (
		<section className="flex self-start flex-col overflow-hidden rounded-lg border border-orange-200 bg-white shadow-card">
				...
				<div className="min-w-0 flex flex-col p-6">
					<div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
						<div>
							...
							<p className="text-sm font-semibold text-slate-500">
								O.S realizadas em{" "}
								{metaStats.mes === "Marco" ? "Março" : metaStats.mes}
							</p>
						</div>
