...
const DEFAULT_BACKUP_DIR = "/opt/retiradas/backups/adm-postgres";
...
function getConfig() {
	return {
		...
		maxBytes: parseBytes(
			process.env.DB_BACKUP_MAX_BYTES || "10gb",
			DEFAULT_MAX_BYTES,
		),
...
function getPgEnv() {
	const fromUrl = process.env.DATABASE_URL
		...
		: {};
	const database = process.env.PGDATABASE || fromUrl.database || "adm";
	...
	return new Promise((resolve, reject) => {
		...
		child.on("close", (code) => {
			...
			const error = new Error(
				stderr || `${command} finalizou com codigo ${code}.`,
			);
			error.code = code;
			error.stdout = stdout;
			error.stderr = stderr;
...
function resolveBackupPath(fileName) {
	...
	if (
		!safeName ||
		safeName !== fileName ||
		(!safeName.endsWith(".dump") && !safeName.endsWith(".dump.age"))
	) {
...
async function listBackups() {
	...
	for (const entry of entries) {
		if (
			!entry.isFile() ||
			(!entry.name.endsWith(".dump") && !entry.name.endsWith(".dump.age"))
		)
			continue;
...
async function createBackup({ reason = "manual", user = null } = {}) {
	...
	if (config.ageRecipient) {
		...
		await runCommand(
			"age",
			["-r", config.ageRecipient, "-o", finalFilePath, filePath],
			env,
		);
		await fs.rm(filePath, { force: true });
		encrypted = true;
...
async function restoreBackup(fileName, { user = null } = {}) {
	...
	if (isEncryptedBackup(fileName)) {
		if (!config.ageIdentityFile) {
			throw new Error(
				"DB_BACKUP_AGE_IDENTITY_FILE nao configurado para restaurar backup criptografado.",
			);
		}
		temporaryRestorePath = path.join(
			getBackupDir(),
			`${path.basename(fileName, ".age")}.restore-tmp`,
		);
		await runCommand(
			"age",
			[
...
async function getBackupStatus() {
	...
	const usedBytes = backups.reduce(
		(sum, backup) => sum + Number(backup.sizeBytes || 0),
		0,
	);
