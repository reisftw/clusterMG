...
const DEFAULT_EMAIL_TEMPLATES = {
	welcome_user: {
		...
		subject: "Seu acesso ao sistema Administrativo",
		title: "Bem-vindo ao Administrativo",
		preview:
			"Sua conta no sistema Administrativo foi criada. Defina sua senha para começar.",
		actionLabel: "Definir minha senha",
		body: "Olá, {nome}.\n\nSua conta foi criada no sistema Administrativo. Para ativar o acesso com segurança, defina sua senha pelo botão abaixo.\n\nE-mail: {email}\nPerfil: {role}\nRegional: {regional}\n\nO link expira em 30 minutos.",
	},
	password_reset: {
		label: "Redefinição de senha",
		subject: "Redefinição de senha - Administrativo",
	...
	password_changed: {
		...
		subject: "Senha alterada - Administrativo",
		title: "Senha alterada com sucesso",
		preview: "Sua senha foi alterada com sucesso.",
		actionLabel: "",
		body: "Olá, {nome}.\n\nSua senha do sistema Administrativo foi alterada com sucesso.\n\nSe você não reconhece esta ação, avise a administração.",
	},
	mfa_login_code: {
		label: "Código MFA por e-mail",
		subject: "Código de segurança - Administrativo",
		title: "Confirme seu acesso",
		preview: "Use o código de segurança para concluir o login.",
		actionLabel: "",
		body: "Olá, {nome}.\n\nRecebemos uma tentativa de login na sua conta do sistema Administrativo.\n\nSeu código de segurança é: {codigo}\n\nEste código expira em {minutos} minutos. Se você não reconhece esta tentativa, avise a administração.",
	},
	smtp_test: {
		label: "Teste SMTP",
		subject: "Teste de e-mail - Administrativo",
		title: "E-mail configurado com sucesso",
		preview: "O SMTP do sistema Administrativo está funcionando.",
		actionLabel: "",
		body: "Este e-mail confirma que o SMTP está autenticando e enviando mensagens pelo sistema Administrativo.\n\nRemetente: {fromEmail}\nResponder para: {replyTo}",
	},
	system_notice: {
		label: "Aviso do sistema",
		subject: "Aviso do sistema Administrativo",
		title: "Aviso importante",
		preview: "Comunicado automático do sistema Administrativo.",
		actionLabel: "Acessar sistema",
		body: "Olá, {nome}.\n\nEste é um aviso automático do sistema Administrativo.\n\nMensagem: {mensagem}\n\nAcesse o sistema para acompanhar os detalhes.",
	},
	insumos_low_stock: {
	...
	totem_insumos_otp: {
		label: "OTP do Totem de Insumos",
		subject: "Código de acesso — Solicitação de Insumos",
		title: "Código de acesso",
		preview: "Use o código para acessar o Totem de Solicitação de Insumos.",
		actionLabel: "",
		body: "Olá,\n\nSeu código para acessar o Totem de Solicitação de Insumos é:\n\n{codigo}\n\nEste código expira em {minutos} minutos.\n\nSe você não realizou esta solicitação, ignore esta mensagem.\n\nBrasil Tecpar",
	},
	totem_insumos_request_created: {
		label: "Comprovante do Totem de Insumos",
		subject: "Solicitação de Insumos — {protocolos}",
		title: "Solicitação registrada",
		preview: "Sua solicitação de insumos foi registrada com sucesso.",
		actionLabel: "",
		body: "Olá, {nome}.\n\nSua solicitação foi registrada com sucesso.\n\n{protocolos}\n\nItens solicitados:\n\n{itens}\n\nStatus:\n\nSolicitado\n\nVocê receberá atualizações conforme o processo avançar.",
	},
};
...
const DEFAULT_CONFIG = {
	...
	fromName: process.env.SMTP_FROM_NAME || "Administrativo",
	...
	mfaEmailEnabled:
		String(process.env.MFA_EMAIL_ENABLED || "true").toLowerCase() === "true",
...
function normalizeAdmEmailTemplate(template = {}) {
	return Object.fromEntries(
		Object.entries(template).map(([key, value]) => [
			key,
			typeof value === "string"
				? value
						.replace(/sistema Retiradas/g, "sistema Administrativo")
						.replace(/Sistema Retiradas/g, "Sistema Administrativo")
						.replace(/Retiradas/g, "Administrativo")
				: value,
		]),
	);
}
...
function renderUserInfo(userInfo = []) {
	const items = Array.isArray(userInfo)
		? userInfo.filter((item) => item?.label)
		: [];
...
function renderInfoBox(infoBox) {
	...
	const highlighted = escapeHtml(value).replace(
		/(30 minutos|10 minutos|24 horas)/gi,
		'<strong style="color:#175CD3">$1</strong>',
	);
	return `
...
function createEmailTemplate({
	...
	buttonText = "",
	buttonUrl = "",
	footerText = "Este e-mail foi enviado automaticamente pelo sistema Administrativo.",
	...
	const safeTitle = escapeHtml(title || "Administrativo");
	const safePreheader = escapeHtml(preheader || title || "Sistema Administrativo");
	const resolvedLogoUrl =
		logoUrl ||
		`${String(appUrl || DEFAULT_CONFIG.appUrl).replace(/\/+$/, "")}/logo-adm.png`;
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td class="email-logo-cell" valign="middle" align="right" width="210" style="padding-left:24px">
                    <img class="email-logo" src="${safeUrl(resolvedLogoUrl)}" width="190" alt="Administrativo" style="display:block;width:190px;max-width:190px;height:auto;border:0;outline:none;text-decoration:none;border-radius:14px">
                  </td>
                </tr>
              </table>
            </td>
...
function parseConfiguredBody(text, templateKey) {
	...
	lines.forEach((line) => {
		...
		if (
			...
			/^perfil\s*:/i.test(line) ||
			/^regional\s*:/i.test(line)
		)
			return;
...
function userInfoForTemplate(templateKey, variables = {}) {
	...
	if (templateKey === "mfa_login_code") {
		return [
			...
			{
				label: "Validade",
				value: `${variables.minutos || 10} minutos`,
				type: "security",
			},
		];
	}
	if (templateKey === "insumos_low_stock") {
		return [
			{ label: "Item", value: variables.produto, type: "success" },
			{
				label: "Estoque atual",
				value:
					`${variables.estoque_atual || "-"} ${variables.unidade || ""}`.trim(),
				type: "role",
			},
			{
				label: "Estoque mínimo",
				value:
					`${variables.estoque_minimo || "-"} ${variables.unidade || ""}`.trim(),
				type: "security",
			},
			{
				label: "Categoria",
				value: variables.categoria || "-",
				type: "location",
			},
			{
				label: "Observação",
...
async function getConfig() {
	...
	return {
		...
		fromName:
			data.fromName && data.fromName !== "Retiradas"
				? data.fromName
				: DEFAULT_CONFIG.fromName,
		mfaEmailEnabled: Boolean(
			data.mfaEmailEnabled ?? DEFAULT_CONFIG.mfaEmailEnabled,
		),
		mfaEmailTtlMinutes: Number(
			data.mfaEmailTtlMinutes || DEFAULT_CONFIG.mfaEmailTtlMinutes,
		),
...
async function saveConfig(patch = {}) {
	...
	const next = {
		...
		mfaEmailTtlMinutes: Math.max(
			...
			Math.min(
				30,
				Number(patch.mfaEmailTtlMinutes || current.mfaEmailTtlMinutes || 10),
			),
		),
...
function renderConfiguredEmail(config, templateKey, variables = {}) {
	const template =
		normalizeTemplates(config.templates)[templateKey] ||
		DEFAULT_EMAIL_TEMPLATES[templateKey];
	...
	await db
     ...
     values ($1, $2, $3, $4, $5, $6, $7, $8::jsonb, now())`,
			[
				id,
				String(
					normalizedMeta.type || normalizedMeta.template || "system_notice",
				),
				String(to || ""),
				String(subject || ""),
				String(status || ""),
...
async function listEmailLogs({
	...
	const countResult = await db.query(
		`select count(*)::int as total from email_logs ${whereSql}`,
		params,
	);
...
async function sendMail({
	...
	const from = `${config.fromName || "Administrativo"} <${config.fromEmail}>`;
	try {
...
async function sendInsumosLowStockEmail({ to, produto = {}, recipients = [] }) {
	const config = await getConfig();
	const appUrl = String(
		config.appUrl || DEFAULT_CONFIG.appUrl || "https://adm.retiradas.tech",
	).replace(/\/+$/, "");
...
async function sendInsumosRequestApprovedEmail({ to, requisicao = {} }) {
	...
	const itens = Array.isArray(requisicao.itens) && requisicao.itens.length
		? requisicao.itens
		: [
				{
					produto_nome: requisicao.produto_nome || "Insumo",
	...
	const itensTexto = itens
		.map((item) =>
			`${item.quantidade || "-"}x ${item.produto_nome || "Insumo"} ${item.unidade || ""}`.trim(),
		)
		.join("\n");
	const prazo = requisicao.expira_em
		? new Date(requisicao.expira_em).toLocaleString("pt-BR", {
				dateStyle: "short",
				timeStyle: "short",
			})
		: "24 horas";
	const rendered = renderConfiguredEmail(config, "insumos_request_approved", {
		nome:
			...
			requisicao.solicitante_email ||
			"solicitante",
		protocolo: requisicao.protocolo || "-",
		produto:
			itens.length === 1
				? itens[0].produto_nome || "Insumo"
				: `${itens.length} itens aprovados`,
		quantidade:
			itens.length === 1 ? itens[0].quantidade || "-" : itens.length,
		unidade: itens.length === 1 ? itens[0].unidade || "" : "itens",
		itens: itensTexto,
	...
	return sendMail({
		...
		meta: {
			...
			protocolo: requisicao.protocolo || "",
			produtoNome: itens[0]?.produto_nome || "",
			quantidade: itens.length,
		},
	});
}

async function sendTotemInsumosOtpEmail({ to, code, ttlMinutes = 5 }) {
	const config = await getConfig();
	const rendered = renderConfiguredEmail(config, "totem_insumos_otp", {
		codigo: code,
		minutos: ttlMinutes,
	});
	return sendMail({
		to,
		...rendered,
		meta: { type: "totem_insumos_otp" },
	});
}

async function sendTotemInsumosRequestCreatedEmail({
	to,
	nome,
	requisicoes = [],
}) {
	const config = await getConfig();
	const protocolos = requisicoes
		.map((item) => item.protocolo)
		.filter(Boolean)
		.join(", ");
	const itens = requisicoes
		.flatMap((item) =>
			Array.isArray(item.itens) && item.itens.length ? item.itens : [item],
		)
		.map((item) =>
			`${item.quantidade || "-"}x ${item.produto_nome || "Insumo"} ${item.unidade || ""}`.trim(),
		)
		.join("\n");
	const rendered = renderConfiguredEmail(
		config,
		"totem_insumos_request_created",
		{
			nome: nome || to,
	...
	return sendMail({
		...
		...rendered,
		meta: {
			type: "totem_insumos_request_created",
			protocolos,
			requisicoes: requisicoes.map((item) => ({
				id: item.id || "",
...
async function sendTestEmail(to) {
	...
	const sampleVariables = {
		...
		mensagem:
			"Este e um exemplo de comunicado para validar o layout do e-mail.",
		codigo: "123456",
