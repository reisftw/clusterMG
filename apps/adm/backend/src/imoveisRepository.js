...
function buildImoveisDashboard({ imoveis, iptus, alugueis, reajustes }) {
	...
	const proprios = ativos.filter((item) => item.tipoContrato !== "alugado");
	...
	const requerAtencao = [
		...contratosAtencao.map((item) => ({
			...
			valor: numberValue(item.valorAluguel),
		})),
	...
	const maioresCustos = [...alugados]
		.sort((a, b) => numberValue(b.valorAluguel) - numberValue(a.valorAluguel))
		.slice(0, 10)
		.map((item) => ({
			imovelId: item.id,
			imovelNome: item.nome || item.seniorId || item.id,
