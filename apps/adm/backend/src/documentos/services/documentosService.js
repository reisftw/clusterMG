...
async function getEmpresa(empresaId) {
	...
	return {
		...
		supervisorTelefone: text(
			data.supervisor?.telefone || data.supervisorTelefone,
		),
...
async function enrichSubmission(submission, { files = null } = {}) {
	...
	const supervisorTelefone = text(
		submission.supervisorTelefone ||
			empresa?.supervisorTelefone ||
...
function canAccessEmpresa(user, empresa) {
	...
	if (["lider_empresa", "supervisor_empresa", "agente_autorizado"].includes(role)) {
		...
		return (
			(uid && uid === empresa.supervisorId) ||
			(email && email === normalize(empresa.supervisorEmail)) ||
...
function canApprove(user, file) {
	...
	if (role === "supervisor_empresa") {
		...
		return (
			(uid && uid === file.supervisorId) ||
			(email && email === normalize(file.supervisorEmail)) ||
			(empresaId && empresaId === file.empresaId) ||
			(empresaNome && empresaNome === normalize(file.empresaNome))
		);
	}
...
function invoiceFileName(empresa, field, mesReferencia, file) {
	const ext = fileExtension(file);
	const label = monthLabel(mesReferencia, " ");
	const model = sanitizeFileName(field?.nome || "Modelo");
	return `${sanitizeFileName(empresa?.nome || "Empresa")} - NF (${model}) - ${sanitizeFileName(label)}${ext}`;
}

function monthlyDocumentFileName(empresa, mesReferencia, file) {
	const month = monthLabel(mesReferencia, " ");
	return `${sanitizeFileName(empresa?.nome || "Empresa")} - Documentos - ${sanitizeFileName(month)}${fileExtension(file)}`;
}
...
async function uploadMonthlyDocumentFile(file, field, ctx) {
	...
	try {
		uploaded = await drive.uploadFile({
			...
			name: field?.nome
				? monthlyDocumentFileName(empresa, month, file)
				: file.originalname,
		});
	} catch (error) {
...
async function uploadSubmissionInvoices({
	...
	for (let index = 0; index < files.length; index += 1) {
		...
		const safeFieldId = uuidOrNull(field.id);
		...
		try {
			uploaded = await drive.uploadFile({
				...
				name: invoiceFileName(empresa, field, submission.mesReferencia, file),
			});
		} catch (error) {
			const uploadError = new Error(
