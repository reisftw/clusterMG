...
const SENSITIVE_ERROR_KEY_PATTERN =
	/(token|secret|password|authorization|cookie|credential|apikey|api_key|refresh)/i;

function sanitizeErrorForLog(value, depth = 0) {
	...
	if (Array.isArray(value)) {
		return value.slice(0, 20).map((item) => sanitizeErrorForLog(item, depth + 1));
	}
			...
			Object.entries(value)
				...
				.map(([key, entry]) => [
					...
					SENSITIVE_ERROR_KEY_PATTERN.test(key)
						? "[redacted]"
						: sanitizeErrorForLog(entry, depth + 1),
				]),
		);
	}
	return value;
...
const TOTEM_OTP_COLLECTION = "totem_insumos_otp_challenges";
const TOTEM_SESSION_COLLECTION = "totem_insumos_sessions";
const TOTEM_ALLOWED_EMAIL_DOMAIN = "brasiltecpar.com.br";
const TOTEM_OTP_TTL_MINUTES = 5;
const TOTEM_OTP_COOLDOWN_SECONDS = 45;
const TOTEM_OTP_MAX_ATTEMPTS = 5;
const TOTEM_SESSION_TTL_MINUTES = 30;
...
function isEmpresaLeader(user) {
	return hasRole(user, ["lider_empresa", "supervisor_empresa", "agente_autorizado"]);
}
...
function canAccessEmpresaRecord(user, documentId, data = {}) {
	...
	const userEmail = normalizeComparableText(user?.email || user?.profile?.email);
	const recordNome = normalizeComparableText(data.nome || data.empresa);
	const supervisorUid = String(data?.supervisor?.uid || data?.supervisorUid || "").trim();
	const supervisorEmail = normalizeComparableText(
		data?.supervisor?.email || data?.supervisorEmail,
	);
	return Boolean(
		(userUid && supervisorUid && userUid === supervisorUid) ||
			(userEmail && supervisorEmail && userEmail === supervisorEmail) ||
...
async function notifyAdministrativeLowStock({
	...
	const appUrl =
		process.env.ADM_PUBLIC_URL ||
		process.env.PUBLIC_APP_URL ||
		"https://adm.retiradas.tech";
...
function normalizeInsumosRequestItems(data = {}) {
	if (Array.isArray(data.itens) && data.itens.length) {
		return data.itens
			.map((item) => ({
				produto_id: String(item.produto_id || item.produtoId || "").trim(),
				produto_nome: String(item.produto_nome || item.produtoNome || "Insumo").trim(),
				unidade: String(item.unidade || "Unidade").trim(),
				categoria: String(item.categoria || "").trim(),
				quantidade: normalizeRequestQuantity(item.quantidade),
			}))
			.filter((item) => item.produto_id && item.quantidade > 0);
	}
	const produtoId = String(data.produto_id || data.produtoId || "").trim();
	const quantidade = normalizeRequestQuantity(data.quantidade);
	if (!produtoId || quantidade <= 0) return [];
	return [
		{
			produto_id: produtoId,
			produto_nome: String(data.produto_nome || data.produtoNome || "Insumo").trim(),
			unidade: String(data.unidade || "Unidade").trim(),
...
function summarizeInsumosRequestItems(items = []) {
	if (!items.length) return "Nenhum item";
	if (items.length === 1) {
		const item = items[0];
		return `${item.quantidade} ${item.unidade || ""} - ${item.produto_nome || "Insumo"}`.trim();
	}
	return `${items.length} itens: ${items
		.slice(0, 3)
		.map((item) => `${item.quantidade} ${item.unidade || ""} ${item.produto_nome || "Insumo"}`.trim())
		.join("; ")}${items.length > 3 ? `; +${items.length - 3}` : ""}`;
}

function normalizeTotemEmail(value) {
	return String(value || "").trim().toLowerCase();
}

function validateTotemCorporateEmail(email) {
	const normalized = normalizeTotemEmail(email);
	const parts = normalized.split("@");
	const domain = parts.length === 2 ? parts[1] : "";
	if (!normalized || parts.length !== 2 || !parts[0] || !domain) {
		const error = new Error("Informe um e-mail corporativo válido.");
		error.statusCode = 400;
		throw error;
	}
	if (domain !== TOTEM_ALLOWED_EMAIL_DOMAIN) {
		const error = new Error("Utilize seu e-mail corporativo Brasil Tecpar.");
		error.statusCode = 400;
		throw error;
	}
	return normalized;
...
function hashTotemSecret(value) {
	return crypto.createHash("sha256").update(String(value || "")).digest("hex");
}

function maskTotemEmail(email) {
	const normalized = normalizeTotemEmail(email);
...
function toTotemSafeUser(user = {}) {
	return {
		...
		insumosBaseId: user.insumos_base_id || "",
		insumosBaseNome: user.insumos_base_nome || "",
	};
}

function getTotemToken(req = {}) {
	const header = String(req.get?.("authorization") || "");
	if (/^Bearer\s+/i.test(header)) return header.replace(/^Bearer\s+/i, "").trim();
	return String(req.get?.("x-totem-token") || req.body?.token || req.query?.token || "").trim();
}

async function recordTotemAudit(action, { email = "", sessionId = "", afterData = {}, req } = {}) {
	return auditLog.recordAuditLog({
		module: "totem",
		entity: "totem_insumos",
		action,
		recordId: sessionId || maskTotemEmail(email),
		userEmail: email,
		ipAddress: req ? undefined : "",
		afterData: {
			email: email ? maskTotemEmail(email) : "",
			...afterData,
		},
	});
}

async function findActiveTotemOtp(email) {
	const result = await db.query(
		`select path, document_id as "documentId", data
		  ...
		  where collection_path = $1
		    ...
		    and nullif(data->>'expires_at', '')::timestamptz > now()
		  order by data->>'created_at' desc
		  limit 1`,
		[TOTEM_OTP_COLLECTION, email],
	);
	return result.rows[0] || null;
}

async function getTotemSessionFromRequest(req) {
	const token = getTotemToken(req);
	if (!token) return null;
	const tokenHash = hashTotemSecret(token);
	const result = await db.query(
		`select path, document_id as "documentId", data
		   from app_documents
		  where collection_path = $1
		    and data->>'token_hash' = $2
		    and data->>'ended_at' is null
		    and nullif(data->>'expires_at', '')::timestamptz > now()
		  limit 1`,
		[TOTEM_SESSION_COLLECTION, tokenHash],
	);
	const session = result.rows[0];
	if (!session) return null;
	return { token, id: session.documentId, path: session.path, ...(session.data || {}) };
}

async function requireTotemSession(req, res) {
	const session = await getTotemSessionFromRequest(req);
	if (!session) {
		res.status(401).json({ error: "Sessão do Totem expirada. Inicie novamente." });
		return null;
	}
	return session;
}

async function createInsumosRequestEntry({
	produtoId,
	quantidade,
	items,
	observacao = "",
	...
	for (const item of rawItems) {
		const normalizedProdutoId = String(item.produtoId || item.produto_id || "").trim();
		const normalizedQuantidade = normalizeRequestQuantity(item.quantidade);
		if (!normalizedProdutoId || normalizedQuantidade <= 0) continue;
		const current = groupedItems.get(normalizedProdutoId) || 0;
		groupedItems.set(normalizedProdutoId, current + normalizedQuantidade);
	}
	if (!groupedItems.size) {
		const error = new Error("Produto obrigatorio.");
		error.statusCode = 400;
		throw error;
	}

	const requestItems = [];
	for (const [normalizedProdutoId, normalizedQuantidade] of groupedItems.entries()) {
		const produto = await documents.getDocument(
			`${INSUMOS_PRODUTOS_COLLECTION}/${normalizedProdutoId}`,
		);
		...
		if (String(produto.data?.status || "ativo") !== "ativo") {
			const error = new Error("Produto inativo nao pode ser solicitado.");
			error.statusCode = 400;
			throw error;
		}
		const estoqueAtual = normalizeStockNumber(produto.data?.estoque_atual);
		if (normalizedQuantidade > estoqueAtual) {
			const error = new Error(
				`Estoque insuficiente para ${produto.data?.nome || "Insumo"}. Disponivel: ${estoqueAtual} ${produto.data?.unidade || ""}.`,
			);
			error.statusCode = 400;
			throw error;
		}
		requestItems.push({
			produto_id: normalizedProdutoId,
	...
	const totalQuantidade = requestItems.reduce(
		(total, item) => total + normalizeRequestQuantity(item.quantidade),
		0,
	);
	const data = {
		id: documentId,
		...
		origin,
		kioskId,
		produto_id: primaryItem.produto_id,
		produto_nome:
			requestItems.length === 1
				? primaryItem.produto_nome
				: `${requestItems.length} itens solicitados`,
		unidade: requestItems.length === 1 ? primaryItem.unidade : "itens",
		categoria: requestItems.length === 1 ? primaryItem.categoria : "Pedido",
		quantidade: totalQuantidade,
		itens: requestItems,
		total_itens: requestItems.length,
		observacao: String(observacao || "").trim(),
		solicitante_uid: user?.uid || "",
		solicitante_nome: displayName,
		solicitante_email: userEmail,
		solicitante_role: normalizeUserRole(user?.role),
		criado_em: now,
		atualizado_em: now,
		auditoria: addInsumosAudit(
			{},
			{
				...
				observacao:
					origin === "KIOSK"
						? `Solicitou ${summarizeInsumosRequestItems(requestItems)} pelo Totem ${kioskId || ""}.`.trim()
						: `Solicitou ${summarizeInsumosRequestItems(requestItems)}.`,
			},
		),
	};
	...
	notificationsService
		.createNotification({
			type: "insumos_requisicao_pendente",
			title: "Nova requisição de insumos",
			message: `${displayName} abriu ${data.protocolo} com ${requestItems.length} item(ns).`,
			targetPath: "/insumos/requisicoes",
			severity: "warning",
			user,
			meta: {
				requisicaoId: documentId,
				protocolo: data.protocolo,
				totalItens: requestItems.length,
				origem: origin,
			},
			dedupeKey: `insumos-requisicao-${documentId}`,
		})
		.catch((error) => {
			console.error(
				"[insumos] Falha ao criar notificacao de requisicao:",
				error?.message || error,
			);
		});
...
async function expireOverdueInsumosRequests() {
	...
	try {
		...
		for (const request of result.rows) {
			...
			const requestItems = normalizeInsumosRequestItems(data);
			for (const item of requestItems) {
				const productPath = `${INSUMOS_PRODUTOS_COLLECTION}/${item.produto_id}`;
				...
				await updateDocumentData(client, productPath, {
					...productData,
					estoque_atual:
						normalizeStockNumber(productData.estoque_atual) + item.quantidade,
...
const REFRESH_COOKIE_NAME = "retiradas_refresh";
const CSRF_COOKIE_NAME = "retiradas_csrf";
const TOKEN_RENEW_WINDOW_SECONDS = Number(
	process.env.ACCESS_TOKEN_RENEW_WINDOW_SECONDS || 60 * 15,
);

function getAllowedOrigins() {
	const configured = String(
		process.env.ADM_CORS_ORIGINS || process.env.CORS_ORIGIN || "",
	)
		.split(",")
...
function setAuthCookie(
	...
	token,
	maxAgeSeconds,
	{ refreshToken = "", refreshMaxAgeSeconds = REFRESH_TOKEN_TTL_SECONDS } = {},
) {
	...
	if (refreshToken) {
		cookies.push(
			`${REFRESH_COOKIE_NAME}=${encodeURIComponent(refreshToken)}; ${getAuthCookieOptions(refreshMaxAgeSeconds).join("; ")}`,
		);
	}
...
function clearAuthCookie(res) {
	res.setHeader("Set-Cookie", [
		...
		`${REFRESH_COOKIE_NAME}=; ${getAuthCookieOptions(0).join("; ")}`,
...
function sendAuthSession(res, session, extra = {}) {
	const csrfToken = setAuthCookie(res, session.token, session.expiresIn, {
		refreshToken: session.refreshToken,
		refreshMaxAgeSeconds: session.refreshExpiresIn,
	});
	res.json({
		ok: true,
		...extra,
...
function createApp() {
	...
	// observabilidade/prometheusMetrics.js). Separado do /api/admin/metrics
	...
	app.get("/api/uploads/avatars/:filename", requireAuthenticated, (_req, res) => {
		res
			.status(200)
			.type("image/svg+xml")
			.set("Cache-Control", "no-store")
			.send(
				`<svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128"><rect width="128" height="128" rx="28" fill="#eaf2ff"/><circle cx="64" cy="48" r="24" fill="#1d4ed8"/><path d="M25 112c6-24 21-36 39-36s33 12 39 36" fill="#1d4ed8"/></svg>`,
			);
	});
	...
	app.get(
		...
		requireAuthenticated,
		requireAnyPermission(["administrativo.auditoria.view", "configuracao.auditoria.view"], ADMIN_ROLES),
		async (req, res, next) => {
			try {
	...
	app.get("/api/public/facilities/qr/:token", publicReadLimiter, async (req, res, next) => {
		try {
			const record = await getPublicQrRecord(req.params.token);
			if (!record) {
				res.status(404).json({ ok: false, error: "Registro não encontrado para este QR Code." });
				return;
			}
	...
	app.get("/api/public/facilities/supplier-evaluation/:token", publicReadLimiter, async (req, res, next) => {
		try {
			const evaluation = await getPublicSupplierEvaluation(req.params.token, req.query.fornecedor);
			if (!evaluation) {
				res.status(404).json({ ok: false, error: "Link de avaliação inválido ou expirado." });
				return;
			}
	...
	app.post("/api/public/facilities/supplier-evaluation/:token", publicReadLimiter, async (req, res, next) => {
		try {
			const evaluation = await submitPublicSupplierEvaluation(req.params.token, req.body || {});
	...
	app.post("/api/auth/refresh", async (req, res, next) => {
		try {
			...
			if (!refreshToken) {
				res.status(401).json({ ok: false, error: "Refresh token ausente." });
				return;
			}

			const session = await rotateRefreshSession(refreshToken);
			const profile = await getImportedUserProfile(session.uid);
			sendAuthSession(res, { ...session, user: profile }, { refreshed: true });
		} catch (error) {
			clearAuthCookie(res);
			if (String(error?.message || "").includes("Refresh token")) {
				res.status(401).json({
					ok: false,
	...
	app.get("/api/auth/me", requireAuthenticated, async (req, res, next) => {
		try {
			...
			if (shouldRenewAuthToken(req.authPayload)) {
				const refreshToken = getRefreshToken(req);
				if (refreshToken) {
					const renewedSession = await rotateRefreshSession(refreshToken);
					const csrfToken = setAuthCookie(
						res,
						renewedSession.token,
						renewedSession.expiresIn,
						{
							refreshToken: renewedSession.refreshToken,
							refreshMaxAgeSeconds: renewedSession.refreshExpiresIn,
						},
					);
					res.json({
						csrfToken,
						expiresIn: renewedSession.expiresIn,
						renewed: true,
	...
	app.post(
		...
		async (req, res, next) => {
			try {
				const email = validateTotemCorporateEmail(req.body?.email);
				const user = await getLocalUserByEmail(email);
				if (!user || user.disabled) {
					await recordTotemAudit("KIOSK_OTP_FAILED", {
						email,
						afterData: { reason: "user_not_found_or_inactive" },
					});
					res.status(404).json({
						error:
							"Não encontramos seu cadastro. Procure o responsável pelo Administrativo.",
					});
					return;
				}

				const active = await findActiveTotemOtp(email);
				const activeCreatedAt = active?.data?.created_at
					? new Date(active.data.created_at).getTime()
					: 0;
				const cooldownMs = TOTEM_OTP_COOLDOWN_SECONDS * 1000;
				const remainingSeconds = Math.ceil(
					(activeCreatedAt + cooldownMs - Date.now()) / 1000,
				);
				if (active && remainingSeconds > 0) {
					res.status(429).json({
						error: `Aguarde ${remainingSeconds}s para reenviar o código.`,
						retryAfterSeconds: remainingSeconds,
						maskedEmail: maskTotemEmail(email),
					});
					return;
				}
				...
				const expiresAt = new Date(
					now.getTime() + TOTEM_OTP_TTL_MINUTES * 60 * 1000,
				).toISOString();
				const documentId = crypto.randomUUID();
				await documents.upsertDocument({
					path: `${TOTEM_OTP_COLLECTION}/${documentId}`,
					collectionPath: TOTEM_OTP_COLLECTION,
					documentId,
					parentPath: null,
					data: {
						email,
						user_uid: user.uid,
						code_hash: hashTotemSecret(code),
						attempts: 0,
						max_attempts: TOTEM_OTP_MAX_ATTEMPTS,
				...
				await emailService.sendTotemInsumosOtpEmail({
					to: email,
					code,
					ttlMinutes: TOTEM_OTP_TTL_MINUTES,
				});
				await recordTotemAudit("KIOSK_OTP_REQUESTED", {
					email,
					afterData: { challengeId: documentId },
				});
				res.json({
					ok: true,
					maskedEmail: maskTotemEmail(email),
					expiresInSeconds: TOTEM_OTP_TTL_MINUTES * 60,
					resendInSeconds: TOTEM_OTP_COOLDOWN_SECONDS,
				});
			} catch (error) {
				next(error);
			}
	...
	app.post(
		...
		async (req, res, next) => {
			try {
				...
				const challenge = await findActiveTotemOtp(email);
				if (!challenge) {
					await recordTotemAudit("KIOSK_OTP_FAILED", {
						email,
						afterData: { reason: "expired_or_missing" },
					});
				...
				if (attempts >= TOTEM_OTP_MAX_ATTEMPTS) {
					res.status(429).json({ error: "Limite de tentativas excedido." });
					return;
				}
				if (data.code_hash !== hashTotemSecret(code)) {
					await documents.upsertDocument({
						path: challenge.path,
						collectionPath: TOTEM_OTP_COLLECTION,
					...
					await recordTotemAudit("KIOSK_OTP_FAILED", {
						email,
						afterData: { reason: "invalid_code", attempts: attempts + 1 },
					});
				...
				await documents.upsertDocument({
					...
					documentId: challenge.documentId,
					parentPath: null,
					data: { ...data, used_at: now.toISOString(), attempts },
				});

				const token = crypto.randomBytes(32).toString("base64url");
				const sessionId = crypto.randomUUID();
				const expiresAt = new Date(
					now.getTime() + TOTEM_SESSION_TTL_MINUTES * 60 * 1000,
				).toISOString();
				const safeUser = toTotemSafeUser(user);
				await documents.upsertDocument({
					path: `${TOTEM_SESSION_COLLECTION}/${sessionId}`,
					collectionPath: TOTEM_SESSION_COLLECTION,
					documentId: sessionId,
					parentPath: null,
					data: {
						token_hash: hashTotemSecret(token),
						email,
						user: safeUser,
						kioskId: String(req.body?.kioskId || "TOTEM-001").trim() || "TOTEM-001",
						created_at: now.toISOString(),
						expires_at: expiresAt,
						ended_at: null,
					},
				});
				await recordTotemAudit("KIOSK_OTP_VALIDATED", { email, sessionId });
				await recordTotemAudit("KIOSK_SESSION_STARTED", { email, sessionId });
				res.json({
					ok: true,
					token,
					expiresAt,
	...
	app.post("/api/totem/session/end", async (req, res, next) => {
		try {
			const session = await getTotemSessionFromRequest(req);
			if (session) {
				await documents.upsertDocument({
					...
					data: {
						...
						end_reason: String(req.body?.reason || "finished").trim(),
					},
				});
				await recordTotemAudit("KIOSK_SESSION_FINISHED", {
					email: session.email,
					sessionId: session.id,
					afterData: { reason: String(req.body?.reason || "finished") },
				});
			}
			res.json({ ok: true });
		} catch (error) {
	...
	app.get("/api/totem/insumos", async (req, res, next) => {
		try {
			const session = await requireTotemSession(req, res);
			if (!session) return;
			const result = await db.query(
				`select document_id as "documentId", data
				  ...
				  where collection_path = $1
				    ...
				    and case
				      when coalesce(data->>'estoque_atual', '') ~ '^[0-9]+(\\.[0-9]+)?$'
				      then (data->>'estoque_atual')::numeric
				      else 0
				    end > 0
				  order by lower(coalesce(data->>'categoria', 'Outros')), lower(data->>'nome')`,
				[INSUMOS_PRODUTOS_COLLECTION],
			);
			const items = result.rows.map((row) => ({
				id: row.documentId,
				nome: row.data?.nome || "Insumo",
				descricao: row.data?.observacao || row.data?.descricao || "",
				categoria: row.data?.categoria || "Outros",
				unidade: row.data?.unidade || "Unidade",
				estoque_atual: normalizeStockNumber(row.data?.estoque_atual),
				base_id: row.data?.base_id || "",
				base_nome: row.data?.base_nome || "",
				imagem: row.data?.imagem || row.data?.imageUrl || "",
			}));
			res.json({
				ok: true,
				user: session.user,
				items,
				categorias: [...new Set(items.map((item) => item.categoria || "Outros"))].sort((a, b) => a.localeCompare(b, "pt-BR")),
			});
		} catch (error) {
			next(error);
		}
	...
	app.post("/api/totem/requisicoes", async (req, res, next) => {
		try {
			const session = await requireTotemSession(req, res);
			if (!session) return;
			...
			const items = rawItems
				.map((item) => ({
					produtoId: String(item.produtoId || item.produto_id || "").trim(),
					quantidade: normalizeRequestQuantity(item.quantidade),
					observacao: String(item.observacao || req.body?.observacao || "").trim(),
				}))
				.filter((item) => item.produtoId && item.quantidade > 0);
			if (!items.length) {
				res.status(400).json({ error: "Selecione ao menos um item." });
				return;
			}
			if (items.length > 30) {
				res.status(400).json({ error: "Limite de 30 itens por pedido." });
				return;
			}

			const requisicao = await createInsumosRequestEntry({
				items,
				observacao: String(req.body?.observacao || "").trim(),
				user: session.user,
				origin: "KIOSK",
				kioskId: session.kioskId || "TOTEM-001",
			});
			const requisicoes = [requisicao];
			await emailService
				.sendTotemInsumosRequestCreatedEmail({
					to: session.email,
					nome: session.user?.nome || session.email,
					requisicoes,
				})
				.catch((error) => {
					console.error(
						"[totem] Falha ao enviar comprovante de requisicao:",
						error?.message || error,
					);
				});
			await recordTotemAudit("KIOSK_REQUEST_CREATED", {
				email: session.email,
				sessionId: session.id,
				afterData: {
					totalItems: requisicao.total_itens || requisicao.itens?.length || 1,
					protocolos: requisicoes.map((item) => item.protocolo),
				},
			});
			res.json({ ok: true, item: requisicao, items: requisicoes });
		} catch (error) {
			next(error);
		}
	});
	...
	app.get(
		...
		async (req, res, next) => {
			try {
				...
				const baseParams = [INSUMOS_REQUISICOES_COLLECTION];
				const baseWhere = ["collection_path = $1"];

				if (!manager) {
					baseParams.push(String(req.user?.uid || ""));
					baseWhere.push(`data->>'solicitante_uid' = $${baseParams.length}`);
				}
				...
				const summaryWhereSql = baseWhere.join(" and ");
				const countResult = await db.query(
					`select count(*)::int as total from app_documents where ${whereSql}`,
					params,
				);
				const summaryResult = await db.query(
					`select coalesce(data->>'status', 'pendente') as status, count(*)::int as total
					   from app_documents
					  where ${summaryWhereSql}
					  group by coalesce(data->>'status', 'pendente')`,
					baseParams,
				);
				const result = await db.query(
					`select path, document_id as "documentId", data
          ...
          limit $${params.length + 1} offset $${params.length + 2}`,
				...
				res.json({
					...
					summary: Object.fromEntries(
						summaryResult.rows.map((row) => [row.status, row.total]),
					),
					limit,
					offset,
					canManage: manager,
	...
	app.post(
		...
		async (req, res, next) => {
			try {
				const item = await createInsumosRequestEntry({
		...
		async (req, res, next) => {
			...
			try {
				...
				for (const item of requestItems) {
					...
					if (!produto) {
						await client.query("rollback");
						res.status(404).json({ error: `Produto ${item.produto_nome || item.produto_id} nao encontrado.` });
						return;
					}
					...
					if (item.quantidade > estoqueAtual) {
						...
						res
							.status(400)
							.json({
								error: `Estoque insuficiente para ${item.produto_nome || productData.nome || "Insumo"}. Disponivel: ${estoqueAtual} ${productData.unidade || item.unidade || ""}.`,
							});
						return;
					}
					productsToUpdate.push({ path: productPath, data: productData, item, estoqueAtual });
				}
				...
				for (const product of productsToUpdate) {
					await updateDocumentData(client, product.path, {
						...product.data,
						estoque_atual: product.estoqueAtual - product.item.quantidade,
				...
				updatedRequest = {
					...
					auditoria: addInsumosAudit(data, {
						...
						observacao: `Estoque reservado por 24h para ${summarizeInsumosRequestItems(requestItems)}. Expira em ${formatSaoPauloDateTime(expiresAt)}.`,
					}),
				};
		...
		async (req, res, next) => {
			...
			try {
				...
				for (const item of requestItems) {
					...
					retiradas.push({ id: retiradaId, ...retirada });
				}
				...
				updatedRequest = {
					...
					retirada_id: retiradas[0]?.id || "",
					retirada_ids: retiradas.map((item) => item.id),
					atualizado_em: now,
					auditoria: addInsumosAudit(data, {
						acao: "entregou",
						usuario_uid: req.user?.uid || "",
						usuario_nome: getUserDisplayName(req.user),
						observacao:
							"Itens entregues ao solicitante e contabilizados como retirada.",
					}),
				};
				await updateDocumentData(client, request.path, updatedRequest);
				await client.query("commit");
	...
	if (!app.locals.supplierEvaluationEmailTimer) {
		const runSupplierEvaluationEmails = () => {
			runSupplierEvaluationEmailRoutine()
				.then((result) => {
					if (result?.checked || result?.sent || result?.errors) {
						console.log("[facilities] rotina de avaliação de fornecedores:", result);
					}
				})
				.catch((error) => {
					console.error(
						"[facilities] Falha na rotina de avaliação de fornecedores:",
						error?.message || error,
					);
				});
		};
		const timer = setInterval(
			runSupplierEvaluationEmails,
			Number(process.env.SUPPLIER_EVALUATION_EMAIL_INTERVAL_MS || 60 * 60 * 1000),
		);
		timer.unref?.();
		app.locals.supplierEvaluationEmailTimer = timer;
		setTimeout(runSupplierEvaluationEmails, 30 * 1000).unref?.();
	}
	...
	app.use((error, req, res, next) => {
		...
		console.error("[api]", sanitizeErrorForLog(error));
