...
function CitiesModal({ regional, onClose, onChange }) {
	...
	const addCity = async (event) => {
		...
		try {
			const city = await createRotCity(regional.id, { name: newName.trim(), code: newCode.trim() });
	...
	const saveRename = async (city) => {
		if ((!editingName.trim() || editingName === city.name) && editingCode === (city.code || "")) { setEditingId(null); return; }
		setBusy(true); setError("");
		try {
			const updated = await updateRotCity(city.id, { name: editingName.trim() || city.name, code: editingCode.trim() });
	...
	return (
		<ModalShell open title={`Cidades — ${regional.name}`} description="Cidades usadas como cidade base dos responsáveis e no cadastro de usuários." icon={<span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-50 text-blue-600"><Building2 size={22} /></span>} onClose={onClose} size="md">
			...
			<div className="space-y-2">
				{cities.map((city) => (
					<div key={city.id} className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
						{editingId === city.id ? (
							<>
								<input value={editingName} onChange={(event) => setEditingName(event.target.value)} autoFocus onKeyDown={(event) => event.key === "Enter" && saveRename(city)} className="h-9 flex-1 rounded-lg border border-blue-300 px-3 text-sm font-semibold outline-none focus:ring-4 focus:ring-blue-100" />
								<input value={editingCode} onChange={(event) => setEditingCode(event.target.value.toUpperCase().slice(0, 8))} onKeyDown={(event) => event.key === "Enter" && saveRename(city)} placeholder="Código" className="h-9 w-24 rounded-lg border border-blue-300 px-2 text-sm font-semibold uppercase outline-none focus:ring-4 focus:ring-blue-100" />
							</>
						) : (
							<span className="flex-1 text-sm font-bold text-slate-800">{city.name}{city.code ? <span className="ml-2 rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-black text-blue-700">{city.code}</span> : null}</span>
						)}
						{editingId === city.id ? <button type="button" onClick={() => saveRename(city)} disabled={busy} className="rot-btn-tactile rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-black text-white hover:bg-emerald-700 disabled:opacity-60">Salvar</button> : <button type="button" onClick={() => { setEditingId(city.id); setEditingName(city.name); setEditingCode(city.code || ""); }} className="rot-btn-tactile flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-blue-50 hover:text-blue-600"><Pencil size={14} /></button>}
			...
			<p className="mt-3 text-xs font-semibold text-slate-400">O código é usado como "base" no cadastro de ativos e em rompimentos (ex: BMO = Brumadinho).</p>
			<form onSubmit={addCity} className="mt-2 flex gap-2 border-t border-slate-100 pt-4">
				<input value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="Nova cidade" className="h-10 flex-1 rounded-lg border border-slate-200 px-3 text-sm font-semibold outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100" />
				<input value={newCode} onChange={(event) => setNewCode(event.target.value.toUpperCase().slice(0, 8))} placeholder="Código" className="h-10 w-24 rounded-lg border border-slate-200 px-2 text-sm font-semibold uppercase outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100" />
				<button type="submit" disabled={busy} className="rot-btn-tactile inline-flex h-10 items-center gap-1.5 rounded-lg bg-blue-600 px-4 text-sm font-black text-white hover:bg-blue-700 disabled:opacity-60"><Plus size={15} /> Adicionar</button>
			</form>
		</ModalShell>
	);
}
