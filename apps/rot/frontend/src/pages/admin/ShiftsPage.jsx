...
function ShiftFormModal({ shift, technicians, regionals, onClose, onSaved }) {
	const isEdit = Boolean(shift);
	const [start, setStart] = useState(toInputValue(shift?.start) || "");
	const [end, setEnd] = useState(toInputValue(shift?.end) || "");
	const [regionalId, setRegionalId] = useState(shift?.regionalId || regionals[0]?.id || "");
	...
	const submit = async (event) => {
		...
		if (!start || !end || !regionalId) {
			setError("Informe início, fim e regional.");
			return;
		}
		setSaving(true);
		setError("");
		try {
			const payload = { start: new Date(start).toISOString(), end: new Date(end).toISOString(), regionalId, teamIds, notes };
			...
			<form onSubmit={submit} className="space-y-4">
				<div className="grid gap-3 sm:grid-cols-2">
					...
					<label className="block">
						...
						<input type="datetime-local" value={end} onChange={(e) => setEnd(e.target.value)} className="h-11 w-full rounded-xl border border-slate-200 px-4 text-sm font-semibold outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100" />
					</label>
				</div>
				<label className="block">
