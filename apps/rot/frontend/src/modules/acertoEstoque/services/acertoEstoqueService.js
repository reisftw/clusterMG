...
export async function updateAcertoEstoqueLancamento(id, payload) {
	const acerto = { ...payload, updatedAt: new Date().toISOString() };
	await updateVpsDocument(`${COLLECTIONS.acertos}/${id}`, acerto);
	return {
		store: await fetchAcertoEstoqueData(),
		acerto: { id, ...acerto },
	};
