...
async function generateAssetCode(client, payload) {
	...
	const code = `${pattern.prefix}-${String(next).padStart(Number(pattern.padding || 6), "0")}`;
...
router.get("/meta", requireRotPermission(["ativos.visualizar", "ativos.criar", "ativos.editar"]), async (_req, res, next) => {
	try {
		const [categories, types, statuses, criticalities, codePatterns, regionals, companies, technicians] = await Promise.all([
...
router.get("/dashboard", requireRotPermission("ativos.visualizar"), async (req, res, next) => {
	try {
		const params = [];
	...
	try {
		...
		const { rows } = await client.query(
			`insert into rot_assets (
				...
				asset_value, acquired_at, company_id, operation_scope, regional_id, status_id, criticality_id, high_value,
				critical_equipment, requires_checklist, inspection_frequency, structural_responsible_type, structural_responsible_id,
				custody_user_id, custody_technician_id, next_inspection_at, notes, active, created_by, last_movement_at
			) values (
			...
			[
				...
				payload.regionalId, payload.statusId, payload.criticalityId, payload.highValue, payload.criticalEquipment, payload.requiresChecklist,
				payload.inspectionFrequency, payload.structuralResponsibleType, payload.structuralResponsibleId, payload.custodyUserId,
				payload.custodyTechnicianId, payload.nextInspectionAt, payload.notes, payload.active, req.rotUser.id,
			],
		);
		await client.query(
...
router.post("/assets/:id/public-token", requireRotPermission("ativos.editar"), async (req, res, next) => {
	try {
		...
		const scope = scopeRegionalFilter(req);
		if (scope && before.regional_id !== scope) fail(403, "Regional não autorizada.");
	...
	try {
		...
		const { rows } = await client.query(
			`update rot_assets set
				...
				custody_technician_id=$24, next_inspection_at=$25, notes=$26, active=$27
...
router.get("/assets/:id/timeline", requireRotPermission("ativos.historico.visualizar"), async (req, res, next) => {
	try {
		const { rows: assetRows } = await db.query(`select regional_id from rot_assets where id=$1 and deleted_at is null`, [req.params.id]);
		if (!assetRows[0]) fail(404, "Ativo não encontrado.");
		const scope = scopeRegionalFilter(req);
		if (scope && assetRows[0].regional_id !== scope) fail(403, "Regional não autorizada.");
		const { rows } = await db.query(
...
router.post("/assets/:id/checklists", requireRotPermission("checklists.executar"), async (req, res, next) => {
	...
	try {
		...
		if (checklist.hasNok) {
			...
			const { rows } = await client.query(
				`update rot_assets set last_inspection_at=now(), next_inspection_at = coalesce(next_inspection_at, now() + interval '7 days') where id=$1 returning *`,
				[asset.id],
			);
			after = rows[0];
		}
...
router.get("/transfers", requireRotPermission("ativos.transferir"), async (req, res, next) => {
	try {
		const { rows } = await db.query(
			`select tr.*, a.code, a.name as asset_name, from_t.nome as from_technician_name, to_t.nome as to_technician_name, u.name as requested_by_name
...
router.post("/assets/:id/transfer", requireRotPermission("ativos.transferir"), async (req, res, next) => {
	...
	try {
		...
		if (!toTechnicianId) fail(400, "Informe o novo técnico responsável.");
		client = await db.connect();
		await client.query("begin");
		const asset = await loadAssetForUpdate(client, req, req.params.id);
		...
		if (req.body?.checklist?.answers?.length) {
			checklist = await executeChecklist(client, req, { assetId: asset.id, templateId: req.body.checklist.templateId, answers: req.body.checklist.answers });
		...
		const { rows: transfers } = await client.query(
			...
			[
				...
				nullableText(req.body?.toUserId),
				toTechnicianId,
		...
		if (!requiresAcceptance) {
			const { rows } = await client.query(
				...
				[asset.id, nullableText(req.body?.toUserId), toTechnicianId],
			);
			after = rows[0];
		}
...
router.post("/transfers/:id/accept", requireRotPermission("ativos.transferir"), async (req, res, next) => {
	let client;
	try {
		client = await db.connect();
		await client.query("begin");
...
router.post("/assets/:id/return", requireRotPermission("ativos.devolver"), async (req, res, next) => {
	let client;
	try {
		client = await db.connect();
		await client.query("begin");
		...
		if (req.body?.checklist?.answers?.length) {
			checklist = await executeChecklist(client, req, { assetId: asset.id, templateId: req.body.checklist.templateId, answers: req.body.checklist.answers });
		} else if (req.body?.checklistRequired) {
			fail(400, "Checklist de devolução obrigatório.");
		}
