...
router.get("/", noStore, async (req, res, next) => {
	try {
		...
		const [{ rows: allEquipments }, { rows: allAudits }, { rows: users }] = await Promise.all([
		...
		// rot.equipments.view so da acesso a pagina (tecnico comum tem isso,
		// pra poder abrir "Equipamentos" e ver o que esta com ele) — ver o
		// parque inteiro da regional exige rot.equipments.manage. Antes
		// qualquer autenticado via todo mundo independente do que tinha.
		const canViewAll = userHasRotPermission(req.rotUser, "rot.equipments.manage");
		const equipments = canViewAll ? allEquipments : allEquipments.filter((e) => e.tech_id === req.rotUser.id);
		const equipmentIds = new Set(equipments.map((e) => e.id));
		const audits = canViewAll ? allAudits : allAudits.filter((a) => equipmentIds.has(a.equipment_id));
		res.json({
			ok: true,
