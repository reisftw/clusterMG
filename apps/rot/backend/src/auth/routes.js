...
// -- Login local (usuario/senha) — preserva o fluxo atual da Operação (login
// por "username", nao e-mail), so trocando comparacao em texto puro por
// argon2, mesmo padrao do Finan. Depois de senha valida, passa por MFA
// por e-mail (se o usuario tiver e-mail cadastrado) antes de emitir o
// JWT — mesmo padrao de segurança do resto do ecossistema.
router.post("/login", loginRateLimit, validateBody(["username", "password"], { allowEmpty: false }), async (req, res, next) => {
	try {
		...
		const user = await loadUserWithRole("lower(u.username) = lower($1)", username);
...
router.post("/login/google", loginRateLimit, validateBody(["idToken"], { allowEmpty: false }), async (req, res, next) => {
	try {
		...
		const clientId = process.env.ROT_GOOGLE_OAUTH_CLIENT_ID;
		if (!clientId) {
		...
		const allowedDomains = String(process.env.ROT_GOOGLE_OAUTH_ALLOWED_DOMAINS || "")
			.split(",")
...
router.post("/forgot-password", passwordResetRateLimit, validateBody(["username"], { allowEmpty: false }), async (req, res, next) => {
	try {
		...
		// cautela padrao contra enumeracao de usuarios.
		const user = username ? await loadUserWithRole("lower(u.username) = lower($1)", username) : null;
		if (user?.email) {
