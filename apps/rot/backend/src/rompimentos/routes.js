...
router.post("/", requireRotPermission(["rot.rompimentos.view", "rot.rompimentos.manage"]), async (req, res, next) => {
	try {
		...
		return;
		const { rows: baseRows } = await db.query(`select * from rot_rompimento_bases where regional_id = $1`, [regionalId]);
...
router.put("/:id", requireRotPermission(["rot.rompimentos.view", "rot.rompimentos.manage"]), async (req, res, next) => {
	try {
		...
		const { rows: baseRows } = await db.query(`select * from rot_rompimento_bases where regional_id = $1`, [novoRegionalId]);
