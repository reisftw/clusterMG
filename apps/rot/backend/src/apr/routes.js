...
function access(req) {
 ...
 where r.id = a.regional_id and p->>'userId' = $2
 and p->>'type' in ('supervisor_field','supervisor_rot')))`,
 params: [scope, req.rotUser.id, broad] };
}
async function managers(client, regional) {
 const refs = Array.isArray(regional.responsaveis) ? regional.responsaveis : [];
 const ids = [...new Set(refs.filter(p => ["supervisor_field","supervisor_rot"].includes(p.type)).map(p => p.userId).filter(Boolean))];
...
router.get("/metadata", async (req,res,next) => {
 try {
  const scope = scopeRegionalFilter(req);
  const { rows } = await db.query("select id,name,responsaveis from rot_regionals where ($1::text is null or id=$1) order by name", [scope]);
  const items = [];
  for (const r of rows) items.push({ id:r.id, name:r.name, managers:await managers(db,r) });
...
router.post("/", createLimit, (req,res,next) => upload.array("photos",10)(req,res,e => {
 ...
 try {
  ...
  const {rows:regionals}=await client.query("select id,name,responsaveis from rot_regionals where id=$1",[regionalId]);
  if(!regionals[0]) fail(400,"Regional inválida.");
  const leads=await managers(client,regionals[0]);
