...
async function imageUrlToDataUrl(url) {
  ...
  if (!response.ok) throw new Error("Não foi possível carregar a logo do Cluster MG.");
...
export async function addClusterLogo(pdf, options = {}) {
  try {
    ...
    if (options.background !== false) {
      ...
      pdf.roundedRect(x - padding, y - padding, width + padding * 2, height + padding * 2, radius, radius, "F");
    }
