...
export const CARGOS_RETIRADAS = [
  ...
  { value: ROLES.SUPERVISOR_ADMINISTRATIVO, label: "Supervisor Administrativo" },
...
export const ROLE_VALUES = Object.freeze(CARGOS_RETIRADAS.map((cargo) => cargo.value));

export const ROLE_LABELS = Object.freeze(
  Object.fromEntries(CARGOS_RETIRADAS.map((cargo) => [cargo.value, cargo.label])),
);
...
export const PERMISSION_LABELS = Object.freeze({
  ...
  "financeiro.chamados.view": "Financeiro - Chamados",
...
export const isValidRole = (role) => ROLE_VALUES.includes(String(role ?? "").toLowerCase());
export const getRoleLabel = (role) => ROLE_LABELS[String(role || "").toLowerCase()] || role || "-";
...
const CATALOG_TO_LEGACY_PERMISSION = Object.freeze({
  ...
  "mensageria.confirmacao_agendamentos.manage": "manage_confirmacao_agendamentos",
...
export const hasPermission = (role, permission) => {
  ...
  const PERMISSIONS = {
    ...
    manage_users: [ROLES.ADMIN, ROLES.SUPERVISOR, ROLES.SUPERVISOR_ADMINISTRATIVO],
  ...
  if (typeof role === "object" && !Array.isArray(role)) {
    const dynamicPermissions = Array.isArray(role.permissions)
      ? role.permissions
      : Array.isArray(role.profile?.permissions)
    ...
    if (dynamicPermissions) {
      ...
      return (
        ...
        Boolean(managePermission && permissionSet.has(managePermission))
      );
    }
    role = role.role;
  }
  ...
  const hasLegacyPermission = userRoles.some(
    (currentRole) =>
      PERMISSIONS[permission]?.includes(String(currentRole).toLowerCase()) ?? false,
  );
  if (hasLegacyPermission) return true;
  const legacyPermission = CATALOG_TO_LEGACY_PERMISSION[permission];
  return Boolean(legacyPermission && hasPermission(userRoles, legacyPermission));
};
