...
const gerarHtmlFerias = (ferias) => `
  <h2 style="font-family:Arial;color:#1e40af;">📅 Férias</h2>
  <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-family:Arial;font-size:13px;width:100%">
    <thead style="background:#dbeafe">
      <tr><th>Colaborador</th><th>Início</th><th>Fim</th><th>Dias</th><th>Status</th></tr>
    </thead>
    <tbody>
      ${ferias.map((f) => `
        <tr>
...
const gerarHtmlFrota = (veiculos) => `
  <h2 style="font-family:Arial;color:#166534;">🚗 Frota</h2>
  <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-family:Arial;font-size:13px;width:100%">
    <thead style="background:#dcfce7">
      <tr><th>Placa</th><th>Modelo</th><th>KM Atual</th><th>Próxima Manutenção</th><th>Status</th></tr>
    </thead>
    <tbody>
      ${veiculos.map((v) => `
        <tr>
          <td>${v.placa}</td>
          <td>${v.modelo}</td>
          <td>${v.km_atual?.toLocaleString('pt-BR')}</td>
          <td>${v.km_proxima_manutencao?.toLocaleString('pt-BR')}</td>
          <td>${v.tecnico_id ? 'Em rota' : 'Disponível'}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
...
const gerarHtmlFeriados = (feriados) => `
  <h2 style="font-family:Arial;color:#7e22ce;">🗓️ Próximos Feriados</h2>
  <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-family:Arial;font-size:13px;width:100%">
...
export const gerarRelatorioHtml = ({ ferias = [], veiculos = [], feriados = [], incluir = {} }) => {
  const partes = [];
  const data   = new Date().toLocaleDateString('pt-BR');

  partes.push(`<div style="font-family:Arial">
    <h1 style="color:#1e293b;">Relatório Operacional — Sempre Internet</h1>
    <p style="color:#64748b;font-size:13px;">Gerado em: ${data}</p>
    <hr/>
  `);

  if (incluir.ferias   !== false) partes.push(gerarHtmlFerias(ferias));
  if (incluir.frota    !== false) partes.push(gerarHtmlFrota(veiculos));
