...
export function isVpsBackendEnabled() {
  return String(import.meta.env.VITE_DATA_BACKEND || "").toLowerCase() === "vps";
}
...
function isCsrfError(response, data) {
  return response.status === 403 && String(data?.error || "").toLowerCase().includes("csrf");
}
...
export async function listPublicVpsDocuments(collectionPath, { limit = 100, offset = 0 } = {}) {
  const params = new URLSearchParams({
    collection: collectionPath,
    limit: String(limit),
    offset: String(offset),
  });
  const response = await fetch(`${getApiBaseUrl()}/public/documents?${params.toString()}`, {
    cache: "no-store",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
...
export async function getPublicVpsDocument(documentPath) {
  const encodedPath = String(documentPath || "")
    ...
    .map(encodeURIComponent)
    .join("/");
  const response = await fetch(`${getApiBaseUrl()}/public/documents/${encodedPath}`, {
    cache: "no-store",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
...
export async function listAllPublicVpsDocuments(collectionPath, { pageSize = 1000, max = Infinity } = {}) {
  const items = [];
  let offset = 0;
...
export async function getVpsStaticSnapshot(domain) {
  const response = await fetch(`${getApiBaseUrl()}/public/static/${encodeURIComponent(domain)}`, {
    cache: "no-store",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
