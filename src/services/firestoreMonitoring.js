const globalMetrics = globalThis.__firestoreMetrics ?? {
  totalReads: 0,
  totalEvents: 0,
  bySource: {},
  byOperation: {},
...
globalThis.__firestoreMetrics = globalMetrics;
...
export function getFirestoreMetricsSummary() {
  return buildSummary();
}

export function printFirestoreMetricsSummary() {
  const summary = buildSummary();

  if (typeof console !== "undefined") {
    console.groupCollapsed(
      `[FIRESTORE] summary | reads=${summary.totalReads} | events=${summary.totalEvents}`,
    );
...
globalThis.__getFirestoreMetricsSummary = getFirestoreMetricsSummary;
globalThis.__printFirestoreMetricsSummary = printFirestoreMetricsSummary;
globalThis.__resetFirestoreMetrics = resetFirestoreMetrics;
...
export function logFirestoreRead({
  ...
  const prefix = cacheHit ? "CACHE" : "FIRESTORE";
