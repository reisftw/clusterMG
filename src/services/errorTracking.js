...
const SENSITIVE_KEY_PATTERN = /(authorization|token|password|senha|secret|csrf|cookie|api[-_]?key)/i;
...
function anonymizeUser(user) {
  ...
  for (let index = 0; index < String(id).length; index += 1) {
    hash = (hash << 5) - hash + String(id).charCodeAt(index);
    hash |= 0;
  }
  return { id: `u_${Math.abs(hash).toString(36)}`, role: user?.role || user?.perfil || "unknown" };
}

export function sanitizeSentryEvent(event) {
  const nextEvent = scrubObject(event);
  if (nextEvent?.request?.headers) nextEvent.request.headers = scrubObject(nextEvent.request.headers);
...
export function initErrorTracking() {
  ...
  const integrations = [];
  if (String(import.meta.env.VITE_SENTRY_REPLAY_ENABLED || "").toLowerCase() === "true") {
    integrations.push(
  ...
  Sentry.init({
    ...
    replaysOnErrorSampleRate: Number(import.meta.env.VITE_SENTRY_REPLAY_ON_ERROR_SAMPLE_RATE || 0),
