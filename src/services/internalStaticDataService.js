...
const INTERNAL_STATIC_DATA_UNAVAILABLE_UNTIL_KEY = "internal-static-data-unavailable-until";
...
function getUnavailableUntil() {
  ...
  const raw = window.localStorage.getItem(INTERNAL_STATIC_DATA_UNAVAILABLE_UNTIL_KEY);
...
function mergeSnapshotValues(base, next) {
  ...
  Object.entries(next).forEach(([key, value]) => {
    result[key] = key in result
      ? mergeSnapshotValues(result[key], value)
      : value;
  });
  return result;
}
