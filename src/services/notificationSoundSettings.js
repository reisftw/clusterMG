...
export function getNotificationSound() {
  ...
  return NOTIFICATION_SOUNDS.some((item) => item.value === saved) ? saved : "ping";
}

export function setNotificationSound(value) {
  if (typeof window === "undefined") return;
  const nextValue = NOTIFICATION_SOUNDS.some((item) => item.value === value) ? value : "ping";
  window.localStorage.setItem(NOTIFICATION_SOUND_STORAGE_KEY, nextValue);
  window.dispatchEvent(new CustomEvent("retiradas:notification-sound-updated", { detail: { sound: nextValue } }));
}
