...
function buildDbQuery({ user, profile, sessionActive = true }) {
  return vi.fn(async (sql, params = []) => {
    ...
    if (text.includes("from app_users") && text.includes("lower(trim(email))")) {
    ...
    if (text.includes("from app_documents") && text.includes("where path = $1")) {
      return { rows: params[0] === `usuarios/${user.uid}` ? [{ id: user.uid, data: profile }] : [] };
    }
    if (text.includes("update app_users") && text.includes("last_login_at")) {
      return { rows: [{ last_login_at: "2026-08-25T10:00:00.000Z", last_login_ip: "", last_login_user_agent: "" }] };
    }
    ...
    if (text.includes("from app_sessions")) return { rows: sessionActive ? [{ jti: params[0] }] : [] };
    return { rows: [] };
  });
}
...
describe("auth/RBAC isolated security tests", () => {
  ...
  it("login accepts valid credentials and rejects invalid credentials", async () => {
    ...
    const profile = { id: "user-1", email: user.email, nome: "Admin", role: "admin" };
    auth = loadAuthWithDb(buildDbQuery({ user, profile }));

    await expect(auth.loginWithPassword(user.email, "SenhaForte123!")).resolves.toMatchObject({
      expiresIn: auth.TOKEN_TTL_SECONDS,
      user: { id: "user-1", role: "admin" },
    });
    await expect(auth.loginWithPassword(user.email, "senha-errada")).rejects.toThrow("E-mail ou senha invalidos.");
  });
  ...
  it("requireAuthenticated blocks missing, invalid and revoked sessions", async () => {
    ...
    await auth.requireAuthenticated(createReq({ token: "" }), missingRes, vi.fn());
    expect(missingRes.statusCode).toBe(401);

    const invalidRes = createRes();
    await auth.requireAuthenticated(createReq({ token: "token-invalido" }), invalidRes, vi.fn());
    expect(invalidRes.statusCode).toBe(401);

    auth = loadAuthWithDb(buildDbQuery({ user, profile, sessionActive: false }));
    const revokedRes = createRes();
    await auth.requireAuthenticated(createReq({ token: session.token }), revokedRes, vi.fn());
  ...
  it("requireAuthenticated accepts active sessions and requireRoles blocks privilege escalation", async () => {
    ...
    const profile = { id: "user-1", email: user.email, nome: "Visitante", role: "visitante" };
    auth = loadAuthWithDb(buildDbQuery({ user, profile }));
    const session = await auth.loginWithPassword(user.email, "SenhaForte123!");
