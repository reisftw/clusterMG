...
function makeAuthMock(overrides = {}) {
  const auth = {
    ...
    requireAuthenticated: vi.fn((req, res, next) => {
      ...
      req.authPayload = { jti: "jti-1", exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 10 };
      req.user = {
        uid: "admin-1",
        ...
        profile: { uid: "admin-1", email: "admin@example.com", role, nome: "Admin" },
      };
      next();
    }),
    ...
    getImportedUserProfile: vi.fn(async () => ({ uid: "admin-1", email: "admin@example.com" })),
    ...
    createPasswordResetToken: vi.fn(async () => ({ resetUrl: "https://retiradas.tech/reset" })),
...
function baseMocks(overrides = {}) {
  ...
  const evolutionMessaging = {
    ...
    getConfig: vi.fn(async () => ({ officialWebhookVerifyToken: "verify-token" })),
  ...
  return {
    ...
    db: { healthcheck: vi.fn(async () => ({ now: "2026-08-16T00:00:00.000Z" })), query: vi.fn() },
    documents,
    evolutionMessaging,
    ...
    agendamentoEsteiraCommands: { runCommand: vi.fn(async () => ({ ok: true })) },
    operationalImports: {
...
function installMocks(overrides = {}) {
  ...
  setMock("./agendamentoEsteiraCommands", currentMocks.agendamentoEsteiraCommands);
...
describe("vps api app characterization - auth", () => {
  it("POST /api/auth/login autentica credenciais validas e define cookies", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body).toMatchObject({ ok: true, csrfToken: "valid-csrf", expiresIn: 3600 });
    expect(response.body.user).toMatchObject({ uid: "admin-1", email: "admin@example.com" });
    expect(response.headers["set-cookie"].join(";")).toContain("retiradas_session=session-token");
  });
  ...
  it("POST /api/auth/login rejeita credenciais invalidas sem status 4xx", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body).toMatchObject({ ok: false, error: "Credenciais invalidas." });
  });
  ...
  it("POST /api/auth/login aplica loginLimiter em excesso de tentativas", async () => {
    ...
    await request(app).post("/api/auth/login").send({ email: "admin@example.com", password: "wrong" });
    const response = await request(app).post("/api/auth/login").send({ email: "admin@example.com", password: "wrong" });

    expect(response.status).toBe(429);
    expect(response.body).toMatchObject({ ok: false, error: "Muitas tentativas. Aguarde alguns minutos." });
  });
  ...
  it("GET /api/auth/me exige autenticacao e retorna perfil quando autenticado", async () => {
    ...
    const authenticated = await request(app).get("/api/auth/me").set("Authorization", "Bearer valid");
  ...
  it("POST /api/auth/logout limpa cookie de sessao", async () => {
    ...
    expect(response.headers["set-cookie"].join(";")).toContain("retiradas_session=;");
  ...
  it("PUT /api/auth/avatar exige CSRF e aceita imagem valida com CSRF", async () => {
    ...
    expect(withoutCsrf.body).toMatchObject({ error: "Token CSRF invalido ou ausente." });
    expect(withCsrf.status).toBe(200);
    expect(withCsrf.body).toMatchObject({ ok: true });
    expect(withCsrf.body.avatarUrl).toMatch(/^\/api\/uploads\/avatars\/user-admin-1-/);
  });
  ...
  it("POST /api/auth/reset-password retorna sucesso para token valido e erro para token invalido", async () => {
    ...
    const success = await request(app).post("/api/auth/reset-password").send({ token: "reset-token", password: "new" });
    const failure = await request(app).post("/api/auth/reset-password").send({ token: "bad-token", password: "new" });
...
describe("vps api app characterization - permission and middleware helpers", () => {
  it("canReadCollection cobre sem permissao, permitido e admin", () => {
    ...
    expect(helpers.canReadCollection({ role: "lider_empresa" }, "ordens_abertas")).toBe(false);
    expect(helpers.canReadCollection({ role: "backoffice_retirada" }, "ordens_abertas")).toBe(true);
    expect(helpers.canReadCollection({ role: "admin" }, "api_integrations")).toBe(true);
  });

  it("canWriteCollection cobre sem permissao, permitido e admin", () => {
    const { helpers } = loadTestables();

    expect(helpers.canWriteCollection({ role: "lider_empresa" }, "ordens_abertas")).toBe(false);
    expect(helpers.canWriteCollection({ role: "supervisor" }, "ordens_abertas")).toBe(true);
    expect(helpers.canWriteCollection({ role: "admin" }, "api_integrations")).toBe(true);
  });

  it("canAccessRegionalRecord limita supervisor a propria regional", () => {
    const { helpers } = loadTestables();

    expect(helpers.canAccessRegionalRecord({ role: "admin" }, { regional: "Outra" })).toBe(true);
    expect(helpers.canAccessRegionalRecord({ role: "supervisor", regional: "Metropolitana SUB2" }, { regional: "Metropolitana SUB2" })).toBe(true);
    expect(helpers.canAccessRegionalRecord({ role: "supervisor", regional: "Metropolitana SUB2" }, { regional: "Alto Paranaiba" })).toBe(false);
  });

  it("canAccessEmpresaRecord limita lider de empresa e supervisor", () => {
    const { helpers } = loadTestables();

    expect(helpers.canAccessEmpresaRecord({ role: "admin" }, "empresa-1", { nome: "Outra" })).toBe(true);
    expect(helpers.canAccessEmpresaRecord({ role: "lider_empresa", empresaId: "empresa-1" }, "empresa-1", { nome: "Empresa" })).toBe(true);
    expect(helpers.canAccessEmpresaRecord({ role: "lider_empresa", empresaId: "empresa-1" }, "empresa-2", { nome: "Empresa" })).toBe(false);
    expect(helpers.canAccessEmpresaRecord({ role: "supervisor", regional: "Metropolitana SUB2" }, "empresa-2", { regional: "Metropolitana SUB2" })).toBe(true);
  });

  it("requireCsrfToken rejeita sem token e passa com token valido", () => {
    const { helpers } = loadTestables({
      auth: makeAuthMock({ verifyCsrfToken: vi.fn((req) => req.get("x-csrf-token") === "valid-csrf") }),
    });
    const res = createRes();
    const next = vi.fn();
    ...
    helpers.requireCsrfToken(createReq({ headers: { "x-csrf-token": "valid-csrf" } }), createRes(), next);
  ...
  it("requireInternalToken rejeita sem token e passa com token valido", () => {
    ...
    helpers.requireInternalToken(createReq({ headers: { "x-internal-api-token": process.env.INTERNAL_API_TOKEN } }), createRes(), next);
    expect(next).toHaveBeenCalledTimes(1);
  });
  ...
  it("rejectLargePublicVisit rejeita payload acima do limite e passa payload pequeno", () => {
    ...
    helpers.rejectLargePublicVisit(createReq({ headers: { "content-length": "11" } }), largeRes, next);
    expect(largeRes.statusCode).toBe(413);
    expect(largeRes.body).toMatchObject({ error: "Payload muito grande." });
    expect(next).not.toHaveBeenCalled();

    helpers.rejectLargePublicVisit(createReq({ headers: { "content-length": "10" } }), createRes(), next);
    expect(next).toHaveBeenCalledTimes(1);
  });
  ...
  it("verifyWebhookSecret rejeita segredo invalido e aceita query, header e body validos", () => {
    ...
    expect(helpers.verifyWebhookSecret(createReq(), invalidRes, "EVOLUTION_WEBHOOK_SECRET", "Evolution")).toBe(false);
    expect(invalidRes.statusCode).toBe(401);

    expect(helpers.verifyWebhookSecret(
    ...
    expect(helpers.verifyWebhookSecret(
      createReq({ headers: { "x-webhook-secret": process.env.EVOLUTION_WEBHOOK_SECRET } }),
      createRes(),
      "EVOLUTION_WEBHOOK_SECRET",
      "Evolution",
    )).toBe(true);
...
describe("vps api app characterization - public webhooks secret gate", () => {
  it("POST /api/webhooks/evolution rejeita sem segredo e aceita segredo via query/header/body", async () => {
    ...
    const rejected = await request(app).post(path).send({ event: "messages.upsert" });
    const viaQuery = await request(app).post(`${path}?secret=${process.env.EVOLUTION_WEBHOOK_SECRET}`).send({ event: "messages.upsert" });
    const viaHeader = await request(app).post(path).set("x-webhook-secret", process.env.EVOLUTION_WEBHOOK_SECRET).send({ event: "messages.upsert" });
    const viaBody = await request(app).post(path).send({ secret: process.env.EVOLUTION_WEBHOOK_SECRET, event: "messages.upsert" });
  ...
  it("POST /api/webhooks/cvortex rejeita sem segredo e aceita com segredo valido", async () => {
    ...
    const rejected = await request(app).post("/api/webhooks/cvortex").send({ event: "message" });
  ...
  it("POST /api/webhooks/whatsapp-official rejeita sem segredo e aceita com segredo valido", async () => {
    ...
    const rejected = await request(app).post("/api/webhooks/whatsapp-official").send({ object: "whatsapp_business_account" });
    const accepted = await request(app)
      .post("/api/webhooks/whatsapp-official")
      .send({ secret: process.env.WHATSAPP_OFFICIAL_WEBHOOK_SECRET, object: "whatsapp_business_account" });
