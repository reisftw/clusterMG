...
describe("antiBot Turnstile validation", () => {
  ...
  it("accepts a token confirmed by Cloudflare", async () => {
    ...
    await expect(antiBot.verifyTurnstileToken("valid-token", {})).resolves.toMatchObject({
      ok: true,
    });
  });
  ...
  it("rejects a token denied by Cloudflare", async () => {
    ...
    await expect(antiBot.verifyTurnstileToken("invalid-token", {})).resolves.toMatchObject({
      ok: false,
      statusCode: 400,
