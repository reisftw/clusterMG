        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-slate-500">
              Configure a API que fará o envio e recebimento das mensagens da mensageria.
            </p>
          </div>
        </div>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <StatusCard
        icon={MessageCircle}
        label="Mensageria"
        value={config.useCvortexForMessaging ? "Cvortex selecionada" : "Não selecionada"}
...
export default function CvortexSettingsPage() {
  ...
  const loadConfig = async () => {
    ...
    setError("");
    try {
      setConfig({ ...DEFAULT_CVORTEX_CONFIG, ...(await buscarConfigCvortex()) });
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setConfig({ ...DEFAULT_CVORTEX_CONFIG, ...saved, webhookSecret: secretPreview });
  ...
  const handleAssociate = async () => {
    ...
    try {
      ...
      setFeedback("Cvortex associada como provedora preferencial de mensageria.");
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Field label="Endpoint de envio">
            <TextInput
              ...
              onChange={(event) => updateConfig("sendEndpoint", event.target.value)}
          ...
          <Field label="Endpoint de status">
            <TextInput
              ...
              onChange={(event) => updateConfig("statusEndpoint", event.target.value)}
          ...
          <Field label="Identificador remetente">
            <TextInput
              ...
              onChange={(event) => updateConfig("defaultFrom", event.target.value)}
          ...
          <Field label="Segredo do webhook">
            <div className="flex flex-col gap-2 sm:flex-row">
              <TextInput
                ...
                placeholder={config.webhookSecretConfigured ? "Segredo já configurado. Preencha apenas para trocar." : "Use um segredo longo"}
              />
              <button
                type="button"
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <span>
              ...
              <span className="text-xs font-semibold text-slate-500">Permite validar status e receber webhooks.</span>
            </span>
          </label>
          <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <input
            ...
            />
            <span>
              <span className="block text-sm font-black text-slate-900">Usar Cvortex na mensageria</span>
              <span className="text-xs font-semibold text-slate-500">Use o botão de confirmação abaixo para virar a chave com segurança.</span>
            </span>
          </label>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          >
            {associating ? <Loader2 className="animate-spin" size={17} /> : <CheckCircle2 size={17} />} Usar Cvortex na mensageria
          </button>
        </div>
      ...
      <section className="rounded-3xl border border-emerald-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-950">Teste de envio</h2>
            <p className="text-sm font-semibold text-slate-500">
              Valide a Cvortex sem trocar a mensageria. O teste usa apenas o endpoint e o token configurados acima.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[320px_1fr_auto] lg:items-end">
          <Field label="Número de teste">
            <TextInput
              ...
              onChange={(event) => setSendTest((current) => ({ ...current, phone: event.target.value }))}
          ...
          <Field label="Mensagem">
            <TextInput
              ...
              onChange={(event) => setSendTest((current) => ({ ...current, message: event.target.value }))}
          ...
          >
            {sendingTest ? <Loader2 className="animate-spin" size={17} /> : <TestTube2 size={17} />} Enviar teste
          </button>
        </div>
        {sendTestResult ? (
          <pre className="mt-5 max-h-72 overflow-auto rounded-2xl bg-slate-950 p-4 text-xs font-semibold text-slate-100">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-950">Webhook de recebimento</h2>
            <p className="text-sm font-semibold text-slate-500">
              Cadastre esta URL na Cvortex para receber mensagens, status e callbacks no Retiradas.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-sm font-bold text-slate-800">Retiradas → Cvortex</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-black uppercase text-slate-500">Recebimento</p>
            <p className="mt-1 text-sm font-bold text-slate-800">Cvortex → Retiradas</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-black uppercase text-slate-500">Segurança</p>
            <p className="mt-1 text-sm font-bold text-slate-800">Secret por query ou header</p>
          </div>
        </div>
      </section>
    </div>
