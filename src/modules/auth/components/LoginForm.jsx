  ...
  return (
    <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs font-bold text-blue-700">
        ...
        <button type="button" onClick={onReset} className="text-blue-800 underline">
          Voltar para login
        </button>
      </div>
    </div>
...
function CredentialFields({ email, password, showPassword, onEmailChange, onPasswordChange, onTogglePassword }) {
  return (
    <>
      <div>
...
const LoginForm = () => {
  const { login, verifyEmailMfa, loginWithGoogle, loginWithOkta, error } = useAuthContext();
  ...
  const handleOktaLogin = () => {
    if (!oktaConfig.enabled || !oktaConfig.issuer || !oktaConfig.clientId) return;
    const nonceBytes = new Uint8Array(16);
    window.crypto.getRandomValues(nonceBytes);
    const nonce = Array.from(nonceBytes, (byte) => byte.toString(16).padStart(2, "0")).join("");
    window.sessionStorage.setItem("retiradas_okta_nonce", nonce);
    const redirectUri = oktaConfig.redirectUri || `${window.location.origin}/login`;
  ...
  useEffect(() => {
    if (!googleConfig.enabled || !googleConfig.clientId || !googleButtonRef.current) return undefined;
    let cancelled = false;
        ...
        <aside className="relative hidden overflow-hidden bg-[radial-gradient(circle_at_78%_22%,rgba(255,255,255,0.18),transparent_24%),linear-gradient(135deg,#0048df_0%,#0068ff_48%,#ff8a00_100%)] px-10 py-9 text-white lg:flex lg:flex-col xl:px-16">
          ...
          <div className="relative z-10">
            <img
              src="/cluster-mg.png"
              alt="Sempre Internet"
              className="h-20 w-auto object-contain drop-shadow-lg xl:h-24"
            />
          </div>
          ...
          <div className="relative z-20 mt-28 max-w-xl xl:mt-32">
            ...
            <p className="mt-6 max-w-lg text-xl leading-relaxed text-white/92">
              Gerencie a operação de retiradas em um único lugar.
            </p>
          </div>
        ...
        <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_82%_20%,rgba(19,94,168,0.42),transparent_28%),radial-gradient(circle_at_12%_84%,rgba(255,122,0,0.09),transparent_28%),linear-gradient(135deg,#082b55_0%,#061f3f_48%,#04162c_100%)] px-5 pb-16 pt-10 sm:px-8">
          ...
          <div className="relative z-10 w-full max-w-[610px]">
            <section className="rounded-[34px] border border-white/80 bg-white/90 px-6 py-8 shadow-[0_30px_90px_rgba(0,0,0,0.32)] backdrop-blur-xl sm:px-12 sm:py-10">
              ...
              <form onSubmit={mfaChallenge ? handleMfaSubmit : handleSubmit} className="mt-9 space-y-5">
              {mfaChallenge ? (
                <MfaCodePanel
              ...
              >
                <LogIn size={24} />
                {isSubmitting ? "Entrando..." : mfaChallenge ? "Validar código" : "Entrar no sistema"}
              </button>
              {!mfaChallenge && googleConfig.enabled && googleConfig.clientId ? (
              ...
              {!mfaChallenge && oktaConfig.enabled && oktaConfig.issuer && oktaConfig.clientId ? (
                <button
                  type="button"
