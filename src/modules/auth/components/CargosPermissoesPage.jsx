...
import { listarCargosPermissoes, salvarCargoPermissoes } from "../services/rolesService";
...
function normalizePermissions(permissions = []) {
  return [...new Set(permissions.filter(Boolean))].sort((a, b) => a.localeCompare(b, "pt-BR"));
}
...
function normalizeCatalog(items = []) {
  return items
    ...
    .sort((a, b) => a.sortOrder - b.sortOrder || a.id.localeCompare(b.id, "pt-BR"));
}
...
function expandCatalogPermissions(rawPermissions = [], catalog = []) {
  if (rawPermissions.includes("*")) return catalog.map((permission) => permission.id);

  const raw = new Set(rawPermissions);
  const expanded = new Set();
  catalog.forEach((permission) => {
    if (raw.has(permission.id) || (permission.legacyPermission && raw.has(permission.legacyPermission))) {
...
function withPermissionPairRules(permissionIds, permission, checked, catalogById) {
...
export default function CargosPermissoesPage() {
  ...
  const groupedCatalog = useMemo(() => groupCatalog(permissionCatalog), [permissionCatalog]);
  ...
  const loadRoles = async () => {
    ...
    try {
      ...
      const nextRoles = data.roles?.length ? data.roles : DEFAULT_ROLE_OPTIONS;
      ...
      setSelectedId((current) => current || DEFAULT_ROLE_OPTIONS[0]?.id || "");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    setDraft({
      ...selectedRole,
      permissions: expandCatalogPermissions(selectedRole.permissions || [], permissionCatalog),
    });
  ...
  const selectedPermissions = useMemo(() => draft?.permissions || [], [draft?.permissions]);

  const selectedCount = useMemo(
    () => selectedPermissions.filter((permission) => catalogIds.has(permission)).length,
  ...
  const handleClone = () => {
    ...
    const next = {
      ...
      permissions: normalizePermissions(
        (draft.permissions || []).filter((permission) => catalogIds.has(permission)),
      ),
    };
    setRoles((current) => [...current, next]);
    setSelectedId(next.id);
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 max-w-3xl text-sm font-medium text-slate-500">
                Configure por seção do menu o que cada cargo pode visualizar ou gerenciar.
              </p>
            </div>
          </div>
      ...
      {!canManage ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">
          Você está em modo somente leitura. Alterações de cargos exigem permissão de gerenciamento.
        </div>
      ) : null}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-slate-500">
                  Simule outro cargo para conferir menus, permissões e visualizações do sistema.
                </p>
                {isViewingAsRole ? (
      ...
      <div className="grid gap-5 xl:grid-cols-[330px_1fr]">
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {roles.map((role) => {
              const expanded = expandCatalogPermissions(role.permissions || [], permissionCatalog);
              return (
                <button
                  type="button"
                  key={role.id}
            ...
            <>
              <div className="grid gap-4 lg:grid-cols-[1fr_1fr_auto]">
                ...
                <label>
                  ...
                  <input
                    value={draft.id || ""}
                    onChange={(event) => setField("id", normalizeId(event.target.value))}
              ...
              {isAdminRole ? (
                <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">
                  O cargo Admin é protegido e mantém acesso total para evitar bloqueio do sistema.
                </div>
              ) : null}

              <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-800">
                {selectedCount} permissão(ões) novas selecionadas. Gerenciar marca Visualizar automaticamente.
              </div>
              ...
              <div className="mt-5 space-y-4">
                {groupedCatalog.length ? (
                          ...
                          return (
                            ...
                            >
                              <div className="min-w-0">
                                <p className="break-anywhere text-sm font-black text-slate-900">{feature.label}</p>
                                {feature.description ? (
                  ...
                  <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm font-bold text-slate-400">
                    Catálogo novo de permissões não carregado. Rode a migration 017 antes de editar cargos.
                  </div>
                )}
              </div>
