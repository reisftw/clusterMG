...
export const useAuth = () => {
	...
	const verifyEmailMfa = useCallback(
		async ({ challengeId, code, turnstileToken = "" }) => {
			setError(null);
			setLoading(true);

			try {
				const credential = await verificarMfaEmail({
					challengeId,
					code,
					turnstileToken,
				});
	...
	const loginWithGoogle = useCallback(
		async (credential, turnstileToken = "") => {
			setError(null);
			setLoading(true);

			try {
				const googleCredential = await loginWithGoogleCredential(
					credential,
					turnstileToken,
				);
				...
				try {
					...
					console.error(
						"Erro ao buscar perfil após login Google:",
						profileError,
					);
				}
				setCurrentUser(profile);
	...
	const loginWithOkta = useCallback(
		async ({ credential, nonce, turnstileToken = "" }) => {
			setError(null);
			setLoading(true);

			try {
				const oktaCredential = await loginWithOktaCredential({
					credential,
					nonce,
					turnstileToken,
				});
