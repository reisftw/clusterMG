...
export const loginWithEmail = async (email, password, turnstileToken = "") => {
	const session = await requestPublicAuth("/auth/login", {
		email,
		password,
		turnstileToken,
	});
...
export const verificarMfaEmail = async ({
	challengeId,
	code,
	turnstileToken = "",
}) => {
	const session = await requestPublicAuth("/auth/mfa/email/verify", {
		challengeId,
		code,
		turnstileToken,
	});
...
export const loginWithGoogleCredential = async (
	...
	const session = await requestPublicAuth("/auth/google", {
		credential,
		turnstileToken,
	});
	if (!session?.user?.id) {
...
export const loginWithOktaCredential = async ({
	credential,
	nonce,
	turnstileToken = "",
}) => {
	const session = await requestPublicAuth("/auth/okta", {
		credential,
		nonce,
		turnstileToken,
	});
...
export const redefinirSenhaComToken = async (
	...
	requestPublicAuth("/auth/reset-password", {
		token,
		password,
		turnstileToken,
	});
...
export const listarEmpresasAdmin = async () => {
	return (
		await listVpsDocuments(COLLECTIONS.EMPRESAS_TECNICOS, { limit: 1000 })
	)
		.map((empresa) => ({
			id: empresa.id,
