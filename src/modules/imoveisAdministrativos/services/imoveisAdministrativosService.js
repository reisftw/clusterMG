...
export async function listarImoveis({ status = "" } = {}) {
  ...
  const response = await requestVpsApi(`${BASE_PATH}${query ? `?${query}` : ""}`);
...
export async function garantirPastaImovel(id) {
  const response = await requestVpsApi(`${BASE_PATH}/${encodeURIComponent(id)}/pasta`, {
    method: "POST",
    body: JSON.stringify({}),
  });
  return response;
...
export async function importarImoveis(items = []) {
...
export async function cadastrarContratoLink(id, dados = {}) {
  return requestVpsApi(`${BASE_PATH}/${encodeURIComponent(id)}/contratos/link`, {
    method: "POST",
    body: JSON.stringify(dados),
  });
}

export async function enviarContratoImovel(id, { file, nome = "", observacao = "" } = {}) {
  const formData = new FormData();
  formData.append("file", file);
  formData.append("nome", nome);
  formData.append("observacao", observacao);
  return requestVpsApi(`${BASE_PATH}/${encodeURIComponent(id)}/contratos/upload`, {
    method: "POST",
    body: formData,
  });
}

export async function enviarAnexoImovel(id, { file, nome = "", categoria = "", tipo = "", data = "", observacao = "" } = {}) {
  const formData = new FormData();
  formData.append("file", file);
...
export async function enviarAditivoImovel(id, { file, nome = "", data = "", observacao = "" } = {}) {
  const formData = new FormData();
...
export async function editarContratoImovel(id, contratoId, dados = {}) {
  return requestVpsApi(`${BASE_PATH}/${encodeURIComponent(id)}/contratos/${encodeURIComponent(contratoId)}`, {
    method: "PUT",
    body: JSON.stringify(dados),
  });
}
...
export async function obterRelatoriosImoveis({ mes = "", ano = "" } = {}) {
  ...
  const query = params.toString();
  return requestVpsApi(`${BASE_PATH}/relatorios/dados${query ? `?${query}` : ""}`);
}
