...
export function calculateLocalReport({ imoveis = [], iptus = [], alugueis = [], reajustes = [] } = {}) {
  const gastosIptu = iptus.reduce((sum, item) => sum + toCurrencyNumber(item.valor), 0);
  ...
  const gastosAluguel = alugueis.length
    ...
    : alugados.reduce((sum, item) => sum + toCurrencyNumber(item.valorAluguel), 0);
