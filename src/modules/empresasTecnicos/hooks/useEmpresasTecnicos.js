...
export function useEmpresasTecnicos() {
  ...
  const carregar = useCallback(async () => {
    ...
    setError("");
    try {
      const [empresasData, supervisoresData] = await Promise.all([
        buscarEmpresasTecnicos(),
        buscarSupervisores().catch(() => []),
      ]);
      setEmpresas(empresasData);
      setSupervisores(supervisoresData);
      setUsuariosEmpresa(await buscarUsuariosEmpresa().catch(() => []));
    } catch (err) {
      setError(err?.message || "Nao foi possivel carregar empresas e tecnicos.");
    } finally {
      setLoading(false);
    }
  }, []);
