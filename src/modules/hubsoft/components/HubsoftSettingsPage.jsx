        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-slate-500">
              Configure o endpoint, valide o OAuth e prepare a leitura das O.S. direto do Hubsoft.
            </p>
          </div>
        </div>
    ...
    <section className="grid gap-4 lg:grid-cols-3">
      ...
      <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4">
        ...
        <p className="mt-1 text-xs font-semibold text-orange-700">
          {config.lastError ? `Erro: ${config.lastError}` : "Sem erro registrado"}
        </p>
      </div>
    </section>
  );
...
export default function HubsoftSettingsPage() {
  ...
  const loadConfig = async () => {
    ...
    setError("");
    try {
      setConfig({ ...DEFAULT_HUBSOFT_CONFIG, ...(await buscarConfigHubsoft()) });
      const runs = await buscarHistoricoSyncHubsoft(8).catch(() => ({ items: [] }));
  ...
  const handleAssociate = async () => {
    ...
    try {
      ...
      setConfig({ ...DEFAULT_HUBSOFT_CONFIG, ...saved });
      setFeedback("Hubsoft associado. A integração ficou marcada como origem preferencial.");
    } catch (err) {
  ...
  const handleSync = async (mode) => {
    ...
    try {
      ...
      setError(err?.message || "Não foi possível iniciar a sincronização Hubsoft.");
      setSyncing(false);
    }
  };
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-5 flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-950">Credenciais OAuth</h2>
            <p className="text-sm font-semibold text-slate-500">
              O backend usa essas credenciais para gerar o Bearer token. Segredos salvos não são exibidos novamente.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => updateConfig("grantType", event.target.value)}
              className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-50"
            />
          </label>
          <label className="block">
          ...
          <label className="block">
            <span className="text-xs font-black uppercase text-slate-500">
              Client Secret {config.clientSecretConfigured ? "(já configurado)" : ""}
            </span>
            <input
              type="password"
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          >
            {testing ? <Loader2 className="animate-spin" size={17} /> : <CheckCircle2 size={17} />} Validar endpoint
          </button>
          <button
            type="button"
          ...
          >
            {associating ? <Loader2 className="animate-spin" size={17} /> : <DatabaseZap size={17} />} Associar Hubsoft
          </button>
        </div>

        {testResult ? (
          <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">
            Token válido. Atualizado em {formatDateTime(testResult.tokenUpdatedAt)} e expira em{" "}
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-5 flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-950">Consulta teste de O.S.</h2>
            <p className="text-sm font-semibold text-slate-500">
              Use uma busca real para confirmar o retorno antes de substituir as planilhas.
            </p>
          </div>
        </div>
        <div className="grid gap-3 lg:grid-cols-[220px_1fr_160px_140px]">
          <select
            value={query.busca}
            onChange={(event) => setQuery((current) => ({ ...current, busca: event.target.value }))}
          ...
          <input
            ...
            onChange={(event) => setQuery((current) => ({ ...current, termo_busca: event.target.value }))}
          ...
          >
            {searching ? <Loader2 className="animate-spin" size={17} /> : <Search size={17} />} Consultar
          </button>
        </div>
        {queryResult ? (
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-950">Sincronização Mapa e Match</h2>
              <p className="text-sm font-semibold text-slate-500">
                Prepare a leitura automática do Hubsoft. Use prévia primeiro; produção substitui os dados atuais.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              {syncing ? <Loader2 className="animate-spin" size={17} /> : <Search size={17} />} Gerar prévia
            </button>
            <button
              type="button"
              disabled={syncing}
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-black text-white transition hover:bg-slate-800 disabled:opacity-60"
            >
              {syncing ? <Loader2 className="animate-spin" size={17} /> : <DatabaseZap size={17} />} Sincronizar produção
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <label className="block">
            <span className="text-xs font-black uppercase text-slate-500">Tipo de busca padrão</span>
            <select
          ...
          <label className="block">
            <span className="text-xs font-black uppercase text-slate-500">Limite por chamada</span>
            <input
              type="number"
              min="1"
              max="50"
              value={config.syncLimit || 50}
              onChange={(event) => updateConfig("syncLimit", Number(event.target.value))}
          ...
          <label className="block lg:col-span-2">
            <span className="text-xs font-black uppercase text-slate-500">Termos de busca opcionais</span>
            <textarea
              value={(Array.isArray(config.syncTermos) ? config.syncTermos : []).join("\n")}
              onChange={(event) => updateConfig("syncTermos", event.target.value.split(/[\n,;]+/).map((item) => item.trim()).filter(Boolean))}
              rows={3}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 p-4">
            <p className="text-xs font-black uppercase text-slate-500">Status de O.S.</p>
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            <p className="text-xs font-black uppercase text-slate-500">Fontes atualizadas</p>
            <div className="mt-3 flex flex-wrap gap-2">
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            ...
            <label className="mt-3 inline-flex items-center gap-2 text-sm font-black text-slate-700">
              <input
                ...
                onChange={(event) => updateConfig("syncMatchEnabled", event.target.checked)}
                className="h-4 w-4"
              />
        ...
        {syncJob ? (
          <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-black text-blue-950">{syncJob.stage || "Processando"}</p>
                <p className="text-xs font-semibold text-blue-700">
                  Status: {syncJob.status || "-"} {syncJob.error ? `| ${syncJob.error}` : ""}
                </p>
              </div>
              <span className="text-lg font-black text-blue-950">{Number(syncJob.percent || 0)}%</span>
            </div>
            <div className="mt-3 h-3 overflow-hidden rounded-full bg-white">
              <div
                className="h-full rounded-full bg-blue-600 transition-all"
                style={{ width: `${Math.min(Math.max(Number(syncJob.percent || 0), 0), 100)}%` }}
              />
            </div>
          ...
          <div className="mt-5 overflow-x-auto rounded-2xl border border-slate-200">
            <table className="min-w-[760px] w-full divide-y divide-slate-200 text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {syncRuns.map((run) => (
                  <tr key={run.id}>
                    <td className="px-4 py-3 font-semibold text-slate-700">{formatDateTime(run.createdAt)}</td>
                    <td className="px-4 py-3 font-semibold text-slate-700">{run.mode || "-"}</td>
                    <td className="px-4 py-3 font-black text-slate-900">{run.status || "-"}</td>
                    <td className="px-4 py-3 font-semibold text-slate-700">{run.totalRows || 0}</td>
                    <td className="px-4 py-3 font-semibold text-slate-700">{run.mapaTotal || 0}</td>
                    <td className="px-4 py-3 font-semibold text-slate-700">{run.matchTotal || 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
