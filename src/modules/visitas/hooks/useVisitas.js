...
export const useVisitas = () => {
  ...
  const atualizar = useCallback(
    async (id, dados) => {
      ...
      setVisitas((prev) =>
        sortVisitas(prev.map((item) => (item.id === id ? { ...item, ...dados } : item))),
      );
    },
    [currentUser?.id, currentUser?.nome],
  );
  ...
  const criarTecnico = useCallback(
    async (dados) => {
      ...
      await registrarAtividade({
        ...
        modulo: "visitas",
        detalhes: { nome: dados?.nome || null, regional: dados?.regional || null },
      });
  ...
  const atualizarTecnico = useCallback(
    async (id, dados) => {
      ...
      setTecnicos((prev) =>
        sortTecnicos(prev.map((item) => (item.id === id ? { ...item, ...dados } : item))),
      );
    },
    [currentUser?.id, currentUser?.nome],
  );
  ...
  const regionais = useMemo(
    () =>
      [...new Set(tecnicos.map((item) => item.regional).filter(Boolean))].sort((a, b) =>
