...
export const gerarRelatorioVisitasPDF = async ({
  ...
  const totalRegionais = visitas.filter((item) => item.tecnico_tipo !== "retirada").length;
  const totalRetirada = visitas.filter((item) => item.tecnico_tipo === "retirada").length;
  ...
  pdf.text(`Tecnicos regionais: ${totalRegionais} - ${currency(totalRegionais * Number(valorVisita || 0))}`, 40, 128);
  pdf.text(`Tecnicos de retirada: ${totalRetirada} - ${currency(totalRetirada * Number(valorVisita || 0))}`, 40, 144);
  ...
  autoTable(pdf, {
    ...
    body: Object.entries(porTecnico).map(([tecnico, items]) => [
      tecnico,
      items.some((item) => item.tecnico_tipo === "retirada") ? "Retirada" : "Regional",
      items.length,
  ...
  autoTable(pdf, {
    ...
    head: [["Data", "Codigo do cliente", "Tecnico", "Tipo", "Status", "Observacao"]],
  ...
  pdf.save(`visitas-${regional}-${mesLabel}.pdf`.replace(/\s+/g, "-").toLowerCase());
};
