...
const VisitasPage = () => {
	...
	const tecnicosFiltrados = useMemo(() => {
		...
		return tecnicos.filter((item) => {
			const regionalOk =
				tecnicoRegionalFiltro === "todas" ||
				item.regional === tecnicoRegionalFiltro;
			const textoOk =
				!termo ||
				String(item.nome || "")
					.toLowerCase()
	...
	const tecnicoTotalPages = Math.max(
		1,
		Math.ceil(tecnicosFiltrados.length / 8),
	);
	...
	const visitasFiltradas = useMemo(() => {
		...
		return visitasDoMesClassificadas.filter((item) => {
			const regionalOk =
				regionalFiltro === "todas" || item.regional === regionalFiltro;
			const textoOk =
				!termo ||
	...
	const resumo = useMemo(() => {
		const porRegional = groupCount(
			visitasDoMesClassificadas,
			(item) => item.regional,
		);
		const porTecnico = groupCount(
			visitasDoMesClassificadas,
			(item) => item.tecnico_nome,
		);
		const validas = visitasDoMesClassificadas.filter(
			(item) => item.status !== "Recusada",
		);
		const regionaisValidas = validas.filter(
			(item) => item.tecnico_tipo !== "retirada",
		);
		const retiradasValidas = validas.filter(
			(item) => item.tecnico_tipo === "retirada",
		);
		return {
	...
	const salvarVisita = async (event) => {
		...
		if (
			!visitaForm.tecnico_nome.trim() ||
			!visitaForm.codigo_cliente.trim() ||
			!visitaForm.data
		) {
			return;
		}
	...
	const salvarValor = async (event) => {
		event.preventDefault();
		await salvarConfig({
			valorVisita: Number(valorConfig ?? config.valorVisita ?? 0),
		});
		setValorConfig(null);
	};
						...
						<div className="space-y-6">
							<div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
								<p className="mb-4 text-sm font-bold text-gray-900">
									Visitas por regional
								</p>
								<div className="space-y-3">
									{sortedEntries(resumo.porRegional).map(
										([regional, total]) => (
											<div key={regional}>
												...
												<div className="h-2 rounded-full bg-gray-100">
													<div
														...
														style={{
															width: `${(total / Math.max(1, resumo.total)) * 100}%`,
														}}
													/>
												</div>
											</div>
					...
					>
						<div className="space-y-4">
							...
							<div>
								...
								<input type="hidden" value={visitaForm.regional} readOnly />
							</div>
							...
							<div className="flex items-center justify-between rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
								...
								<div className="flex items-center gap-2">
									<button
										type="button"
										onClick={() =>
											setTecnicoPage((current) => Math.max(1, current - 1))
										}
									...
									<button
										type="button"
										onClick={() =>
											setTecnicoPage((current) =>
												Math.min(tecnicoTotalPages, current + 1),
											)
										}
				...
				<div className="space-y-4">
					...
					<div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
						{regionais.map((regional) => {
							const dados = visitasDoMesClassificadas.filter(
								(item) =>
									item.regional === regional && item.status !== "Recusada",
							);
							...
							return (
								...
								>
									...
									<div className="space-y-2">
										{sortedEntries(
											groupCount(dados, (item) => item.tecnico_nome),
										).map(([tecnico, total]) => {
											const tipo =
												dados.find((item) => item.tecnico_nome === tecnico)
													?.tecnico_tipo === "retirada"
													? "Retirada"
													: "Regional";
											return (
												<div
						...
						{!regionais.length && (
							<div className="rounded-2xl border border-gray-100 bg-white p-12 text-center shadow-sm lg:col-span-2">
								...
								<p className="text-sm text-gray-400">
									Cadastre tecnicos para gerar regionais.
								</p>
							</div>
						)}
					</div>
			...
			{activeTab === "config" && (
				<div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
					...
					<div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
						...
						<p className="mt-2 text-sm text-gray-500">
							O valor configurado e usado para calcular o gasto mensal e os PDFs
							por regional.
						</p>
