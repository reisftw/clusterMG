  ...
  return Object.fromEntries(
    META_BASES.map(({ id }) => {
      ...
      const seasonalPercentByMonth = Object.fromEntries(
        META_MESES.map((mes) => [
          mes,
          sanitizePercent(rawSeasonal[mes], defaults.seasonalPercentByMonth[mes]),
        ]),
      );
      ...
      return [
        ...
        {
          mode,
          fixedPercent: sanitizePercent(raw.fixedPercent ?? raw.percentualFixo, defaults.fixedPercent),
...
export function getBaseIdFromMetaData(data = {}) {
  ...
  if (origem.includes("ONNET") && origem.includes("SEMPRE")) return "onnetSempre";
...
function applyMetaConfigToRecord(data, config, baseId, mes) {
  ...
  return {
    ...
    meta,
    metaSazonal,
    metaMode: normalizeMetasBaseConfig(config)[baseId]?.mode || META_MODES.SAZONAL,
    metaModeLabel,
    percentAchieved,
    saldoDiario,
    technicians: recalculatePerformanceItems(data.technicians, meta, metaOriginal),
...
export function applyMetasBaseConfigToAllData(allData = {}, config = null) {
  ...
  return Object.fromEntries(
    Object.entries(allData || {}).map(([mes, dadosMes]) => {
      ...
      const sempre = applyMetaConfigToRecord(dadosMes, normalized, "sempre", mes);
      return [
        mes,
        {
          ...sempre,
          onnet: applyMetaConfigToRecord(dadosMes.onnet, normalized, "onnet", mes),
