...
async function gerarPDF(dados, feriadosSet) {
  ...
  const resumoRows = MESES
    ...
    .map((mes) => {
      ...
      const status =
        parseFloat(m.percentAchieved) >= 100
          ? "Atingido"
          : falta > 0
  ...
  autoTable(doc, {
    startY: y,
    head: [["Mes","Cancelamentos","Regra Meta","Meta OS","Realizado","% Total","Saldo Final","Status"]],
    body: resumoRows,
  ...
  for (const mes of mesesComDados) {
    ...
    if (m.saldoDiario?.length) {
      ...
      autoTable(doc, {
        startY: y,
        head: [["Dia","Equipe Tec.","Agente","Loja","Regionais","Total Dia","Meta Dia","Saldo Dia","Saldo Mes"]],
