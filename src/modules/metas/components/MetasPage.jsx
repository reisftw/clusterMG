...
function temDadosOperacionais(dadosMes) {
  return Boolean(
    dadosMes &&
      (
        Number(dadosMes.totalOS || 0) > 0 ||
        Number(dadosMes.meta || 0) > 0
      ),
  );
}
...
function getForcaTaskContext(config, allData, agentesData) {
  if (!config?.ativa) {
    return { available: false, reason: "Ative a forca tarefa para gerar os textos." };
  }

  const start = parseLocalDate(config.inicio);
  const end = parseLocalDate(config.fim);
  if (!start || !end || start > end) {
    return { available: false, reason: "Configure um periodo valido para a forca tarefa." };
  }
  ...
  const startIndex = start.getMonth() === yesterday.getMonth() ? start.getDate() - 1 : 0;
  const endIndex = end.getMonth() === yesterday.getMonth() ? end.getDate() - 1 : dayIndex;
  ...
  const regionaisDetalhes = (dadosMes.regionais || [])
    .map((item) => ({
      ...
      acumulado: (item.daily || []).reduce(
        (sum, value, index) => (inRange(index) ? sum + Number(value || 0) : sum),
        0,
      ),
    }))
  ...
  const acumuladoRegionais = regionaisDetalhes.reduce((sum, item) => sum + item.acumulado, 0);
  const acumuladoAgentes = agentesDetalhes.reduce((sum, item) => sum + item.acumulado, 0);
...
function buildForcaTaskMessages(context) {
  ...
  const linhasRegionais = context.regionais.detalhes.length
    ? context.regionais.detalhes
        .map((item, index) => `${index + 1}. ${item.name}: ${formatNumber(item.ontem)} ontem | ${formatNumber(item.acumulado)} no periodo`)
        .join("\n")
...
function ForceTaskCopyMessages({ config, allData, agentesData }) {
  ...
  const cards = [
    { key: "regionais", title: "Mensagem para regionais", text: messages.regionais },
    { key: "agentes", title: "Mensagem para agentes autorizados", text: messages.agentes },
  ];
  ...
  return (
    <section className="rounded-2xl border border-blue-100 bg-blue-50 p-5 lg:col-span-2">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {cards.map((card) => (
          <div key={card.key} className="rounded-2xl border border-blue-100 bg-white p-4">
    ...
    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-2xl border border-gray-100 bg-white p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-sm font-bold text-gray-900">Configurar forca tarefa</h2>
        ...
        <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <label className="space-y-1.5">
              <span className="text-xs font-semibold text-gray-600">Meta regionais</span>
              <input
                type="number"
                min="0"
                className="input-field"
                ...
                disabled={!canManage || saving}
                onChange={(event) => updateMeta("regionais", event.target.value)}
              />
            </label>
            <label className="space-y-1.5">
              <span className="text-xs font-semibold text-gray-600">Meta agente autorizado</span>
              <input
                type="number"
                min="0"
                className="input-field"
            ...
            <label className="space-y-1.5">
              <span className="text-xs font-semibold text-gray-600">Meta tecnicos retirada</span>
              <input
                type="number"
                min="0"
                className="input-field"
      ...
      <section className="rounded-2xl border border-orange-100 bg-orange-50 p-5">
        ...
        <h3 className="mt-2 text-xl font-extrabold text-gray-900">Forca tarefa</h3>
        <p className="mt-2 text-sm text-gray-600">
          O /painel vai somar apenas as entregas entre as datas configuradas e comparar
          com as metas abaixo.
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl bg-white p-4">
            <p className="text-xs text-gray-500">Regionais</p>
            <p className="mt-1 text-2xl font-black text-blue-700">{form.metas.regionais}</p>
          </div>
          <div className="rounded-xl bg-white p-4">
            <p className="text-xs text-gray-500">Agente autorizado</p>
            <p className="mt-1 text-2xl font-black text-blue-700">{form.metas.agentes}</p>
          </div>
          <div className="rounded-xl bg-white p-4">
            <p className="text-xs text-gray-500">Tecnicos retirada</p>
            <p className="mt-1 text-2xl font-black text-blue-700">{form.metas.tecnicos}</p>
          </div>
        </div>
      </section>
...
function MetasBaseConfigModal({
  ...
  return (
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          {error ? (
            ...
            <p className="text-xs font-semibold text-slate-500">
              A alteração recalcula os cards e relatórios sem precisar resubir a planilha.
            </p>
          )}
    ...
    >
      <form id="metas-base-config-form" className="space-y-4" onSubmit={handleSubmit}>
        {META_BASES.map((base) => {
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Esta base é independente das outras e será avaliada pela regra abaixo.
                  </p>
                </div>
                <div className="grid gap-2 sm:grid-cols-2">
                  <label className="rounded-xl border border-white bg-white px-3 py-2 shadow-sm">
                    ...
                    <select
                      ...
                      onChange={(event) => updateBase(base.id, { mode: event.target.value })}
                  ...
                  <label className="rounded-xl border border-white bg-white px-3 py-2 shadow-sm">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateBase(base.id, { fixedPercent: event.target.value })
                      }
                      disabled={!isFixa}
                    ...
                    >
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <input
                          ...
                          onChange={(event) =>
                            updateSeasonalMonth(base.id, mes, event.target.value)
                          }
            ...
            <div className="flex flex-wrap items-center gap-2">
              {MESES.map((m) => (
                <button
                  ...
                  disabled={!temDadosNaFonte(allData[m], fonteDados) && mesSelecionado !== m}
                >
                  {m}
                </button>
              ))}
                ...
                {FONTES_DADOS.map((fonte) => {
                  ...
                  return (
                    <button
                      ...
                      title={disponivel ? fonte.label : "Sem dados nesta fonte para o mes selecionado"}
                    >
                      {fonte.label}
                    </button>
                  );
