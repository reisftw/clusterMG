...
const MetasUpload = ({ onUpload, uploading, lastUpdate, onRefresh }) => {
	...
	const handleFile = async (e) => {
		...
		try {
			...
			alert(
				"Erro ao processar a planilha. Verifique o arquivo e tente novamente.",
			);
		}
		e.target.value = "";
	};
	...
	return (
		<div className="flex items-center gap-2 flex-wrap">
			...
			<StaticDataRefreshLink className="flex items-center gap-2 px-4 py-2 rounded-xl border border-blue-200 text-blue-700 hover:bg-blue-50 hover:border-blue-300 text-sm font-semibold transition-colors" />
		</div>
	);
};
