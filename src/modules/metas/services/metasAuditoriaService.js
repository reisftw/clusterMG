...
function shouldReplaceCidade(current, next) {
  ...
  const currentTotal = Number(current.total || 0) + Number(current.cancelamentos || 0);
...
export const salvarAuditoriaAgentes = async (mes, cidades) => {
  ...
  cidades.forEach((cidade) => {
    const key = normalizaTexto(normalizaCidade(cidade.cidade)).replace(/\s+/g, "_");
  ...
  const { data } = await getOrLoadCachedValue(
    ...
    async () => {
      ...
      for (const mes of meses) {
        const item = await getVpsDocument(`${COL}/${mes}/cidades/${cidadeKey}`).catch(() => null);
