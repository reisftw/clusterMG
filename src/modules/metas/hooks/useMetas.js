...
const DASHROW = {
  Janeiro: 4, Fevereiro: 5, 'Marco': 6, Abril: 7,
...
const TECHNAMES = [
  ...
  'JOAO EZIQUIEL', 'PAULO XAVIER', 'CLAUDSON FARIA', 'ANDRE PAULA',
];
const REGNAMES = [
...
function findRowByLabelInColumn(ws, label, column = 'B', startRow = 1, endRow = 60) {
  if (!ws) return null;
  const target = normalizeSheetName(label);
  for (let row = startRow; row <= endRow; row++) {
    if (normalizeSheetName(ws[`${column}${row}`]?.v ?? '') === target) return row;
  }
  return null;
}
...
function findCellColumnByLabel(ws, label, row, startCol = 0, endCol = 60) {
  ...
  for (let col = startCol; col <= endCol; col++) {
    if (normalizeSheetName(ws[encCol(col) + row]?.v ?? '') === target) return col;
  }
  return null;
}
...
function findMonthlyLayout(ws, fallbackCfg) {
  ...
  const agenteRow = findRowByLabelInColumn(ws, 'Agente Autorizado') ?? fallbackCfg.agenteRow;
  const lojaRow = findRowByLabelInColumn(ws, 'Entregue em Loja') ?? fallbackCfg.lojaRow;
...
function parseOnnetMonth(wb, mes, feriadosExtras = []) {
  ...
  if (firstRegionalRow && lastRegionalRow && lastRegionalRow >= firstRegionalRow) {
  ...
  const totalRegionais = regionais.reduce((sum, item) => sum + Number(item.total || 0), 0);
  ...
  for (let index = 0; index < dayCount; index++) {
    ...
    const regTot = regionais.reduce((sum, item) => sum + Number(item.daily?.[index] || 0), 0);
...
function mergePerformanceItems(items = []) {
  ...
  items.forEach((item) => {
    ...
    current.daily = Array.from({ length: maxLength }, (_, index) =>
...
function combineMonthlyData(sempreData, onnetData, mes, feriadosExtras = []) {
  ...
  const sourceTotalOS = Number(sempreData.totalOS || 0) + Number(onnetData.totalOS || 0);
  const cancelamentos =
    Number(sempreData.cancelamentos || 0) + Number(onnetData.cancelamentos || 0);
  const metaSazonal = Number(sempreData.metaSazonal || onnetData.metaSazonal || 80);

  const days = new Map();
  [sempreData, onnetData].forEach((source) => {
    const rawDays = Array.isArray(source.rawDays) && source.rawDays.length
      ? source.rawDays
  ...
  const totalOS = totalDiarioConsolidado > 0 ? totalDiarioConsolidado : sourceTotalOS;
  const percentAchieved =
    meta > 0 ? Number(((totalOS / meta) * 100).toFixed(1)) : 0;
...
export function parseMetasWorkbook(wb, feriadosExtras = []) {
  ...
  MONTHORDER.forEach((mes) => {
    ...
    const multCols = {
      Janeiro: 9, Fevereiro: 10, 'Marco': 11, Abril: 12,
    ...
    const { diasUteis: du, metaDiariaMedia, metaPorDia, metaAcumuladaPorDia } =
    ...
    const percentAchieved =
      pctRaw > 0
        ...
        : meta > 0
          ...
          : 0;
    const propMult =
      totalOS > 0 && totalMultas > 0
        ? (totalOS / totalMultas).toFixed(1)
        : 0;
    const agenteDai = [];
    const lojaDai = [];
    for (let d = cfg.dayColStart; d <= cfg.dayColEnd; d++) {
    ...
    result[mes] = {
      ...
      ...(onnet
        ? {
            onnet,
            onnetSempre: combineMonthlyData(sempreData, onnet, mes, feriadosExtras),
          }
        : {}),
    };
  });
...
export const useMetas = () => {
  ...
  useEffect(() => {
    return subscribeRealtimeTopics(["metas"], () => {
      invalidateMetasCache();
      invalidateForcaTarefaConfigCache();
      invalidateMetasBaseConfigCache();
  ...
  const salvarConfiguracaoMetasBase = useCallback(async (config) => {
    ...
    try {
      ...
      setAllData((current) => applyMetasBaseConfigToAllData(current, nextConfig));
      invalidateMetasCache();
      invalidateMetasBaseConfigCache();
      invalidateDashboardDataCache();
      invalidateInternalStaticDataCache();
