...
describe("useMetas", () => {
  ...
  it("detecta coluna total e linhas pela aba mensal", () => {
    ...
    expect(result.Junho.saldoDiario).toEqual([
      expect.objectContaining({ dia: 1, equipe: 1, agente: 2, loja: 3, totalDia: 6 }),
    ]);
  });
  ...
  it("interpreta abas onnet e monta a visao consolidada com sempre", () => {
    ...
    expect(result.Julho.saldoDiario[0]).toEqual(
      expect.objectContaining({ dia: 1, equipe: 4, agente: 3, loja: 3, totalDia: 10 }),
    );
    expect(result.Julho.saldoDiario.find((item) => item.dia === 15)).toEqual(
      expect.objectContaining({ dia: 15, equipe: 0, agente: 2, loja: 1, regionais: 5, totalDia: 8 }),
    );
    ...
    expect(result.Julho.onnetSempre.rawDays.find((item) => item.dia === 15)).toEqual(
      expect.objectContaining({ dia: 15, agente: 2, loja: 1, regionais: 15, totalDia: 18 }),
    );
    expect(result.Julho.onnetSempre.saldoDiario.find((item) => item.dia === 15)).toEqual(
      expect.objectContaining({ dia: 15, agente: 2, loja: 1, regionais: 15, totalDia: 18 }),
    );
  });
  ...
  it("continua carregando com feriados vazios quando a consulta de feriados falha", async () => {
    metasMocks.buscarFeriados.mockRejectedValue(new Error("feriados indisponiveis"));
    metasMocks.buscarTodasMetas.mockResolvedValue({
  ...
  it("encerra o carregamento mesmo quando a leitura inicial falha", async () => {
    metasMocks.buscarTodasMetas.mockRejectedValue(new Error("backend indisponivel"));
