...
export const exportMetasPDF = (dados) => {
  ...
  // KPIs
  doc.text(`Cancelamentos: ${Math.round(dados.meta / 0.8).toLocaleString('pt-BR')}`, 14, 45);
  doc.text(`Meta 80%: ${Math.round(dados.meta).toLocaleString('pt-BR')}`, 14, 55);
  doc.text(`Realizado: ${Number(dados.totalOS).toLocaleString('pt-BR')}`, 14, 65);
