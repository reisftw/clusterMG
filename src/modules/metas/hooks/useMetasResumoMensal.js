...
export function useMetasResumoMensal(
  ...
  const kpis = useMemo(() => {
    ...
    const percentualAtingido = parseFloat(dadosMesSelecionado.percentAchieved);
    ...
    return [
      {
        label: "Cancelamentos",
        value: Math.round(dadosMesSelecionado.cancelamentos).toLocaleString("pt-BR"),
      ...
      },
      {
        label: "Falta",
        value:
          Math.max(0, dadosMesSelecionado.meta - dadosMesSelecionado.totalOS) > 0
            ? Math.max(
                0,
