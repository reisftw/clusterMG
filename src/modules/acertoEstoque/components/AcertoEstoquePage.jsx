...
const AcertoEstoquePage = () => {
  ...
  const agendasOrdenadas = useMemo(
    () =>
      [...store.agendas].sort((left, right) => {
        const weekdayDiff = getAgendaWeekdayIndex(left) - getAgendaWeekdayIndex(right);
  ...
  const historicoTotalPages = Math.max(1, Math.ceil(acertosFiltrados.length / PAGE_SIZE));
  ...
  const previewData = useMemo(() => {
    ...
    const tecnicoLancamentos = (lancamentoForm.lancamentos || [])
      .map((lancamento) => {
        const tecnico = store.tecnicos.find((item) => item.id === lancamento.tecnicoId);
      ...
      return [
        {
          ...
          itens: Array.isArray(selectedAcerto.itens) ? selectedAcerto.itens : [],
        },
      ];
    }
  ...
  const handleAgendaSubmit = async (event) => {
    ...
    const ok = await salvarEntidade(
      "agendas",
      agendaForm,
      agendaForm.id ? "Agenda atualizada com sucesso." : "Agenda criada com sucesso.",
    );
  ...
  const handleProdutoSubmit = async (event) => {
    ...
    const ok = await salvarEntidade(
      "produtos",
      produtoForm,
      produtoForm.id ? "Produto atualizado com sucesso." : "Produto criado com sucesso.",
    );
  ...
  const handleLancamentoSubmit = async (event) => {
    ...
    const lancamentos = (lancamentoForm.lancamentos || [])
      ...
      .filter((lancamento) => lancamento.tecnicoId && lancamento.itens.length > 0);
    ...
    const tecnicoNomes = [...new Set(tecnicoLancamentos.map((item) => item.tecnicoNome).filter(Boolean))];
    const empresaNomes = [...new Set(tecnicoLancamentos.map((item) => item.empresaNome).filter(Boolean))];
    ...
    const acerto = await lancarAcerto({
      ...
      createdBy: currentUser?.id || currentUser?.uid || currentUser?.email || "",
      createdByName: currentUser?.nome || currentUser?.name || currentUser?.email || "",
    ...
    setLancamentoForm((current) => ({
      ...
      lancamentos: current.lancamentos.map((lancamento, index) =>
        index === lancamentoIndex
          ? {
              ...lancamento,
              itens: [...(lancamento.itens || []), { produtoId: "", quantidade: 1 }],
            }
          : lancamento,
      ),
    }));
          ...
          ? {
              ...
              itens:
                lancamento.itens.length === 1
                  ? lancamento.itens
                  : lancamento.itens.filter((_, currentItemIndex) => currentItemIndex !== itemIndex),
            }
          : lancamento,
      ),
    }));
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-orange-100 bg-gradient-to-r from-orange-50 via-white to-blue-50 p-6 shadow-sm xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Acerto de Estoque</h1>
          </div>
        </div>
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              ...
              >
                ...
                <p className="mt-1 text-xs text-gray-500">Configurar cidade, dia e turno</p>
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              >
                ...
                <p className="mt-1 text-xs text-gray-500">Adicionar item ao estoque de acerto</p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="mt-1 text-xs text-orange-50">Ir direto para o lancamento</p>
              </button>
            </div>
          </SectionCard>

          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <MetricCard label="Empresas" value={metrics.totals.empresas} tone="blue" />
            <MetricCard label="Tecnicos" value={metrics.totals.tecnicos} tone="orange" />
            <MetricCard label="Produtos" value={metrics.totals.produtos} tone="emerald" />
            <MetricCard label="Acertos" value={metrics.totals.acertos} tone="slate" />
            <MetricCard
                    ...
                    <tbody>
                      {metrics.latestAcertos.map((item) => (
                        <tr key={item.id} className="border-b border-gray-50 last:border-0">
                          <td className="px-2 py-3 font-semibold text-gray-800">{item.codigo}</td>
                          <td className="px-2 py-3 text-gray-500">{formatDate(item.dataAcerto)}</td>
                          <td className="px-2 py-3 text-gray-500">{item.cidade}</td>
                          <td className="px-2 py-3 text-gray-500">
                            {item.totalTecnicos || item.tecnicoNomes?.length || 1}
                          </td>
                          <td className="px-2 py-3 text-gray-500">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {item.cidade} - {item.totalTecnicos || item.tecnicoNomes?.length || 1} tecnico(s)
                          </p>
                        </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {metrics.porCidade.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-2xl font-extrabold text-orange-600">{item.total}</p>
                      <p className="text-xs text-gray-400">acerto(s) registrados</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="space-y-6">
          ...
          {cadastroTab === "agendas" ? (
            <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
              <SectionCard title="Cadastro de agenda" subtitle="Cidade, dia fixo e turno do acerto">
                <form className="space-y-4" onSubmit={handleAgendaSubmit}>
                  <InputField label="Cidade / localidade">
                    <input
                      ...
                      onChange={(event) =>
                        setAgendaForm((current) => ({ ...current, cidade: event.target.value }))
                      }
                      required
                    />
                  </InputField>
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <InputField label="Turno">
                      <select
                        ...
                        onChange={(event) =>
                          setAgendaForm((current) => ({ ...current, turno: event.target.value }))
                        }
                      >
              ...
              <SectionCard title="Agendas cadastradas" subtitle={`${store.agendas.length} agenda(s)`}>
                {agendasOrdenadas.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {agendasOrdenadas.map((item) => (
                      ...
                      >
                        <div>
                          <p className="font-semibold text-gray-900">{item.cidade}</p>
                          <p className="text-xs text-gray-500">
                            {formatDiaSemana(item.diaSemana)} - {formatTurno(item.turno)}
                          </p>
                        </div>
                        <div className="flex gap-2">
                          <button
          ...
          {cadastroTab === "produtos" ? (
            <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
              <SectionCard title="Cadastro de produto" subtitle="Materiais usados no acerto">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <InputField label="Status">
                      <select
                        ...
                        onChange={(event) =>
                          setProdutoForm((current) => ({ ...current, status: event.target.value }))
                        }
                      >
              ...
              <SectionCard title="Produtos cadastrados" subtitle={`${store.produtos.length} produto(s)`}>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {paginatedCadastroProdutos.map((item) => (
                            <tr key={item.id} className="border-b border-gray-50 last:border-0">
                              <td className="px-2 py-3 font-semibold text-gray-800">{item.nome}</td>
                              <td className="px-2 py-3 text-gray-500">{item.categoria || "-"}</td>
                              <td className="px-2 py-3 text-gray-500">{item.unidade}</td>
                              <td className="px-2 py-3">
                                <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${getStatusBadgeClasses(item.status)}`}>
                                  {item.status}
                                </span>
                              </td>
                              <td className="px-2 py-3">
                                <div className="flex gap-1">
                                  ...
                                  <button
                                    type="button"
                                    className="rounded-lg px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-50"
                                    onClick={() => handleDelete("produtos", item.id)}
                                  >
                                    Excluir
                                  </button>
                                </div>
      ...
      {activeTab === "lancamento" ? (
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <SectionCard title="Novo acerto" subtitle="Lancamento por cidade com varios tecnicos no mesmo acerto">
            <form className="space-y-5" onSubmit={handleLancamentoSubmit}>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-bold text-gray-900">Tecnicos incluidos no acerto</p>
                  <button
                    type="button"
                ...
                {(lancamentoForm.lancamentos || []).map((lancamento, lancamentoIndex) => (
                  <div
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        <InputField label={"Tecnico " + (lancamentoIndex + 1)}>
                          <select
                            className="input-field"
                          ...
                          >
                            ...
                            {tecnicosDisponiveis.map((item) => (
                              <option key={item.id} value={item.id}>
                                {item.nome} • {empresasMap.get(item.empresaId)?.nome || "-"}
                              </option>
                            ))}
                          </select>
                        </InputField>
                      ...
                      <button
                        ...
                        onClick={() => removeTecnicoLancamento(lancamentoIndex)}
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                      ...
                      {(lancamento.itens || []).map((item, itemIndex) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => removeTecnicoItem(lancamentoIndex, itemIndex)}
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
              ...
              action={
                <button
                  ...
                  onClick={async () => {
                    ...
                    showCopiedMessage("preview-whatsapp", "WhatsApp copiado com sucesso.");
                  }}
                >
                  <Copy size={14} />
                  Copiar WhatsApp
              ...
              action={
                <button
                  ...
                  onClick={async () => {
                    ...
                    showCopiedMessage("preview-email", "E-mail copiado com sucesso.");
                  }}
                >
                  <Copy size={14} />
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-gray-500">
                            Feito por: {item.createdByName || item.createdBy || "-"}
                          </p>
                        </div>
              ...
              selectedAcerto ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-50"
                    onClick={async () => {
                      await copyHtml(selectedAcertoEmailHtml, selectedAcertoWhatsapp);
                      showCopiedMessage(
              ...
              <div className="space-y-5">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl bg-gray-50 p-4">
                    ...
                    <p className="mt-2 font-bold text-gray-900">{selectedAcerto.codigo}</p>
                  </div>
                  <div className="rounded-2xl bg-gray-50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">
                      Registrado em
                  ...
                  <div className="rounded-2xl bg-gray-50 p-4">
                    ...
                    <p className="mt-2 font-bold text-gray-900">
                      {selectedAcerto.createdByName || selectedAcerto.createdBy || "-"}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-gray-50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">
                    ...
                    <p className="mt-2 font-bold text-gray-900">
                      {selectedAcerto.cidade} - {formatTurno(selectedAcerto.turno)}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-gray-50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">
                    ...
                    <p className="mt-2 font-bold text-gray-900">
                      {selectedAcerto.totalTecnicos || selectedAcerto.tecnicoNomes?.length || 1} tecnico(s) - {(selectedAcerto.empresaNomes || [selectedAcerto.empresaNome]).filter(Boolean).join(", ")}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-bold text-gray-900">Tecnicos e materiais do acerto</p>
                    ...
                    {selectedAcertoLancamentos.map((lancamento, index) => (
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 px-4 py-4">
                          <div>
                            <p className="font-semibold text-gray-900">{lancamento.tecnicoNome || "-"}</p>
                            <p className="mt-1 text-xs text-gray-500">
                              {lancamento.empresaNome || "-"}
                              {lancamento.responsavel ? ` - Responsavel: ${lancamento.responsavel}` : ""}
                            </p>
                          </div>
                ...
                <div className="space-y-5">
                  <div>
                    <p className="mb-2 text-sm font-bold text-gray-900">WhatsApp</p>
                    <textarea
                      readOnly
                  ...
                  <div>
                    <p className="mb-2 text-sm font-bold text-gray-900">E-mail</p>
                    <div
                      className="rounded-2xl border border-gray-200 bg-white"
