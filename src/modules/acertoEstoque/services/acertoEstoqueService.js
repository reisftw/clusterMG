...
function sortByName(items = []) {
  return [...items].sort((a, b) =>
    String(a.nome || a.tecnicoNome || a.cidade || a.codigo || "")
      .localeCompare(String(b.nome || b.tecnicoNome || b.cidade || b.codigo || ""), "pt-BR"),
  );
}

function sortAcertos(items = []) {
  return [...items].sort((a, b) =>
    String(b.createdAt || b.dataAcerto || "").localeCompare(String(a.createdAt || a.dataAcerto || "")),
  );
}
  ...
  if (entity === "empresas") {
    return sortByName(
      items.map((item) => ({
        ...
        emailResponsavel:
          typeof item.responsavel === "object" ? item.responsavel?.email || "" : item.emailResponsavel || "",
...
export async function fetchAcertoEstoqueData() {
  ...
  const acertosNormalizados = acertos.map((acerto) => {
    ...
    const tecnicoLancamentos = Array.isArray(acerto.tecnicoLancamentos) && acerto.tecnicoLancamentos.length > 0
      ? acerto.tecnicoLancamentos
      : (Array.isArray(acerto.lancamentos) ? acerto.lancamentos : []).map((lancamento) => {
          ...
          return {
            ...
            itens: (Array.isArray(lancamento.itens) ? lancamento.itens : []).map((item) => {
    ...
    const tecnicoNomes = uniqueValues(tecnicoLancamentos.map((item) => item.tecnicoNome));
    const empresaNomes = uniqueValues(tecnicoLancamentos.map((item) => item.empresaNome));
...
export async function saveAcertoEstoqueEntity(entity, item) {
  if (entity === "empresas" || entity === "tecnicos") {
    throw new Error("Empresas e tecnicos devem ser cadastrados no modulo Empresas.");
  }
  ...
  if (entity === "empresas") {
    ...
    payload.responsavel = {
      nome: typeof item?.responsavel === "string" ? item.responsavel : item?.responsavel?.nome || "",
