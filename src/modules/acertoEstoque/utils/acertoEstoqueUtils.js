...
function normalizeTecnicoLancamentos(acerto = {}) {
  if (Array.isArray(acerto.tecnicoLancamentos) && acerto.tecnicoLancamentos.length > 0) {
...
export function buildDashboardMetrics(store) {
  ...
  const acertosSemana = acertos.filter((item) => isSameWeek(now, item.dataAcerto));
  const porEmpresa = empresas
    .map((empresa) => ({
      empresaId: empresa.id,
      nome: empresa.nome,
      totalTecnicos: tecnicos.filter((tecnico) => tecnico.empresaId === empresa.id).length,
      totalAcertos: acertos.filter((acerto) =>
