...
const EquipamentosPage = () => {
  ...
  const { equipamentos, loading, carregar, criar, atualizar, excluir } = useEquipamentos();
  ...
  const handleSalvar = async (dados) => {
    if (dados.id) { const { id, ...rest } = dados; await atualizar(id, rest); }
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{equipamentos.length} equipamento(s) cadastrado(s)</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {lista.length === 0 ? (
                <tr>
                  <td colSpan={podeEditar ? 7 : 6} className="px-5 py-12 text-center">
                ...
                <tr key={eq.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3 font-semibold text-gray-800">{eq.modelo}</td>
                  <td className="px-5 py-3 font-mono text-xs text-gray-500">{eq.patrimonio || '—'}</td>
                  <td className="px-5 py-3">
                    {eq.responsavel ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">{eq.responsavel}</span>
                      </div>
                    ) : (
                  ...
                  <td className="px-5 py-3">
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg border ${STATUS_BADGE[eq.status] ?? 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                      {eq.status}
                    </span>
                  </td>
