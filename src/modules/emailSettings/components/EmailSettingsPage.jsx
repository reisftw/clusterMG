...
import {
	...
	enviarTesteEmailSistema,
	listarLogsEmailSistema,
	salvarConfigEmailSistema,
...
function useEmailSettingsController() {
	...
	const [activeTemplate, setActiveTemplate] = useState("welcome_user");
	...
	const loadData = async () => {
		...
		setFeedback("");
		try {
			const next = await buscarConfigEmailSistema();
			setConfig(next);
			setTestTo(next.replyTo || next.smtpUser || "");
		} catch (error) {
			setFeedback(
				error?.message || "Não foi possível carregar a configuração de e-mail.",
			);
		} finally {
			setLoading(false);
		}
	...
	const loadLogs = async (page = logsPage) => {
		setLogsLoading(true);
		try {
			const response = await listarLogsEmailSistema({
			...
			setFeedback(
				error?.message || "Não foi possível carregar os logs de e-mail.",
			);
		} finally {
	...
	const updateTemplate = (field, value) => {
		setConfig((current) => ({
			...current,
			templates: {
				...(current.templates || DEFAULT_EMAIL_TEMPLATES),
				[activeTemplate]: {
	...
	const save = async () => {
		...
		setFeedback("");
		try {
			const saved = await salvarConfigEmailSistema(config);
			setConfig(saved);
	...
	const test = async () => {
		...
		setFeedback("");
		try {
			const response = await enviarTesteEmailSistema(testTo);
			setFeedback(
				`${response?.sent || templateKeys.length} e-mails de teste enviados para ${testTo}.`,
			);
...
// Extraidos de EmailSettingsPage (achado javascript:S3776,
// docs/SONARQUBE-MAP.md) — as duas abas (logs/config) eram os dois lados
// de um unico `activeTab === "logs" ? (...) : (...)` gigante dentro do
// componente principal; viraram componentes dedicados, mesma
// JSX/logica de antes.
	...
	return (
		<section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
			<div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
				<div>
					<h2 className="text-lg font-black text-slate-950">Logs de e-mails</h2>
					<p className="mt-1 text-sm font-medium text-slate-500">
						Veja todos os e-mails enviados e os erros retornados pelo SMTP.
					</p>
				</div>
				<div className="text-sm font-bold text-slate-500">
					{logsTotal} registro(s)
				</div>
			</div>
							...
							{logsLoading ? (
								...
								logs.map((item) => (
									<tr key={item.id} className="hover:bg-slate-50">
										...
										<td className="max-w-[360px] px-4 py-3 text-xs font-semibold leading-relaxed text-red-700">
											{item.error || "-"}
										</td>
									</tr>
								))
			...
			<div className="mt-4 flex flex-col gap-3 text-sm font-semibold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
				<span>
					Mostrando {logs.length ? (logsPage - 1) * EMAIL_LOG_PAGE_SIZE + 1 : 0}{" "}
					a {Math.min(logsPage * EMAIL_LOG_PAGE_SIZE, logsTotal)} de {logsTotal}
				</span>
				<div className="flex items-center gap-2">
					...
					<span className="rounded-lg bg-slate-100 px-3 py-2 font-black text-slate-800">
						Página {logsPage}
					</span>
					<button
						type="button"
			...
			<section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
						...
						>
							...
							<span>
								<span className="block text-sm font-black text-blue-950">
									Exigir código por e-mail no login
								</span>
								<span className="block text-xs font-semibold text-blue-700">
									Após senha correta, o usuário precisa confirmar o código
									enviado por e-mail.
								</span>
							</span>
						</label>
				...
				<div className="rounded-lg border border-blue-200 bg-white p-5 shadow-sm">
					<div className="flex items-center gap-2 text-slate-950">
						...
						<h2 className="text-lg font-black">Envio de teste</h2>
					</div>
					<p className="mt-2 text-sm font-medium text-slate-500">
						Envie uma prévia real de todos os templates para validar
						autenticação, remetente e layout.
					</p>
					...
					>
						<Send size={17} />
						{testing ? "Enviando..." : "Enviar e-mails de teste"}
					</button>
					<div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm font-semibold leading-relaxed text-slate-600">
						Os e-mails de criação de conta, redefinição e MFA usam
						tokens/códigos temporários.
					</div>
				</div>
			</section>
			...
			<section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
				<div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
					<div>
						...
						<p className="mt-1 text-sm font-medium text-slate-500">
							Edite o texto enviado em cada ação do sistema. As variáveis são
					...
					<div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-bold leading-relaxed text-blue-800">
						Variáveis: {"{nome}"}, {"{email}"}, {"{role}"}, {"{regional}"},{" "}
						{"{resetUrl}"}, {"{appUrl}"}, {"{codigo}"}, {"{minutos}"}
					</div>
				</div>
				...
				<div className="mt-5 grid gap-4 md:grid-cols-2">
					...
					<label>
						...
						<input
							className={inputClass}
							value={activeTemplateConfig.title || ""}
					...
					<label>
						...
						<input
							className={inputClass}
							value={activeTemplateConfig.preview || ""}
							onChange={(e) => updateTemplate("preview", e.target.value)}
						/>
					</label>
					<label>
						<span className="text-sm font-bold text-slate-700">
		...
		<div className="space-y-6">
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					...
					<div className="flex flex-wrap gap-2">
						<button
							type="button"
							onClick={
								activeTab === "logs" ? () => loadLogs(logsPage) : loadData
							}
			...
			{activeTab === "logs" ? (
				...
				<EmailConfigSection
					config={config}
					update={update}
					mfaEmailToggleId={mfaEmailToggleId}
					testTo={testTo}
					setTestTo={setTestTo}
					test={test}
					testing={testing}
