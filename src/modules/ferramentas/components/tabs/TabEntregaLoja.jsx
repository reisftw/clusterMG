...
const buildAtendimento = (row) => {
	const codigo = normalizeCode(
		readField(row, ["codigo_cliente", "codigo", "codigocliente"]),
	);
	return {
		codigo,
		nome: String(
			readField(row, ["nome_razaosocial", "nome", "cliente", "razao_social"]),
		).trim(),
...
const buildOs = (row) => {
	...
	const tipo = String(
		readField(row, ["tipo_ordem_servico", "tipo", "tipoos"]),
	).trim();
	...
	return {
		...
		numeroOs: String(
			readField(row, ["numero_ordem_servico", "num_os", "numeroos", "os"]),
		).trim(),
		tipo,
		...
		servico: String(readField(row, ["servico", "plano"])).trim(),
		telefone: String(
			readField(row, ["telefone_primario", "telefone", "telefones"]),
		).trim(),
		raw: row,
	};
};
...
const countBy = (items, getKey) =>
	[
		...items.reduce((map, item) => {
			const key =
				String(getKey(item) || "Não informado").trim() || "Não informado";
...
const exportWorkbook = async ({
	...
	const addTableSheet = (name, rows, columns) => {
		...
		sheet.autoFilter = {
			from: "A1",
			to: `${String.fromCodePoint(64 + columns.length)}1`,
		};
		return sheet;
	};
	...
	addTableSheet("Clientes Entregues", clientesEntregues, [
		{ header: "Código cliente", key: "codigo", width: 16 },
	...
	triggerDownload(
		buffer,
		`verificacao-entrega-loja-${new Date().toLocaleDateString("pt-BR").replace(/\//g, "-")}.xlsx`,
	);
};
...
const TabEntregaLoja = () => {
	...
	const analysis = useMemo(() => {
		const atendimentos = atRows
			.map(buildAtendimento)
			.filter((item) => item.codigo);
		const os = osRows.map(buildOs).filter((item) => item.codigo);
		const atendimentoByCode = new Map();
		atendimentos.forEach((item) => {
			if (!atendimentoByCode.has(item.codigo))
				atendimentoByCode.set(item.codigo, item);
		});
		const matches = os
			...
			.map((order) => ({
				...
				cidade:
					order.cidade || atendimentoByCode.get(order.codigo)?.cidade || "",
			}))
			.sort(
				(a, b) => (b.dataObj?.getTime?.() || 0) - (a.dataObj?.getTime?.() || 0),
			);
		const clientesEntregues = dedupeBy(matches, (item) => item.codigo).map(
			(item) => ({
				codigo: item.codigo,
				...
				totalOs: matches.filter((osItem) => osItem.codigo === item.codigo)
					.length,
			}),
		);
		return {
			...
			osUnicas: dedupeBy(
				os,
				(item) =>
					item.numeroOs || `${item.codigo}-${item.tipo}-${item.dataCadastro}`,
			),
			matches,
			clientesEntregues,
	...
	const tableRows = useMemo(() => {
		const source =
			activeTab === "clientes" ? analysis.clientesEntregues : analysis.matches;
		const search = normalizeText(query);
		if (!search) return source;
		return source.filter((item) =>
			normalizeText(
				`${item.codigo} ${item.nome} ${item.numeroOs || ""} ${item.servico || ""} ${item.cidade || ""} ${item.status || ""}`,
			).includes(search),
		);
		...
		<div className="space-y-6">
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					<div>
						...
						<h2 className="mt-2 text-2xl font-black text-slate-900">
							Verificar entrega loja
						</h2>
						<p className="mt-1 max-w-3xl text-sm font-medium text-slate-500">
							Cruze o relatório de atendimentos com o relatório de ordens de
							serviço pelo código do cliente. O foco é identificar quais
							clientes possuem atendimento aberto para as ordens, com nome,
							código e serviço.
						</p>
					</div>
					<button
			...
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
				<KpiCard
					...
					value={analysis.atendimentosUnicos.length}
					icon={FileSpreadsheet}
					tone="blue"
				/>
				<KpiCard
					label="O.S no relatório"
					value={analysis.osUnicas.length}
					icon={BarChart3}
					tone="slate"
				/>
				<KpiCard
					label="O.S com atendimento"
					value={analysis.matches.length}
					icon={Link2}
					tone="orange"
				/>
				<KpiCard
					label="Clientes entregues"
					value={analysis.clientesEntregues.length}
					icon={CheckCircle2}
					tone="green"
				/>
			</section>
			...
			{hasData ? (
					...
					<div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
										...
										<tbody className="divide-y divide-slate-100 bg-white">
											{visibleRows.map((item) => (
												<tr key={`${item.numeroOs}-${item.codigo}`}>
													...
													<td className="px-4 py-3 text-slate-600">
														{item.atendimento?.servico || item.servico || "-"}
													</td>
													<td className="px-4 py-3 text-slate-600">
														{item.status || "-"}
													</td>
						...
						{tableRows.length > visibleRows.length ? (
							<p className="mt-3 text-xs font-semibold text-slate-500">
								Mostrando os primeiros {visibleRows.length} de{" "}
								{tableRows.length.toLocaleString("pt-BR")} registros filtrados.
								Use a busca ou exporte para ver tudo.
							</p>
						) : null}
					</div>
				...
				<section className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
					...
					<p className="mt-3 text-lg font-black text-slate-900">
						Envie os dois relatórios para gerar a verificação.
					</p>
					<p className="mt-1 text-sm font-semibold text-slate-500">
						O match será feito pelo código do cliente.
					</p>
				</section>
			)}
		</div>
