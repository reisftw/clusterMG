...
const extractCidade = (row) => {
  const cidade =
    row.cidade || row.cidadedeinstalacao || row.municipio || row.cidadecliente || "";
...
const TabCancelamentosMes = () => {
      ...
      (regional.cidades || []).forEach((cidade) => {
        const nomeCidade =
          typeof cidade === "string"
            ? cidade
            : cidade?.nome || cidade?.cidade || cidade?.name || cidade?.label || "";
  ...
  const rows = useMemo(
    () =>
      rawRows.map((row) => {
        ...
        const motivo =
          String(row.motivocancelamento || row.motivo || "Não informado").trim() ||
          "Não informado";
        ...
        return {
          ...row,
          _codigoCliente: String(row.codigocliente || row.idcliente || "").trim(),
          _cidade: cidade,
          _regional: regional,
          _empresa: empresa,
          _motivo: motivo,
  ...
  const analytics = useMemo(() => {
    ...
    return {
      ...
      periodo: formatPeríodo(rows.map((row) => row._dataCancelamento).filter(Boolean)),
  ...
  const gerar = async () => {
    ...
    try {
      ...
      const setTitle = (sheet, row, columns, text, bg = "C2410C") => {
        ...
        cell.value = text;
        cell.font = { name: "Arial", bold: true, size: 13, color: color("FFFFFF") };
      ...
      const setHeader = (sheet, row, col, text, bg = "C2410C") => {
        ...
        cell.value = text;
        cell.font = { name: "Arial", bold: true, size: 10, color: color("FFFFFF") };
        cell.fill = { type: "pattern", pattern: "solid", fgColor: color(bg) };
      ...
      const setCell = (sheet, row, col, value, bg = "FFFFFF", align = "left", bold = false) => {
        const cell = sheet.getCell(row, col);
        cell.value = value ?? "";
      ...
      const setKpi = (sheet, row, col, label, value, accent) => {
        ...
        labelCell.font = { name: "Arial", bold: true, size: 8, color: color("FFFFFF") };
        labelCell.fill = { type: "pattern", pattern: "solid", fgColor: color(accent) };
        labelCell.alignment = { horizontal: "center", vertical: "middle" };

        const valueCell = sheet.getCell(row + 1, col);
        valueCell.value = value;
        valueCell.font = { name: "Arial", bold: true, size: 20, color: color(accent) };
        valueCell.fill = { type: "pattern", pattern: "solid", fgColor: color("F9FAFB") };
      ...
      setTitle(cidadeSheet, 1, 4, "RANKING DE CANCELAMENTOS POR CIDADE", "1D4ED8");
      ["Cidade", "Regional", "Total", "% Total"].forEach((label, index) =>
      ...
      [
        ...
        "Data Cancelamento",
        "O.S Cancel. Abertas",
      ].forEach((label, index) => setHeader(dataSheet, 2, index + 1, label, "334155"));
      rows.forEach((row, index) => {
        ...
        setCell(dataSheet, index + 3, 6, row._isFtth ? "Sim" : "Não", bg, "center");
        setCell(dataSheet, index + 3, 7, row._tecnologia, bg, "center");
        setCell(dataSheet, index + 3, 8, row._motivo, bg);
        setCell(dataSheet, index + 3, 9, formatDate(row._dataCancelamento), bg, "center");
        ...
        {rows.length > 0 ? (
          ...
          <>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Campos esperados:{" "}
              <code>cidade • tecnologia • motivo_cancelamento • data_cancelamento</code>
            </p>
          </>
        )}
      </div>
      ...
      {rows.length > 0 && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
            {[
              ["Cancelamentos", analytics.totalCancelamentos, "text-orange-700"],
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              ...
              <div className="space-y-2">
                {analytics.byCidadeRows.slice(0, 5).map((item) => (
                  ...
                  >
                    <div>
                      <p className="font-semibold text-gray-800">{item.cidade}</p>
              ...
              <div className="space-y-2">
                {analytics.byTipoRows.slice(0, 5).map((item) => (
                  ...
                  >
                    ...
                    <span className="font-bold text-purple-700">{item.total}</span>
                  </div>
                ))}
              </div>
            </div>
