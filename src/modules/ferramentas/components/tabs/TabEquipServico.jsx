...
const DEFAULT_AC = ["AC", "300 MB", "300MB", "400 MB", "400MB", "500 MB", "500MB"];
const DEFAULT_AX = ["AX", "600 MB", "600MB", "700 MB", "800 MB", "900 MB", "1 GB", "1GB", "GIGA"];
...
const parsePlanSpeedMbps = (servico) => {
  ...
  const regex = /(\d+(?:\.\d+)?)\s*(giga|gbps|gb|g|megas|mega|mbps|mb|k)(?=[^a-z0-9]|$)/g;
  let match;
  let best = null;
  ...
  while ((match = regex.exec(text))) {
    ...
    const unit = match[2];
    const mbps = unit.startsWith("g") ? value * 1000 : unit === "k" ? value / 1000 : value;
...
const resolveRegional = (row, cidade, cityMap) => {
  ...
  if (normalizeText(fromRow).includes("onnet")) return fallbackOnnet || "Sem Regional";
...
const resolveCompany = (row, regional, servico) => {
  const source = [
    regional,
    servico,
    pickFirst(row, ["empresa", "provedor", "marca", "filial", "unidade", "grupo", "origem"]),
  ].join(" ");

  return normalizeText(source).includes("onnet") || ONNET_REGIONAIS_KEYS.has(normalizeKey(regional))
    ? "ONNET"
    : "SEMPRE";
};
...
const sortEntriesByTotal = (entries) => entries.sort((a, b) => b[1].total - a[1].total);

const renderKwGroup = ({ label, type, kws, newKw, setNewKw, addKw, removeKw }) => (
  <div className="rounded-xl border border-gray-200 p-4 bg-white">
    <div className="flex items-center gap-2 mb-3">
      <span
        className="w-3 h-3 rounded-full"
        style={{ background: "#" + { fast: COLORS.Fast, ac: COLORS.AC, ax: COLORS.AX }[type] }}
      />
      <span className="font-semibold text-sm text-gray-700">{label}</span>
      <span className="ml-auto text-xs text-gray-400">{kws.length} palavras-chave</span>
    </div>
    ...
    <div className="flex gap-2">
      <input
        type="text"
        value={newKw[type]}
        onChange={(event) => setNewKw((prev) => ({ ...prev, [type]: event.target.value }))}
...
const TabEquipServiço = () => {
          ...
          new Promise((resolve, reject) => {
            ...
            reader.onload = (ev) => {
              try {
                ...
                resolve({
                  name: file.name,
                  rows: normKeys(XLSX.utils.sheet_to_json(worksheet, { raw: false, defval: "" })),
                });
              } catch {
                reject(file.name);
              }
  ...
  const enrichRows = (rows) =>
    rows.map((row) => {
      const servico = pickFirst(row, ["servico", "plano", "produto", "servicoplano", "planoservico"]);
      const cidade = pickFirst(row, ["cidade", "municipio", "cidadecliente", "cidadedeinstalacao"]) || "N/A";
      ...
      return {
        ...
        _os: pickFirst(row, ["numeroordemservico", "numos", "numeroos", "ordemservico", "os"]),
  ...
  const gerar = async () => {
    ...
    try {
      ...
      const setCell = (ws, row, col, value, bg = "FFFFFF", align = "left", bold = false) => {
        const cell = ws.getCell(row, col);
        cell.value = value ?? "";
      ...
      const setFormula = (
        ...
        cell.font = { name: "Arial", size: 10, bold, ...(fontColor ? { color: C(fontColor) } : {}) };
        cell.fill = { type: "pattern", pattern: "solid", fgColor: C(bg) };
      ...
      const setKpiFormula = (ws, row, col, label, formula, result, color, numFmt = "#,##0") => {
      ...
      const byCity = groupRows(data, (row) => row._cidade, (row) => ({
      ...
      const cityRows = sortEntriesByTotal(Object.entries(byCity)).map(([cidade, info]) => ({
        cidade,
        ...info,
      }));
      ...
      const dataRef = (col) => `'Dados Geral'!$${col}$${dataStart}:$${col}$${dataEnd}`;
      const totalFormula = `COUNTA(${dataRef("B")})`;
      const paramRowByPlan = { Fast: 5, AC: 6, AX: 7, Outros: 8 };
      const paramValueRef = (plan) => `'Parametros'!$B$${paramRowByPlan[plan] || 8}`;
      const paramCostRef = (plan) => `'Parametros'!$E$${paramRowByPlan[plan] || 8}`;

      const writeSummarySheet = (sheetName, title, rows, columns, color) => {
        const ws = wb.addWorksheet(sheetName);
        ws.views = [{ showGridLines: false }];
        setTitle(ws, 1, columns.length, title, color);
        columns.forEach((col, index) => setHeader(ws, 3, index + 1, col.label, color));
        rows.forEach((item, index) => {
          ...
          columns.forEach((col, colIndex) => {
            ...
            if (typeof value === "object" && value?.formula) {
              ...
              setCell(ws, excelRow, colIndex + 1, value, bg, col.align || "left", col.bold);
            }
          });
        });
      ...
      PLAN_ORDER.forEach((plan, index) => {
        ...
        setFormula(wsP, excelRow, 3, `COUNTIF(${dataRef("A")},A${excelRow})`, qty, bg);
        setFormula(wsP, excelRow, 4, `B${excelRow}*C${excelRow}`, (planValues[plan] || 0) * qty, bg, "center", true, '"R$" #,##0.00');
        setCell(wsP, excelRow, 5, withdrawalUnitCost, bg, "center");
        wsP.getCell(excelRow, 5).numFmt = '"R$" #,##0.00';
        setFormula(wsP, excelRow, 6, `C${excelRow}*E${excelRow}`, qty * withdrawalUnitCost, bg, "center", true, '"R$" #,##0.00');
        setFormula(
          wsP,
          excelRow,
          7,
      ...
      setFormula(
        ...
        4,
        "SUM(D5:D8)",
        ["Fast", "AC", "AX"].reduce((sum, plan) => sum + (planValues[plan] || 0) * (byPlan[plan]?.total || 0), 0),
        "E0F2FE",
        "center",
        true,
        '"R$" #,##0.00',
      ...
      const totalRecoverable = ["Fast", "AC", "AX"].reduce(
        (sum, plan) => sum + (planValues[plan] || 0) * (byPlan[plan]?.total || 0),
        0,
      );
      ...
      ["Cenario", "Custo Unit.", "Qtd. Retiradas", "Valor Recuperavel", "Previsao Custo", "Lucro Previsto", "Observacao"].forEach((label, index) =>
        setHeader(wsC, 4, index + 1, label, "F97316"),
      );
      [
        { name: "Otimista", formula: "'Parametros'!E5*0.8", result: withdrawalUnitCost * 0.8, note: "Custo 20% abaixo do base" },
        { name: "Base", formula: "'Parametros'!E5", result: withdrawalUnitCost, note: "Custo informado na tela/Parametros" },
        { name: "Conservador", formula: "'Parametros'!E5*1.2", result: withdrawalUnitCost * 1.2, note: "Custo 20% acima do base" },
        { name: "Personalizado", formula: "'Parametros'!E5", result: withdrawalUnitCost, note: "Edite o custo unitario desta linha para simular" },
        ...
        if (scenario.name === "Personalizado") {
          ...
          setFormula(wsC, row, 2, scenario.formula, scenario.result, bg, "center", false, '"R$" #,##0.00');
        }
        setFormula(wsC, row, 3, "'Parametros'!C10", data.length, bg, "center", false, "#,##0");
        setFormula(wsC, row, 4, "'Parametros'!D10", totalRecoverable, bg, "center", true, '"R$" #,##0.00');
        setFormula(wsC, row, 5, `B${row}*C${row}`, scenario.result * data.length, bg, "center", true, '"R$" #,##0.00');
        setFormula(wsC, row, 6, `D${row}-E${row}`, totalRecoverable - scenario.result * data.length, bg, "center", true, '"R$" #,##0.00', "117A65");
        setCell(wsC, row, 7, scenario.note, bg, "left");
      });
      setCell(wsC, 11, 1, "Como usar", "E0F2FE", "left", true);
      setCell(wsC, 11, 2, "Altere o custo unitario na linha Personalizado ou na aba Parametros para recalcular as previsoes.", "E0F2FE", "left");
      ...
      writeSummarySheet(
        ...
        data,
        [
          { label: "Plano", value: (item) => item._plano, width: 14, align: "center", bold: true },
          { label: "Código Cliente", value: (item) => item._codigo, width: 18, align: "center" },
          { label: "Nome Razaosocial", value: (item) => item._nome, width: 36 },
          { label: "Empresa", value: (item) => item._empresa, width: 14, align: "center" },
          { label: "Cidade", value: (item) => item._cidade, width: 24 },
          { label: "Serviço", value: (item) => item._servico, width: 70 },
          { label: "Velocidade Mbps", value: (item) => item._velocidade || "", width: 16, align: "center" },
          { label: "Regional", value: (item) => item._regional, width: 24 },
          { label: "O.S.", value: (item) => item._os, width: 22, align: "center" },
          { label: "Status", value: (item) => item._status, width: 22, align: "center" },
      ...
      setKpiFormula(wsD, 4, 1, "TOTAL", totalFormula, data.length, COLORS.title);
      setKpiFormula(wsD, 4, 2, "FAST <=299", `COUNTIF(${dataRef("A")},"Fast")`, byPlan.Fast?.total || 0, COLORS.Fast);
      setKpiFormula(wsD, 4, 3, "AC 300-500", `COUNTIF(${dataRef("A")},"AC")`, byPlan.AC?.total || 0, COLORS.AC);
      setKpiFormula(wsD, 4, 4, "AX >500", `COUNTIF(${dataRef("A")},"AX")`, byPlan.AX?.total || 0, COLORS.AX);
      setKpiFormula(wsD, 4, 5, "ONNET", `COUNTIF(${dataRef("D")},"ONNET")`, byCompany.ONNET?.total || 0, COLORS.ONNET);
      setKpiFormula(wsD, 4, 6, "SEMPRE", `COUNTIF(${dataRef("D")},"SEMPRE")`, byCompany.SEMPRE?.total || 0, COLORS.SEMPRE);
      setKpiFormula(wsD, 4, 7, "OUTROS", `COUNTIF(${dataRef("A")},"Outros")`, byPlan.Outros?.total || 0, COLORS.Outros);

      setKpiFormula(wsD, 7, 1, "VALOR FAST", `'Parametros'!D5`, (byPlan.Fast?.total || 0) * planValues.Fast, COLORS.Fast, '"R$" #,##0.00');
      setKpiFormula(wsD, 7, 2, "VALOR AC", `'Parametros'!D6`, (byPlan.AC?.total || 0) * planValues.AC, COLORS.AC, '"R$" #,##0.00');
      setKpiFormula(wsD, 7, 3, "VALOR AX", `'Parametros'!D7`, (byPlan.AX?.total || 0) * planValues.AX, COLORS.AX, '"R$" #,##0.00');
      setKpiFormula(
        wsD,
        7,
        4,
        "TOTAL PARADO",
        `'Parametros'!D10`,
        ["Fast", "AC", "AX"].reduce((sum, plan) => sum + (byPlan[plan]?.total || 0) * (planValues[plan] || 0), 0),
        "F97316",
        '"R$" #,##0.00',
      );
      setKpiFormula(
      ...
      PLAN_ORDER.forEach((plan, index) => {
        ...
        setFormula(wsD, rowCursor, 2, `COUNTIF(${dataRef("A")},A${rowCursor})`, planRows.length, bg);
        setFormula(wsD, rowCursor, 3, `IFERROR(B${rowCursor}/${totalFormula},0)`, planRows.length / (data.length || 1), bg, "center", false, "0.0%");
        setFormula(
          wsD,
          rowCursor,
          4,
      ...
      rowCursor += 2;
      ["Cidade", "Empresa", "Regional", "Total", "Fast", "AC", "AX"].forEach((label, index) =>
        setHeader(wsD, rowCursor, index + 1, label, "117A65"),
      );
      rowCursor += 1;
      cityRows
        .slice(0, 15)
        .forEach((info, index) => {
          ...
          setFormula(wsD, rowCursor, 2, `IFERROR(INDEX(${dataRef("D")},MATCH(${cidadeCell},${dataRef("E")},0)),"")`, info.empresa, bg, "center");
          setFormula(wsD, rowCursor, 3, `IFERROR(INDEX(${dataRef("H")},MATCH(${cidadeCell},${dataRef("E")},0)),"")`, info.regional, bg);
          setFormula(wsD, rowCursor, 4, `COUNTIF(${dataRef("E")},${cidadeCell})`, info.total, bg);
          setFormula(wsD, rowCursor, 5, `COUNTIFS(${dataRef("E")},${cidadeCell},${dataRef("A")},"Fast")`, info.Fast, bg);
          setFormula(wsD, rowCursor, 6, `COUNTIFS(${dataRef("E")},${cidadeCell},${dataRef("A")},"AC")`, info.AC, bg);
          setFormula(wsD, rowCursor, 7, `COUNTIFS(${dataRef("E")},${cidadeCell},${dataRef("A")},"AX")`, info.AX, bg);
          rowCursor += 1;
        });
      ...
      PLAN_ORDER.forEach((plan, index) => {
        ...
        setFormula(wsD, row, 10, `COUNTIF(${dataRef("A")},I${row})`, planRows.length, bg);
        setFormula(
          wsD,
          row,
          11,
          `REPT("█",IFERROR(ROUND(J${row}/MAX($J$13:$J$16)*22,0),0))`,
          "█".repeat(Math.round((planRows.length / Math.max(...PLAN_ORDER.map((p) => data.filter((item) => item._plano === p).length), 1)) * 22)),
          bg,
          "left",
          true,
        );
        wsD.getCell(row, 11).font = { name: "Arial", bold: true, size: 10, color: C(COLORS[plan]) };
      });

      setCell(wsD, 19, 9, "TOP PRIORIDADE AX/AC", "117A65", "center", true);
      wsD.mergeCells(19, 9, 19, 14);
      ["Cidade", "AX", "AC", "Lucro s/ FAST", "Prioridade", "Barra"].forEach((label, index) =>
        setHeader(wsD, 20, index + 9, label, "117A65"),
      );
      cityPriorityRows.slice(0, 8).forEach((info, index) => {
        ...
        setFormula(wsD, row, 10, `COUNTIFS(${dataRef("E")},I${row},${dataRef("A")},"AX")`, info.AX, bg);
        setFormula(wsD, row, 11, `COUNTIFS(${dataRef("E")},I${row},${dataRef("A")},"AC")`, info.AC, bg);
        setFormula(
          wsD,
          row,
          12,
        ...
        setFormula(
          ...
          "█".repeat(Math.max(0, Math.round((score / Math.max(...cityPriorityRows.slice(0, 8).map(priorityScore), 1)) * 18))),
          bg,
          "left",
          true,
          null,
      ...
      writeSummarySheet(
        ...
        PLAN_ORDER.map((plan) => {
          ...
          return {
            ...
            sempre: planRows.filter((item) => item._empresa === "SEMPRE").length,
          };
        }),
        [
          { label: "Plano", value: (item) => item.plano, width: 18, bold: true },
          { label: "Total", value: (item, row) => ({ formula: `COUNTIF(${dataRef("A")},A${row})`, result: item.total }), width: 12, align: "center" },
          {
            label: "% Geral",
            value: (item, row) => ({ formula: `IFERROR(B${row}/${totalFormula},0)`, result: item.total / (data.length || 1) }),
            width: 12,
            align: "center",
            numFmt: "0.0%",
          },
          {
            label: "ONNET",
            value: (item, row) => ({ formula: `COUNTIFS(${dataRef("A")},A${row},${dataRef("D")},"ONNET")`, result: item.onnet }),
            width: 12,
            align: "center",
          },
          {
            label: "SEMPRE",
            value: (item, row) => ({ formula: `COUNTIFS(${dataRef("A")},A${row},${dataRef("D")},"SEMPRE")`, result: item.sempre }),
            width: 12,
            align: "center",
          },
          {
          ...
          {
            label: "Valor Parado",
            value: (item, row) => ({ formula: `B${row}*F${row}`, result: item.total * (planValues[item.plano] || 0) }),
            width: 16,
            align: "center",
            numFmt: '"R$" #,##0.00',
          },
          ...
          },
          {
            ...
            value: (item, row) => ({
              formula: `G${row}-H${row}`,
              result: item.total * (planValues[item.plano] || 0) - item.total * withdrawalUnitCost,
            }),
            width: 18,
            align: "center",
            bold: true,
      ...
      writeSummarySheet(
        ...
        COMPANY_ORDER.map((company) => {
          ...
          return {
            ...
            outros: companyRows.filter((item) => item._plano === "Outros").length,
          };
        }),
        [
          { label: "Empresa", value: (item) => item.empresa, width: 16, bold: true },
          { label: "Total", value: (item, row) => ({ formula: `COUNTIF(${dataRef("D")},A${row})`, result: item.total }), width: 12, align: "center" },
          {
            label: "% Geral",
            value: (item, row) => ({ formula: `IFERROR(B${row}/${totalFormula},0)`, result: item.total / (data.length || 1) }),
            width: 12,
          ...
          { label: "Fast", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("D")},A${row},${dataRef("A")},"Fast")`, result: item.fast }), width: 12, align: "center" },
          { label: "AC", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("D")},A${row},${dataRef("A")},"AC")`, result: item.ac }), width: 12, align: "center" },
          { label: "AX", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("D")},A${row},${dataRef("A")},"AX")`, result: item.ax }), width: 12, align: "center" },
          { label: "Outros", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("D")},A${row},${dataRef("A")},"Outros")`, result: item.outros }), width: 12, align: "center" },
          {
            label: "Valor Parado",
            value: (item, row) => ({
              formula: `E${row}*${paramValueRef("Fast")}+F${row}*${paramValueRef("AC")}+G${row}*${paramValueRef("AX")}`,
              result: item.fast * planValues.Fast + item.ac * planValues.AC + item.ax * planValues.AX,
            }),
            width: 18,
            align: "center",
            numFmt: '"R$" #,##0.00',
          ...
          },
          {
            ...
            value: (item, row) => ({
              formula: `I${row}-J${row}`,
              result: item.fast * planValues.Fast + item.ac * planValues.AC + item.ax * planValues.AX - item.total * withdrawalUnitCost,
            }),
            width: 18,
            align: "center",
            bold: true,
      ...
      writeSummarySheet(
        ...
        cityRows,
        [
          { label: "Cidade", value: (item) => item.cidade, width: 28, bold: true },
          { label: "Empresa", value: (item, row) => ({ formula: `IFERROR(INDEX(${dataRef("D")},MATCH(A${row},${dataRef("E")},0)),"")`, result: item.empresa }), width: 14, align: "center" },
          { label: "Regional", value: (item, row) => ({ formula: `IFERROR(INDEX(${dataRef("H")},MATCH(A${row},${dataRef("E")},0)),"")`, result: item.regional }), width: 28 },
          { label: "Total", value: (item, row) => ({ formula: `COUNTIF(${dataRef("E")},A${row})`, result: item.total }), width: 12, align: "center" },
          {
            label: "% Geral",
            value: (item, row) => ({ formula: `IFERROR(D${row}/${totalFormula},0)`, result: item.total / (data.length || 1) }),
            width: 12,
            align: "center",
            numFmt: "0.0%",
          },
          { label: "Fast", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},A${row},${dataRef("A")},"Fast")`, result: item.Fast }), width: 12, align: "center" },
          { label: "AC", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},A${row},${dataRef("A")},"AC")`, result: item.AC }), width: 12, align: "center" },
          { label: "AX", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},A${row},${dataRef("A")},"AX")`, result: item.AX }), width: 12, align: "center" },
          { label: "Outros", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},A${row},${dataRef("A")},"Outros")`, result: item.Outros }), width: 12, align: "center" },
          {
            label: "Valor Parado",
            value: (item, row) => ({
      ...
      writeSummarySheet(
        ...
        cityPriorityRows,
        [
          { label: "Rank", value: (_item, _row, index) => index + 1, width: 8, align: "center", bold: true },
          { label: "Cidade", value: (item) => item.cidade, width: 28, bold: true },
          { label: "Empresa", value: (item, row) => ({ formula: `IFERROR(INDEX(${dataRef("D")},MATCH(B${row},${dataRef("E")},0)),"")`, result: item.empresa }), width: 14, align: "center" },
          { label: "Regional", value: (item, row) => ({ formula: `IFERROR(INDEX(${dataRef("H")},MATCH(B${row},${dataRef("E")},0)),"")`, result: item.regional }), width: 28 },
          { label: "Total", value: (item, row) => ({ formula: `COUNTIF(${dataRef("E")},B${row})`, result: item.total }), width: 12, align: "center" },
          { label: "Fast", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},B${row},${dataRef("A")},"Fast")`, result: item.Fast }), width: 10, align: "center" },
          { label: "AC", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},B${row},${dataRef("A")},"AC")`, result: item.AC }), width: 10, align: "center" },
          { label: "AX", value: (item, row) => ({ formula: `COUNTIFS(${dataRef("E")},B${row},${dataRef("A")},"AX")`, result: item.AX }), width: 10, align: "center" },
          {
            label: "Valor AX/AC",
            value: (item, row) => ({
          ...
          {
            label: "Custo Ret.",
            value: (item, row) => ({ formula: `(G${row}+H${row})*'Parametros'!$E$5`, result: (item.AC + item.AX) * withdrawalUnitCost }),
            width: 16,
            align: "center",
            numFmt: '"R$" #,##0.00',
          },
          ...
          {
            label: "Prioridade",
            value: (_item, _row, index) => (index < 10 ? "ALTA" : index < 30 ? "MEDIA" : "BAIXA"),
            width: 14,
            align: "center",
            bold: true,
          },
          {
            ...
            value: (item, row) => {
              ...
              const maxScore = Math.max(...cityPriorityRows.map(priorityScore), 1);
              return {
                ...
                result: "█".repeat(Math.max(0, Math.round((score / maxScore) * 24))),
              };
            },
            width: 30,
            align: "left",
      ...
      PLAN_ORDER.forEach((plan) => {
        ...
        ws.views = [{ showGridLines: false }];
        setTitle(ws, 1, 10, `${plan.toUpperCase()} - DETALHE DINAMICO`, COLORS[plan]);
        [
          "Código Cliente",
          "Nome Razaosocial",
          "Empresa",
          ...
          "Status",
          "Tipo O.S.",
        ].forEach((label, index) => setHeader(ws, 3, index + 1, label, COLORS[plan]));
    ...
    <div className="space-y-4">
      ...
      >
        ...
        {files.length > 0 ? (
          ...
          <>
            <p className="font-semibold text-gray-700">Clique ou arraste os arquivos .xlsx</p>
            <p className="text-xs text-gray-400 mt-1">
              Campos esperados: <code>codigo_cliente · nome_razaosocial · servico · cidade</code>
            </p>
          </>
        )}
      </div>
      ...
      <div className="rounded-xl border border-blue-100 bg-white p-4">
        <div className="mb-3">
          <p className="text-sm font-bold text-gray-800">Valores unitarios dos equipamentos</p>
          <p className="text-xs text-gray-500">
            Esses valores alimentam a aba Parametros e a previsao de valor parado no dashboard.
          </p>
        </div>
          ...
          {PLAN_ORDER.filter((plan) => plan !== "Outros").map((plan) => (
            <label key={plan} className="block">
              ...
              <input
                ...
                onChange={(event) =>
                  setEquipValues((prev) => ({ ...prev, [plan]: event.target.value }))
                }
      ...
      {status === "gerando" && (
        <p className="text-sm text-blue-600 font-medium animate-pulse">Gerando Excel...</p>
      )}
      {status === "err" && (
