...
const TabMesInicial = () => {
      ...
      try {
        ...
        const enriched = data.map((row) => {
          ...
          return {
            ...
            _isFibra: norm(tecnologia) === "fibra",
            _cidade: cidade,
  ...
  const gerar = async () => {
    ...
    try {
      ...
      const xD = (ws, row, col, val, bg = "FFFFFF", align = "left", bold = false) => {
        const cell = ws.getCell(row, col);
        cell.value = val ?? "";
      ...
      fibra.forEach((row) => {
        ...
        if (norm(row._status) === "cancelado") byReg[reg].cancelados += 1;
      });
      ...
      fibra.forEach((row) => {
        ...
        if (norm(row._status) === "cancelado") byCidade[cidade].cancelados += 1;
      });
      ...
      xKpi(wsD, 3, 2, "REGIONAIS", new Set(fibra.map((r) => r._regional)).size, "117A65");
      xKpi(wsD, 3, 3, "CIDADES", new Set(fibra.map((r) => r._cidade)).size, "1F618D");
      ...
      rr++;
      ["Regional", "Fibra", "% Fibra", "Cidades", "Ag. Aut.", "Cancelados"].forEach(
        (h, i) => xH(wsD, rr, i + 1, h, "2471A3"),
      );
      rr++;
      Object.entries(byReg)
        ...
        .forEach(([reg, data], index) => {
          ...
          xD(wsD, rr, 5, data.agentes, data.agentes > 0 ? "FEF9E7" : bg, "center");
      ...
      Object.entries(byReg)
        ...
        .forEach(([reg, data], index) => {
          ...
          xD(wsR, row, 5, data.agentes, data.agentes > 0 ? "FEF9E7" : bg, "center");
      ...
      ["Cidade", "Regional", "Fibra", "% Fibra", "Ag. Aut.", "Cancelados"].forEach(
        (h, i) => xH(wsC, 2, i + 1, h, "1E8449"),
      );
      Object.entries(byCidade)
        ...
        .forEach(([cidade, data], index) => {
          ...
          xD(wsC, row, 5, data.agentes, data.agentes > 0 ? "FEF9E7" : bg, "center");
      ...
      if (!Object.keys(byAgenteCidade).length) {
        xD(wsA, 3, 1, "Nenhum agente autorizado com fibra encontrado.", "FFFFFF");
      }
        ...
        .forEach(([reg, tecnologias]) => {
          Object.entries(tecnologias)
            ...
            .forEach(([tech, total], index) => {
              ...
              xD(wsO, outrosRow, 4, percent(total, outros.length), bg, "center");
              outrosRow++;
            });
        });
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            [
              "Cidades",
              new Set(rows.filter((r) => r._isFibra).map((r) => r._cidade)).size,
              "text-teal-700",
            ],
