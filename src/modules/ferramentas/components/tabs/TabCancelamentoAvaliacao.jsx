...
const isFibraTecnologia = (row) => normalizeText(row.tecnologia).includes("fibra");
...
const buildAnalytics = (rows) => {
  ...
  const regionalRows = Object.keys(byRegional)
    .sort((a, b) => byRegional[b] - byRegional[a] || a.localeCompare(b, "pt-BR"))
    .map((regional) => ({
      regional,
...
const TabCancelamentoAvaliacao = () => {
    ...
    () =>
      rawRows
        .map((row) => {
          ...
          const dataCancelamento = parseDate(
            pickFirst(row, [
              "datacancelamento",
              "dataalteracostatus",
              "datadecancelamento",
  ...
  const gerar = async () => {
    ...
    try {
      ...
      setTitle(dashboard, 7, matrixColumns, "FTTH POR REGIONAL E MES", "15803D");
      ...
      analytics.regionalRows.forEach((item, index) => {
        ...
        analytics.months.forEach((monthKey, monthIndex) => {
          setCell(dashboard, row, monthIndex + 2, item.months[monthKey], bg, "center");
        });
        setCell(dashboard, row, analytics.months.length + 2, item.total, bg, "center", true);
      });

      const totalRow = analytics.regionalRows.length + 9;
      setCell(dashboard, totalRow, 1, "Total", "DCFCE7", "left", true);
      analytics.months.forEach((monthKey, index) => {
        const total = analytics.monthRows.find((item) => item.monthKey === monthKey)?.total || 0;
        setCell(dashboard, totalRow, index + 2, total, "DCFCE7", "center", true);
      });
      setCell(
        dashboard,
        totalRow,
      ...
      setHeader(regionalSheet, 2, analytics.months.length + 2, "Total", "15803D");
      analytics.regionalRows.forEach((item, index) => {
        const row = index + 3;
        const bg = stripe(index, "ECFDF5");
        setCell(regionalSheet, row, 1, item.regional, bg, "left", true);
        analytics.months.forEach((monthKey, monthIndex) => {
          setCell(regionalSheet, row, monthIndex + 2, item.months[monthKey], bg, "center");
        });
        setCell(
          regionalSheet,
          row,
      ...
      rows.forEach((row, index) => {
        ...
        setCell(dataSheet, index + 3, 5, formatDate(row._dataCancelamento), bg, "center");
    ...
    <div className="space-y-4">
        ...
        {rawRows.length > 0 ? (
          <>
            ...
            <p className="text-sm text-green-600">
              {analytics.totalRetiradas} clientes FIBRA em {rawRows.length} linhas
            </p>
          </>
        ) : status === "loading" ? (
          ...
          <>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Agrupa clientes FIBRA por regional e mes usando cidade e data de cancelamento
            </p>
          </>
        )}
      </div>
      ...
      {rawRows.length > 0 && rows.length === 0 && (
        <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-3">
          Nenhuma linha com tecnologia FIBRA foi identificada na planilha enviada.
        </p>
      )}
