...
export async function consultarHistoricoSempre(mac, { limit = 20, page = 1 } = {}) {
  const normalized = normalizeMacInput(mac);
  ...
  return requestVpsApi(`/integrations/sempre/equipment/history?${params.toString()}`);
}
...
export async function listarEquipamentosMapaSempre({
  ...
  return requestVpsApi(`/integrations/sempre/equipment/mapa?${params.toString()}`);
}
