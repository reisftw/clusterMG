...
function StatusBadge({ status, label }) {
	return (
		<span
			className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${CLASS_TONES[status] || CLASS_TONES.indefinido}`}
		>
		...
		<div className="mt-3 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-[180px_1fr_auto]">
			...
			<div>
				...
				{treatment.updatedAt ? (
					<p className="mt-1 text-[11px] font-semibold text-slate-400">
						Ultima tratativa:{" "}
						{TREATMENT_LABELS[treatmentStatus] || treatmentStatus} por{" "}
						{treatment.updatedByName || treatment.responsavel || "-"}
					</p>
				) : null}
			</div>
			<div className="flex items-end">
				<button
					type="button"
					onClick={() =>
						onSaveTreatment?.(item, {
							status: draftStatus,
							observacao: draftNote,
						})
					}
					disabled={saving}
...
function EquipmentRow({ item, onSaveTreatment, saving }) {
	...
	const linked =
		...
		mainResult.primary?.vinculadoEm ||
		mainResult.stockLocal?.display ||
		"-";
		...
		<article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
			...
			<div className="mt-3 grid gap-3 text-sm md:grid-cols-3">
				<div className="rounded-xl bg-slate-50 p-3">
					...
					<p className="mt-1 break-words font-black text-slate-700">
						{item.macs?.join(" / ") || "-"}
					</p>
				</div>
				<div className="rounded-xl bg-slate-50 p-3">
			...
			{item.acertoProgramado ? (
				<div className="mt-3 rounded-2xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-bold text-blue-800">
					Acerto programado: {item.acertoProgramado.codigo} -{" "}
					{item.acertoProgramado.cidade || "-"} -{" "}
					{item.acertoProgramado.dataAcerto || "-"}
				</div>
			) : null}
			...
			{item.results?.length ? (
				<div className="mt-3 flex flex-wrap gap-2">
					{item.results.map((result) => (
						...
						>
							{result.mac}:{" "}
							{result.classification?.reason || result.error || "consultado"}
						</span>
					))}
				</div>
			) : null}
...
export default function EstoqueIntegradoPage() {
	...
	async function load(options = {}) {
		...
		try {
			...
			setError(
				err?.message || "Nao foi possivel consultar os equipamentos do mapa.",
			);
		} finally {
			setLoading(false);
		}
	...
	const divergentItems =
		data?.items?.filter(
			(item) => item.classificationStatus !== "com_cliente",
		) || [];
	...
	return (
		<div className="mx-auto flex max-w-7xl flex-col gap-6">
				...
				<div className="flex flex-wrap items-start justify-between gap-4">
					<div>
						...
						<p className="mt-1 max-w-2xl text-sm font-semibold text-slate-500">
							Le apenas ordens de retirada/cancelamento do mapa, consulta a API
							Sempre e separa os equipamentos por vinculo atual.
						</p>
					</div>
			...
			<section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
				<div className="flex flex-wrap items-center justify-between gap-4">
					<div>
						<h2 className="text-xl font-black text-slate-950">
							Equipamentos do Mapa
						</h2>
						<p className="mt-1 text-sm font-semibold text-slate-500">
							Considera Retirada FTTH, Cancelamento FTTH, Cancelamento Loja e
				...
				{data?.refreshing ? (
					<div className="mt-4 flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-700">
						<Loader2 size={18} className="animate-spin" />
						Leitura completa em segundo plano. Atualize a pagina em alguns
			...
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
				<InfoCard
					icon={ClipboardList}
					label="Ordens com MAC"
					value={data?.totalOrdensComMac ?? 0}
					helper={`${data?.totalOrdensElegiveis ?? 0} elegiveis / ${data?.showing ?? 0} na tela`}
					tone="blue"
				/>
				<InfoCard
					icon={CheckCircle2}
					label="Com cliente"
					value={summary.com_cliente ?? 0}
					tone="green"
				/>
				<InfoCard
					icon={AlertCircle}
					label="Divergencias"
					value={summary.divergentes ?? 0}
					tone={summary.divergentes ? "red" : "green"}
				/>
				<InfoCard
					icon={Boxes}
					label="Em estoque"
					value={summary.em_estoque ?? 0}
					tone="blue"
				/>
				<InfoCard
					icon={UsersRound}
					label="Outro vinculo"
					value={summary.outro_vinculo ?? 0}
					tone="orange"
				/>
			</section>
			...
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
				<InfoCard
					...
					helper={`${treatmentSummary.resolvidos ?? 0} resolvida(s)`}
					tone={(alertas.divergenciasAbertas ?? 0) ? "red" : "green"}
				/>
				<InfoCard
					icon={UsersRound}
					label="Tecnico cobrado"
					value={treatmentSummary.cobrado_tecnico ?? 0}
					tone="orange"
				/>
				<InfoCard
					icon={PackageSearch}
					label="MAC duplicado"
					value={alertas.macsDuplicados ?? 0}
					helper="Mesmo MAC em mais de uma ordem"
					tone={(alertas.macsDuplicados ?? 0) ? "red" : "green"}
				/>
				<InfoCard
					icon={Boxes}
					label="Com acerto"
					value={alertas.acertoProgramado ?? 0}
					helper="Responsavel tem acerto recente"
					tone="blue"
				/>
				<InfoCard
					icon={CheckCircle2}
					label="Resolvidos"
					value={treatmentSummary.resolvido ?? 0}
					tone="green"
				/>
			</section>
			...
			{alertas.duplicateMacs?.length ? (
				<section className="rounded-3xl border border-red-200 bg-red-50 p-5 shadow-sm">
					<h2 className="text-lg font-black text-red-900">
						MACs duplicados no mapa
					</h2>
					<div className="mt-3 grid gap-2">
						{alertas.duplicateMacs.slice(0, 10).map((item) => (
							<div
								key={item.mac}
								className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-red-800"
							>
								{item.mac}: {item.total} ordem(ns) -{" "}
								{item.ordens.map((ordem) => ordem.numero).join(", ")}
							</div>
						))}
					</div>
				</section>
				...
				<section className="rounded-3xl border border-orange-200 bg-orange-50 p-5 shadow-sm">
					<div className="mb-4">
						<h2 className="text-lg font-black text-orange-950">
							Equipamento nao localizado
						</h2>
						<p className="text-xs font-semibold text-orange-800">
							{data.equipamentosNaoLocalizados.total} ordem(ns) com MAC sem
							retorno na API Sempre. Elas nao entram em divergencia por tecnico.
						</p>
					</div>
			...
			{data && !data.totalOrdensComMac ? (
				<div className="rounded-3xl border border-amber-200 bg-amber-50 p-8 text-center text-sm font-bold text-amber-800">
					Nenhuma ordem do mapa possui Mac Addr ou Phy Addr salvo. Confira se o
					ultimo upload foi publicado depois de adicionar as colunas.
				</div>
			) : null}
					...
					<div className="mb-4 flex items-center gap-2">
						...
						<div>
							<h2 className="text-lg font-black text-slate-950">
								Divergencias por Tecnico
							</h2>
							<p className="text-xs font-semibold text-slate-500">
								Equipamentos com vinculo em tecnico/local diferente. Nao
								localizados ficam na seção propria.
							</p>
						</div>
					</div>
					...
					<div className="flex flex-wrap items-center justify-between gap-3">
						<div>
							...
							<p className="mt-1 text-xs font-semibold text-slate-500">
								...
								{divergentItems.length} divergencia(s) nesta pagina, com{" "}
								{data.totalFiltered ?? data.totalConsultadas ?? 0} ordem(ns)
								filtradas.
							</p>
						</div>
