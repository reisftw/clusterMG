...
function HistoryItem({ note }) {
  ...
  const origem = firstItem.estoqueOrigem?.display || firstItem.estoqueOrigem?.descricao || "-";
  const destino = firstItem.estoqueDestino?.display || firstItem.estoqueDestino?.descricao || "-";
    ...
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500">
            {note.tipoOperacao?.descricao || note.status || note.situacao || "Movimentacao"}
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-xl bg-slate-50 p-3">
          <p className="text-[11px] font-black uppercase tracking-wide text-slate-400">Origem</p>
          <p className="mt-1 text-sm font-bold text-slate-700">{origem}</p>
        </div>
        <div className="rounded-xl bg-slate-50 p-3">
          <p className="text-[11px] font-black uppercase tracking-wide text-slate-400">Destino</p>
          <p className="mt-1 text-sm font-bold text-slate-700">{destino}</p>
        </div>
      </div>

      {firstItem.produto?.descricao ? (
        <p className="mt-3 text-sm font-semibold text-slate-600">{firstItem.produto.descricao}</p>
      ) : null}
      {note.observacao ? (
        <p className="mt-2 text-xs font-semibold text-slate-500">{note.observacao}</p>
      ) : null}
    </article>
  );
}
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-4">
      <InfoCard
        ...
        label="Status"
        value={equipment.found ? primary?.status || "Encontrado" : "Nao encontrado"}
        helper={primary?.vinculadoTipo ? `Vinculo: ${primary.vinculadoTipo}` : ""}
      ...
      <InfoCard
        ...
        helper={primary?.cpfCnpjMascarado ? `Doc: ${primary.cpfCnpjMascarado}` : ""}
      />
    </section>
  );
}
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-950">Historico de Movimentacoes</h2>
            <p className="text-xs font-semibold text-slate-500">
              {history.meta?.totalItems ?? history.notes?.length ?? 0} registro(s) encontrados para {history.mac}
            </p>
          </div>
        </div>
      </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-black text-slate-950">Consulta de Estoque</h1>
            <p className="mt-1 max-w-2xl text-sm font-semibold text-slate-500">
              Consulte um MAC especifico para ver vinculo atual, estoque/local e historico de movimentacoes.
            </p>
          </div>
