...
export default function BolsaTecnicoPage() {
  ...
  const totalEquipamentos = typeGroups.reduce((sum, group) => sum + (group.totalEquipamentos || 0), 0);
  const responsaveis = new Set(
    typeGroups.flatMap((group) => (group.responsaveis || []).map((holder) => holder.nome)),
  );
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-black text-slate-950">Bolsa Tecnico</h1>
            <p className="mt-1 max-w-2xl text-sm font-semibold text-slate-500">
              Mostra equipamentos das ordens de retirada do mapa separados por tipo e pelo responsavel atual na API Sempre.
            </p>
          </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">
              A bolsa usa a ultima leitura salva. Em novo mapa, o sistema consulta apenas MACs novos e registra devolucoes.
            </p>
          </div>
        ...
        {data?.refreshing ? (
          <div className="mt-4 flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-700">
            ...
            Leitura em segundo plano. A bolsa sera atualizada assim que terminar.
          </div>
        ) : null}
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs font-semibold text-slate-500">
              A bolsa nao contabiliza vinculo que bate com o nome do cliente da O.S.
            </p>
          </div>
          <button
            type="button"
      ...
      {typeGroups.map((group) => (
        <section key={group.tipo} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-black uppercase tracking-wide text-slate-950">{group.tipo}</h2>
              <p className="text-xs font-semibold text-slate-500">
                {group.totalEquipamentos} equipamento(s) em {group.totalOrdens} ordem(ns)
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {(group.responsaveis || []).slice(0, 6).map((holder) => (
              <article key={`${group.tipo}-${holder.nome}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="break-words text-sm font-black text-slate-900">{holder.nome}</p>
                ...
                <div className="mt-3 grid gap-2">
                  {(holder.items || []).slice(0, 6).map((item) => (
                    <div key={`${item.documentId}-${item.macs?.join("-")}`} className="rounded-xl bg-white p-3 text-xs font-semibold text-slate-600">
                      <p className="font-black text-slate-800">{item.nomeCliente || "Cliente sem nome"}</p>
                      <p>OS {item.numero || "-"} - {item.cidade || "-"}</p>
                      <p className="break-words">MACs: {item.macs?.join(" / ") || "-"}</p>
                    </div>
                  ))}
                </div>
              </article>
          ...
          {(group.responsaveis || []).length > 6 ? (
            <p className="mt-3 text-xs font-bold text-slate-500">
              Mostrando os 6 maiores responsaveis deste tipo. Use os filtros ou a lista paginada por O.S abaixo.
            </p>
          ) : null}
        </section>
      ))}
      ...
      {data?.items?.length ? (
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-black text-slate-950">O.S da bolsa</h2>
              <p className="text-xs font-semibold text-slate-500">
                Pagina {data.page || page} de {data.totalPages || 1}. {data.totalFiltered ?? 0} ordem(ns) filtradas.
              </p>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {(data.items || []).filter((item) => item.responsibleName && !item.responsibleIsClient && item.classificationStatus !== "nao_encontrado").map((item) => (
              <article key={`${item.documentId}-${item.macs?.join("-")}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="break-words text-sm font-black text-slate-900">{item.numero || "-"}</p>
                    <p className="mt-1 text-xs font-semibold text-slate-500">{item.nomeCliente || "-"} - {item.cidade || "-"}</p>
                  </div>
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-blue-700">
                    {item.macs?.length || 0}
                  </span>
                ...
                <div className="mt-3 grid gap-2 text-xs font-semibold text-slate-600">
                  ...
                  <p>Tecnico mapa: <strong>{item.tecnico || "-"}</strong></p>
                  <p>Vinculo atual: <strong>{item.responsibleName || "-"}</strong></p>
                  <p className="break-words">MACs: <strong>{item.macs?.join(" / ") || "-"}</strong></p>
                </div>
              </article>
            ))}
          </div>
      ...
      {data?.logsRecent?.length ? (
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4">
            <h2 className="text-lg font-black text-slate-950">Log de Devolucoes</h2>
            <p className="text-xs font-semibold text-slate-500">
          ...
          <div className="grid gap-3">
            {data.logsRecent.slice(0, 20).map((log) => (
              <article key={`${log.id}-${log.createdAt}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-black text-slate-900">{log.mac}</p>
                    <p className="mt-1 text-xs font-semibold text-slate-500">
                      Saiu de {log.responsavelAnterior || "-"} - OS {log.osAnterior || "-"} - {log.cidadeAnterior || "-"}
                    </p>
                  </div>
                ...
                <p className="mt-2 text-xs font-semibold text-slate-600">{log.message}</p>
              </article>
            ))}
          </div>
        </section>
      ...
      {data && !typeGroups.length ? (
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-8 text-center text-sm font-bold text-amber-800">
          Nenhum equipamento com responsavel atual encontrado nas ordens de retirada do mapa.
        </div>
      ) : null}
    </div>
  );
