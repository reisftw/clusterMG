...
import { BellRing, Camera, Clock, Loader2, Trash2, Volume2 } from "lucide-react";
...
const NOTIFICATION_TYPES = [
  { key: "documentos_pendentes", label: "Documentos pendentes", description: "Avisar quando uma empresa enviar documentos para aprovação." },
  { key: "whatsapp_agendamento_auto", label: "Agendamento automático", description: "Avisar quando o WhatsApp gerar um agendamento automático." },
  { key: "sistema_critico", label: "Alertas críticos", description: "Avisar sobre API offline, falha de backup ou serviço importante indisponível." },
  { key: "backup", label: "Backups", description: "Avisos de backup gerado, falha ou restauração." },
  { key: "api", label: "APIs", description: "Avisos de integrações e APIs monitoradas." },
  { key: "geral", label: "Geral", description: "Comunicados internos do sistema." },
];
...
export default function ConfiguracoesGeraisPage() {
  ...
  const savePreferences = async (nextPreferences, successMessage = "Configuração salva.") => {
    if (!canManage) return;
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("retiradas:notification-preferences-updated", { detail: nextPreferences }));
  ...
  const handleDefaultAvatar = async (event) => {
    ...
    try {
      ...
      await savePreferences({ ...preferences, defaultAvatarUrl: avatarUrl }, "Avatar padrão salvo.");
    } catch (error) {
    ...
    <div className="space-y-5">
      {!canManage ? (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">
          Você está em modo somente leitura. Alterações de configuração exigem permissão de gerenciamento.
        </div>
      ) : null}
            ...
            <div className="flex items-start gap-4">
              <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-blue-50 text-blue-700">
                {preferences.defaultAvatarUrl ? (
                  <img src={preferences.defaultAvatarUrl} alt="Avatar padrão" className="h-full w-full object-cover" />
                ) : (
                  <Camera size={24} />
                )}
              </div>
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm font-semibold leading-relaxed text-slate-500">
                  Usuários sem avatar próprio exibirão esta imagem. Aceita apenas JPG ou PNG até 600 KB.
                </p>
              </div>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              {preferences.defaultAvatarUrl ? (
                <button
                  type="button"
                  disabled={saving || !canManage}
                  onClick={() => savePreferences({ ...preferences, defaultAvatarUrl: "" }, "Avatar padrão removido.")}
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-2xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-black text-slate-950">Som das notificações</h2>
                <p className="text-sm font-semibold text-slate-500">
                  Escolha o som tocado quando chegar uma nova notificação no sininho.
                </p>
              </div>
            </div>
              ...
              {NOTIFICATION_SOUNDS.map((option) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    {option.value === "none" ? "Não tocar som." : "Tocar quando houver alerta novo."}
                  </p>
                </button>
              ))}
            </div>
          ...
          >
            {saving ? <Loader2 className="animate-spin" size={17} /> : <Volume2 size={17} />} Testar som
          </button>
        </div>
      </section>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${enabled ? "bg-emerald-600 text-white" : "bg-slate-200 text-slate-600"}`}>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-950">Horário silencioso</h2>
            <p className="text-sm font-semibold text-slate-500">
              Use para manter o sino visual, mas sem som em um período específico.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[1fr_160px_160px]">
          <label className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 p-4">
            <span>
              <span className="block text-sm font-black text-slate-950">Ativar horário silencioso</span>
              <span className="text-xs font-semibold text-slate-500">Quando ativo, o sistema não toca som no período abaixo.</span>
            </span>
            <input
              type="checkbox"
          ...
          <label className="rounded-2xl border border-slate-200 p-4">
            <span className="text-xs font-black uppercase text-slate-500">Início</span>
            <input
              type="time"
