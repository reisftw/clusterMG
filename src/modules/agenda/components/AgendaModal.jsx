...
const TIPOS = ['Viagem', 'Reunião', 'Visita Técnica', 'Treinamento', 'Outro'];
...
const AgendaModal = ({ evento, onSalvar, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col border border-gray-100">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {/* Atividade */}
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Atividade *</label>
            <input
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">
              <span className="flex items-center gap-1"><MapPin size={11} /> Cidade</span>
            </label>
            <input
              className={inputClass}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">
              <span className="flex items-center gap-1"><Users size={11} /> Participantes</span>
            </label>
              ...
              {tecnicos.map((c) => (
                ...
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-[10px] font-bold ${
                    isSelected(c.id) ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                    {c.nome.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className={`text-xs font-semibold truncate ${isSelected(c.id) ? 'text-blue-700' : 'text-gray-700'}`}>
                      {c.nome}
                    </p>
            ...
            {form.participantes.length > 0 && (
              <p className="text-[11px] text-blue-600 font-medium mt-1.5">
                {form.participantes.length} participante(s) selecionado(s)
              </p>
            )}
          </div>
