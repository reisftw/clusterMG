    ...
    <div className="relative inline-flex items-center">
      ...
      {show && (
        <div className="absolute bottom-full left-0 mb-2 z-20 bg-white border border-gray-100 rounded-xl shadow-lg p-3 min-w-[160px]">
          <p className="text-[10px] font-semibold text-gray-400 uppercase mb-2">Participantes</p>
            ...
            {participantes.map((p) => (
              <li key={p.id} className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center shrink-0">
                  <span className="text-white text-[9px] font-bold">{p.nome.charAt(0)}</span>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-800">{p.nome.split(" ").slice(0, 2).join(" ")}</p>
                  {p.cargo && <p className="text-[10px] text-gray-400">{p.cargo}</p>}
                </div>
              </li>
            ))}
          </ul>
      ...
      {!proximos.length ? (
        ...
        <ul className="space-y-2">
          {proximos.map((e) => {
            ...
            return (
              <li key={e.id} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 hover:bg-blue-50 transition-colors">
                <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${TIPO_DOT[e.tipo] ?? "bg-gray-400"}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate">{e.atividade}</p>
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-semibold text-blue-600">{formatCurto(e.data_inicio)}</p>
                  {isMultiDia && (
                    <p className="text-[10px] text-gray-400">→ {formatCurto(e.data_fim)}</p>
                  )}
                </div>
              </li>
            );
