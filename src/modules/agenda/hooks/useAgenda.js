...
const sortByDataInicio = (items = []) =>
  [...items].sort((a, b) => String(a?.data_inicio ?? '').localeCompare(String(b?.data_inicio ?? '')));
...
export const useAgenda = ({ preferStatic = false } = {}) => {
  ...
  const criar = useCallback(async (dados) => {
    ...
    await registrarAtividade({
      ...
      modulo: 'agenda',
      detalhes: { titulo: dados?.titulo || null, data_inicio: dados?.data_inicio || null },
    });
    invalidateCache(CACHE_KEY);
    setEventos((prev) => sortByDataInicio([...prev, { id: ref.id, ...dados }]));
  ...
  const atualizar = useCallback(async (id, dados) => {
    ...
    invalidateCache(CACHE_KEY);
    setEventos((prev) =>
      sortByDataInicio(prev.map((item) => (item.id === id ? { ...item, ...dados } : item))),
    );
    void regenerateStaticData().catch(() => null);
  }, [currentUser?.id, currentUser?.nome]);
