...
function phoneKey(value) {
	...
	return (
		variants.sort((a, b) => a.length - b.length)[0] || normalizeDigits(value)
	);
}
...
function isTestRecord(item = {}) {
	...
	const cliente = normalizeText(
		item.cliente || item.cliente_nome || item.nome || item.raw?.cliente || "",
	);
	...
	const codigo = normalizeText(
		item.codigo_cliente || item.codigoCliente || item.raw?.codigo_cliente || "",
	);
...
function buildTestContext({ historico = [], callbacks = [], fila = [] }) {
	...
	return {
		historyIds: new Set(
			testHistory
				.map((item) => String(item.id || item.historicoId || ""))
				.filter(Boolean),
		),
		queueIds: new Set(
		...
		callbackIds: new Set(
			testCallbacks
				.map((item) => String(item.id || item.callbackId || ""))
				.filter(Boolean),
		),
		...
		phones: new Set(
			[...testHistory, ...testCallbacks, ...testQueue]
				.flatMap((item) =>
					phoneVariants(item.telefone || item.telefone_digits || item.phone),
				)
				.filter(Boolean),
		),
	};
...
function isLinkedToTest(item = {}, context = {}) {
	...
	const appointmentId = String(
		item.agendamento_id || item.agendamentoId || item.id || "",
	);
	const phoneMatches = phoneVariants(
		item.telefone || item.telefone_digits || item.phone,
	).some((phone) => context.phones?.has(phone));
...
function summarize(
	...
	const realHistorico = historico.filter(
		(item) => !isLinkedToTest(item, testContext),
	);
	const realCallbacks = callbacks.filter(
		(item) => !isLinkedToTest(item, testContext),
	);
	const realFila = fila.filter((item) => !isLinkedToTest(item, testContext));
	const realAgendamentos = agendamentos.filter(
		(item) => !isLinkedToTest(item, testContext),
	);

	const sentHistory = realHistorico.filter(
		(item) => isSent(item) && inPeriod(getReferenceDate(item), bounds),
	);
	const failedHistory = realHistorico.filter(
		(item) => isFailed(item) && inPeriod(getReferenceDate(item), bounds),
	);
	...
	callbacksInPeriod.forEach((item) => {
		...
		const city =
			String(item.cidade || queue?.cidade || "Sem cidade").trim() ||
			"Sem cidade";
		const current = byCity.get(city) || {
			cidade: city,
			enviados: 0,
...
async function exportPdf({ summary, periodLabel }) {
	...
	autoTable(pdf, {
		...
		body: [
			[
				...
				String(summary.totalMessages),
				"Histórico enviado",
			],
			[
				"Contatos únicos impactados",
				String(summary.totalUnique),
				"Telefone/código",
			],
			[
				"Taxa de resposta",
				pctText(summary.responses, summary.totalMessages),
				`${summary.responses} de ${summary.totalMessages} responderam`,
			],
			[
				"Taxa de agendamento",
				pctText(summary.scheduled, summary.totalMessages),
				`${summary.scheduled} de ${summary.totalMessages} agendaram`,
			],
			[
				"Taxa de recolhimento final",
				pctText(summary.collected, summary.scheduledDue),
				`${summary.collected} de ${summary.scheduledDue} agendamentos ate ${summary.cutoffLabel}`,
			],
			[
				"Agendamentos hoje/futuros",
				String(summary.scheduledFuture),
				"Nao entram na taxa de recolhimento",
			],
			[
				"Diferenca em aberto",
				String(summary.pendingCollection),
				`Agendados ate ${summary.cutoffLabel} ainda sem recolhimento`,
			],
			[
				"Conversão de atendimento",
				pctText(summary.scheduled, summary.responses),
				"Agendados sobre respondidos",
			],
			[
				"Sucesso técnico/campo",
				pctText(summary.collected, summary.scheduledDue),
				"Recolhidos sobre agendados",
			],
		],
	...
	autoTable(pdf, {
		...
		head: [
			["Cidade", "Enviadas", "Respostas", "Agendamentos", "Recolhimentos"],
		],
	...
	pdf.save(
		`relatorio-mensageria-${periodLabel.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.pdf`,
	);
}
...
export default function MensageriaRelatoriosPage() {
	...
	const load = async () => {
		...
		try {
			...
			setFeedback(
				error?.message ||
					"Não foi possível carregar os relatórios da Mensageria.",
			);
		} finally {
			setLoading(false);
		}
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-sm text-slate-500">
								Acompanhe resposta, agendamento e recolhimento final com base na
								conferência do mapa.
							</p>
						</div>
					</div>
			...
			<section className="grid gap-4 xl:grid-cols-2">
				<div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<p className="mt-1 text-sm text-slate-500">
						Em relação ao total de mensagens enviadas no período.
					</p>
					<div className="mt-6 space-y-5">
						...
						/>
						<FunnelBar
							label={`Recolhimento até ${summary.cutoffLabel}`}
							value={summary.collected}
							total={summary.scheduledDue}
							tone="bg-orange-500"
						/>
					</div>
					<div className="mt-5 rounded-xl border border-orange-100 bg-orange-50 px-4 py-3 text-sm font-bold text-orange-800">
						{summary.collected} recolhidos sobre {summary.scheduledDue}{" "}
						agendamentos até {summary.cutoffLabel}. Diferença em aberto:{" "}
						{summary.pendingCollection}. Hoje/futuros: {summary.scheduledFuture}
						.
					</div>
				</div>
				<div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
					<h2 className="text-lg font-black text-slate-950">
						Visão por etapa do funil
					</h2>
					<p className="mt-1 text-sm text-slate-500">
						Efetividade depois que o cliente engaja.
					</p>
					<div className="mt-6 space-y-5">
						<FunnelBar
							label="Conversão de atendimento"
							value={summary.scheduled}
							total={summary.responses}
							tone="bg-blue-600"
						/>
						<FunnelBar
							label="Sucesso do técnico/campo"
							value={summary.collected}
							total={summary.scheduledDue}
							tone="bg-emerald-600"
						/>
						<FunnelBar
							label="Falhas de envio registradas"
							value={summary.failures}
							total={summary.totalMessages + summary.failures}
							tone="bg-red-500"
						/>
					</div>
				</div>
				...
				<div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
					<span>
						...
						{summary.byCity.length
							? (normalizedCityPage - 1) * cityPageSize + 1
							: 0}{" "}
						a{" "}
						{Math.min(normalizedCityPage * cityPageSize, summary.byCity.length)}{" "}
						de {summary.byCity.length} cidades
					</span>
					<div className="flex items-center gap-2">
						...
						<button
							...
							onClick={() =>
								setCityPage((page) => Math.min(cityTotalPages, page + 1))
							}
						>
							Próxima
						</button>
