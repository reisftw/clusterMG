...
import { AlertTriangle, ChevronLeft, ChevronRight, MessageCircleReply, RefreshCw, Send } from "lucide-react";
...
const getPhoneGroupKey = (value) => {
  ...
  return variants.sort((a, b) => a.length - b.length)[0] || normalizePhone(value);
};
...
const groupItemsByPhone = (items = []) => {
  ...
  items.forEach((item) => {
    ...
    if (nextDate > currentDate) {
      groups.set(key, { ...item, groupedItems: [...current.groupedItems, item] });
    } else {
      current.groupedItems.push(item);
    }
  });
  return [...groups.values()].sort((a, b) => getItemTimestamp(b) - getItemTimestamp(a));
};
...
const buildCallbackIndexes = (callbacks = []) => {
  ...
  callbacks.forEach((callback) => {
    ...
    getPhoneVariants(callback.telefone).forEach((phone) => addToIndex(byPhone, phone, callback));
  });
...
const findResponsesForItem = (item, indexes) => {
  ...
  const addResponses = (responses = []) => {
    responses.forEach((response) => {
      responseMap.set(response.id || `${response.telefone}-${getItemTimestamp(response)}-${readCallbackMessage(response)}`, response);
    });
  };
  ...
  getPhoneVariants(item.telefone).forEach((phone) => addResponses(indexes.byPhone.get(phone)));

  return [...responseMap.values()].sort((a, b) => getItemTimestamp(b) - getItemTimestamp(a));
};

const MensageriaEnviadosPage = () => {
  const [items, setItems] = useState([]);
  const [callbacks, setCallbacks] = useState([]);
  const [resumo, setResumo] = useState({ enviados: 0, falhas: 0, respostas: 0 });
  ...
  const visibleItems = useMemo(() => {
    ...
    const grouped = groupItemsByPhone(items).map((item) => {
      const relatedItems = item.groupedItems?.length ? item.groupedItems : [item];
      const responseMap = new Map();
      relatedItems.forEach((relatedItem) => {
        findResponsesForItem(relatedItem, callbackIndexes).forEach((response) => {
          responseMap.set(response.id || `${response.telefone}-${getItemTimestamp(response)}-${readCallbackMessage(response)}`, response);
        });
      });
      const responses = [...responseMap.values()].sort((a, b) => getItemTimestamp(b) - getItemTimestamp(a));
      return { ...item, groupedItems: relatedItems, responses };
    });
    const tabFiltered =
      viewTab === "agendados"
        ? grouped.filter((item) => item.responses?.some((response) => response.agendado || response.status === "agendado"))
        : grouped;
    const filtered = search
      ? tabFiltered.filter((item) =>
      [item.cliente, item.telefone, item.os, item.cidade, item.status, item.origem]
        .join(" ")
        .toLowerCase()
        .includes(search),
      )
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
          <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Enviadas</p>
          <p className="mt-2 text-2xl font-bold text-emerald-900">{Number(resumo.enviados || 0).toLocaleString("pt-BR")}</p>
        </div>
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
          <p className="text-xs font-bold uppercase tracking-wide text-blue-700">Respostas</p>
          <p className="mt-2 text-2xl font-bold text-blue-900">{Number(resumo.respostas || 0).toLocaleString("pt-BR")}</p>
        </div>
        <div className="rounded-lg border border-red-200 bg-red-50 p-4">
          <p className="text-xs font-bold uppercase tracking-wide text-red-700">Falhas</p>
          <p className="mt-2 text-2xl font-bold text-red-900">{Number(resumo.falhas || 0).toLocaleString("pt-BR")}</p>
        </div>
      </section>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {latestCallbacksByPhone.length ? (
            latestCallbacksByPhone.slice(0, 8).map((callback) => (
              <div key={callback.id} className="rounded-lg border border-blue-100 bg-blue-50 p-3">
                ...
                <p className="mt-2 line-clamp-3 text-sm text-slate-700">
                  {readCallbackMessage(callback) || "Mensagem sem texto legível"}
                </p>
                <p className="mt-2 text-xs font-bold text-blue-700">{callback.status || "-"}</p>
              </div>
            ))
          ) : (
        ...
        <div className="mb-4 inline-flex rounded-lg border border-slate-200 bg-slate-50 p-1">
          <button
            ...
            className={`rounded-md px-4 py-2 text-sm font-bold transition ${
              viewTab === "todos" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-white"
            }`}
          >
            Todos
          </button>
          <button
            ...
            className={`rounded-md px-4 py-2 text-sm font-bold transition ${
              viewTab === "agendados" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-white"
            }`}
          >
              ...
              columns={[
                {
                  ...
                  render: (item) => (
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{item.os || "-"} • {item.cidade || "-"}</p>
                    </div>
                  ),
                },
                { key: "telefone", header: "Telefone", render: (item) => <span className="font-semibold">{item.telefone || "-"}</span> },
                { key: "data", header: "Data", render: (item) => formatDateTime(item.criadoEm || item.criado_em) },
                {
                  key: "status",
                  header: "Status",
                  render: (item) => (
                    <span className={`inline-flex h-fit w-fit items-center gap-1 rounded-full px-2 py-1 text-xs font-bold ${
                      item.status === "falhou" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"
                    }`}>
                      {item.status === "falhou" ? <AlertTriangle size={13} /> : <Send size={13} />}
                      {item.status || "enviado"}
                    </span>
                  ),
                },
                    ...
                    return latestResponse ? (
                      <div className="space-y-2">
                        <div className="rounded-lg border border-blue-100 bg-blue-50 p-2">
                          ...
                          <p className="mt-1 text-sm font-medium text-slate-800">{readCallbackMessage(latestResponse) || "-"}</p>
                          <p className="mt-1 text-xs text-slate-500">{latestResponse.status || "-"}</p>
                          {responses.length > 1 ? (
                            <button
                              type="button"
                              onClick={() => setConversationModal({ item, responses, sentItems: relatedItems })}
      ...
      {conversationModal ? (
        ...
        >
            <div className="grid gap-4 lg:grid-cols-2">
              <div>
                <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-blue-700">Recebidas</h3>
                <div className="space-y-3">
                  {conversationModal.responses?.length ? conversationModal.responses.map((response) => (
                <div key={response.id} className="rounded-lg border border-blue-100 bg-blue-50 p-3">
                  ...
                  <p className="mt-2 whitespace-pre-wrap text-sm font-medium text-slate-800">
                    {readCallbackMessage(response) || "Mensagem sem texto legível"}
                  </p>
                  {response.motivo ? (
                    <p className="mt-2 text-xs font-semibold text-slate-500">{response.motivo}</p>
                  ) : null}
                </div>
              )) : (
              ...
              <div>
                <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-emerald-700">Enviadas</h3>
                <div className="space-y-3">
                  {(conversationModal.sentItems || []).map((sentItem) => (
                    <div key={sentItem.id} className="rounded-lg border border-emerald-100 bg-emerald-50 p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700">
                          <Send size={13} />
                          {formatDateTime(sentItem.criadoEm || sentItem.criado_em)}
                        </span>
                        <span className="rounded-full bg-white px-2 py-1 text-xs font-bold text-slate-600">
                      ...
                      <p className="mt-2 whitespace-pre-wrap text-sm font-medium text-slate-800">
                        {sentItem.mensagem || "Mensagem enviada sem corpo registrado."}
                      </p>
