...
const VARIABLES = [
  { token: "{cliente}", label: "Cliente", field: "cliente", sample: "Maria Oliveira" },
  { token: "{primeiro_nome}", label: "Primeiro nome", field: "primeiro_nome", sample: "Maria" },
  { token: "{codigo_cliente}", label: "Código do cliente", field: "codigo_cliente", sample: "48291" },
  { token: "{contrato}", label: "Contrato", field: "contrato", sample: "CTR-10482" },
  { token: "{os}", label: "O.S.", field: "os", sample: "OS-72913" },
  { token: "{cidade}", label: "Cidade", field: "cidade", sample: "São Paulo" },
  { token: "{regional}", label: "Regional", field: "regional", sample: "Leste" },
  { token: "{endereco}", label: "Endereço", field: "endereco", sample: "Rua das Flores, 120 - Centro" },
  { token: "{telefone}", label: "Telefone", field: "telefone", sample: "(11) 98888-2211" },
  { token: "{data_cancelamento}", label: "Data do cancelamento", field: "data_cancelamento", sample: "01/08/2026" },
  { token: "{data_abertura_os}", label: "Data de abertura da O.S.", field: "data_abertura_os", sample: "01/08/2026" },
  { token: "{dias_aberta}", label: "Dias em aberto", field: "dias_aberta", sample: "3" },
  { token: "{status_os}", label: "Status da O.S.", field: "status_os", sample: "Pendente" },
  { token: "{tipo}", label: "Tipo da O.S.", field: "tipo", sample: "Retirada" },
  { token: "{protocolo}", label: "Protocolo", field: "protocolo", sample: "PRT-20260801" },
  { token: "{central_whatsapp}", label: "WhatsApp da central", field: "central_whatsapp", sample: "+55 31 3987-0880" },
  { token: "{link_agendamento}", label: "Link de agendamento", field: "link_agendamento", sample: "https://wa.me/553139870880" },
];
...
const replaceVariables = (text, item) => {
  ...
  return VARIABLES.reduce(
    (current, variable) =>
      current.replaceAll(variable.token, data[variable.field] ?? variable.sample),
    text || "",
  );
};
...
const formatInterval = (value, unit) => {
  ...
  if (unit === "minutes") return `${safeValue} minuto${safeValue > 1 ? "s" : ""}`;
...
const MensageriaPage = () => {
  ...
  const providerLabel = getProviderLabel(config.whatsappProvider || "evolution");
  const canManage =
  ...
  const selectedQueueItem =
    fila.find((item) => item.id === selectedQueueId) || fila[0] || SAMPLE_QUEUE_ITEMS[0];
  ...
  const stats = useMemo(() => {
    const sentToday = historico.filter((item) => {
      const date = item.criadoEm?.toDate?.() || (item.criadoEm ? new Date(item.criadoEm) : null);
    ...
    return {
      fila: fila.filter((item) => !["enviado", "ignorado"].includes(item.status)).length,
      enviadas: sentToday,
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setActiveTemplateId(nextConfig.activeTemplateId || nextTemplates[0]?.id || "cancelamento");
      setTestTemplateId(nextConfig.activeTemplateId || nextTemplates[0]?.id || "cancelamento");
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all([
        ...
        salvarTemplateMensageria({ ...activeTemplate, requiredCentralButton: false }),
      ]);
  ...
  const handleNewTemplate = async () => {
    ...
    const template = {
      ...
      conteudo: "Olá, {primeiro_nome}! Podemos ajudar com o agendamento da coleta?",
    };
  ...
  const handleSimulateMapDiff = async () => {
    ...
    try {
      const existingKeys = new Set(
        fila.map((item) => `${item.os || ""}|${String(item.telefone || "").replace(/\D/g, "")}`),
      );
      const created = [];
  ...
  const handleRegisterSend = async (item) => {
    ...
    setFeedback(
      result?.sent
        ...
        : result?.skipped || "Envio acionado, mas nenhum item apto foi encontrado na fila.",
    );
    await loadData();
  };
  ...
  const handleToggleEvolutionPause = async () => {
    ...
    try {
      if (config.evolutionPaused) {
        ...
        setConfig((current) => ({ ...current, evolutionEnabled: true, evolutionPaused: false, autoSend: true }));
        setFeedback("Automação iniciada. Novos clientes do Mapa entram na fila conforme as regras configuradas.");
      } else {
      ...
      setFeedback(error?.message || "N?o foi poss?vel iniciar ou pausar a automação.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Central para mensagens automáticas de coleta geradas pela diferença do Mapa.
            </p>
          </div>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Na fila", value: stats.fila, icon: Clock, tone: "text-amber-600 bg-amber-50" },
          { label: "Enviadas hoje", value: stats.enviadas, icon: Send, tone: "text-blue-600 bg-blue-50" },
          { label: "Aprovadas", value: stats.aprovadas, icon: CheckCircle2, tone: "text-emerald-600 bg-emerald-50" },
          { label: "Falhas", value: stats.falhas, icon: AlertTriangle, tone: "text-red-600 bg-red-50" },
        ].map(({ label, value, icon: Icon, tone }) => (
          <div key={label} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</p>
                <p className="mt-2 text-2xl font-bold text-slate-900">{Number(value || 0).toLocaleString("pt-BR")}</p>
              </div>
      ...
      <section className="rounded-lg border border-emerald-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-slate-900">
              ...
              <h2 className="text-lg font-bold">Teste e automação {providerLabel}</h2>
            </div>
            <p className="mt-1 text-sm text-slate-500">
              Envie uma mensagem de teste com qualquer modelo cadastrado antes de liberar a fila automática.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_auto]">
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Número de teste</span>
            <input
              value={testPhone}
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Tipo de mensagem</span>
            <select
        ...
        <div className="mt-4 grid gap-3 text-xs text-slate-500 md:grid-cols-4">
          <span>Worker: {evolutionStatus?.worker?.workerActive ? "ativo" : "inativo"}</span>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(380px,0.8fr)]">
        <div className="space-y-6">
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2 text-slate-900">
              <Settings size={18} />
              <h2 className="text-lg font-bold">Atualização do Mapa e regras de envio</h2>
            </div>
            <p className="mt-1 text-sm text-slate-500">
              Defina a frequência, janela permitida, duplicidade e retentativas antes de conectar a API.
            </p>

            <div className="mt-5 grid gap-4 md:grid-cols-5">
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Intervalo</span>
                <input
                  type="number"
                  min="1"
                  value={config.intervalValue}
                  onChange={(event) => updateConfig("intervalValue", event.target.value)}
                  disabled={!canManage}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </label>
              <label className="block">
                ...
                <select
                  ...
                  onChange={(event) => updateConfig("intervalUnit", event.target.value)}
                  disabled={!canManage}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                >
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) => updateConfig("sendWindowStart", event.target.value)}
                  disabled={!canManage}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </label>
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) => updateConfig("sendWindowEnd", event.target.value)}
                  disabled={!canManage}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </label>
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Limite diário</span>
                <input
                  type="number"
                  min="1"
                  value={config.dailySendLimit || 100}
                  onChange={(event) => updateConfig("dailySendLimit", event.target.value)}
                  disabled={!canManage}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </label>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-5">
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Bloquear duplicidade por</span>
                <div className="mt-1 flex items-center gap-2">
                  <input
                    type="number"
                    min="1"
                    value={config.duplicateBlockDays}
                    onChange={(event) => updateConfig("duplicateBlockDays", event.target.value)}
                    disabled={!canManage}
              ...
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Limite de tentativas</span>
                <input
                  type="number"
                  min="1"
                  value={config.retryLimit}
                  onChange={(event) => updateConfig("retryLimit", event.target.value)}
                  disabled={!canManage}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </label>
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Retentar após</span>
                <div className="mt-1 flex items-center gap-2">
                  <input
                    type="number"
                    min="1"
                    value={config.retryAfterMinutes}
                    onChange={(event) => updateConfig("retryAfterMinutes", event.target.value)}
                    disabled={!canManage}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                  />
              ...
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Delay mínimo</span>
                <div className="mt-1 flex items-center gap-2">
                  <input
                    type="number"
                    min="5"
                    value={config.evolutionMinDelaySeconds || 45}
                    onChange={(event) => updateConfig("evolutionMinDelaySeconds", event.target.value)}
                    disabled={!canManage}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                  />
              ...
              <label className="block">
                <span className="text-sm font-semibold text-slate-700">Delay máximo</span>
                <div className="mt-1 flex items-center gap-2">
                  <input
                    type="number"
                    min="10"
                    value={config.evolutionMaxDelaySeconds || 120}
                    onChange={(event) => updateConfig("evolutionMaxDelaySeconds", event.target.value)}
                    disabled={!canManage}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                  />
                  <span className="text-sm font-semibold text-slate-500">s</span>
            ...
            <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
              A automação respeita o limite diário e envia apenas dentro da janela configurada. Com delay inteligente ativo, o sistema calcula o intervalo ideal para distribuir a fila do mapa até o fim do período.
            </div>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-4">
              {[
                ["autoSync", "Atualização automática", "Buscar diferenças a cada " + formatInterval(config.intervalValue, config.intervalUnit)],
                ["approvedTemplate", "Modelo aprovado", "Libera o envio quando a API estiver conectada."],
                ["autoSend", "Envio automático", "Envia assim que uma diferença nova entrar."],
                ["autoEnqueueMapDiff", "Fila pelo mapa", "Novos clientes do mapa entram automaticamente na fila."],
                ["smartDelayEnabled", "Delay inteligente", "Distribui o limite diário do começo ao fim da janela."],
                ["avoidDuplicates", "Bloquear duplicidade", "Evita repetir mensagem para a mesma O.S. ou telefone."],
              ].map(([field, title, description]) => (
                <label key={field} className="flex items-start gap-3 rounded-lg border border-slate-200 p-3 text-sm text-slate-700">
                  <input
                    type="checkbox"
                    checked={Boolean(config[field])}
                    onChange={(event) => updateConfig(field, event.target.checked)}
                    disabled={!canManage}
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Modelos de mensagem</h2>
                <p className="mt-1 text-sm text-slate-500">
                  Cadastre templates por situação e use variáveis para personalizar cada cliente.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-[220px_minmax(0,1fr)]">
              <div className="space-y-2">
                {templates.map((template) => (
                  ...
                  >
                    ...
                    <span className="text-xs opacity-75">{template.situacao}</span>
                  </button>
                ))}
              </div>
              ...
              <div>
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-700">Situação</span>
                    <input
                      value={activeTemplate?.situacao || ""}
                      onChange={(event) => updateTemplate("situacao", event.target.value)}
                      disabled={!canManage}
                      className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    />
                  </label>
                ...
                <textarea
                  ...
                  onChange={(event) => updateTemplate("conteudo", event.target.value)}
                  disabled={!canManage}
                  rows={9}
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-700">WhatsApp da central</span>
                    <input
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-700">Mensagem do botão</span>
                    <input
                      value={config.buttonMessage}
                      onChange={(event) => updateConfig("buttonMessage", event.target.value)}
                      disabled={!canManage}
                      className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    />
                  </label>
        ...
        <aside className="space-y-6">
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <label className="mt-4 block">
              <span className="text-sm font-semibold text-slate-700">Cliente da prévia</span>
              <select
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-600">
              <p>Limite diário: {Number(config.dailySendLimit || 100).toLocaleString("pt-BR")} mensagem(ns)</p>
              <p>Janela: {config.sendWindowStart} até {config.sendWindowEnd}</p>
              <p>Delay: {config.smartDelayEnabled ? "inteligente pela janela de envio" : "manual por intervalo"}</p>
              <p>Intervalo de segurança: {config.evolutionMinDelaySeconds || 45}s até {config.evolutionMaxDelaySeconds || 120}s</p>
              <p>Dias: {(config.sendDays || []).join(", ") || "Nenhum dia selecionado"}</p>
              <p>Duplicidade: {config.avoidDuplicates ? `${config.duplicateBlockDays} dias` : "desativada"}</p>
              <p>Retentativa: até {config.retryLimit} tentativa(s), a cada {config.retryAfterMinutes} minutos</p>
            </div>
          </div>
        </aside>
      </section>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Fila de aprovação</h2>
            <p className="mt-1 text-sm text-slate-500">
              Clientes detectados pela diferença do Mapa antes do disparo pelo WhatsApp.
            </p>
          </div>
          <button
            type="button"
              ...
              {fila.length ? (
                fila.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-semibold text-slate-900">{item.cliente}</td>
                    <td className="px-4 py-3 text-slate-600">{item.os}</td>
                    <td className="px-4 py-3 text-slate-600">{item.cidade}</td>
                    <td className="px-4 py-3 text-slate-600">{item.telefone}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${STATUS_CLASSES[item.status] || STATUS_CLASSES.novo}`}>
                        {STATUS_LABELS[item.status] || item.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-600">{Number(item.tentativas || 0)}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2">
                        <button type="button" disabled={!canManage} onClick={() => handleQueueStatus(item, "aprovado")} className="rounded-lg border border-emerald-200 px-2.5 py-1 text-xs font-bold text-emerald-700 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60">
                          Aprovar
                        </button>
                        <button type="button" disabled={!canManage} onClick={() => handleRegisterSend(item)} className="rounded-lg border border-blue-200 px-2.5 py-1 text-xs font-bold text-blue-700 hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-60">
                          Enviar
                        </button>
                        <button type="button" disabled={!canManage} onClick={() => handleRetry(item)} className="rounded-lg border border-amber-200 px-2.5 py-1 text-xs font-bold text-amber-700 hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-60">
                          <RotateCcw size={12} className="mr-1 inline" />
                          Retentar
                        </button>
                        <button type="button" disabled={!canManage} onClick={() => handleQueueStatus(item, "ignorado")} className="rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
                          Ignorar
                        </button>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-sm text-slate-500">
                    Nenhum cliente na fila. Use a simulação para preparar dados de teste.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <table className="min-w-[760px] w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {historico.length ? (
                historico.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 text-slate-600">{formatDateTime(item.criadoEm)}</td>
                    <td className="px-4 py-3 font-semibold text-slate-900">{item.cliente}</td>
                    <td className="px-4 py-3 text-slate-600">{item.telefone}</td>
                    <td className="px-4 py-3 text-slate-600">{item.os}</td>
