...
const getSendForecast = (item, config = {}) => {
  ...
  if (item.proximaTentativaEm) return `Após ${formatDateTime(item.proximaTentativaEm)}`;
  if (config.evolutionPaused) return "Pausada";
  return "Apto para envio";
};

const getProviderLabel = (config = {}) => {
  if (config.whatsappProvider === "official_whatsapp") return "WhatsApp oficial";
  return "Evolution API";
};

const MensageriaFilaPage = () => {
  const { currentUser } = useAuthContext();
  const canManage = hasPermission(currentUser, "mensageria.fila.manage") ||
  ...
  const [filters, setFilters] = useState({ status: "pendentes", origem: "todas", search: "" });
  ...
  const origemOptions = useMemo(() => {
    const values = new Set(fila.map((item) => String(item.origem || "Sem origem")).filter(Boolean));
  ...
  const filteredFila = useMemo(() => {
    ...
    return fila.filter((item) => {
      ...
      if (filters.status === "pendentes" && ["enviado", "ignorado"].includes(itemStatus)) return false;
      if (filters.status !== "todos" && filters.status !== "pendentes" && itemStatus !== filters.status) return false;
      if (filters.origem !== "todas" && String(item.origem || "Sem origem") !== filters.origem) return false;
      if (!text) return true;
      return normalize([item.cliente, item.telefone, item.os, item.cidade, item.origem, item.tipo].join(" ")).includes(text);
    });
  }, [fila, filters]);

  const totalPages = Math.max(1, Math.ceil(filteredFila.length / pageSize));
  const currentPage = Math.min(page, totalPages);
  const pageItems = filteredFila.slice((currentPage - 1) * pageSize, currentPage * pageSize);

  const stats = useMemo(
    () => ({
      total: fila.length,
      pendentes: fila.filter((item) => !["enviado", "ignorado"].includes(String(item.status || "novo"))).length,
      aprovados: fila.filter((item) => String(item.status || "") === "aprovado").length,
      falhas: fila.filter((item) => String(item.status || "") === "falhou").length,
    }),
    [fila],
  );
  ...
  const queueWaiting = queueRunning && !queueHasError && Boolean(status?.worker?.lastSkipped);
  ...
  const handleStartQueue = async () => {
    ...
    try {
      ...
      setFeedback("Fila iniciada. O sistema vai enviar uma mensagem por vez respeitando o delay configurado.");
      await loadData();
    } catch (error) {
    ...
    try {
      ...
      setFeedback(
        result?.sent
          ...
          : result?.skipped || "Item priorizado, mas nenhum envio foi executado.",
      );
      await loadData();
    } catch (error) {
  ...
  const toggleSendDay = (day) => {
    setConfig((current) => {
      ...
      return {
        ...current,
        sendDays: days.includes(day) ?days.filter((item) => item !== day) : [...days, day],
      };
    });
  };
  ...
  const handleSaveQueueConfig = async () => {
    ...
    setFeedback("");
    try {
      const minDelay = Math.max(5, Number(config.evolutionMinDelaySeconds || 45));
      const nextConfig = {
        ...config,
        dailySendLimit: Math.max(1, Number(config.dailySendLimit || 100)),
        evolutionMinDelaySeconds: minDelay,
        evolutionMaxDelaySeconds: Math.max(minDelay, Number(config.evolutionMaxDelaySeconds || 120)),
        sendDays: Array.isArray(config.sendDays) && config.sendDays.length
          ?config.sendDays
      ...
      setFeedback(error?.message || "Não foi possível salvar as configurações da fila.");
    } finally {
      setWorking(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Veja quem está aguardando envio, quando será enviado e de onde o cliente entrou na fila.
                </p>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              <StartButtonIcon size={16} className={workerRunning ? "animate-pulse" : ""} />
              {startButtonLabel}
            </button>
          </div>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          { label: "Na fila", value: stats.pendentes, icon: Clock, tone: "border-amber-200 bg-amber-50 text-amber-700" },
          { label: "Aprovadas", value: stats.aprovados, icon: CheckCircle2, tone: "border-emerald-200 bg-emerald-50 text-emerald-700" },
          { label: "Falhas", value: stats.falhas, icon: AlertTriangle, tone: "border-red-200 bg-red-50 text-red-700" },
          { label: "Total", value: stats.total, icon: Send, tone: "border-blue-200 bg-blue-50 text-blue-700" },
        ].map((card) => {
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 md:grid-cols-4">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-bold uppercase text-slate-500">Sistema</p>
            <p className="mt-1 text-sm font-black text-slate-900">{getProviderLabel(config)}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-bold uppercase text-slate-500">Automação</p>
            <p className="mt-1 text-sm font-black text-slate-900">{config.evolutionPaused ?"Pausada" : "Ativa"}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-bold uppercase text-slate-500">Próxima execução</p>
            <p className="mt-1 text-sm font-black text-slate-900">{formatDateTime(status?.worker?.nextRunAt)}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-bold uppercase text-slate-500">Última execução</p>
            <p className="mt-1 text-sm font-black text-slate-900">{formatDateTime(status?.worker?.lastRun)}</p>
          </div>
        </div>
      ...
      <section className="rounded-lg border border-blue-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Janela de envio</h2>
            <p className="text-sm text-slate-500">
              Configure aqui quando a fila pode enviar automaticamente e qual intervalo será usado entre uma mensagem e outra.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <label className="block">
            <span className="text-xs font-bold uppercase text-slate-500">Início</span>
            <input
              type="time"
              value={config.sendWindowStart || "08:00"}
              onChange={(event) => updateQueueConfig("sendWindowStart", event.target.value)}
              disabled={!canManage}
          ...
          <label className="block">
            <span className="text-xs font-bold uppercase text-slate-500">Fim</span>
            <input
              type="time"
              value={config.sendWindowEnd || "18:00"}
              onChange={(event) => updateQueueConfig("sendWindowEnd", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="block">
            <span className="text-xs font-bold uppercase text-slate-500">Limite diário</span>
            <input
              type="number"
              min="1"
              value={config.dailySendLimit || 100}
              onChange={(event) => updateQueueConfig("dailySendLimit", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="block">
            <span className="text-xs font-bold uppercase text-slate-500">Delay mínimo</span>
            <input
              type="number"
              min="5"
              value={config.evolutionMinDelaySeconds || 45}
              onChange={(event) => updateQueueConfig("evolutionMinDelaySeconds", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="block">
            <span className="text-xs font-bold uppercase text-slate-500">Delay máximo</span>
            <input
              type="number"
              min="5"
              value={config.evolutionMaxDelaySeconds || 120}
              onChange={(event) => updateQueueConfig("evolutionMaxDelaySeconds", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
        ...
        <div className="mt-4">
          <p className="text-xs font-bold uppercase text-slate-500">Dias de envio</p>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                ...
                pageItems.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-slate-500">{item.codigo_cliente || item.contrato || "-"}</p>
                    </td>
                    <td className="px-4 py-3 font-semibold text-slate-700">{item.telefone || "-"}</td>
                    ...
                    <td className="px-4 py-3">
                      <p className="font-semibold text-slate-800">{getSendForecast(item, config)}</p>
                      {item.ultimoErro ?<p className="mt-1 max-w-xs text-xs text-red-600">{item.ultimoErro}</p> : null}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-bold ${STATUS_CLASSES[item.status] || STATUS_CLASSES.novo}`}>
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        disabled={working || !canManage || ["enviado", "ignorado"].includes(String(item.status || ""))}
        ...
        <div className="mt-4 flex flex-col gap-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
          <span>
            ...
            {Math.min(currentPage * pageSize, filteredFila.length)} de {filteredFila.length} item(ns)
          </span>
