...
import { LogOut, MessageCircle, QrCode, RefreshCw, Save, Server, ShieldCheck } from "lucide-react";
import {
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">API da Mensageria</h1>
            <p className="mt-1 text-sm text-slate-500">
              Configure a Evolution API, gere o QR Code e acompanhe o estado da conexão.
            </p>
          </div>
        </div>
  ...
  return (
    <section className="grid gap-4 md:grid-cols-5">
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Worker</p>
        <p className="mt-2 text-lg font-bold text-slate-900">
          {status?.worker?.workerActive ? "Ativo" : "Inativo"}
        </p>
      </div>
      <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-blue-700">Sistema</p>
        <p className="mt-2 text-lg font-bold text-blue-950">
          {getProviderLabel(config.whatsappProvider || "evolution")}
        </p>
      </div>
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Fila</p>
        <p className="mt-2 text-lg font-bold text-slate-900">
          {config.evolutionPaused ? "Pausada" : "Automática"}
        </p>
      </div>
      <div className={`rounded-lg border p-4 shadow-sm ${connection.connected ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"}`}>
        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">WhatsApp</p>
        <p className={`mt-2 text-lg font-bold ${connection.connected ? "text-emerald-800" : "text-slate-900"}`}>
          {connectionLabel}
        </p>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Último erro</p>
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="inline-flex rounded-lg border border-slate-200 bg-slate-50 p-1">
          <button type="button" onClick={() => onChange("evolution")} disabled={!canManage} className={providerButtonClass(provider === "evolution")}>
            Evolution API
          </button>
          <button type="button" onClick={() => onChange("official_whatsapp")} disabled={!canManage} className={providerButtonClass(provider === "official_whatsapp")}>
            WhatsApp oficial
          </button>
        </div>
      </div>
...
function OfficialWhatsappSection({ config, updateConfig, canManage }) {
  ...
  return (
    <section className="rounded-lg border border-emerald-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">
        Configuração compatível com WhatsApp Cloud API. O envio fora da janela de atendimento deve usar template aprovado.
      </p>
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="block">
          <span className="text-sm font-semibold text-slate-700">Base URL Graph API</span>
          <input
            value={config.officialWhatsappBaseUrl || ""}
            onChange={(event) => updateConfig("officialWhatsappBaseUrl", event.target.value)}
            disabled={!canManage}
        ...
        <label className="block">
          <span className="text-sm font-semibold text-slate-700">Phone Number ID</span>
          <input
            value={config.officialWhatsappPhoneNumberId || ""}
            onChange={(event) => updateConfig("officialWhatsappPhoneNumberId", event.target.value)}
            disabled={!canManage}
            className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
          />
        </label>
        <label className="block">
          <span className="text-sm font-semibold text-slate-700">Business Account ID</span>
          <input
            value={config.officialWhatsappBusinessAccountId || ""}
            onChange={(event) => updateConfig("officialWhatsappBusinessAccountId", event.target.value)}
            disabled={!canManage}
            className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
          />
        </label>
        <label className="block">
          <span className="text-sm font-semibold text-slate-700">Webhook Verify Token</span>
          <input
            value={config.officialWebhookVerifyToken || ""}
            onChange={(event) => updateConfig("officialWebhookVerifyToken", event.target.value)}
            disabled={!canManage}
            className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
          />
        </label>
        <label className="block md:col-span-2">
          <span className="text-sm font-semibold text-slate-700">Access Token</span>
          <input
            type="password"
            value={config.officialWhatsappAccessToken || ""}
            onChange={(event) => updateConfig("officialWhatsappAccessToken", event.target.value)}
            disabled={!canManage}
            className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
          />
        </label>
        <label className="block">
          <span className="text-sm font-semibold text-slate-700">Template aprovado</span>
          <input
            value={config.officialWhatsappTemplateName || ""}
            onChange={(event) => updateConfig("officialWhatsappTemplateName", event.target.value)}
            disabled={!canManage}
        ...
        <label className="block">
          <span className="text-sm font-semibold text-slate-700">Idioma do template</span>
          <input
            value={config.officialWhatsappTemplateLanguage || "pt_BR"}
            onChange={(event) => updateConfig("officialWhatsappTemplateLanguage", event.target.value)}
            disabled={!canManage}
            className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
          />
        </label>
...
const MensageriaApiPage = () => {
  ...
  const canManage = hasPermission(currentUser, "mensageria.api.manage") ||
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setFeedback(error?.message || "Não foi possível carregar a API da Mensageria.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const syncSelectedEvolutionAccount = (nextConfig) => {
    const accounts = Array.isArray(nextConfig.evolutionAccounts) ?nextConfig.evolutionAccounts : [];
    const selected = accounts.find((account) => account.id === nextConfig.evolutionSelectedAccountId);
  ...
  const updateProvider = (provider) => {
    ...
    setConfig((current) => ({
      ...
      evolutionEnabled: provider === "evolution" ?current.evolutionEnabled : false,
  ...
  const handleSave = async () => {
    ...
    setFeedback("");
    try {
      await salvarConfigMensageria(singleEvolutionConfig(syncSelectedEvolutionAccount(config)));
  ...
  const handleConnect = async () => {
    ...
    setQr(null);
    try {
      await salvarConfigMensageria(singleEvolutionConfig(syncSelectedEvolutionAccount({ ...config, whatsappProvider: "evolution" })));
      const result = await conectarEvolutionMensageria();
      const raw = findEvolutionQrValue(result);
      setQr({ raw, imageSrc: toQrImageSrc(raw) });
      setFeedback(raw ?"QR Code gerado. Escaneie pelo WhatsApp." : "Instância acionada, mas a Evolution não retornou QR Code.");
      await loadData();
    } catch (error) {
  ...
  const handleConfigureWebhook = async () => {
    ...
    try {
      ...
      setFeedback("Webhook de respostas configurado. As próximas respostas dos clientes serão registradas.");
      await loadData();
    } catch (error) {
      setFeedback(error?.message || "Não foi possível configurar o webhook de respostas.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <ApiHeader onRefresh={loadData} onSave={handleSave} saving={saving} canManage={canManage} />
      <FeedbackBanner feedback={feedback} />
      <StatusCards status={status} config={config} connection={connection} connectionLabel={connectionLabel} />
      <ProviderSelector provider={provider} onChange={updateProvider} canManage={canManage} />

      <OfficialWhatsappSection config={config} updateConfig={updateConfig} canManage={canManage} />
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-sm font-bold text-blue-900">Conta principal Evolution</p>
                <p className="mt-1 text-xs font-semibold text-blue-700">
                  O envio utiliza somente a instância configurada abaixo. Ao pausar e iniciar novamente, a fila continua do próximo cliente pendente.
                </p>
              </div>
              <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${
                connection.connected ?"bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"
              }`}>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-blue-200 bg-white px-3 py-2">
                <p className="text-xs font-bold uppercase text-blue-500">Instância</p>
                <p className="mt-1 text-sm font-bold text-blue-950">{config.evolutionInstance || "-"}</p>
              </div>
              <div className="rounded-lg border border-blue-200 bg-white px-3 py-2">
                <p className="text-xs font-bold uppercase text-blue-500">Número conectado</p>
                <p className="mt-1 text-sm font-bold text-blue-950">{connection.number || "-"}</p>
              </div>
              <div className="rounded-lg border border-blue-200 bg-white px-3 py-2">
                <p className="text-xs font-bold uppercase text-blue-500">Fila</p>
                <p className="mt-1 text-sm font-bold text-blue-950">{config.evolutionPaused ?"Pausada" : "Automática"}</p>
              </div>
            </div>
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <label className="block">
              <span className="text-sm font-semibold text-slate-700">URL da Evolution</span>
              <input
            ...
            <label className="block">
              <span className="text-sm font-semibold text-slate-700">Instância</span>
              <input
            ...
            <label className="block">
              <span className="text-sm font-semibold text-slate-700">Delay mínimo entre envios</span>
              <input
                type="number"
                min="5"
            ...
            <label className="block">
              <span className="text-sm font-semibold text-slate-700">Delay máximo entre envios</span>
              <input
                type="number"
                min="10"
            ...
            <label className="block md:col-span-2">
              <span className="text-sm font-semibold text-slate-700">Webhook de respostas</span>
              <input
              ...
              <p className="mt-1 text-xs font-medium text-slate-500">
                Esse endpoint recebe as mensagens dos clientes para registrar respostas e criar agendamentos.
              </p>
            </label>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-blue-200 bg-white p-5 shadow-sm">
          ...
          {config.evolutionWebhookConfiguredAt ?(
            <p className="mt-2 text-xs font-semibold text-emerald-700">
              Webhook configurado em {new Date(config.evolutionWebhookConfiguredAt).toLocaleString("pt-BR")}
            </p>
          ) : null}
          ...
          {qr ?(
            <div className="mt-4">
              ...
              <p className="mt-3 text-sm text-slate-600">
                Abra o WhatsApp, acesse aparelhos conectados e escaneie o QR Code.
              </p>
            </div>
          ) : (
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Textos enviados automaticamente quando o cliente responde pelo WhatsApp.
        </p>
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <label className="flex items-center gap-3 rounded-lg border border-blue-100 bg-blue-50 p-4 lg:col-span-2">
            <input
              ...
              onChange={(event) => updateConfig("guidedScheduleEnabled", event.target.checked)}
              disabled={!canManage}
              className="h-5 w-5 rounded border-slate-300 text-blue-600"
            />
            <span>
              <span className="block text-sm font-bold text-slate-900">Fluxo guiado de agendamento</span>
              <span className="text-xs font-semibold text-slate-600">
                Quando o cliente responder SIM, o bot envia opções de data e depois opções de horário.
              </span>
            </span>
          </label>
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Sem data ou horário</span>
            <textarea
              rows={5}
              value={config.replyNoScheduleMessage || ""}
              onChange={(event) => updateConfig("replyNoScheduleMessage", event.target.value)}
              disabled={!canManage}
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Cliente não localizado</span>
            <textarea
              rows={5}
              value={config.replyUnmatchedMessage || ""}
              onChange={(event) => updateConfig("replyUnmatchedMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Após agendamento criado</span>
            <textarea
              rows={5}
              value={config.replyScheduledConfirmationMessage || ""}
              onChange={(event) => updateConfig("replyScheduledConfirmationMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
            <p className="mt-1 text-xs font-medium text-slate-500">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Opções de data</span>
            <textarea
              rows={5}
              value={config.guidedScheduleDateMessage || ""}
              onChange={(event) => updateConfig("guidedScheduleDateMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
            <p className="mt-1 text-xs font-medium text-slate-500">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Opções de horário</span>
            <textarea
              rows={5}
              value={config.guidedScheduleTimeMessage || ""}
              onChange={(event) => updateConfig("guidedScheduleTimeMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
            <p className="mt-1 text-xs font-medium text-slate-500">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Data não entendida</span>
            <textarea
              rows={4}
              value={config.guidedScheduleInvalidDateMessage || ""}
              onChange={(event) => updateConfig("guidedScheduleInvalidDateMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Horário não entendido</span>
            <textarea
              rows={4}
              value={config.guidedScheduleInvalidTimeMessage || ""}
              onChange={(event) => updateConfig("guidedScheduleInvalidTimeMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="block">
            <span className="text-sm font-semibold text-slate-700">Resposta após já estar agendado</span>
            <textarea
              rows={5}
              value={config.replyAfterScheduledMessage || ""}
              onChange={(event) => updateConfig("replyAfterScheduledMessage", event.target.value)}
              disabled={!canManage}
              className="mt-1 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
            />
          </label>
