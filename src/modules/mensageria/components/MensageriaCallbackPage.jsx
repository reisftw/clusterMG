...
const MensageriaCallbackPage = () => {
  ...
  const canManage = hasPermission(currentUser, "mensageria.callback.manage") ||
  ...
  const stats = useMemo(
    () => ({
      ...
      semData: callbacks.filter((item) => item.status === "sem_data_horario").length,
    }),
    [callbacks],
  );
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Retornos do WhatsApp que o Retorninho interpreta para agendar automaticamente no sistema.
            </p>
          </div>
        </div>
        <button
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          ...
          { label: "Sem data/horário", value: stats.semData, icon: TriangleAlert },
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
        <form onSubmit={handleSubmit} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-bold text-slate-900">Teste de callback</h2>
          <p className="mt-1 text-sm text-slate-500">
            Simula o retorno que a API do WhatsApp enviará. Para agendar automático, informe o código do cliente.
          </p>

          <div className="mt-4 space-y-3">
            <label className="block">
              <span className="text-sm font-semibold text-slate-700">Código do cliente</span>
              <input
                value={form.codigo_cliente}
                onChange={(event) => setField("codigo_cliente", event.target.value)}
                disabled={!canManage}
            ...
            <label className="block">
              <span className="text-sm font-semibold text-slate-700">Mensagem recebida</span>
              <textarea
                value={form.mensagem}
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-bold text-slate-900">Retornos recebidos</h2>
                ...
                {loading ? (
                  ...
                  callbacks.map((item) => (
                    <tr key={item.id} className="align-top hover:bg-slate-50">
                      <td className="px-4 py-3 text-slate-600">{formatDateTime(getCallbackDate(item))}</td>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-900">{item.cliente || "Cliente não informado"}</p>
                        <p className="text-xs text-slate-500">Código {item.codigo_cliente || "-"} · {item.telefone || "-"}</p>
                      </td>
                      <td className="max-w-[320px] px-4 py-3 text-slate-600">{item.mensagem || "-"}</td>
                      <td className="px-4 py-3 text-slate-600">{formatSchedule(item.schedule)}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${STATUS_CLASSES[item.status] || STATUS_CLASSES.recebido}`}>
                          {STATUS_LABELS[item.status] || item.status || "Recebido"}
                        </span>
                        {item.motivo ? <p className="mt-1 text-xs text-slate-400">{item.motivo}</p> : null}
                      </td>
                      <td className="px-4 py-3 text-slate-600">
                        {item.agendamento_id ? (
                          <span className="font-semibold text-emerald-700">{item.agendamento_id}</span>
                        ) : (
                          "Não agendado"
                        )}
                      </td>
