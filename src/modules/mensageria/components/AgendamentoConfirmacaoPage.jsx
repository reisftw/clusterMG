...
export default function AgendamentoConfirmacaoPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const nextEvolutionStatus = await buscarStatusEvolutionConfirmacaoAgendamentos().catch(() => null);
      setEvolutionStatus(nextEvolutionStatus);
    } catch (error) {
      setFeedback(error?.message || "Não foi possível carregar a confirmação de agendamentos.");
    } finally {
      setLoading(false);
    }
  }, [page, pageSize]);
  ...
  const loadLogs = useCallback(async ({ nextPage = logsPage, nextPageSize = logsPageSize } = {}) => {
    setLogsLoading(true);
    setFeedback("");
    try {
      const offset = (nextPage - 1) * nextPageSize;
      const nextLogs = await buscarLogsConfirmacaoAgendamentos({ limit: nextPageSize, offset });
      setLogs(nextLogs);
    } catch (error) {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { label: "Escalonamentos", value: totals.escalonamentos || 0, tone: "amber" },
      { label: "Sem resposta final", value: totals.semRespostaFinal || 0, tone: "red" },
    ];
  }, [report]);
  ...
  const addAcceptedReply = () => {
    ...
    setConfig((current) => {
      ...
      if (currentReplies.some((item) => item.toUpperCase() === value)) return current;
      return { ...(current || {}), acceptedReplies: [...currentReplies, value] };
    });
  ...
  const removeAcceptedReply = (value) => {
    setConfig((current) => ({
      ...(current || {}),
      acceptedReplies: safeArray(current?.acceptedReplies).filter((item) => item !== value),
    }));
  };
  ...
  const handleSaveAcceptedReplies = async () => {
    ...
    try {
      ...
      setFeedback(error?.message || "Não foi possível salvar as respostas aceitas.");
    } finally {
      setWorking(false);
    }
  };
  ...
  const handleConnectEvolution = async () => {
    ...
    try {
      ...
      setFeedback(raw ? "QR Code gerado. Escaneie pelo WhatsApp." : "A Evolution não retornou QR Code.");
      await loadData();
    } catch (error) {
  ...
  const handleConfigureEvolutionWebhook = async () => {
    ...
    try {
      const saved = await salvarConfigConfirmacaoAgendamentos(config || {});
      setConfig(saved);
      await configurarWebhookEvolutionConfirmacaoAgendamentos(saved.evolutionWebhookUrl || "");
  ...
  const handleSendTest = async () => {
    ...
    try {
      ...
      setFeedback(`TESTE enviado para ${result?.telefone || testForm.telefone}.`);
      await loadData();
    } catch (error) {
  ...
  const handleSaveManualResponsible = async () => {
    ...
    setFeedback("");
    try {
      await preencherResponsavelConfirmacaoAgendamentos(manualResponsibleTrack.id, manualResponsibleForm);
  ...
  const handleToggleAutomation = async () => {
    ...
    try {
      ...
      const saved = await salvarConfigConfirmacaoAgendamentos({ ...(config || {}), enabled: nextEnabled });
      setConfig(saved);
      setFeedback(nextEnabled ? "Rotina automática ativada." : "Rotina automática pausada.");
      await loadData();
    } catch (error) {
      setFeedback(error?.message || "Não foi possível alterar a rotina automática.");
    } finally {
      setWorking(false);
    }
  };
  ...
  const openTomorrowPreview = async () => {
    ...
    setFeedback("");
    try {
      const result = await buscarPreviaConfirmacaoAgendamentos({ daysAhead: 1 });
      setPreview(result);
    } catch (error) {
      setFeedback(error?.message || "Não foi possível carregar a prévia de amanhã.");
    } finally {
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-950">Confirmação de Agendamentos</h2>
            <p className="text-sm text-slate-500">
              Rotina automática das 08h e monitoramento de novos agendamentos do dia.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        {stats.map((item) => (
          ...
          >
            <p className="text-xs font-black uppercase tracking-wide text-slate-600">{item.label}</p>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(380px,0.9fr)]">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-lg font-black text-slate-950">Configuração</h3>
              <p className="text-sm text-slate-500">
                As respostas aceitas param a escalação: SIM ou AGENDADO por padrão.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Horário da rotina">
              <input
                ...
                onChange={(event) => updateConfig("dailySendTime", event.target.value)}
              />
            </Field>
            <Field label="ESCALAR EM MINUTOS">
              <input
                ...
                onChange={(event) => updateConfig("escalationMinutes", event.target.value)}
              />
            </Field>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="mb-3 flex flex-wrap gap-2">
                  ...
                  {!safeArray(config?.acceptedReplies).length ? (
                    <span className="text-sm font-semibold text-slate-500">Nenhuma resposta cadastrada.</span>
                  ) : null}
                </div>
                <button
                  type="button"
            ...
            <Field label="Novos agendamentos do dia">
              <select
                ...
                onChange={(event) => updateConfig("sendSameDayNewAppointments", event.target.value === "sim")}
              >
          ...
          <div className="mt-5 space-y-4">
            <Field label="Mensagem da rotina das 08h">
              <textarea
                ...
                onChange={(event) => updateConfig("morningTemplate", event.target.value)}
              />
            </Field>
            <Field label="Mensagem de agendamento novo durante o dia">
              <textarea
                ...
                onChange={(event) => updateConfig("sameDayTemplate", event.target.value)}
              />
            </Field>
            <Field label="Mensagem de escalonamento">
              <textarea
                ...
                onChange={(event) => updateConfig("escalationTemplate", event.target.value)}
              />
            </Field>
            <p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-600">
              Variáveis: {"{responsavel_nome}"}, {"{total}"}, {"{data}"}, {"{lista_clientes}"}, {"{cliente_nome}"}, {"{codigo_cliente}"}, {"{hora}"}, {"{cidade}"}, {"{agendado_por}"}, {"{responsavel_anterior}"} e {"{minutos}"}.
            </p>
          </div>
        </section>
        ...
        <section className="space-y-5">
          <div className="rounded-2xl border border-blue-200 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="text-lg font-black text-slate-950">API Evolution</h3>
            </div>
            <div className="space-y-2 text-sm font-semibold text-slate-600">
              <p>
                WhatsApp:{" "}
                <span className="font-black text-slate-950">
                  {evolutionStatus?.connection?.connected ? "conectado" : evolutionStatus?.connection?.state || "não conectado"}
                </span>
              </p>
              <p>
                Número:{" "}
            ...
            <div className="mt-4 space-y-3">
              <Field label="URL da Evolution">
                <input
                  ...
                  onChange={(event) => updateConfig("evolutionBaseUrl", event.target.value)}
              ...
              <Field label="Instância da confirmação">
                <input
                  ...
                  onChange={(event) => updateConfig("evolutionInstance", event.target.value)}
              ...
              <Field label="Apikey da confirmação">
                <input
                  ...
                  onChange={(event) => updateConfig("evolutionApiKey", event.target.value)}
              ...
              <Field label="Webhook da confirmação">
                <input
                  ...
                  onChange={(event) => updateConfig("evolutionWebhookUrl", event.target.value)}
            ...
            <div className="space-y-3">
              <Field label="Telefone">
                <input
                  ...
                  onChange={(event) =>
                    setTestForm((current) => ({ ...current, telefone: event.target.value }))
                  }
                />
              </Field>
              <Field label="Nome do responsável">
                <input
                  ...
                  value={testForm.nome}
                  onChange={(event) =>
                    setTestForm((current) => ({ ...current, nome: event.target.value }))
                  }
                />
              </Field>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="grid gap-3 text-sm font-semibold text-slate-600">
              <p>
                Estado:{" "}
                <span className={`font-black ${worker?.enabled ? "text-emerald-700" : "text-amber-700"}`}>
                  {worker?.enabled ? (worker?.workerRunning ? "rodando agora" : "ativo") : "rotina desativada ou pausada"}
                </span>
              </p>
              <p>
                Última mensagem enviada:{" "}
                <span className="font-black text-slate-950">{formatDateTime(worker?.lastMessageSentAt)}</span>
              </p>
              <p>
                Próxima rotina:{" "}
                <span className="font-black text-slate-950">
                  {worker?.enabled ? formatDateTime(worker?.nextDailyRunAt) : "rotina desativada ou pausada"}
                </span>
              </p>
              <p>Erro: <span className="font-black text-red-700">{worker?.lastError || "-"}</span></p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="text-lg font-black text-slate-950">Responsáveis</h3>
            </div>
            <div className="space-y-2">
              {safeArray(report?.responsaveis).slice(0, 8).map((item) => (
                <div key={item.nome} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2 text-sm">
                  <span className="font-bold text-slate-800">{item.nome}</span>
                  <span className="font-black text-slate-950">{item.confirmados}/{item.enviados}</span>
                </div>
              ))}
              {!safeArray(report?.responsaveis).length ? (
                <p className="text-sm font-semibold text-slate-500">Sem dados ainda.</p>
              ) : null}
            </div>
          </div>
        </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Cada linha é uma trilha auditável de confirmação.</p>
          </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200">
          <table className="min-w-[960px] w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {tracks.map((track) => {
                const step = safeArray(track.steps)[Number(track.currentStepIndex || 0)] || {};
                return (
                  <tr key={track.id} className="align-top">
                    <td className="px-4 py-3 font-bold text-slate-900">{track.regional || "-"}</td>
                    <td className="px-4 py-3 font-bold text-slate-700">{track.dateKey ? track.dateKey.split("-").reverse().join("/") : "-"}</td>
                    <td className="px-4 py-3 text-slate-600">{track.origin === "rotina_08h" ? "Rotina 08h" : "Agendamento do dia"}</td>
                    <td className="px-4 py-3 text-slate-700">
                      <p className="font-black">{safeArray(track.appointments).length} cliente(s)</p>
                      <p className="mt-1 line-clamp-2 text-xs font-semibold text-slate-500">
                        {safeArray(track.appointments).map((item) => `${item.cliente_nome || "Cliente"} (${item.codigo_cliente || "-"})`).join(", ")}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex rounded-full border px-2 py-1 text-xs font-black ${statusClass[track.status] || statusClass.novo}`}>
                        {statusLabels[track.status] || track.status || "Novo"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-700">
                      <p className="font-bold">{step.nome || track.confirmedBy?.nome || "-"}</p>
                      <p className="text-xs text-slate-500">{step.label || track.confirmedBy?.label || ""}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-600">{formatDateTime(track.nextCheckAt)}</td>
                    <td className="px-4 py-3">
      ...
      {logsOpen ? (
        <ModalShell onClose={() => setLogsOpen(false)} showClose={false} size="4xl" bodyClassName="p-0">
          <div className="flex min-h-0 flex-col p-5">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 className="text-xl font-black text-slate-950">Logs da confirmação</h3>
                ...
                {logsLoading ? (
                  ...
                  <div key={log.id} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="font-black text-slate-900">{log.type || "evento"}</p>
                      <p className="text-xs font-bold text-slate-500">{formatDateTime(log.criadoEm)}</p>
                    </div>
                    <p className="mt-1 text-sm font-semibold text-slate-600">
                      {log.regional || "-"} {log.step?.nome ? `• ${log.step.nome}` : ""} {log.responder?.nome ? `• ${log.responder.nome}` : ""}
                    </p>
      ...
      {previewOpen ? (
        <ModalShell onClose={() => setPreviewOpen(false)} showClose={false} size="5xl" bodyClassName="p-0">
          <div className="flex min-h-0 flex-col p-5">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 className="text-xl font-black text-slate-950">Prévia de amanhã</h3>
                <p className="text-sm font-semibold text-slate-500">
                  Simulação da rotina das 08h para {formatDateKey(preview?.dateKey)}. Nenhuma mensagem é enviada aqui.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
            ...
            <div className="mb-4 grid gap-3 md:grid-cols-3">
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                <p className="text-xs font-black uppercase tracking-wide text-blue-700">Data</p>
                <p className="mt-1 text-2xl font-black text-slate-950">{formatDateKey(preview?.dateKey)}</p>
              </div>
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                <p className="text-xs font-black uppercase tracking-wide text-emerald-700">Clientes</p>
                <p className="mt-1 text-2xl font-black text-slate-950">{preview?.total || 0}</p>
              </div>
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                <p className="text-xs font-black uppercase tracking-wide text-amber-700">Regionais</p>
                <p className="mt-1 text-2xl font-black text-slate-950">{safeArray(preview?.groups).length}</p>
              </div>
            </div>
              ...
              {previewLoading ? (
                ...
                <div className="space-y-3">
                  {safeArray(preview?.groups).map((group) => (
                    <div key={group.regional} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <h4 className="text-base font-black text-slate-950">{group.regional || "Sem regional"}</h4>
                          <p className="text-sm font-semibold text-slate-500">
                            {group.total} cliente(s) • Responsável inicial:{" "}
                            <strong>{group.responsavelInicial?.nome || "não configurado"}</strong>
                            {group.responsavelInicial?.label ? ` (${group.responsavelInicial.label})` : ""}
                          </p>
                        </div>
                      ...
                      <div className="mt-3 grid gap-2 md:grid-cols-2">
                        {safeArray(group.appointments).map((item) => (
                          <div key={item.id || `${item.codigo_cliente}-${item.hora}`} className="rounded-xl border border-white bg-white p-3">
                            <p className="font-black text-slate-900">{item.cliente_nome || "Cliente sem nome"}</p>
                            <p className="text-sm font-semibold text-slate-600">
                              Código {item.codigo_cliente || "-"} • {item.hora || "-"} • {item.cidade || "-"}
                            </p>
                            <p className="text-xs font-bold text-slate-400">Agendado por: {item.agendado_por || "-"}</p>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {acceptedRepliesOpen ? (
        <ModalShell onClose={() => setAcceptedRepliesOpen(false)} showClose={false} size="lg" bodyClassName="p-0">
          <div className="p-5">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h3 className="text-xl font-black text-slate-950">Respostas aceitas</h3>
                <p className="text-sm font-semibold text-slate-500">
                  Cadastre as palavras que confirmam ciência e param o escalonamento.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {!safeArray(config?.acceptedReplies).length ? (
                <p className="text-sm font-semibold text-slate-500">Nenhuma resposta cadastrada.</p>
              ) : null}
            </div>
      ...
      {manualResponsibleTrack ? (
        <ModalShell onClose={() => setManualResponsibleTrack(null)} showClose={false} size="lg" bodyClassName="p-0">
          <div className="p-5">
            <div className="mb-4">
              <h3 className="text-xl font-black text-slate-950">Preencher responsável</h3>
              <p className="mt-1 text-sm font-semibold text-slate-500">
                Este envio não encontrou BackOffice. Ao salvar, o contato será gravado na regional{" "}
                <strong>{manualResponsibleTrack.regional || "-"}</strong> e a mensagem será reenviada.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <Field label="Nome do responsável">
                <input
                  ...
                  onChange={(event) =>
                    setManualResponsibleForm((current) => ({ ...current, nome: event.target.value }))
                  }
              ...
              <Field label="Telefone">
                <input
                  ...
                  onChange={(event) =>
                    setManualResponsibleForm((current) => ({ ...current, telefone: event.target.value }))
                  }
                  placeholder="31999999999"
                />
              </Field>
              <Field label="E-mail">
                <input
                  ...
                  onChange={(event) =>
                    setManualResponsibleForm((current) => ({ ...current, email: event.target.value }))
                  }
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button
                type="button"
                disabled={working || !manualResponsibleForm.nome.trim() || !manualResponsibleForm.telefone.trim()}
