...
export function normalizeMensageriaConfig(config = {}) {
	return {
		...
		evolutionSelectedAccountId: "default",
		whatsappProvider:
			config.whatsappProvider || DEFAULT_MENSAGERIA_CONFIG.whatsappProvider,
...
export async function buscarFilaMensageria() {
	return (
		...
		const dateA =
			new Date(
				a.prioridadeEm || a.proximaTentativaEm || a.criadoEm || 0,
			).getTime() || 0;
		const dateB =
			new Date(
				b.prioridadeEm || b.proximaTentativaEm || b.criadoEm || 0,
			).getTime() || 0;
		return dateA - dateB;
	});
}
...
export async function buscarHistoricoMensageriaPaginado({
	limit = 20,
	offset = 0,
} = {}) {
	return (
...
export async function buscarLogsDisconnectEvolutionMensageria(limit = 50) {
	return requestVpsApi(
		`/mensageria/evolution/disconnect-logs?limit=${encodeURIComponent(limit)}`,
	);
}
