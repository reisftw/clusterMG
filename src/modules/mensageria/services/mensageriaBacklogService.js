...
function normalizeBacklogOrder(ordem) {
	...
	const codigoCliente =
		String(readField(ordem, ["codigo_cliente", "codigo"])) ||
		extractCustomerCode(clienteRaw);
	const telefone = String(
		readField(ordem, ["telefone", "celular", "fone", "whatsapp", "contato"]),
	);
	...
	const endereco = String(
		readField(ordem, ["endereco_resumo", "endereco", "logradouro"]),
	);
	const numero = String(readField(ordem, ["numero"]));
	const bairro = String(readField(ordem, ["bairro"]));
	const enderecoCompleto =
		endereco ||
		[readField(ordem, ["rua"]), numero, bairro].filter(Boolean).join(", ");
	...
	return {
		...
		status_mensageria: String(
			readField(ordem, ["status_mensageria", "status_contato"]),
		),
