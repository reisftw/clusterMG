...
function IntegrationForm({ initialValue, onCancel, onSave, saving }) {
  ...
  const [modulesText, setModulesText] = useState(() => (form.modules || []).join(", "));
  const [headersText, setHeadersText] = useState(() =>
    listToText(form.headers, (item) => `${item.key}: ${item.value}`),
  );
  const [mappingsText, setMappingsText] = useState(() =>
    listToText(form.fieldMappings, (item) => `${item.localField} = ${item.remotePath}`),
  );
  ...
  return (
    <section className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Cadastre URLs, ambiente, referência de credencial e campos usados pelas sincronizações.
          </p>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid gap-3 md:grid-cols-6">
        ...
        <label className="block md:col-span-2">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Fornecedor</span>
          <input
            value={form.provider}
        ...
        <label className="block md:col-span-2">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Ambiente</span>
          <select
        ...
        <label className="block md:col-span-4">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">URL base</span>
          <input
            type="url"
            value={form.baseUrl}
        ...
        <label className="block md:col-span-2">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Healthcheck</span>
          <input
            value={form.healthcheckPath}
            onChange={(event) => handleChange("healthcheckPath", event.target.value)}
            className="input-field"
        ...
        <label className="block md:col-span-2">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Autenticação</span>
          <select
            value={form.authType}
        ...
        <label className="block md:col-span-2">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Local da credencial</span>
          <select
            value={form.authLocation}
            onChange={(event) => handleChange("authLocation", event.target.value)}
            className="input-field"
          >
        ...
        <label className="block md:col-span-2">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Frequência</span>
          <select
            value={form.syncFrequency}
            onChange={(event) => handleChange("syncFrequency", event.target.value)}
            className="input-field"
          >
        ...
        <label className="block md:col-span-3">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Referencia do segredo</span>
          <input
            value={form.credentialRef}
            onChange={(event) => handleChange("credentialRef", event.target.value)}
            className="input-field"
        ...
        <label className="block md:col-span-3">
          ...
          <input
            ...
            onChange={(event) => handleChange("secretValue", event.target.value)}
            className="input-field"
            placeholder={
          ...
          <span className="mt-1 block text-[11px] font-semibold text-gray-400">
            O valor não aparece na listagem. Ao editar, deixe em branco para manter o token atual.
          </span>
        </label>
        ...
        <label className="flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 md:col-span-6">
          ...
          />
          <span className="text-sm font-semibold text-gray-700">Integração ativa</span>
        </label>

        <label className="block md:col-span-3">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Módulos que usam</span>
          <input
            value={modulesText}
        ...
        <label className="block md:col-span-3">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Headers não sensíveis</span>
          <textarea
            value={headersText}
        ...
        <label className="block md:col-span-3">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Mapeamento de campos</span>
          <textarea
            value={mappingsText}
            onChange={(event) => setMappingsText(event.target.value)}
            className="input-field min-h-[88px]"
            placeholder={"tecnicoNome = data.tecnico.nome\nnumeroOs = data.ordem.id"}
          />
        </label>

        <label className="block md:col-span-6">
          <span className="mb-1.5 block text-xs font-semibold text-gray-600">Observações</span>
          <textarea
            value={form.notes}
        ...
        >
          <CheckCircle2 size={16} /> {saving ? "Salvando..." : "Salvar integração"}
        </button>
      </form>
    </section>
  );
...
function OAuthSettingsPanel({
  ...
  const googleNeedsDomain = googleOAuth.autoProvision && !String(googleOAuth.allowedDomains || "").trim();
  const oktaNeedsDomain = oktaOAuth.autoProvision && !String(oktaOAuth.allowedDomains || "").trim();
      ...
      <section className="rounded-xl border border-emerald-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-base font-black text-gray-900">Login com Google</h2>
              <p className="mt-1 max-w-2xl text-sm font-semibold leading-relaxed text-gray-500">
                Ative o botão de login Google. O sistema só libera acesso para e-mails já cadastrados e ativos.
              </p>
            </div>
          </div>
          <label className="inline-flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm font-black text-gray-700">
            <input
              ...
              onChange={(event) => setGoogleOAuth((current) => ({ ...current, enabled: event.target.checked }))}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <label className="block">
            <span className="text-xs font-black uppercase tracking-wide text-gray-500">Google Client ID</span>
            <input
              value={googleOAuth.clientId || ""}
              onChange={(event) => setGoogleOAuth((current) => ({ ...current, clientId: event.target.value }))}
              placeholder="Cole o Client ID do OAuth Web"
              className="input-field mt-2"
            />
          </label>
          <label className="block">
            <span className="text-xs font-black uppercase tracking-wide text-gray-500">Domínios permitidos</span>
            <input
              value={googleOAuth.allowedDomains || ""}
              onChange={(event) => setGoogleOAuth((current) => ({ ...current, allowedDomains: event.target.value }))}
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
            <label className="flex items-start gap-3 text-sm font-black text-blue-950">
              <input
                ...
                onChange={(event) => setGoogleOAuth((current) => ({ ...current, autoProvision: event.target.checked }))}
              ...
              <span>
                ...
                <span className="mt-1 block text-xs font-semibold leading-relaxed text-blue-700">
                  Se o e-mail Google for de um domínio permitido e ainda não existir no sistema, ele será criado como Visitante.
                </span>
              </span>
            </label>
          </div>
          <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
            <p className="text-xs font-black uppercase tracking-wide text-gray-500">Cargo padrão automático</p>
            ...
            <p className="mt-1 text-xs font-semibold leading-relaxed text-gray-500">
              Depois do primeiro login, o admin pode alterar o cargo em Configurações &gt; Usuários.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold text-gray-500">
              No Google Cloud, use JavaScript origin <strong>https://retiradas.tech</strong>.
            </p>
            {googleNeedsDomain ? (
              <p className="mt-1 text-xs font-bold text-orange-700">
                Para criação automática, informe pelo menos um domínio permitido.
              </p>
            ) : null}
            {googleMessage ? <p className="mt-1 text-xs font-bold text-blue-700">{googleMessage}</p> : null}
          </div>
          <button
            type="button"
          ...
          >
            {googleSaving ? <Loader2 className="animate-spin" size={17} /> : <KeyRound size={17} />}
            Salvar Google OAuth
          </button>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-base font-black text-gray-900">Login com Okta Verify</h2>
              <p className="mt-1 max-w-2xl text-sm font-semibold leading-relaxed text-gray-500">
                Ative o login corporativo com MFA pela Okta. O Okta Verify fica responsável pela segunda etapa.
              </p>
            </div>
          </div>
          <label className="inline-flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm font-black text-gray-700">
            <input
              ...
              onChange={(event) => setOktaOAuth((current) => ({ ...current, enabled: event.target.checked }))}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <label className="block">
            <span className="text-xs font-black uppercase tracking-wide text-gray-500">Issuer Okta</span>
            <input
              value={oktaOAuth.issuer || ""}
              onChange={(event) => setOktaOAuth((current) => ({ ...current, issuer: event.target.value }))}
              placeholder="Ex: https://suaempresa.okta.com/oauth2/default"
              className="input-field mt-2"
            />
          </label>
          <label className="block">
            <span className="text-xs font-black uppercase tracking-wide text-gray-500">Okta Client ID</span>
            <input
              value={oktaOAuth.clientId || ""}
              onChange={(event) => setOktaOAuth((current) => ({ ...current, clientId: event.target.value }))}
              placeholder="Client ID da aplicação OIDC SPA"
              className="input-field mt-2"
            />
          </label>
          <label className="block">
            <span className="text-xs font-black uppercase tracking-wide text-gray-500">Redirect URI</span>
            <input
              value={oktaOAuth.redirectUri || ""}
              onChange={(event) => setOktaOAuth((current) => ({ ...current, redirectUri: event.target.value }))}
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => setOktaOAuth((current) => ({ ...current, allowedDomains: event.target.value }))}
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
            <label className="flex items-start gap-3 text-sm font-black text-blue-950">
              <input
                ...
                onChange={(event) => setOktaOAuth((current) => ({ ...current, autoProvision: event.target.checked }))}
                className="mt-0.5 h-5 w-5 accent-blue-600"
              />
              <span>
                Criar conta visitante automaticamente
                <span className="mt-1 block text-xs font-semibold leading-relaxed text-blue-700">
                  Se o e-mail Okta for de um domínio permitido e ainda não existir no sistema, ele será criado como Visitante.
                </span>
              </span>
            </label>
          </div>
          <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
            <p className="text-xs font-black uppercase tracking-wide text-gray-500">Configuração na Okta</p>
            <p className="mt-1 text-sm font-black text-gray-900">OIDC SPA + ID Token</p>
            <p className="mt-1 text-xs font-semibold leading-relaxed text-gray-500">
              Cadastre o Redirect URI acima em Login redirect URIs e exija Okta Verify na política do app.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold text-gray-500">
              Login Redirect URI sugerido: <strong>{oktaOAuth.redirectUri || "https://retiradas.tech/login"}</strong>.
            </p>
            {oktaNeedsDomain ? (
              <p className="mt-1 text-xs font-bold text-orange-700">
                Para criação automática, informe pelo menos um domínio permitido.
            ...
            {oktaMessage ? <p className="mt-1 text-xs font-bold text-blue-700">{oktaMessage}</p> : null}
          </div>
          <button
            type="button"
            disabled={oktaSaving}
          ...
          >
            {oktaSaving ? <Loader2 className="animate-spin" size={17} /> : <ShieldCheck size={17} />}
            Salvar Okta
          </button>
        </div>
      </section>
...
export default function IntegracoesPage() {
  ...
  useEffect(() => {
    if (!isAdmin) return;
    let active = true;
    Promise.allSettled([obterConfigGoogleOAuthAdmin(), obterConfigOktaOAuthAdmin()])
      ...
      .catch(() => {
        if (active) setGoogleMessage("Não foi possível carregar as configurações OAuth.");
      });
    return () => {
      active = false;
    };
  ...
  const saveGoogleOAuth = async () => {
    ...
    try {
      ...
      setGoogleMessage(saveError?.message || "Não foi possível salvar o login com Google.");
    } finally {
  ...
  const saveOktaOAuth = async () => {
    ...
    try {
      ...
      setOktaMessage(saveError?.message || "Não foi possível salvar o login com Okta.");
    } finally {
      setOktaSaving(false);
    }
  };
    ...
    <div className="space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Configure APIs externas para os módulos usarem sem URL fixa no código.
              </p>
            </div>
          </div>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-lg border border-gray-100 bg-white px-4 py-3">
          ...
          <p className="mt-1 text-2xl font-black text-gray-900">{integracoes.length}</p>
        </div>
        ...
        {integracoes.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Comece por Hubsoft e Playground para preparar o match de O.S. com equipamentos.
            </p>
          </div>
        ) : (
                    ...
                    <td className="px-5 py-4">
                      ...
                      {item.loginConfigured || item.tokenExpiresAt ? (
                        <p className="mt-1 text-xs text-gray-400">
                          ...
                          {item.loginConfigured && item.tokenExpiresAt ? " · " : ""}
                          {item.tokenExpiresAt ? `Token expira em ${new Date(item.tokenExpiresAt).toLocaleString("pt-BR")}` : ""}
                        </p>
                      ) : null}
                    </td>
                    <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-1">
                          ...
                          <button
                            ...
                            title={item.systemManaged ? "Integração gerenciada pelo sistema" : "Excluir integração"}
                          >
                            <Trash2 size={15} />
                          </button>
                        </div>
