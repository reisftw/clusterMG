...
export const SYSTEM_INTEGRATIONS = Object.freeze([
  {
    ...
    notes: "Usada para consultar equipamento por MAC, estoque local e historico de movimentacoes.",
    ...
    fieldMappings: [
      ...
      { localField: "historico", remotePath: "nota?filter.itens.serie=$eq:MAC" },
    ],
  ...
  },
  {
    ...
    notes: "Usada para consultar clientes, atendimentos e ordens de serviço diretamente no Hubsoft.",
    statusServiceId: "hubsoft",
    headers: [],
    fieldMappings: [
      { localField: "codigo_cliente", remotePath: "cliente/ordem_servico?busca=codigo_cliente" },
      { localField: "num_os", remotePath: "cliente/ordem_servico?busca=numero_ordem_servico" },
  ...
  },
  {
    ...
    notes: "Usada para envio e recebimento das mensagens da mensageria quando associada como provedora.",
  ...
  },
  {
    ...
    syncFrequency: "manual",
    healthcheckPath: "/status",
    notes: "Pré-integração preparada para conectar o Sapiens quando a Brasil TecPar liberar os endpoints oficiais.",
  ...
  },
  {
    ...
    notes: "Healthcheck principal da API, validando Nginx/SSL, Node e conexao com PostgreSQL.",
  ...
  },
  {
    ...
    notes: "Valida usuarios locais, segredo de autenticacao e geracao de sessao.",
  ...
  },
  {
    ...
    syncFrequency: "manual",
    healthcheckPath: "/admin/api-status",
    notes: "Valida leitura e contagem dos documentos persistidos no PostgreSQL.",
  ...
  },
  {
    ...
    notes: "Valida snapshots usados pelas telas publicas e pelo acompanhamento.",
  ...
  },
  {
    ...
    syncFrequency: "manual",
    healthcheckPath: "/admin/api-status",
    notes: "Valida colecoes alimentadas por uploads e importacoes operacionais.",
  ...
  },
  {
    ...
    notes: "Valida pasta de backups, retencao, quota e ultimo backup disponivel.",
  ...
  },
  {
    ...
    notes: "Valida canal Server-Sent Events usado para atualizacao em tempo real.",
...
function mergeSystemIntegrations(integrations = []) {
  ...
  const missingSystemItems = SYSTEM_INTEGRATIONS
    .filter((item) =>
      !registeredKeys.has(item.id) &&
      !registeredKeys.has(`${item.name}|${item.provider}|${item.baseUrl}`.toLowerCase()),
    )
...
export async function buscarIntegracoes() {
  return mergeSystemIntegrations(
    (await listVpsDocuments(COLLECTIONS.INTEGRACOES_API, { limit: 300 }))
      .map((item) => normalizeIntegration(item.id, item)),
  );
}
