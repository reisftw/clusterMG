...
import { buscarEmpresasTecnicos, normalizeText } from "../../empresasTecnicos/services/empresasTecnicosService";
...
export function normalizeProvider(provider = {}) {
  const id = text(provider.id || provider.nome).toLowerCase().replace(/[^a-z0-9_]+/g, "_");
  return {
    id,
...
export function normalizeCotacao(id, data = {}) {
  return {
    ...
    lalamoveQuotationId: text(data.lalamoveQuotationId || data.lalamove_quotation_id),
...
export function buildCotacaoPayload(form = {}, ponto = null, config = DEFAULT_LOGISTICA_CONFIG, user = null) {
  const provider = (config.providers || []).find((item) => item.id === form.fornecedor);
  return {
    ...
    pontoEndereco: [ponto?.endereco, ponto?.numero, ponto?.bairro, ponto?.cidade].filter(Boolean).join(", "),
...
export async function buscarLogisticaConfig() {
  ...
  return {
    ...
    ...doc,
    providers: (doc.providers?.length ? doc.providers : DEFAULT_LOGISTICA_CONFIG.providers).map(normalizeProvider),
  };
}
...
export async function salvarLogisticaConfig(config) {
  return requestVpsApi("/logistica/config", {
    ...
    body: JSON.stringify({
      ...DEFAULT_LOGISTICA_CONFIG,
      ...config,
      providers: (config.providers || DEFAULT_LOGISTICA_CONFIG.providers).map(normalizeProvider),
...
export async function buscarPontosLogistica() {
  return (await listAllVpsDocuments(COLLECTIONS.LOGISTICA_PONTOS, { pageSize: 500 }))
...
export async function buscarCotacoesLogistica() {
  return (await listAllVpsDocuments(COLLECTIONS.LOGISTICA_COTACOES, { pageSize: 500 }))
    .map((item) => normalizeCotacao(item.id, item))
    .sort((a, b) => String(b.criadoEm || "").localeCompare(String(a.criadoEm || "")));
}

export async function salvarCotacaoManual(form, pontos = [], config = DEFAULT_LOGISTICA_CONFIG, user = null) {
  ...
  if (form.id) {
    await updateVpsDocument(`${COLLECTIONS.LOGISTICA_COTACOES}/${form.id}`, data);
    return form.id;
  }
...
export async function obterCidadesEmpresas() {
  ...
  regionais.forEach((regional) => {
    (regional.cidades || []).forEach((cidade) => addCidade(cidade, regional.nome));
  });
...
export function normalizeClienteLogistica(id, data = {}) {
  ...
  return {
    id,
    cliente: pickFirst(data.cliente, data.nome_cliente, data.nomeCliente, data.assinante, data.nome),
    codigoCliente: pickFirst(data.codigo_cliente, data.codigoCliente, data.cod_cliente, data.contrato, data.id_cliente),
...
export async function buscarClientesLogistica() {
  const docs = await listAllVpsDocuments("ordens_abertas", { pageSize: 1000, max: 20000 }).catch(() => []);
  const seen = new Set();
  return docs
    .map((item) => normalizeClienteLogistica(item.id, item))
    .filter((item) => {
      const key = normalizeText(`${item.codigoCliente}-${item.cliente}-${item.os}`);
...
export async function solicitarCotacaoLalamove(form = {}, pontos = []) {
  ...
  const pickupAddress = [form.enderecoColeta, form.numeroColeta, form.bairroColeta, form.cidade]
    .filter(Boolean)
    .join(", ");
  const dropoffAddress = [ponto.endereco, ponto.numero, ponto.bairro, ponto.cidade]
    .filter(Boolean)
    .join(", ");
...
export async function criarPontosBasePorCidades(cidades = [], pontosExistentes = []) {
  const existing = new Set(pontosExistentes.map((item) => normalizeText(item.cidade)));
  let created = 0;
  for (const item of cidades) {
    ...
    await salvarPontoLogistica({
      ...
      ativo: true,
      referencia: "Cadastro inicial criado pela Logística. Complete o endereço antes de usar.",
    });
    created += 1;
  }
  return created;
