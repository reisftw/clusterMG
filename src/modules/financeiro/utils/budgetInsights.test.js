...
describe("budgetInsights", () => {
	...
	it("considera todos os grupos do realizado e remove apenas desconsiderados", () => {
		const insights = getBudgetInsights(
			{
				...config,
				accounts: [
				...
				centers: [
					...
					},
					{
						...
						realizedByCompanyBranch: [
							...
							{ year: 2026, month: 8, accountId: "nao-basal", realized: 900, grupo: "Onnet", quebra2: "DESCONSIDERAR" },
						],
					},
					{
						id: "project-center",
		...
		expect(insights.budgetCategoryGroups.find((item) => item.id === "basal").realized).toBe(300);
		expect(insights.budgetCategoryGroups.find((item) => item.id === "nao_basal").realized).toBe(700);
		expect(insights.budgetCategoryGroups.find((item) => item.id === "projetos").realized).toBe(500);
