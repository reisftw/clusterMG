    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className={`mt-3 text-3xl font-bold ${offlineServices.length ? "text-red-600" : "text-slate-900"}`}>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-3 text-lg font-bold text-emerald-600">
            {status?.runtime?.uptimeLabel || formatDuration(status?.runtime?.uptimeSeconds)}
          </p>
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-4">
          <h2 className="text-base font-bold text-slate-900">Uptime e Reinícios da API</h2>
        ...
        <div className="grid gap-3 p-4 md:grid-cols-3">
          <div className="rounded-lg bg-slate-50 p-3">
            <p className="text-xs font-bold uppercase text-slate-400">PID atual</p>
            <p className="mt-1 font-bold text-slate-900">{status?.runtime?.pid || "-"}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            <p className="text-xs font-bold uppercase text-slate-400">Último restart</p>
            <p className="mt-1 font-bold text-slate-900">{formatDateTime(status?.runtime?.lastRestartAt)}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3">
            <p className="text-xs font-bold uppercase text-slate-400">Memória RSS</p>
            <p className="mt-1 font-bold text-slate-900">{formatBytes(status?.runtime?.memory?.rss)}</p>
          </div>
        </div>
        <div className="border-t border-slate-100 p-4">
          ...
          <div className="mt-3 divide-y divide-slate-100 rounded-lg border border-slate-100">
            {(status?.runtime?.events || []).slice(0, 10).map((event) => (
              <div key={event.id || `${event.type}-${event.createdAt}`} className="flex flex-col gap-1 p-3 text-sm md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="font-bold text-slate-900">{runtimeEventLabel(event.type)}</p>
                  <p className="text-xs font-semibold text-slate-500">
                    {event.details?.reason || event.details?.signal || "Sem motivo informado"}
                  </p>
                </div>
            ...
            {!(status?.runtime?.events || []).length ? (
              <div className="p-4 text-center text-sm text-slate-500">
                Nenhum evento de reinício registrado ainda. O primeiro aparecerá após o próximo restart da API.
              </div>
            ) : null}
          </div>
        </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-4">
          <h2 className="text-base font-bold text-slate-900">Histórico de Status dos Serviços</h2>
          <p className="text-xs text-slate-500">
            Registra quando cada API entra ou sai do ar, com motivo e tempo de resposta.
          </p>
        </div>
        <div className="divide-y divide-slate-100">
          {serviceEvents.slice(0, 12).map((event) => (
            <div key={event.id || `${event.serviceId}-${event.createdAt}`} className="flex flex-col gap-2 p-4 text-sm md:flex-row md:items-center md:justify-between">
              <div>
                <p className="font-bold text-slate-900">{event.serviceName || event.serviceId}</p>
                <p className="text-xs font-semibold text-slate-500">
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-4">
          <h2 className="text-base font-bold text-slate-900">APIs Monitoradas</h2>
          <p className="text-xs text-slate-500">
                ...
                >
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={`mt-1 text-sm ${online ? "text-emerald-600" : "text-red-600"}`}>
