...
function latestFilesByField(files = []) {
	...
	files.forEach((file) => {
		...
		const currentDate = current
			? new Date(current.createdAt || current.updatedAt || 0).getTime()
			: 0;
...
export function buildDocumentosFinanceiroEmail({
	empresa = {},
	submission = {},
	template,
} = {}) {
	const files = latestFilesByField(submission.files || []).filter(
		(file) => String(file.status || "").toLowerCase() === "aprovado",
	);
	const documentosLista = files.length
		? files
				.map(
					(file, index) =>
						`${index + 1}. ${text(file.fieldNome || file.tipo || file.nome || "Documento")}`,
				)
				.join("\n")
	...
	const values = {
		...
		empresa_cnpj:
			text(empresa.cnpj || empresa.documento || submission.empresaCnpj) ||
			"Não informado",
		regional: text(empresa.regional || submission.regional) || "-",
		supervisor_nome:
			text(empresa.supervisor?.nome || submission.supervisorNome) || "-",
