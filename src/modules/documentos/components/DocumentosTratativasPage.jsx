			...
			<div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
				<div className="min-w-0">
					...
					<h2 className="mt-4 truncate text-xl font-black text-slate-950">
						{item.empresaNome || "Empresa sem nome"}
					</h2>
					<p className="mt-1 text-sm font-semibold text-slate-500">
						Enviado por {item.submittedByName || item.submittedByEmail || "-"}{" "}
						em {formatDateTime(item.submittedAt)}
					</p>
				</div>
...
export default function DocumentosTratativasPage() {
	...
	const filtered = useMemo(() => {
		...
		return items.filter((item) =>
			normalize(
				`${item.empresaNome} ${item.regional} ${item.supervisorNome} ${item.submittedByName}`,
			).includes(term),
		);
	}, [items, search]);
	...
	const oldest = filtered.reduce(
		(max, item) => Math.max(max, Number(item.tempoSupervisorMs || 0)),
		0,
	);
				...
				<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="mt-1 text-sm font-semibold text-slate-500">
								Empresas que enviaram documentos e aguardam avaliação do
			...
			<section className="grid gap-4 md:grid-cols-3">
				...
				<div className="rounded-2xl border border-blue-100 bg-blue-50 p-5">
					...
					<p className="mt-2 text-3xl font-black text-blue-950">
						{oldest ? elapsedLabel(Date.now() - oldest, nowTick) : "-"}
					</p>
				</div>
				...
				{!pageItems.length ? (
					<div className="rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
						...
						<p className="mt-3 text-sm font-bold text-slate-500">
							Nenhuma tratativa aguardando supervisor.
						</p>
					</div>
				) : null}
			</section>
