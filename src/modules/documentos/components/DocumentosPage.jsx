...
const STATUS_CONFIG = {
  pendente: {
    ...
    description: "Envios mensais aguardando validação do supervisor ou do administrativo.",
    icon: FileText,
  },
  aprovado: {
...
function isAdministrativeRole(role) {
  return [ROLES.ADMIN, ROLES.SUPERVISOR_ADMINISTRATIVO, ROLES.ANALISTA_ADMINISTRATIVO].includes(String(role || "").toLowerCase());
}

function isOperationalReviewer(role) {
  return [ROLES.ADMIN, ROLES.SUPERVISOR, ROLES.BACKOFFICE_RETIRADA].includes(String(role || "").toLowerCase());
}
...
function PaginationControls({ page, totalPages, totalItems, pageSize, onPageChange, onPageSizeChange }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-slate-100 px-4 py-3 text-sm font-bold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
      <p>Mostrando {start} a {end} de {totalItems} registro(s)</p>
...
function SubmissionModal({ submission, currentUser, onClose, onChanged }) {
  const [selectedFileId, setSelectedFileId] = useState(submission?.files?.[0]?.id || "");
  ...
  const selectedFile = (submission?.files || []).find((file) => file.id === selectedFileId) || submission?.files?.[0] || null;
  ...
  const selectedStatus = String(selectedFile?.status || "pendente").toLowerCase();
  const selectedAdminStatus = String(selectedFile?.adminStatus || "pendente").toLowerCase();
  const canReviewAdministrative = selectedStatus === "aprovado" && selectedAdminStatus !== "aprovado" && isAdministrativeRole(role);
  const canReviewOperational = selectedStatus === "pendente" && isOperationalReviewer(role);
  ...
  const approveLabel = canReviewAdministrative ? "Aprovar administrativo" : "Aprovar documento";
  const rejectLabel = canReviewAdministrative ? "Reprovar administrativo" : "Reprovar documento";
  ...
  const reviewFile = async (status) => {
    ...
    try {
      ...
      setModalMessage(error?.message || "Não foi possível atualizar o documento.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex min-h-0 flex-col overflow-hidden">
        <header className="flex flex-col gap-3 border-b border-slate-100 p-5 md:flex-row md:items-center md:justify-between">
          <div className="min-w-0">
            <h2 className="break-words text-xl font-black text-slate-950">{submission.empresaNome}</h2>
            <p className="break-words text-sm font-semibold text-slate-500">
              Mês {formatMonthLabel(submission.mesReferencia)} - enviado em {formatDate(submission.submittedAt)}
            </p>
          </div>
          <button type="button" onClick={onClose} className="shrink-0 rounded-xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-700 hover:bg-slate-50">
            Fechar
          </button>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="mb-4 grid gap-3 md:grid-cols-3">
            <div className="min-w-0 rounded-2xl bg-slate-50 p-4">
              ...
              <span className={statusBadge(submission.status)}>{statusLabel(submission.status)}</span>
            </div>
            <div className="min-w-0 rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-black uppercase text-slate-500">Supervisor</p>
              <p className="break-words font-black text-slate-900">{submission.supervisorNome || "-"}</p>
            </div>
            <div className="min-w-0 rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-black uppercase text-slate-500">Enviado por</p>
              <p className="break-words font-black text-slate-900">{submission.submittedByName || "-"}</p>
            </div>
          </div>
          ...
          <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(260px,310px)_minmax(0,1fr)]">
              ...
              {(submission.files || []).map((file) => (
                ...
                >
                  <div className="flex min-w-0 items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="break-words text-sm font-black text-slate-950">{file.fieldNome || file.tipo || "Documento"}</p>
                      <p className="mt-1 line-clamp-2 break-all text-xs font-semibold text-slate-500">{file.nome}</p>
                    </div>
                    <span className={`${statusBadge(file.status)} shrink-0`}>{statusLabel(file.status)}</span>
                  </div>
                  ...
                  {file.motivoReprovacao ? (
                    <p className="mt-2 break-words rounded-xl bg-red-50 p-2 text-xs font-bold text-red-700">{file.motivoReprovacao}</p>
                  ) : null}
                  {file.adminMotivoReprovacao ? (
                    <p className="mt-2 break-words rounded-xl bg-red-50 p-2 text-xs font-bold text-red-700">Administrativo: {file.adminMotivoReprovacao}</p>
                  ) : null}
                </button>
              ))}
            </div>
            ...
            <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
              <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-white p-3">
                <div className="min-w-0">
                  <p className="break-words text-sm font-black text-slate-950">{selectedFile?.fieldNome || selectedFile?.tipo || "Documento"}</p>
                  <p className="break-all text-xs font-semibold text-slate-500">{selectedFile?.nome || "-"}</p>
                </div>
                <div className="flex shrink-0 flex-wrap justify-end gap-2">
                  ...
                  {selectedFile ? (
                    <button type="button" onClick={() => baixarDocumento(selectedFile)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50">
              ...
              {renameOpen ? (
                <div className="border-b border-slate-200 bg-blue-50 p-3">
                  <label className="block">
                    <span className="mb-1 block text-xs font-black uppercase tracking-wide text-blue-700">Nome do arquivo</span>
                    <input
                      value={renameName}
                  ...
                  <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:justify-end">
                    ...
                    >
                      {renaming ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Salvar nome
                    </button>
                  </div>
                </div>
              ) : null}
              <div className="h-[48vh] min-h-[360px]">
                {loadingPdf ? (
                  <div className="flex h-full items-center justify-center text-sm font-black text-slate-500">
                    <Loader2 className="mr-2 animate-spin" size={18} /> Carregando PDF...
                  </div>
                ) : pdfUrl && String(selectedFile?.mimeType || "").startsWith("image/") ? (
                  <div className="flex h-full items-center justify-center p-3">
                    <img src={pdfUrl} alt={selectedFile?.nome || "Documento"} className="max-h-full max-w-full rounded-xl object-contain shadow-sm" />
                  </div>
                ) : pdfUrl ? (
                  <iframe title={selectedFile?.nome || "Documento"} src={pdfUrl} className="h-full w-full" />
                ) : (
          ...
          {showRejectReason ? (
            <div className="mt-4 rounded-2xl border border-red-100 bg-red-50 p-4">
              <label className="block">
                <span className="mb-1 block text-xs font-black uppercase tracking-wide text-red-700">Motivo da reprovação deste documento</span>
                <textarea
                  ref={motivoRef}
                  value={motivo}
              ...
              <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:justify-end">
                ...
                <button type="button" disabled={saving || !motivo.trim()} onClick={() => reviewFile("reprovado")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-red-600 px-5 py-2 text-sm font-black text-white hover:bg-red-700 disabled:opacity-60">
                  {saving ? <Loader2 className="animate-spin" size={16} /> : <XCircle size={16} />} Confirmar reprovação
                </button>
              </div>
            </div>
          ) : null}
        ...
        <footer className="flex flex-col gap-3 border-t border-slate-100 p-5 sm:flex-row sm:justify-end">
          {submission.status === "aprovado" ? (
            <>
              <button
                type="button"
                disabled={saving}
                onClick={() => baixarEnvioDocumentosZip(submission).catch((error) => setModalMessage(error?.message || "Não foi possível baixar o ZIP."))}
          ...
          {canReviewSelected ? (
            <>
              ...
              <button type="button" disabled={saving || !selectedFile} onClick={() => reviewFile("aprovado")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-black text-white hover:bg-emerald-700 disabled:opacity-60">
        ...
        {items.length ? items.map((item) => (
          <tr key={item.id}>
            <td className="px-4 py-3">
              ...
              <p className="text-xs font-semibold text-slate-500">{item.regional || "-"}</p>
            </td>
            <td className="px-4 py-3 font-black text-slate-800">{formatMonthLabel(item.mesReferencia)}</td>
            <td className="max-w-xs px-4 py-3 font-semibold text-slate-700">
              <span className="line-clamp-2 break-words">{item.documento}</span>
            </td>
            <td className="px-4 py-3"><span className={statusBadge(item.status)}>{statusLabel(item.status)}</span></td>
            <td className="px-4 py-3 font-black text-slate-800">{item.responsavel}</td>
            <td className="px-4 py-3 font-semibold text-slate-600">{formatDateOnly(item.data)}</td>
            <td className="px-4 py-3 font-semibold text-slate-600">{formatTimeOnly(item.data)}</td>
            <td className="max-w-sm px-4 py-3 text-xs font-semibold text-slate-500">
              <span className="line-clamp-2 break-words">{item.motivo || "-"}</span>
            </td>
          </tr>
        )) : (
          <tr>
...
export default function DocumentosPage({ status = "historico" }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextItems, config] = await Promise.all([
        listarEnviosDocumentos({ status: status === "historico" ? "" : status, limit: 100 }),
        obterCobrancaDocumentos().catch(() => null),
      ]);
      setItems(nextItems);
      setFinanceiroTemplate(config?.financeEmailTemplate || "");
    } catch (error) {
      setSavingMessage(error?.message || "Não foi possível carregar os documentos.");
    } finally {
      setLoading(false);
    }
  }, [status]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("retiradas:documentos-pendentes-updated", handlePendingUpdate);
    return () => {
      window.removeEventListener("retiradas:documentos-pendentes-updated", handlePendingUpdate);
    };
  }, [load, status]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) =>
      [item.empresaNome, item.supervisorNome, item.status, item.mesReferencia, item.submittedByName]
  ...
  const historyItems = useMemo(() => {
    ...
    items.forEach((submission) => {
      (submission.files || []).forEach((file) => {
        if (!file.approvedAt || !["aprovado", "reprovado"].includes(file.status)) return;
        events.push({
        ...
        if (file.adminReviewedAt && ["aprovado", "reprovado"].includes(String(file.adminStatus || "").toLowerCase())) {
          events.push({
            ...
            responsavel: file.adminReviewedByName || submission.adminReviewedByName || "-",
  ...
  const refreshAfterModalChange = async () => {
    const nextItems = await listarEnviosDocumentos({ status: status === "historico" ? "" : status, limit: 100 });
    setItems(nextItems);
  ...
  const handleFinanceiroEmail = async (submission) => {
    ...
    try {
      ...
      setSavingMessage(error?.message || "Não foi possível gerar o e-mail ao financeiro.");
    } finally {
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <button type="button" onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-700 hover:bg-slate-50">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-950">{status === "historico" ? "Histórico de aprovação" : "Envios mensais"}</h2>
            <p className="text-sm text-slate-500">{status === "historico" ? `${historyItems.length} registro(s)` : `${filteredItems.length} envio(s)`}</p>
          </div>
          <label className="relative w-full md:max-w-sm">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={17} />
            <input value={query} onChange={(event) => setQuery(event.target.value)} className={`${inputClass} pl-11`} placeholder={status === "historico" ? "Buscar por empresa, documento ou aprovador" : "Buscar por empresa, mês ou supervisor"} />
          </label>
        </div>

        <div className="overflow-hidden rounded-2xl border border-slate-100">
          <div className="overflow-x-auto">
            {status === "historico" ? <HistoryTable items={paginatedItems} /> : null}
            <table className={`${status === "historico" ? "hidden " : ""}min-w-[860px] w-full divide-y divide-slate-100 text-sm`}>
              <thead className="bg-slate-50 text-left text-xs font-black uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-4 py-3">Empresa</th>
                  <th className="px-4 py-3">Mês</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedItems.length ? paginatedItems.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-4 py-3 font-black text-slate-800">{formatMonthLabel(item.mesReferencia)}</td>
                    <td className="px-4 py-3 font-semibold text-slate-600">{item.files?.length || 0}</td>
                    <td className="px-4 py-3"><span className={statusBadge(item.status)}>{statusLabel(item.status)}</span></td>
                    <td className="px-4 py-3 font-semibold text-slate-500">{formatDate(item.submittedAt)}</td>
                      ...
                      <div className="flex justify-end gap-2">
                        {item.status === "aprovado" ? (
                          <>
                            <button
                              type="button"
                              onClick={() => baixarEnvioDocumentosZip(item).catch((error) => setSavingMessage(error?.message || "Não foi possível baixar o ZIP."))}
                            ...
                            >
                              {financeiroLoadingId === item.id ? <RefreshCw className="animate-spin" size={13} /> : <Send size={13} />}
                              E-mail
                            </button>
                          </>
                        ) : null}
                        <button type="button" onClick={() => setSelected(item)} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50">
                          Abrir
                        </button>
                      </div>
                    </td>
      ...
      <SubmissionModal submission={selected} currentUser={currentUser} onClose={() => setSelected(null)} onChanged={refreshAfterModalChange} />
    </div>
  );
}
