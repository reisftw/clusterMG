...
import { getVpsAuthToken, getVpsCsrfToken } from "../../../services/vpsAuthSession";

export async function listarDocumentos({ status, empresaId, limit = 20, offset = 0 } = {}) {
...
export async function listarCamposDocumentos({ includeInactive = false } = {}) {
  const response = await requestVpsApi(`/documentos/fields?includeInactive=${includeInactive ? "true" : "false"}`);
  return response?.items || [];
}
...
export async function listarCamposNotasFiscais({ includeInactive = false } = {}) {
  const response = await requestVpsApi(`/documentos/invoice-fields?includeInactive=${includeInactive ? "true" : "false"}`);
  return response?.items || [];
}
...
export async function limparHistoricoDocumentos({ confirmation, deleteDriveFiles = false, resetFolders = false }) {
...
export async function listarEnviosDocumentos({ status, mine = false, empresaId, limit = 20, offset = 0 } = {}) {
  const params = new URLSearchParams({
    limit: String(limit),
    offset: String(offset),
  });
  ...
  if (empresaId) params.set("empresaId", empresaId);
  const response = await requestVpsApi(`/documentos/submissions?${params.toString()}`);
  return response?.items || [];
}

export async function listarTratativasDocumentos({ limit = 20, offset = 0 } = {}) {
  const params = new URLSearchParams({
    limit: String(limit),
    offset: String(offset),
  });
  const response = await requestVpsApi(`/documentos/submissions/tratativas?${params.toString()}`);
  return response?.items || [];
}
...
export async function enviarNotasFiscaisMensais({ submissionId, filesByField }) {
  const formData = new FormData();
  const fieldIds = [];
  const valores = [];
  ...
  return requestVpsApi(`/documentos/submissions/${encodeURIComponent(submissionId)}/invoices`, {
    method: "POST",
    body: formData,
  });
}
...
export async function revisarEnvioDocumento(id, { status, motivo = "" }) {
  return requestVpsApi(`/documentos/submissions/${encodeURIComponent(id)}/review`, {
    method: "PATCH",
...
export async function conectarGoogleDrive() {
  ...
  if (!response?.url) throw new Error("Backend não retornou URL de autorização do Google.");
...
export async function criarSubpastaEmpresa(empresaId, name) {
  return requestVpsApi(`/documentos/folders/${encodeURIComponent(empresaId)}/subfolders`, {
    method: "POST",
...
export async function listarPastaDriveEmpresa(empresaId, folderId = "") {
  ...
  return requestVpsApi(`/documentos/folders/${encodeURIComponent(empresaId)}/browser${suffix}`);
}
...
export async function baixarDocumento(documento) {
  ...
  const response = await fetch(`${getApiBaseUrl()}/documentos/${encodeURIComponent(documento.id)}/download`, {
    cache: "no-store",
    credentials: "include",
    headers: {
...
export async function carregarDriveItemEmpresaUrl(empresaId, item, folderId = "") {
  const token = getVpsAuthToken();
  const response = await fetch(buildDriveItemUrl(empresaId, item, folderId, true), {
    cache: "no-store",
    credentials: "include",
    headers: {
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
export async function baixarEnvioDocumentosZip(submission) {
  const token = getVpsAuthToken();
  const response = await fetch(`${getApiBaseUrl()}/documentos/submissions/${encodeURIComponent(submission.id)}/zip`, {
    cache: "no-store",
    credentials: "include",
    headers: {
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
