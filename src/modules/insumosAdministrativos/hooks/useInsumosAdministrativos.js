...
export function useInsumosAdministrativos(usuario) {
			...
			try {
				...
				setProdutos((current) => {
					...
					return sortProdutos(
						exists
							? current.map((item) =>
									item.id === produto.id ? { ...item, ...produto } : item,
								)
							: [...current, produto],
					);
				});
				if (produto.reposicao) {
					setReposicoes((current) =>
						sortReposicoes([produto.reposicao, ...current]),
					);
				}
				return produto;
			} catch (err) {
	...
	const atualizarStatusProduto = useCallback(
		async (id, status) => {
			...
			setError("");
			try {
				const updated = await atualizarStatusProdutoAdministrativo(
					id,
					status,
					usuario,
				);
				setProdutos((current) => {
					...
					return sortProdutos(
						current.map((produto) =>
							produto.id === id ? { ...produto, ...updated } : produto,
						),
					);
				});
				return updated;
	...
	const resumo = useMemo(() => {
		...
		const itensEstoque = produtos.reduce(
		...
		const setoresAtendidos = new Set(
			retiradas
				.filter((retirada) => {
					const date = new Date(
						retirada.retirado_em || retirada.criado_em || "",
					);
					const now = new Date();
					return (
						date.getFullYear() === now.getFullYear() &&
