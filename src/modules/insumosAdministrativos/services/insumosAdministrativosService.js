...
function normalizeConfig(config = {}) {
  ...
  return {
    unidades: uniqueList(safeConfig.unidades?.length ? safeConfig.unidades : DEFAULT_INSUMOS_CONFIG.unidades),
    setores: uniqueList(safeConfig.setores?.length ? safeConfig.setores : DEFAULT_INSUMOS_CONFIG.setores),
    categorias: uniqueList(safeConfig.categorias?.length ? safeConfig.categorias : DEFAULT_INSUMOS_CONFIG.categorias),
  };
}
...
export async function salvarProdutoAdministrativo(dados = {}, usuario = {}) {
  ...
  const existing = produtoId ? await getVpsDocument(`${PRODUTOS}/${produtoId}`).catch(() => null) : null;
...
export async function atualizarStatusProdutoAdministrativo(id, status, usuario = {}) {
...
export async function registrarRetiradaAdministrativa(dados = {}, usuario = {}) {
  ...
  if (!produto || produto.status === "excluido") throw new Error("Produto nao encontrado.");

  const estoqueAtual = number(produto.estoque_atual);
  if (quantidade > estoqueAtual) {
    throw new Error(`Estoque insuficiente. Disponivel: ${estoqueAtual} ${produto.unidade || ""}.`);
  }
...
export async function salvarConfigInsumosAdministrativos(dados = {}, usuario = {}) {
  const now = new Date().toISOString();
  const payload = {
...
export async function listarInsumosRequisicoes({ status = "todos", limit = 20, offset = 0 } = {}) {
  const params = new URLSearchParams({
    status,
    limit: String(limit),
    offset: String(offset),
  });
  const response = await requestVpsApi(`/insumos/requisicoes?${params.toString()}`);
  return {
...
export async function aprovarInsumosRequisicao(id) {
  const response = await requestVpsApi(`/insumos/requisicoes/${encodeURIComponent(id)}/aprovar`, {
    method: "POST",
    body: JSON.stringify({}),
  });
  return response?.item;
...
export async function rejeitarInsumosRequisicao(id, motivo = "") {
  const response = await requestVpsApi(`/insumos/requisicoes/${encodeURIComponent(id)}/rejeitar`, {
    method: "POST",
...
export async function entregarInsumosRequisicao(id, observacao = "") {
  const response = await requestVpsApi(`/insumos/requisicoes/${encodeURIComponent(id)}/entregar`, {
    method: "POST",
