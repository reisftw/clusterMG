...
const TipoBadge = ({ tipo }) => (
  <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg border capitalize ${TIPO_BADGE[tipo] ?? 'bg-gray-100 text-gray-500 border-gray-200'}`}>
    {tipo}
  </span>
);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-gray-100">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-gray-900">Cadastrar Feriado</h3>
          </div>
        </div>
...
const FeriadosPage = () => {
  ...
  return (
    <div className="space-y-5">
        ...
        {[
          ...
          >
            ...
            <span className={`ml-1 text-xs font-bold px-1.5 py-0.5 rounded-md ${
              aba === key ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-500'
            }`}>
      ...
      {/* Erro */}
      {error && (
        <div className="px-4 py-3 bg-red-50 border border-red-100 rounded-2xl text-sm text-red-600">{error}</div>
      )}
        ...
        {aba === 'nacionais' && (
          feriadosApi.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-gray-400">Nenhum feriado nacional encontrado.</p>
            </div>
          ) : (
                ...
                <tbody>
                  {feriadosApi.map((f, i) => (
                    <tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                      <td className="px-5 py-3 text-sm font-medium text-gray-700">{formatarData(f.date)}</td>
                      <td className="px-5 py-3 text-sm text-gray-800">{f.name}</td>
                      <td className="px-5 py-3"><TipoBadge tipo={f.type} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {aba === 'manuais' && (
          feriados.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-gray-400">Nenhum feriado cadastrado manualmente.</p>
              {podeGerenciar && (
                <button
                  onClick={() => setShowForm(true)}
                ...
                <tbody>
                  {feriados.map((f) => (
                    <tr key={f.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                      <td className="px-5 py-3 text-sm font-medium text-gray-700">{formatarData(f.data)}</td>
                      <td className="px-5 py-3 text-sm text-gray-800 font-semibold">{f.nome}</td>
                      <td className="px-5 py-3"><TipoBadge tipo={f.tipo} /></td>
                      {podeGerenciar && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={async () => { await deletar(confirmarDel.id); setConfirmarDel(null); }}
