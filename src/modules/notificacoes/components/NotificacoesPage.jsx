...
export default function NotificacoesPage() {
  ...
  useEffect(() => {
    ...
    load()
      .catch((error) => {
        if (active) setMessage(error?.message || "Não foi possível carregar as notificações.");
      })
      .finally(() => {
  ...
  const checkCritical = async () => {
    ...
    try {
      ...
      setMessage(error?.message || "Não foi possível verificar os alertas críticos.");
    } finally {
      setChecking(false);
    }
  };
    ...
    <div className="space-y-5">
      <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-950">Central de Notificações</h1>
              <p className="text-sm font-semibold text-slate-500">
                Histórico de documentos, WhatsApp, APIs, backups e alertas do sistema.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {checking ? <Loader2 className="animate-spin" size={16} /> : <ShieldAlert size={16} />}
              Verificar críticos
            </button>
            <button
              type="button"
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
          ...
          <p className="mt-2 text-2xl font-black text-blue-950">{stats?.total || 0}</p>
        </div>
        <div className="rounded-2xl border border-orange-100 bg-orange-50 p-4">
          <p className="text-xs font-black uppercase text-orange-700">Não lidas</p>
          <p className="mt-2 text-2xl font-black text-orange-950">{stats?.unread || unread}</p>
        </div>
        <div className="rounded-2xl border border-red-100 bg-red-50 p-4">
          <p className="text-xs font-black uppercase text-red-700">Críticas</p>
          <p className="mt-2 text-2xl font-black text-red-950">{stats?.bySeverity?.critical || 0}</p>
        </div>
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
          <p className="text-xs font-black uppercase text-emerald-700">WhatsApp</p>
          <p className="mt-2 text-2xl font-black text-emerald-950">{stats?.byType?.whatsapp_agendamento_auto || 0}</p>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-500">{items.length} notificação(ões) encontrada(s)</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-black text-slate-700">
              <input
                ...
                onChange={(event) => setFilters((current) => ({ ...current, unread: event.target.checked }))}
            ...
            >
              <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold leading-relaxed">{item.message}</p>
                </div>
                <div className="flex shrink-0 flex-wrap gap-2 text-xs font-black">
                  <span className="rounded-full bg-white/75 px-2.5 py-1">{item.read ? "Lida" : "Nova"}</span>
                  <span className="rounded-full bg-white/75 px-2.5 py-1">{formatDate(item.createdAt)}</span>
                </div>
              </div>
            </a>
          )) : (
