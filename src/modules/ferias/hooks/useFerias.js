...
export const useFerias = (currentUser) => {
	...
	const carregar = useCallback(
		async (force = false) => {
			...
			try {
				const colaboradorId = getCurrentColaboradorId(currentUser);
				const scopedToCurrentUser =
					!canReadAllFerias(currentUser) && Boolean(colaboradorId);
				const cacheKey = buildCacheKey(currentUser);
				const { data } = await getOrLoadCachedValue(
					cacheKey,
					async () => {
						const items = scopedToCurrentUser
						...
						logDataRead({
							source: "useFerias",
							type: "sql-list",
							path: scopedToCurrentUser
								? `ferias:colaborador:${colaboradorId}`
								: "ferias",
							count: items.length,
						});
		...
		async (dados) => {
			try {
				const payload = {
					...dados,
					colaborador_id:
						dados?.colaborador_id ?? getCurrentColaboradorId(currentUser),
				};
