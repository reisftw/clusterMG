...
const FeriasStatusBadge = ({ status }) => {
	...
	return (
		<span
			className={`text-xs font-semibold px-2.5 py-1 rounded-lg border ${config.bg} ${config.text} ${config.border}`}
		>
			{config.label}
		</span>
	);
		...
		<div className="space-y-5">
			...
			<div className="grid grid-cols-3 gap-3">
				{[
					...
					<div
						key={label}
						className={`rounded-2xl border ${bg} ${border} p-4 text-center`}
					>
			...
			{visualizacao === "calendario" && (
				<FeriasCalendario ferias={ferias} colaboradores={colaboradores} />
			)}
				...
				<div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
					<div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100 bg-gray-50/50 flex-wrap">
						...
						<span className="ml-auto text-xs text-gray-400 font-medium">
							{feriasFiltradas.length} registro(s)
						</span>
					</div>
												...
												<div className="flex items-center gap-2">
													<div className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-400 to-orange-500 flex items-center justify-center shrink-0">
														<span className="text-white text-[10px] font-bold">
															{String(
																getNomeColaborador(item.colaborador_id) || "?",
															)
																.charAt(0)
																.toUpperCase()}
														</span>
					...
					<div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 w-full max-w-sm">
						...
						<p className="text-sm text-gray-500 text-center mb-6">
							...
							<span className="font-semibold text-gray-700">
								{getNomeColaborador(confirmarDel.colaborador_id)}
							</span>{" "}
							({formatarData(confirmarDel.data_inicio)} →{" "}
							{formatarData(confirmarDel.data_fim)}) serao removidas.
						</p>
