...
const FeriasLancamentoForm = ({ colaboradores, onSubmit, onClose }) => {
  ...
  const inputClass = "w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm";
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select value={colaboradorId} onChange={(e) => setColaboradorId(e.target.value)} required className={inputClass}>
          ...
          <div>
            ...
            <input type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)} required className={inputClass} />
          </div>

          {dataInicio && dataFim && (
            <p className="text-sm text-blue-600 dark:text-blue-400 font-medium">Total: {calcularDias()} dias</p>
          )}
