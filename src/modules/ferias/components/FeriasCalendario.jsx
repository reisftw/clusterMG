      ...
      <div className="p-4">
        <div className="grid grid-cols-7 mb-2">
          {DIAS_SEMANA.map((diaSemana) => (
            <div key={diaSemana} className="text-center text-xs font-bold text-gray-400 uppercase tracking-wide py-1">
              {diaSemana}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {celulas.map((dia, index) => {
            ...
            return (
              ...
              >
                <span className={`text-xs font-bold mb-1 ${isHoje(dia) ? "text-blue-600" : "text-gray-500"}`}>
                  {dia}
                </span>
          ...
          <div className="flex flex-wrap gap-2">
            {colaboradoresComFerias.map((colaborador) => {
              ...
              return (
                ...
                >
                  <span className={`w-2 h-2 rounded-full ${cor.dot} shrink-0`} />
                  <span className="text-xs font-semibold text-gray-700">
                    {getPrimeiroNome(colaborador.nome ?? colaborador.id)}
                  </span>
                  <span className="text-[10px] text-gray-400">{colaborador.cargo ?? ""}</span>
                </div>
              );
            })}
          </div>
