  ...
  if (!lista.length) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-center justify-center h-48">
        <p className="text-sm text-gray-400">Nenhum colaborador com data de nascimento cadastrada.</p>
      </div>
    );
  }
      ...
      {proximo && (
        <div className="bg-gradient-to-r from-orange-50 to-orange-100 border border-orange-100 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs font-semibold text-orange-600">{formatarData(proximo.aniv)}</p>
            <p className="text-xs text-orange-400 mt-0.5">
              {proximo.diffDias === 0 ? "🎉 Hoje!" : `em ${proximo.diffDias} dias`}
            </p>
          </div>
        </div>
      )}
      ...
      <ul className="space-y-2">
        {lista.slice(1).map((c) => (
          <li key={c.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
            ...
            <div className="text-right">
              <p className="text-xs text-gray-600 font-medium">{formatarData(c.aniv)}</p>
              {c.diffDias > 0 && c.diffDias <= 7 ? (
                <span className="text-[10px] bg-orange-100 text-orange-600 font-semibold px-1.5 py-0.5 rounded-md">
                  esta semana
                </span>
              ) : (
                <span className="text-[10px] text-gray-400">{c.diffDias} dias</span>
              )}
            </div>
          </li>
        ))}
