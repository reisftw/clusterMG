...
function normalizeAppointmentDateKey(item = {}) {
	...
	const brDate = brDateRegex.exec(raw);
	if (brDate) {
...
function countByMonth(items = [], monthKey) {
	return items.filter((item) =>
		normalizeAppointmentDateKey(item).startsWith(monthKey),
	).length;
}
...
const ModernKpiGrid = ({
	...
	const dynamicTrends = {
		agendamentosMes: buildMonthTrend(
			agendamentosMesValue,
			agendamentosMesPrevious,
		),
	};
	...
	return (
		<section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
			{KPI_CONFIG.filter((item) => !hiddenKeys.includes(item.key)).map(
				(item) => (
					<KpiCard
						key={item.key}
						{...item}
						trend={dynamicTrends[item.key] || item.trend}
