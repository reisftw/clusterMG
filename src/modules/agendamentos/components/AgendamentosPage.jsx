...
const uniqueSorted = (items) =>
  [...new Set(items.filter(Boolean))].sort((a, b) => a.localeCompare(b, "pt-BR"));
...
const getLogDateKey = (log) =>
  String(log?.data_agendamento || log?.data || log?.agendamento_data || "").slice(0, 10);

const getLogCreatedTime = (log) => {
  const createdAt = new Date(log?.criado_em || log?.created_at || log?.createdAt || 0);
...
const getLogRunKey = (log) => {
  ...
  const rawCreatedAt = log?.criado_em || log?.created_at || log?.createdAt || "";
...
const isTecnicoAtivo = (colaborador) => {
  ...
  return (
    ...
    (cargo.includes("tecnico") || cargo.includes("tecnico"))
  );
};
...
const AgendamentosPage = () => {
  ...
  useEffect(() => {
    ...
    getConfig()
      .then((config) => {
        if (!mounted) return;
        setTecnicosConfig((config?.tecnicos || []).map((item) => item?.nome).filter(Boolean));
      })
      .catch(() => {
  ...
  const tecnicos = useMemo(() => {
    const fallback = colaboradores.filter(isTecnicoAtivo).map((item) => item.nome);
  ...
  const opcoesTecnico = useMemo(
    () => uniqueSorted([...tecnicos, ...agendamentos.map((item) => item.tecnico_nome)]),
  ...
  const listaFiltrada = useMemo(() => {
    ...
    return agendamentos.filter((item) => {
      ...
      const tecnicoOk = filtroTecnico === "todos" || item.tecnico_nome === filtroTecnico;
      const textoOk =
        !termo ||
  ...
  const handleNaoRecolhido = async () => {
    ...
    try {
      ...
      setDesfechoErro(error.message || "Nao foi possivel enviar para reagendamento.");
    } finally {
      setSavingDesfecho(false);
    }
  };
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{resumo.concluidos}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex flex-col gap-3 border-b border-gray-100 p-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Apenas a última rodada do mapa para os agendamentos do dia anterior.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="overflow-y-auto p-5">
              {!logsMapa.length ? (
                <p className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4 text-sm text-gray-500">
                  Nenhuma alteração do dia anterior foi registrada na última verificação.
                </p>
              ) : (
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-500">
                              Código {log.codigo_cliente || "-"} • {formatData(log.data_agendamento)}
                          ...
                          >
                            {recolhido ? <CheckCircle2 size={12} /> : <PackageX size={12} />}
                            {log.status_novo}
                          </span>
                        </div>
                        <p className="mt-3 text-xs text-gray-600">{log.motivo}</p>
                        <p className="mt-2 text-[11px] font-semibold text-gray-400">
                          {formatDateTime(log.criado_em)}
                          {log.os_encontrada ? ` • O.S. ${log.os_encontrada}` : ""}
                        </p>
                      </div>
                    );
                  })}
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  {agendamento.observacao && (
                    <p className="mt-3 text-sm text-gray-500">{agendamento.observacao}</p>
                  )}
                </div>
                ...
                {podeEditar && (
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    {agendamento.origem === "esteira_backoffice" &&
                      !["Entregue", "Concluido", "Nao recolhido", "Cancelado"].includes(
                        agendamento.status,
                      ) && (
                        <button
                          type="button"
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        atualizar(agendamento.id, { ...agendamento, status: event.target.value })
                      }
        ...
        <div className="mt-4 divide-y divide-gray-100">
          {!agendamentosDoDia.length ? (
            ...
            agendamentosDoDia.map((agendamento) => {
              const classified = ["Entregue", "Concluido", "Nao recolhido"].includes(
                agendamento.status,
              );
              return (
                <div
            ...
            <div className="space-y-4 p-5">
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setDesfecho((current) => ({ ...current, tipo: "nao_recolhido" }))
                  }
                  className={`flex min-h-24 flex-col items-center justify-center gap-2 rounded-lg border text-sm font-bold ${
