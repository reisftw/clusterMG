...
export async function submitRetiradaRequest(form) {
  ...
  const ref = await createVpsDocument(COLLECTIONS.RETIRADAS_SOLICITACOES, payload);
...
export function listenRetiradaRequests(onData, onError) {
  ...
  const load = async () => {
    try {
      const items = await listVpsDocuments(COLLECTIONS.RETIRADAS_SOLICITACOES, { limit: 1000 });
      if (!active) return;
      onData(
        items
...
export async function updateRetiradaRequest(id, payload = {}) {
  const current = await getVpsDocument(`${COLLECTIONS.RETIRADAS_SOLICITACOES}/${id}`);
  const updates = {
    nome: text(payload.nome),
  ...
  if (text(payload.status) === "concluido" && text(current.status) !== "concluido") {
    updates.concluidoEm = new Date().toISOString();
  }

  return updateVpsDocument(`${COLLECTIONS.RETIRADAS_SOLICITACOES}/${id}`, updates);
}
