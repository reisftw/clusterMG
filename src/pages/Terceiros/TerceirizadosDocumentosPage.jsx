...
const TERCEIRIZADOS_MFA_STORAGE_KEY = "retiradas_terceirizados_mfa_challenge_v1";
...
function storeMfaChallenge(challenge) {
  ...
  window.sessionStorage.setItem(TERCEIRIZADOS_MFA_STORAGE_KEY, JSON.stringify(challenge));
}
...
function PaginationControls({ page, totalPages, totalItems, pageSize, onPageChange, onPageSizeChange }) {
  ...
  return (
    <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 text-sm font-bold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
      <p>Mostrando {start} a {end} de {totalItems} envio(s)</p>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="rounded-xl bg-blue-600 px-3 py-2 text-white">{page}</span>
        <button type="button" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="rounded-xl border border-slate-200 px-3 py-2 text-slate-700 hover:bg-slate-50 disabled:opacity-40">
          ›
        </button>
...
function DocumentUploadCard({
  ...
  const currentAdminStatus = String(currentFile?.adminStatus || "").toLowerCase();
  const isRejected = currentStatus === "reprovado" || currentAdminStatus === "reprovado";
  const rejectionReason = currentFile?.adminMotivoReprovacao || currentFile?.motivoReprovacao;
    ...
    >
      <div className="flex flex-col gap-3 min-[380px]:flex-row min-[380px]:items-center min-[380px]:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${tone.icon} text-white shadow-lg shadow-slate-200`}>
            {isRejected ? <AlertTriangle size={27} /> : selectedFile ? <CheckCircle2 size={27} /> : <FileText size={27} />}
          </div>
          <div className="min-w-0">
            <h3 className="truncate text-base font-black text-slate-950">
              {field.nome} {field.obrigatorio ? <span className="text-red-600">*</span> : null}
            </h3>
        ...
        {selectedFile ? (
          ...
          >
            {saving ? <Loader2 className="animate-spin" size={18} /> : <CloudUpload className="transition group-hover:-translate-y-0.5" size={18} />}
            Enviar arquivo
          </label>
        )}
      </div>
      ...
      {isRejected && rejectionReason ? (
        <div className="mt-3 rounded-2xl border border-red-100 bg-red-50 px-3 py-2 text-xs font-bold leading-relaxed text-red-700">
          <div className="flex items-start gap-2">
            ...
            <span className="min-w-0 break-words">{rejectionReason}</span>
          </div>
        </div>
      ) : null}
    </article>
...
function latestFilesByField(files = []) {
  ...
  files.forEach((file) => {
    ...
    if (!current || new Date(file.createdAt || 0).getTime() >= new Date(current.createdAt || 0).getTime()) {
      latest.set(key, file);
    }
  });
  return latest;
...
function getInitialLoginMessage() {
  return readStoredMfaChallenge() ? "Código MFA pendente. Informe o código recebido por e-mail." : "";
}
...
function useTerceirosLoginFlow(onLoggedIn) {
  ...
  const [mfaChallenge, setMfaChallenge] = useState(() => readStoredMfaChallenge());
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (result?.mfaRequired) {
        ...
        setMfaCode("");
        setLocalMessage("Código enviado. Verifique seu e-mail e informe o código para continuar.");
        return;
      }
      ...
      setLocalMessage(submitError?.message || "Não foi possível entrar. Confira e-mail e senha.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const recoverPassword = async () => {
    if (!email.trim()) {
      setRecoverMessage("Informe seu e-mail para receber o link de recuperação.");
      return;
    }
    setRecovering(true);
    setRecoverMessage("");
    try {
      ...
      setRecoverMessage("Se esse e-mail estiver cadastrado, enviaremos um link para redefinir a senha.");
    } catch (recoverError) {
      setRecoverMessage(recoverError?.message || "Não foi possível solicitar a redefinição de senha.");
    } finally {
      setRecovering(false);
    }
  };
...
function LoginBox({ onLoggedIn }) {
  ...
  return (
    <div onKeyDown={handleKeyDown} className="mx-auto w-full max-w-md rounded-[28px] border border-white/15 bg-white/95 p-6 text-slate-950 shadow-2xl">
      <div className="mb-6 text-center">
        <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-600 text-white">
          <Lock size={22} />
        </div>
        <h1 className="text-2xl font-black">{mfaChallenge ? "Confirme seu acesso" : "Portal terceirizado"}</h1>
        <p className="mt-1 text-sm font-semibold text-slate-500">
          {mfaChallenge
      ...
      <div className="space-y-3">
        {mfaChallenge ? (
          <div className="rounded-3xl border border-blue-100 bg-blue-50 p-4">
            <label className="block">
              <span className="text-xs font-black uppercase tracking-[0.12em] text-blue-700">Código de segurança</span>
              <input
                type="text"
                required
                inputMode="numeric"
          ...
          <>
            <label className="block">
              <span className="mb-2 block text-xs font-black uppercase tracking-[0.12em] text-slate-500">E-mail</span>
            ...
            <label className="block">
              <span className="mb-2 block text-xs font-black uppercase tracking-[0.12em] text-slate-500">Senha</span>
              <div className="relative">
        ...
        {error ? <div className="rounded-2xl border border-red-100 bg-red-50 p-3 text-sm font-bold text-red-700">{error}</div> : null}
        {localMessage ? (
        ...
        >
          <LogIn size={18} /> {saving ? "Validando..." : mfaChallenge ? "Validar código" : "Entrar"}
        </button>
        {mfaChallenge ? (
          <button
            type="button"
...
export default function TerceirizadosDocumentosPage() {
  ...
  const audience = currentRole === "agente_autorizado" ? "agente" : "tecnico";
  ...
  const load = useCallback(async () => {
    ...
    setLoadingData(true);
    try {
      const [requiredFields, requiredInvoiceFields, submissions] = await Promise.all([
        listarCamposDocumentos(),
        listarCamposNotasFiscais(),
        listarEnviosDocumentos({ mine: true, limit: 100 }),
      ]);
      setFields(requiredFields.filter((field) => {
  ...
  useEffect(() => {
    load().catch((error) => setMessage(error?.message || "Não foi possível carregar documentos."));
  }, [load]);
  ...
  const monthSubmission = useMemo(() => {
    ...
    return monthItems.sort((a, b) => new Date(b.submittedAt || b.createdAt || 0) - new Date(a.submittedAt || a.createdAt || 0))[0] || null;
  }, [items, mesReferencia]);

  const latestMonthFiles = useMemo(
    () => latestFilesByField((monthSubmission?.files || []).filter((file) => !isInvoiceFile(file))),
    [monthSubmission],
  );
  ...
  const approvedFields = useMemo(
    () => fields.filter((field) => {
      ...
      return String(file?.status || "").toLowerCase() === "aprovado" && String(file?.adminStatus || "").toLowerCase() === "aprovado";
    }),
  ...
  const pendingFields = useMemo(
    () => fields.filter((field) => {
      ...
      return status === "pendente" || (status === "aprovado" && adminStatus !== "aprovado" && adminStatus !== "reprovado");
    }),
    [fields, latestMonthFiles],
  );
  ...
  const visibleFields = useMemo(
    () => fields.filter((field) => {
      const status = String(latestMonthFiles.get(field.id)?.status || "").toLowerCase();
      const adminStatus = String(latestMonthFiles.get(field.id)?.adminStatus || "").toLowerCase();
      if (status === "reprovado" || adminStatus === "reprovado") return true;
      return !(status === "aprovado" && adminStatus === "aprovado") && status !== "pendente" && !(status === "aprovado" && adminStatus !== "aprovado");
    }),
    [fields, latestMonthFiles],
  );
  ...
  const selectedFiles = useMemo(
    () => visibleFields.map((field) => ({ fieldId: field.id, file: filesByField[field.id] })).filter((item) => item.file),
  ...
  const monthApproved = String(monthSubmission?.status || "").toLowerCase() === "aprovado";
  const existingInvoices = useMemo(
    () => (monthSubmission?.files || []).filter(isInvoiceFile),
    [monthSubmission],
  );
  const selectedFilesSize = useMemo(
    () => selectedFiles.reduce((total, item) => total + Number(item.file?.size || 0), 0),
    [selectedFiles],
  );
  ...
  const submit = async (event) => {
    event.preventDefault();
    if (waitingReview) {
      setMessage("Existe envio aguardando avaliação para este mês. Aguarde o supervisor avaliar antes de enviar novamente.");
      return;
    }
    ...
    setSuccessMessage("");
    try {
      const response = await enviarDocumentosMensais({ mesReferencia, filesByField: selectedFiles });
      setFilesByField({});
      ...
      setSuccessMessage(`Documentos de ${formatMonthLabel(mesReferencia)} enviados com sucesso. Novo envio bloqueado até a avaliação do supervisor.`);
      setSaving(false);
      load().catch((error) => setMessage(error?.message || "Envio salvo, mas não foi possível atualizar a lista automaticamente."));
    } catch (error) {
  ...
  const submitInvoices = async () => {
    if (!monthSubmission?.id || !monthApproved) {
      setMessage("As notas fiscais só ficam disponíveis após a aprovação completa dos documentos.");
      return;
    }
    ...
    const withoutValue = selectedInvoices.find((item) => Number(String(item.valor).replace(/\./g, "").replace(",", ".")) <= 0);
    if (withoutValue) {
      setMessage(`Informe o valor da nota fiscal "${withoutValue.field?.nome || "selecionada"}".`);
      return;
    }
    setSaving(true);
    setMessage("");
    ...
    try {
      ...
      setSuccessMessage(`Notas fiscais de ${formatMonthLabel(mesReferencia)} enviadas com sucesso.`);
      setSaving(false);
      load().catch((error) => setMessage(error?.message || "Notas salvas, mas não foi possível atualizar a lista automaticamente."));
    } catch (error) {
      ...
      <main className="mx-auto flex min-h-dvh w-full max-w-6xl flex-col px-4 pb-6 pt-5 sm:px-6 lg:px-8">
        <header className="relative min-h-72 shrink-0 overflow-hidden rounded-b-[42px] px-2 pb-12 pt-7 sm:min-h-80 sm:px-6 lg:rounded-[42px] lg:border lg:border-white/10 lg:bg-white/5 lg:px-10 lg:shadow-2xl lg:shadow-black/10">
          ...
          <div className="pointer-events-none absolute -right-3 top-11 hidden h-48 w-48 sm:block">
            ...
            <CloudUpload className="absolute right-16 top-16 text-white drop-shadow-xl" size={54} />
            <div className="absolute right-0 top-20 h-28 w-28 rounded-full border-2 border-dashed border-blue-300/50" />
          </div>
          <div className="relative max-w-xl">
            <img src="/cluster-mg.png" alt="Cluster MG" className="mb-8 h-auto w-40 max-w-full object-contain sm:w-52" decoding="async" />
            <p className="text-xs font-black uppercase tracking-[0.34em] text-orange-300 sm:text-sm">Portal de documentos</p>
            <h1 className="mt-3 text-5xl font-black leading-none tracking-tight text-white sm:text-6xl">Terceirizados</h1>
        ...
        <section className="-mt-10 flex flex-1 flex-col pb-7 lg:mt-7">
          {!currentUser ? (
            <LoginBox onLoggedIn={() => setSessionTick((value) => value + 1)} />
          ) : !isLeader ? (
            <div className="mx-auto max-w-xl rounded-3xl border border-red-200 bg-white p-6 text-center text-red-700 shadow-xl">
              ...
              <p className="mt-2 text-sm font-bold">Esta área é destinada aos líderes de empresa e agentes autorizados.</p>
              <button onClick={signOut} className="mt-5 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Sair</button>
            </div>
          ) : (
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start">
              <form onSubmit={submit} className="relative z-10 rounded-[34px] border border-white/80 bg-white p-5 text-slate-950 shadow-[0_28px_90px_rgba(15,23,42,0.22)] sm:p-8">
                <div className="mb-8 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="text-3xl font-black tracking-tight text-slate-950">Enviar documentos</h2>
                    <div className="mt-5 flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-sm font-bold text-slate-400">
                          {currentUser?.empresaNome || currentUser?.profile?.empresaNome || "Sua empresa"}
                        </p>
                      </div>
                    </div>
                  </div>
                  <button type="button" onClick={signOut} className="inline-flex min-h-12 shrink-0 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-600 shadow-sm transition hover:bg-slate-50">
                    <LogOut size={18} />
                    Sair
                  </button>
                </div>
                ...
                <div className="mb-8 space-y-3">
                  <p className="text-sm font-black uppercase tracking-[0.14em] text-slate-500">Mês de referência</p>
                ...
                <div className="border-t border-slate-200 pt-7">
                  <h3 className="mb-5 text-sm font-black uppercase tracking-[0.14em] text-slate-500">Documentos para envio</h3>
                    ...
                    <div className="mb-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-bold text-emerald-800">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {approvedFields.map((field) => (
                          <span key={field.id} className="rounded-full bg-white px-3 py-1 text-xs font-black text-emerald-700">
                            {field.nome}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold text-amber-800">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1">Você já enviou documento(s) para {formatMonthLabel(mesReferencia)}. Novo envio fica bloqueado até o supervisor avaliar.</p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            {pendingFields.map((field) => (
                              <span key={field.id} className="rounded-full bg-white px-3 py-1 text-xs font-black text-amber-700">
                                {field.nome}
                              </span>
                            ))}
                          </div>
                  ...
                  <div className="space-y-4">
                    {visibleFields.length ? visibleFields.map((field, index) => (
                      <DocumentUploadCard
                        ...
                        saving={saving}
                        onSelect={(file) => setFilesByField((current) => ({ ...current, [field.id]: file }))}
                        onRemove={() => setFilesByField((current) => {
                          const next = { ...current };
                          delete next[field.id];
                          return next;
                        })}
                      />
                    )) : fields.length && !waitingReview ? (
                ...
                {monthApproved ? (
                  <section className="mt-7 rounded-[28px] border border-orange-100 bg-orange-50/70 p-4 sm:p-5">
                    <div className="mb-4 flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-black text-slate-950">Notas fiscais do mês</h3>
                        <p className="text-sm font-bold text-orange-800">
                          Documentos aprovados. Agora você pode anexar as notas fiscais e informar o valor de cada uma.
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="mb-4 rounded-2xl border border-emerald-100 bg-white p-3 text-sm font-bold text-emerald-800">
                        ...
                        <div className="space-y-2">
                          {existingInvoices.map((invoice) => (
                            <div key={invoice.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-emerald-50 px-3 py-2">
                    ...
                    <div className="space-y-3">
                      {invoiceFields.length ? invoiceFields.map((field, index) => {
                        const inputId = `terceirizados-nota-${field.id}`;
                        const current = invoiceFilesByField[field.id] || {};
                        const tone = documentTones[index % documentTones.length];
                        return (
                          <article key={field.id} className="rounded-2xl border border-orange-100 bg-white p-4">
                            <div className="grid gap-3 lg:grid-cols-[1fr_150px_190px] lg:items-end">
                              <div>
                                ...
                                <p className="text-xs font-bold text-slate-500">Anexe a nota fiscal e informe o valor.</p>
                              </div>
                              <label className="space-y-1">
                                ...
                                <input
                                  value={current.valor || ""}
                                  disabled={saving}
                                  onChange={(event) => setInvoiceFilesByField((state) => ({
                                    ...state,
                                    [field.id]: { ...(state[field.id] || {}), valor: event.target.value },
                                  }))}
                                  placeholder="0,00"
                              ...
                              <div>
                                <input
                                  ...
                                  onChange={(event) => setInvoiceFilesByField((state) => ({
                                    ...state,
                                    [field.id]: { ...(state[field.id] || {}), file: event.target.files?.[0] || null },
                                  }))}
                                  className="sr-only"
                                />
                                <label htmlFor={inputId} className={`inline-flex min-h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed px-4 py-3 text-sm font-black transition hover:scale-[1.02] ${tone.dashed}`}>
                                  <CloudUpload size={18} />
                ...
                {selectedFiles.length && !waitingReview ? (
                  <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm font-bold text-slate-700">
                    {selectedFiles.length} arquivo(s) pronto(s) para envio - {formatBytes(selectedFilesSize)} no total.
                  </div>
                ) : null}
                ...
                {successMessage ? <div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-3 text-sm font-bold text-emerald-800">{successMessage}</div> : null}
                {message ? <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-3 text-sm font-bold text-blue-800">{message}</div> : null}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-blue-800/80">
                        O envio passa pelo backend autenticado do sistema e não gera links públicos.
                      </p>
                    </div>
                  </div>
                </div>

                <button disabled={saving || loadingData || waitingReview || !visibleFields.length || !selectedFiles.length} className="mt-5 inline-flex min-h-16 w-full items-center justify-center gap-3 rounded-[22px] bg-gradient-to-r from-indigo-600 to-blue-600 px-5 py-4 text-lg font-black text-white shadow-[0_20px_42px_rgba(37,99,235,0.32)] transition hover:-translate-y-0.5 hover:shadow-[0_24px_50px_rgba(37,99,235,0.38)] disabled:translate-y-0 disabled:cursor-not-allowed disabled:from-slate-300 disabled:to-slate-400 disabled:shadow-none">
                  {saving ? <Loader2 className="animate-spin" size={22} /> : <Send size={22} />}
                  {saving ? "Enviando arquivos..." : waitingReview ? "Aguardando avaliação" : hasRejectedFields ? "Reenviar para aprovação" : "Enviar para aprovação"}
                </button>
              </form>
              ...
              <section className="rounded-[30px] border border-white/80 bg-white/95 p-5 text-slate-950 shadow-[0_22px_70px_rgba(15,23,42,0.16)] lg:sticky lg:top-6">
                <div className="mb-5 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slate-500">Acompanhe aprovados, pendentes e reprovados.</p>
                  </div>
                  <button type="button" onClick={load} className="rounded-xl border border-slate-200 p-2 text-slate-700 hover:bg-slate-50">
                    <RefreshCw size={17} />
                  </button>
                </div>
                <div className="space-y-3">
                  {paginatedItems.length ? paginatedItems.map((item) => (
                    <article key={item.id} className="rounded-2xl border border-slate-200 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-lg font-black text-slate-950">{formatMonthLabel(item.mesReferencia)}</p>
                          <p className="text-xs font-semibold text-slate-500">{formatDate(item.submittedAt)}</p>
                        </div>
                        <span className={statusBadge(item.status)}>{item.status}</span>
                      </div>
                      <p className="mt-2 text-sm font-semibold text-slate-600">{item.files?.length || 0} arquivo(s)</p>
