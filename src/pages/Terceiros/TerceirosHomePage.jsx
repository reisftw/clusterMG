...
const cardStyles = {
	blue: {
		...
		iconBg: "bg-blue-50",
		arrow:
			"bg-blue-500/20 text-blue-300 group-hover:bg-blue-500/30 group-hover:text-blue-100",
	...
	green: {
		...
		arrow:
			"bg-emerald-500/20 text-emerald-300 group-hover:bg-emerald-500/30 group-hover:text-emerald-100",
	...
	violet: {
		...
		iconBg: "bg-violet-50",
		arrow:
			"bg-violet-500/20 text-violet-300 group-hover:bg-violet-500/30 group-hover:text-violet-100",
	...
	orange: {
		...
		iconBg: "bg-orange-50",
		arrow:
			"bg-orange-500/20 text-orange-300 group-hover:bg-orange-500/30 group-hover:text-orange-100",
...
function MenuButton({ icon: Icon, title, helper, href, tone = "blue" }) {
	...
	return (
		...
		>
			<span
				className={`pointer-events-none absolute bottom-5 left-0 h-16 w-2 rounded-r-full bg-gradient-to-b ${styles.glow} to-transparent opacity-90 blur-[1px] transition group-hover:h-20`}
			/>
			...
			<span
				className={`relative flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-[24px] ${styles.iconBg} ${styles.icon} shadow-[0_18px_34px_rgba(0,0,0,0.18)] transition duration-200 group-hover:scale-105 sm:h-20 sm:w-20`}
			>
				<Icon size={34} strokeWidth={2.35} />
			</span>
			<span className="relative min-w-0 flex-1">
				<span className="block truncate text-2xl font-black tracking-tight text-white sm:text-3xl">
					{title}
				</span>
				<span className="mt-2 block text-base font-semibold leading-snug text-blue-100/85 sm:text-lg">
					{helper}
				</span>
			</span>
			<span
				className={`relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full transition duration-200 group-hover:translate-x-1 ${styles.arrow}`}
			>
...
export default function TerceirosHomePage({ portal = "terceiros" }) {
	...
	const terceirosParam = isTerceirizados
		? "?from=terceirizados"
		: "?from=terceiros";
			...
			<main className="relative mx-auto flex min-h-screen w-full max-w-md flex-col px-5 pb-6 pt-9 sm:max-w-lg sm:px-7 lg:max-w-xl">
				...
				<section className="relative flex flex-1 flex-col gap-4 py-8 sm:gap-5">
					...
					<MenuButton
						...
						href={
							isTerceirizados
								? ROUTES.TERCEIRIZADOS_CONSULTA_MAC
								: ROUTES.TERCEIROS_CONSULTA_MAC
						}
						tone="violet"
					/>
					<MenuButton
