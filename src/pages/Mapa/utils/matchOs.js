...
function buildCidadeMatches(grupo, cidade, listaOrdens, { isAgente = false } = {}) {
  const ordensComCoord = listaOrdens.filter(hasCoordinates);
  const retiradas = ordensComCoord.filter((ordem) => isRetiradaTipo(ordem.tipo));
  const servicos = ordensComCoord.filter((ordem) => !isRetiradaTipo(ordem.tipo));
  ...
  const matches = servicos
    .map((principal) => {
      ...
      const relacionadas = retiradas
        ...
        .map((retirada) => {
          const distanceMeters = Math.round(calcDistanceMeters(principal, retirada));
          const sameStreet =
            ruaPrincipal &&
            ruaPrincipal === normalizeStreet(retirada.endereco);
  ...
  return Object.entries(agrupado)
    .map(([grupo, cidadesMap]) => {
      ...
      return {
        regional: grupo,
        cidades,
        totalMatches: cidades.reduce((sum, cidade) => sum + cidade.totalMatches, 0),
