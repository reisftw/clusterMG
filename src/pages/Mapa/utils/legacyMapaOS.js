...
function parseExcelDate(rawValue) {
	return validDateOrNull(
		new Date(Math.round((rawValue - 25569) * 86400 * 1000)),
	);
}
...
function buildLegacyCidadeMatches(
	grupo,
	cidade,
	legacyOrdens,
	recentOrdens,
			...
			const relacionadas = relacionadasBase
				.map((ordem) => {
					const distanceMeters = Math.round(
						calcDistanceMeters(principal, ordem),
					);
					const sameStreet =
...
export async function refreshLegacyMapaSnapshot({
	...
	const legacyOrdens = Array.isArray(ordens)
		? ordens
		: await loadAllLegacyOrdens();
	const summary = buildPublicMapaSnapshot(legacyOrdens);
	const oldestPreview = [...legacyOrdens]
		.sort((a, b) =>
			String(a.dataAberturaSort || "").localeCompare(
				String(b.dataAberturaSort || ""),
			),
		)
		.slice(0, 120)
...
export async function loadLegacyDashboardDoc() {
	return getPublicVpsDocument(`public_dashboard/${LEGACY_PUBLIC_DOC}`).catch(
		() => null,
	);
}

export async function loadLegacyOrdensPage({
	after = null,
