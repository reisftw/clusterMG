...
export function formatarRegionalWhatsapp(regional, cidades) {
	...
	const cidadesOrdenadas = Object.entries(cidades).sort((a, b) => {
		const totalA =
			Object.values(a[1].pendente || {}).reduce((s, v) => s + v, 0) +
			Object.values(a[1].aguardando || {}).reduce((s, v) => s + v, 0);
		const totalB =
			Object.values(b[1].pendente || {}).reduce((s, v) => s + v, 0) +
	...
	cidadesOrdenadas.forEach(([cidade, data]) => {
		const partesPendente = Object.entries(data.pendente || {})
			.map(([tipo, qtd]) => `_${tipo}:_ *${qtd}*`)
			.join(" | ");
		const partesAguardando = Object.entries(data.aguardando || {})
			.map(([tipo, qtd]) => `_${tipo}:_ *${qtd}*`)
			.join(" | ");

		const todasPartes = [partesPendente, partesAguardando]
			.filter(Boolean)
			.join(" | ");
...
export function formatarAgenteWhatsapp(cidade, data) {
	...
	const total =
		Object.values(data.pendente || {}).reduce((s, v) => s + v, 0) +
