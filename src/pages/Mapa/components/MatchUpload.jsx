      ...
      <div className="fixed inset-0 z-layout-modal flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Informe o período usado {isMultiplo ? "nas planilhas" : "na planilha"} do
            Match antes de importar.
      ...
      <div className="fixed inset-0 z-layout-modal flex items-center justify-center p-4">
        ...
        >
          <div className="mb-5 flex items-start justify-between gap-3">
            <div>
              <h3 id="match-fields-help-title" className="font-bold text-gray-900">
          ...
          <div>
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200">
              {camposObrigatorios.map(([campo, descricao]) => (
                ...
                >
                  <code className="text-xs font-semibold text-emerald-700">{campo}</code>
          ...
          <p className="mt-4 text-xs leading-5 text-gray-500">
            Preencha <strong>coordenadas</strong> no formato latitude, longitude para que
            o sistema consiga localizar O.S. próximas.
          </p>
                ...
                {extras.length ? (
                  extras.map((tipo) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setExtras((current) => current.filter((item) => item !== tipo))
                      }
  ...
  return (
    <div className={`min-w-0 rounded-2xl border p-4 ${toneClass}`}>
      <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="min-w-0 text-sm font-bold leading-5 [overflow-wrap:normal]">{titulo}</p>
        <button
          type="button"
          onClick={copiarTexto}
  ...
  return (
    <div className="w-full rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-black">{job.stage || "Processando no backend"}</p>
...
export default function MatchUpload({ onConcluido }) {
  ...
  async function handleConfirmar(periodo) {
    ...
    try {
      const res = await processarUploadMatch(filesPendentes, setProgresso, periodo, setJobStatus);
      setResultado(res);
      if (Array.isArray(res?.tiposIgnoradosAdicionais)) {
        setMatchConfig({
          tiposIgnoradosAdicionais: res.tiposIgnoradosAdicionais,
          tiposIgnoradosAplicados:
            res.tiposIgnoradosAplicados?.length
          ...
          <>
            <div className="flex gap-2 flex-wrap">
              <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg px-3 py-1 text-xs font-semibold">
                ...
                {resultado.fonteLabel || "SEMPRE + ONNET"} · Match: {resultado.total}
              </span>
              {resultado.totalGeral && resultado.totalGeral !== resultado.total ? (
            ...
            {Object.keys(resultado.ignoradasTipos || {}).length ? (
              <div className="flex flex-wrap gap-2">
                {Object.entries(resultado.ignoradasTipos).map(([tipo, total]) => (
                  <span
                    key={tipo}
