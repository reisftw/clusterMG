...
function readLocalStorageCache() {
  ...
  if (window.localStorage.getItem(STATIC_DATA_CACHE_DAY_KEY) !== getTodayKey()) {
    return null;
  }
  ...
  pendingRequest = Promise.resolve()
    ...
    .then((cachedJson) => {
      ...
      return fetch(buildVpsDashboardUrl(), {
        ...
        .catch((error) => {
          logger.warn("[useDashboardData] VPS indisponivel, usando JSON publico legado.", error);
...
export function useDashboardData(options = {}) {
  ...
  useEffect(() => {
    ...
    const unsubscribeRealtime = subscribeRealtimeTopics(
      ...
      (event) => {
        invalidateDashboardDataCache(event?.emittedAt || new Date().toISOString());
        loadData(true, { silent: true });
      },
      { debounceMs: 250 },
    );
