...
function normalizeDaily(totalDaily) {
  if (Array.isArray(totalDaily)) return totalDaily.map((v) => Math.round(Number(v) || 0));
...
function recalcPcts(data) {
  ...
  for (const month of Object.keys(out)) {
    const d = out[month]; if (!d) continue;
    ...
    d.cidades.forEach((cidade) => {
      ...
      cidade.pct   = cancelamentos > 0
        ? Number(((realizado / cancelamentos) * 100).toFixed(1)) : 0;
    });

    d.cidadesRanking = [...d.cidades].sort((a, b) => b.realizado - a.realizado);
    const dailySum   = d.totalDaily.reduce((acc, v) => acc + (Number(v) || 0), 0);
    d.totalRealizado = dailySum > 0 ? dailySum : Math.round(Number(d.totalRealizado) || 0);
    ...
    d.percentAchieved   = d.totalCancelamentos > 0
      ? Number(((d.totalRealizado / d.totalCancelamentos) * 100).toFixed(1)) : 0;
    d.status = d.percentAchieved >= 80
      ? "Meta atingida!"
