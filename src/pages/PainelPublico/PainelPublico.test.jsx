...
describe("PainelPublico", () => {
  ...
  it("renderiza a aba de retiradas e permite alterar o mes pelo cabecalho", async () => {
    ...
    expect(screen.getByTestId("tab-retiradas")).toHaveTextContent("Retiradas Março");

    await user.click(screen.getByRole("button", { name: /trocar mes/i }));

    expect(screen.getByTestId("tab-retiradas")).toHaveTextContent("Retiradas Janeiro");
  });
  ...
  it("usa o mes mais recente com dados quando o mes atual nao possui indicadores", async () => {
    ...
    expect(screen.getByTestId("tab-retiradas")).toHaveTextContent("Retiradas Julho");
  });
