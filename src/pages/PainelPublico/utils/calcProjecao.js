...
function buildProjectionSeries({ month, year, feriadosSet, projection, totalAtual }) {
...
export async function calcSaldoDiario(d, month, feriadosInput = null) {
  ...
  const lista = dias.map(r => {
    ...
    const feriado = monthIndex >= 0 && feriadosSet.has(formatarChaveFeriado(monthIndex + 1, dia));
...
export async function calcProjecao(d, month, feriadosInput = null) {
  ...
  const saldoProjecao = Array.isArray(d?.saldoDiario) && d.saldoDiario.length
    ? d.saldoDiario
    : lista;
  ...
  return {
    ...
    pctProjecao:
      Number(d.meta || 0) > 0
        ? ((Number(projection.projecaoFinal || 0) / Number(d.meta || 0)) * 100).toFixed(1)
        : 0,
