...
function getMonthsFromCurrent(month, allData) {
  ...
  return Array.from(
    ...
    (_, offset) =>
      MONTH_ORDER[(currentIndex - offset + MONTH_ORDER.length) % MONTH_ORDER.length],
  ).filter((item) => Number(getMonthRecord(allData, item)?.totalRealizado || 0) > 0);
}
...
function getFilteredSummary(cidades = [], fallback = {}) {
  ...
  const totals = cidades.reduce(
    (acc, cidade) => ({
      ...
      totalCancelamentos: acc.totalCancelamentos + Number(cidade.cancelamentos || 0),
...
function StatusMap({ cidades = [] }) {
  ...
  return (
    <div className="agentes-status-map">
      {groups.map((group) => (
        <section className={`agentes-status-group ${group.tone}`} key={group.key}>
...
export default function TabAgentes({ allData, month, lastUpdate }) {
  ...
  const cidades = useMemo(() => {
    ...
    return [...base].sort((a, b) => Number(b.realizado || 0) - Number(a.realizado || 0));
  }, [d]);

  const filteredCities = useMemo(() => {
    return cidades.filter((cidade) => {
      const cityMatches =
        cityFilter === "todas" || normalizeMonthName(cidade.nome) === cityFilter;
      const statusMatches =
  ...
  const abaixoCount = cidades.filter((cidade) => Number(cidade.pct || 0) < 50).length;
  const periodLabel = useMemo(() => getPeriodLabel(dataMonthKey), [dataMonthKey]);
        ...
        datasets: [
          {
            ...
            data: mesesComDados.map((m) =>
              Math.round(Number(getMonthRecord(allData, m)?.totalRealizado || 0)),
            ),
            backgroundColor: "#FF6B00",
            borderRadius: 8,
          },
    ...
    <div className="agentes-dashboard">
      ...
      <div className="kpis agentes-kpis">
        ...
        <KpiCard
          ...
          value={intFmt.format(Math.max(0, Math.round(Number(summary.totalFalta || 0))))}
      ...
      <div className="agentes-focus-grid">
        <section className="agentes-performance-card">
          ...
          <p>
            A carteira está em {pctFmt.format(Number(summary.percentAchieved || 0))}% da meta.
            {topCity
          ...
          <div className="agentes-main-progress">
            <span style={{ width: `${Math.min(Number(summary.percentAchieved || 0), 100)}%` }} />
            <strong>{pctFmt.format(Number(summary.percentAchieved || 0))}%</strong>
          </div>

          <div className="agentes-summary-row">
            <div>
              <span>Realizado</span>
              <strong>{intFmt.format(Number(summary.totalRealizado || 0))}</strong>
            </div>
            <div>
              <span>Meta</span>
              <strong>{intFmt.format(Math.round(Number(summary.totalMeta || 0)))}</strong>
            </div>
            <div>
              <span>Falta</span>
              <strong>{intFmt.format(Math.max(0, Math.round(Number(summary.totalFalta || 0))))}</strong>
            </div>
          </div>
      ...
      <div className="agentes-secondary-grid">
        <section className="card">
          ...
          <RankingList items={rankItems} metaRef={Number(summary.totalMeta) || 0} label="retiradas" />
      ...
      <section className="card agentes-chart-wide">
        <div className="card-title">Evolução Diária Acumulada — Total Geral</div>
