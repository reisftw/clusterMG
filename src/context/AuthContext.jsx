...
export const AuthProvider = ({ children }) => {
  ...
  const isAdmin = String(auth.currentUser?.role || "").toLowerCase() === ROLES.ADMIN;
  const effectiveRole = isAdmin && viewAsRole ? viewAsRole : auth.currentUser?.role;
  const safeViewAsRoles = Array.isArray(viewAsRoles) ? viewAsRoles : EMPTY_VIEW_AS_ROLES;
  ...
  const effectiveUser = useMemo(
    () => {
      ...
      if (isAdmin && viewAsRole) {
        ...
        const { permissions: _permissions, isAdmin: _isAdmin, ...user } = auth.currentUser;
        return { ...user, role: effectiveRole, permissions: selectedViewAsRole?.permissions || [] };
      }
      return { ...auth.currentUser, role: effectiveRole };
    },
    [auth.currentUser, effectiveRole, isAdmin, selectedViewAsRole?.permissions, viewAsRole, viewAsRolesLoading],
  );
  const value = useMemo(
    () => ({
      ...auth,
    ...
    [auth, effectiveUser, effectiveRole, isAdmin, safeViewAsRoles, viewAsRole, viewAsRolesLoading],
  );
