...
function TopicSwitch({ checked, label, description, onChange }) {
  return (
    <label className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
    ...
    <div className="mt-4 flex flex-col gap-3">
      {(!support.supported || !support.hasNotificationApi) && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">
          Este dispositivo nao oferece suporte completo a notificacoes
          push.
        </div>
      )}
...
function NotificationsModalHeader({ enabled, support, activeTopicsCount, onClose }) {
  return (
    <div className="shrink-0 bg-[linear-gradient(135deg,#0b1f6a_0%,#1447b8_100%)] px-6 py-5 text-white">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/70">Status</p>
          <p className="mt-1 text-sm font-semibold text-white">{enabled ? "Ativado" : "Desativado"}</p>
        </div>
        <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/70">Permissao</p>
          <p className="mt-1 text-sm font-semibold text-white">{getPermissionLabel(support.permission)}</p>
        </div>
        <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/70">Canais ativos</p>
          <p className="mt-1 text-sm font-semibold text-white">{activeTopicsCount} de {PUSH_TOPIC_OPTIONS.length}</p>
        </div>
      </div>
    </div>
  );
...
function NotificationsModalBody({ enabled, support, topics, message, onToggleTopic }) {
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50 px-6 py-5">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`shrink-0 rounded-full border px-3 py-1 text-[11px] font-bold uppercase tracking-[0.1em] ${getStatusTone(enabled)}`}>
...
function NotificationsModalFooter({ enabled, busy, canActivate, onDisable, onPrimaryAction }) {
  return (
    <div className="shrink-0 flex flex-col gap-3 border-t border-slate-200 bg-white px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      >
        <Check size={16} />
        {busy ? "Salvando..." : enabled ? "Salvar preferencias" : "Ativar notificacoes"}
      </button>
    </div>
  );
}
