...
const Topbar = ({ onOpenMobileMenu }) => {
  ...
  const avatarSrc = displayUser?.avatarUrl || displayUser?.avatarDataUrl || defaultAvatarUrl || "";

  const publishCounters = useCallback((counters = {}) => {
    window.dispatchEvent(new CustomEvent("retiradas:notification-counters", { detail: counters }));
  }, []);
  ...
  const loadNotifications = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const visibleByPreference = nextItems.filter((item) => enabledTypes[item.type] !== false);
      const nextUnreadCount = visibleByPreference.filter((item) => !item.read).length;
      const latestCreatedAt = nextItems[0]?.createdAt || "";
      const hasNewItem = latestCreatedAt && lastSeenCreatedAtRef.current && latestCreatedAt !== lastSeenCreatedAtRef.current;
      ...
      if (hasNewItem && !silent) {
        ...
        window.dispatchEvent(new CustomEvent("retiradas:documentos-pendentes-updated"));
      }
  ...
  useEffect(() => {
    obterPreferenciasNotificacoes()
      .then((preferences) => {
        ...
        setDefaultAvatarUrl(preferences?.defaultAvatarUrl || preferences?.defaultAvatarDataUrl || "");
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const handlePreferenceUpdate = (event) => {
      ...
      setDefaultAvatarUrl(nextPreferences.defaultAvatarUrl || nextPreferences.defaultAvatarDataUrl || "");
    };
    window.addEventListener("retiradas:notification-preferences-updated", handlePreferenceUpdate);
    return () => {
      window.clearTimeout(initialTimer);
      window.clearInterval(interval);
      window.removeEventListener("retiradas:notification-preferences-updated", handlePreferenceUpdate);
    };
  }, [loadNotifications]);
            ...
            <div className="absolute right-0 top-12 z-layout-dropdown w-[min(92vw,390px)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 p-4">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500">Atualiza automaticamente a cada 30s</p>
                </div>
                <button
                  type="button"
                  onClick={markAllRead}
                ...
                {notifications.length ? notifications.map((item) => (
                  <Link
                    ...
                    className={`block rounded-xl border p-3 transition hover:bg-slate-50 ${
                      item.read ? "border-transparent" : "border-orange-200 bg-orange-50/60"
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-sm font-black text-slate-950">{item.title}</p>
                      <span className="shrink-0 text-[11px] font-bold text-slate-400">{formatNotificationDate(item.createdAt)}</span>
                    </div>
                    <p className="mt-1 text-xs font-semibold leading-relaxed text-slate-600">{item.message}</p>
                  </Link>
                )) : (
