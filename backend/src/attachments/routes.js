...
async function assertEntityAccess(client, req, entityType, entityId, mode = "view") {
	...
	if (entityType === "SST_PROTOCOL") {
		const { rows } = await client.query("select * from sst_protocols where id=$1", [entityId]);
		const item = rows[0];
		if (!item) fail(404, "Protocolo não encontrado.");
		...
		`select a.* from rot_aprs a
		 where a.id=$1
		   ...
		   and (a.author_id=$3 or $4::boolean or exists (
		     ...
		     where rr.regional_id=a.regional_id and rr.source_payload->>'userId'=$3
