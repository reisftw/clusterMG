...
// Formata a data local (nao UTC) no mesmo formato ISO (YYYY-MM-DD) usado
// em row.data, pra comparar como string sem depender de fuso horario do
// Date/toISOString (que usa UTC e pode "voltar" um dia perto da meia-noite).
...
function shouldKeepAccessImportedRow(row = {}) {
	...
	// Engenharia reversa do Access (query Cria_APagar): WHERE F15 >= Now()
	// — so entra em "Contas a Pagar" (PREVISTO) quem tem Dt. Prev. Pgto
	// ainda no futuro (ou hoje). Sem esse corte, o Finan mantinha titulos
	// ja vencidos/pagos no passado (confirmado com a planilha real: 10
	// linhas com Dt. Prev. Pgto entre julho e inicio de setembro, R$
	// 12.532,16, que o Access ja tinha descartado por estarem vencidas).
	// row.data e sempre a Dt. Prev. Pgto quando presente (ver
	// normalizeFpcp302Row: dueDateInfo tem prioridade sobre a data base),
	// e comparacao textual "YYYY-MM-DD" >= "YYYY-MM-DD" funciona como
	// comparacao cronologica.
	if (row.data && row.data < todayLocalIsoDate()) return false;
