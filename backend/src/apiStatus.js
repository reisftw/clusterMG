...
async function buildApiStatus() {
	...
	const services = await Promise.all([
		...
		checkService("public-data", "API de Dados Administrativos", async () => {
		...
		checkService("imports", "API de Importações Administrativas", async () => {
			const result = await db.query(
				`select collection_path as "collectionPath", count(*)::bigint as total
           from app_documents
          ...
          order by collection_path`,
				[
					[
						"documentos_empresas_envios",
						"documentos_empresas_arquivos",
						"empresas_tecnicos",
