...
router.post("/login", loginRateLimit, async (req, res, next) => {
	try {
		...
		// SEC-004: sessao vai so no cookie HttpOnly — nao devolve mais o token
...
router.post("/mfa/email/verify", mfaVerifyRateLimit, async (req, res, next) => {
	try {
...
router.post("/password/forgot", passwordResetRateLimit, async (req, res, next) => {
	try {
		const email = String(req.body?.email || "").trim().toLowerCase();
		if (!email) {
...
router.post("/password/reset", passwordResetRateLimit, async (req, res, next) => {
	try {
