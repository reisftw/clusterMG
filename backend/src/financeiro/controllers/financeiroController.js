...
const finan_READ_CACHE_TTL_MS = Math.max(
	Number(process.env.finan_READ_CACHE_TTL_MS || 10000),
	0,
);
const financeiroReadCache = new Map();

function getCachedFinanceiroResponse(key) {
	if (!finan_READ_CACHE_TTL_MS || !key) return null;
	const cached = financeiroReadCache.get(key);
	if (!cached) return null;
	if (Date.now() - cached.createdAt > finan_READ_CACHE_TTL_MS) {
	...
	async function getBudgetCostCenters(req, res, next) {
		try {
			...
			const cacheKey = [
				...
				query.startDate || "",
				query.endDate || "",
				// Faltavam aqui (bug real, achado ao implementar o modo leve pro
				// teste de carga k6, 2026-09-10): sem isso, uma chamada com
				// `incluirMovimentacoes=1&centroCustoId=X` (o modal "Ver
				// detalhes" de UM centro) tinha a MESMA cache key que a chamada
				// padrão da lista — quem chegasse primeiro "vencia" o cache pros
				// dois, devolvendo ou o payload errado (leve demais pro modal,
				// sem `movements`) ou o cache pesado sendo reaproveitado à toa
				// pra lista inteira.
				query.incluirMovimentacoes || "",
				query.centroCustoId || "",
			].join(":");
			res.json(
