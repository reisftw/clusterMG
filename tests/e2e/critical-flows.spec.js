...
test.describe("fluxos críticos E2E de segurança", () => {
	test.beforeEach(() => {
		test.skip(
			!HAS_E2E_TARGET,
			"Defina E2E_BASE_URL para rodar contra staging/local.",
		);
	});
	...
	test("login válido autentica sessão quando credenciais de staging forem fornecidas", async ({
		request: api,
	}) => {
		test.skip(
			!LOGIN_EMAIL || !LOGIN_PASSWORD,
			"Defina E2E_LOGIN_EMAIL/E2E_LOGIN_PASSWORD em staging para rodar o login real.",
		);
	...
	test("RBAC permite admin em rota administrativa", async () => {
		test.skip(
			!ADMIN_TOKEN,
			"Defina E2E_ADMIN_TOKEN ou AUTH_TOKEN para rodar E2E autenticado.",
		);
	...
	test("RBAC bloqueia visitante em rota administrativa", async () => {
		test.skip(
			!VISITOR_TOKEN,
			"Defina E2E_VISITOR_TOKEN para validar negação por papel real.",
		);
	...
	test("abre leituras críticas autenticadas sem erro 5xx", async () => {
		...
		const endpoints = [
			...
			[
				"/api/agendamentos/confirmacao/envios?limit=5&offset=0",
				"envios de mensageria",
			],
	...
	test("ações críticas rejeitam requisições sem CSRF", async () => {
		...
		const probes = [
			...
			[
				"/api/documentos/submissions/e2e-invalid/review",
		...
		for (const [endpoint, label] of probes) {
			...
			expect(
				response.status(),
				`${label} deve exigir CSRF/permissão`,
	...
	test("operações de escrita críticas rodam somente em staging mutável", async () => {
		test.skip(
			!RUN_MUTATIONS,
			"Defina E2E_MUTATION_ENABLED=1 somente em staging preparado para mutações.",
		);
		test.skip(
			!ADMIN_TOKEN,
			"Defina E2E_ADMIN_TOKEN ou AUTH_TOKEN para rodar E2E autenticado.",
		...
		const invalidFinanceiro = await writeApi.post("/api/financeiro/mockup", {
			data: { invalid: true },
		});
		expect(invalidFinanceiro.status()).toBeLessThan(500);

		const invalidAtendimento = await writeApi.post(
			"/api/atendimento/cases/e2e-invalid/reply",
			{
				data: { message: "" },
			},
		);
