...
function assertBaseConfig(config = {}) {
  ...
  if (missing.length) {
    const error = new Error(`Configure Senior/Sapiens antes de validar: ${missing.join(", ")}.`);
...
async function testConnection() {
  ...
  if (!response.ok) {
    const message = payload?.message || payload?.error || payload?.raw || `Senior/Sapiens HTTP ${response.status}`;
