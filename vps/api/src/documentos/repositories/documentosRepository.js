...
async function createRequiredField(field = {}) {
  const result = await db.query(
    `insert into document_required_fields (nome, obrigatorio, ordem, publico_alvo, ativo)
     values ($1, $2, $3, $4, $5)
     returning *`,
    [field.nome, Boolean(field.obrigatorio), Number(field.ordem || 0), field.publicoAlvo || "ambos", field.ativo !== false],
  );
  return mapRequiredField(result.rows[0]);
}

async function updateRequiredField(id, field = {}) {
  const current = await db.query(`select * from document_required_fields where id = $1`, [id]);
  ...
  const result = await db.query(
    `update document_required_fields set
     ...
     where id = $1
     returning *`,
    [id, next.nome, Boolean(next.obrigatorio), Number(next.ordem || 0), next.publicoAlvo || "ambos", next.ativo !== false],
  );
  return mapRequiredField(result.rows[0]);
}
...
async function listSubmissions({ status, statuses, empresaId, regional, submittedBy, limit = 20, offset = 0 } = {}) {
  const filters = [];
  const params = [];
...
async function getSubmission(id) {
  const result = await db.query(`select * from document_submissions where id = $1`, [id]);
  return mapSubmission(result.rows[0]);
}
...
async function getFile(id) {
  const result = await db.query(`select * from document_files where id = $1`, [id]);
  return mapFile(result.rows[0]);
}
...
async function deleteFile(id) {
  const result = await db.query(`delete from document_files where id = $1 returning *`, [id]);
  return mapFile(result.rows[0]);
}
