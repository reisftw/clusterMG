...
function createZipArchive(options = {}) {
  if (typeof createArchiver === "function") return createArchiver("zip", options);
  if (typeof archiverModule.ZipArchive === "function") return new archiverModule.ZipArchive(options);
  const error = new Error("Dependencia archiver carregada em formato invalido. Rode npm install --omit=dev na VPS.");
...
async function getEmpresa(empresaId) {
  ...
  return {
    ...
    responsavelNome: text(data.responsavel?.nome || data.responsavel_nome || data.nomeResponsavel),
    responsavelEmail: text(data.responsavel?.email || data.email || data.emailResponsavel),
  };
}
...
async function getRegionalByName(regionalName) {
  ...
  const regionais = await documents.listAllDocuments("regionais").catch(() => []);
  const match = regionais.find((item) => normalize(item?.data?.nome || item?.documentId) === target);
...
async function enrichSubmission(submission) {
  ...
  const supervisorNome = text(submission.supervisorNome || empresa?.supervisorNome || regionalSupervisor.nome);
  const supervisorEmail = text(submission.supervisorEmail || empresa?.supervisorEmail || regionalSupervisor.email);
  const supervisorTelefone = text(regionalSupervisor.telefone || regionalSupervisor.whatsapp || regionalSupervisor.celular);
  const submittedAt = submission.submittedAt ? new Date(submission.submittedAt) : null;
  const tempoSupervisorMs = submittedAt && !Number.isNaN(submittedAt.getTime())
...
function canAccessEmpresa(user, empresa) {
  ...
  if (["admin", "backoffice_retirada", "supervisor_administrativo", "analista_administrativo"].includes(role)) return true;
  if (role === "supervisor") return normalize(user?.regional || user?.profile?.regional) === normalize(empresa.regional);
  if (["lider_empresa", "agente_autorizado"].includes(role)) {
    ...
    const empresaNome = normalize(user?.empresaNome || user?.empresa_nome || user?.profile?.empresaNome || user?.profile?.empresa_nome);
    return empresaId === empresa.id || (empresaNome && empresaNome === normalize(empresa.nome));
  }
  return false;
}
...
function canApprove(user, file) {
  ...
  return normalize(user?.regional || user?.profile?.regional) === normalize(file.regional);
}

function canAdministrativeReview(user) {
  return ["admin", "supervisor_administrativo", "analista_administrativo"].includes(normalize(user?.role));
}

function canViewSupervisorTreatments(user) {
  return ["admin", "supervisor", "supervisor_administrativo", "analista_administrativo"].includes(normalize(user?.role));
}

function requireAdminUser(user) {
  if (["admin", "supervisor_administrativo"].includes(normalize(user?.role))) return;
...
async function requireEmpresaAccess(user, empresaId) {
  ...
  if (!canAccessEmpresa(user, empresa)) {
    const error = new Error("Sem permissão para acessar documentos desta empresa.");
    error.statusCode = 403;
    throw error;
  }
  return empresa;
...
async function ensureClientFolder(empresa, user) {
  ...
  if (existing?.driveFolderId && existing.rootDriveFolderId === rootDriveFolderId) return existing;
  const folder = await drive.createFolder(`${empresa.nome} - ${empresa.id}`, rootDriveFolderId);
...
async function ensureMonthFolder(empresa, mesReferencia, user) {
  ...
  const monthFolder = await drive.createFolder(mesReferencia, folder.driveFolderId);
...
async function listCompanyDriveFolder({ empresaId, folderId, user }) {
  const { clientFolder, targetFolderId } = await resolveCompanyBrowserFolder({ empresaId, folderId, user });
  const folderMeta = await drive.getFileMetadata(targetFolderId, "id,name,mimeType,parents,createdTime,modifiedTime");
...
async function downloadCompanyDriveFile({ empresaId, fileId, folderId, user }) {
  const { targetFolderId } = await resolveCompanyBrowserFolder({ empresaId, folderId, user });
  const metadata = await drive.getFileMetadata(fileId, "id,name,mimeType,size,parents");
  if (!Array.isArray(metadata.parents) || !metadata.parents.includes(targetFolderId)) {
...
function normalizeMesReferencia(value) {
  ...
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(textValue)) {
    const error = new Error("Mês de referência inválido. Use o formato AAAA-MM.");
    error.statusCode = 400;
    throw error;
  }
  return textValue;
...
function computeSubmissionStatus(files = []) {
  ...
  const adminStatuses = latestFiles.map((item) => normalize(item.adminStatus || "pendente"));
...
async function markMissingDriveFileForResubmission(submission, file) {
  const motivo = "Arquivo aprovado não foi encontrado no Google Drive. Reenvie este documento.";
  ...
  await repository.updateSubmissionOnly(submission.id, {
    status: nextStatus,
    motivoReprovacao: latestFiles
      .filter((item) => normalize(item.status) === "reprovado")
      .map((item) => `${item.fieldNome || item.nome}: ${item.motivoReprovacao || "-"}`)
      .join("\n") || null,
...
async function sendDocumentEmail({ to, subject, title, lines, actionPath = "/documentos/pendentes", meta = {} }) {
  const target = text(to);
  if (!target) return null;
  const config = await emailService.getConfig().catch(() => null);
  const appUrl = String(config?.appUrl || process.env.PUBLIC_APP_URL || "https://retiradas.tech").replace(/\/+$/, "");
  ...
  return emailService.sendMail({
    ...
    html: emailService.createEmailTemplate({
      ...
      buttonUrl: actionUrl,
      infoBox: meta?.event === "file_rejected"
...
async function notifySubmissionReviewed(submission) {
  return sendDocumentEmail({
    ...
    title: submission.status === "aprovado" ? "Documentos aprovados" : "Documentos reprovados",
    lines: [
      `Empresa: ${submission.empresaNome}`,
      `Mês de referência: ${submission.mesReferencia}`,
      `Status: ${submission.status}`,
      submission.status === "reprovado" ? `Motivo: ${submission.motivoReprovacao || "-"}` : "Tudo certo com os documentos enviados.",
    ],
...
async function notifyAdministrativeQueue(submission) {
  await notificationsService.createNotification({
    ...
    targetPath: "/documentos/pendentes",
    severity: "info",
    targets: { roles: ["admin", "supervisor_administrativo", "analista_administrativo"] },
    meta: {
      submissionId: submission.id,
      empresaId: submission.empresaId,
      empresaNome: submission.empresaNome,
...
async function notifyDocumentRejected(submission, file) {
  return sendDocumentEmail({
    ...
    meta: { submissionId: submission.id, fileId: file.id, event: "file_rejected" },
  });
}
...
function notifyInBackground(promiseFactory) {
  Promise.resolve()
    .then(promiseFactory)
    .catch((error) => {
      console.warn("[documentos] Falha ao enviar notificacao:", error?.message || error);
    });
}
...
function fieldAppliesToAudience(field, audience) {
  const target = normalizePublicoAlvo(field?.publicoAlvo || field?.publico_alvo || "ambos");
  return target === "ambos" || target === audience;
}

async function saveRequiredField({ id, nome, obrigatorio = true, ordem = 0, publicoAlvo = "ambos", ativo = true }) {
  ...
  if (id) return repository.updateRequiredField(id, { nome: cleanName, obrigatorio, ordem, publicoAlvo: target, ativo });
  return repository.createRequiredField({ nome: cleanName, obrigatorio, ordem, publicoAlvo: target, ativo });
}
...
function normalizeBillingConfig(input = {}) {
  const firstReminderDay = Math.min(Math.max(Number(input.firstReminderDay || 25), 1), 31);
  const repeatUntilDay = Math.min(Math.max(Number(input.repeatUntilDay || 31), firstReminderDay), 31);
  ...
  return {
    ...
    repeatUntilDay,
    windowStart: windowPattern.test(text(input.windowStart)) ? text(input.windowStart) : "08:00",
    windowEnd: windowPattern.test(text(input.windowEnd)) ? text(input.windowEnd) : "18:00",
    emailSubject: text(input.emailSubject) || DEFAULT_BILLING_CONFIG.emailSubject,
    financeEmailTemplate: text(input.financeEmailTemplate) || DEFAULT_BILLING_CONFIG.financeEmailTemplate,
...
function shouldRunBilling(config, now = new Date(), force = false) {
  ...
  const inDay = currentDay === Number(config.firstReminderDay)
    || (config.repeatDailyAfterFirst && currentDay > Number(config.firstReminderDay) && currentDay <= Number(config.repeatUntilDay));
  if (!inDay) return false;
  const currentTime = timeStamp(now);
  return currentTime >= config.windowStart && currentTime <= config.windowEnd;
}

async function runBillingNotifications({ force = false, now = new Date() } = {}) {
  ...
  const fields = (await repository.listRequiredFields()).filter((field) => field.obrigatorio);
  if (!fields.length) return { ok: true, skipped: true, reason: "sem_campos_obrigatorios", sent: 0 };
  ...
  for (const empresaDoc of empresas) {
    ...
    try {
      await sendDocumentEmail({
        ...
        actionPath: "/terceirizados/login",
        meta: { event: "billing_reminder", empresaId: empresa.id, mesReferencia: month },
      });
      await documents.upsertDocument({
        path: logPath,
      ...
      errors.push({ empresaId: empresa.id, error: error?.message || String(error) });
    }
  }
...
async function createMonthlySubmission({ mesReferencia, files = [], fieldIds = [], user }) {
  const role = normalize(user?.role);
  if (!["lider_empresa", "agente_autorizado", "admin"].includes(role)) {
    const error = new Error("Apenas líderes de empresa ou agentes autorizados podem enviar documentos por aqui.");
    error.statusCode = 403;
    throw error;
  }
  const empresaId = text(user?.empresaId || user?.empresa_id || user?.profile?.empresaId || user?.profile?.empresa_id);
  ...
  const activeFields = (await repository.listRequiredFields())
    .filter((field) => fieldAppliesToAudience(field, audience));
  const fieldMap = new Map(activeFields.map((field) => [field.id, field]));
  let submission = await repository.getLatestSubmissionByEmpresaMes(empresa.id, month);
  const existingFiles = submission ? await repository.listSubmissionFiles(submission.id) : [];
  if (submission && computeSubmissionStatus(existingFiles) === "aprovado") {
    const error = new Error("Os documentos deste mês já foram aprovados. Não é possível enviar novos documentos para este mês.");
    error.statusCode = 400;
    throw error;
  }
  if (submission && computeSubmissionStatus(existingFiles) === "aguardando_administrativo") {
    const error = new Error("Os documentos deste mês já foram aprovados pelo supervisor e aguardam aprovação administrativa.");
  ...
  if (missing.length) {
    const error = new Error(`Documentos obrigatorios ausentes: ${missing.map((item) => item.nome).join(", ")}.`);
    error.statusCode = 400;
    throw error;
  }
  if (!files.length) {
  ...
  const updatedSubmission = await repository.updateSubmissionOnly(submission.id, {
    status: nextStatus,
    motivoReprovacao: nextStatus === "reprovado"
    ...
    reviewedByName: nextStatus === "pendente" ? null : submission.reviewedByName,
...
async function listSubmissions({ status, mine = false, empresaId, limit, offset, user }) {
  ...
  if (normalize(status) === "pendente") {
    ...
    if (["supervisor_administrativo", "analista_administrativo"].includes(role)) {
  ...
  if (["lider_empresa", "agente_autorizado"].includes(role) || mine) {
    filters.empresaId = text(user?.empresaId || user?.empresa_id || user?.profile?.empresaId || user?.profile?.empresa_id);
  }
  const submissions = await repository.listSubmissions(filters);
  return Promise.all(submissions.map(enrichSubmission));
}

async function listSupervisorTreatments({ limit, offset, user }) {
  if (!canViewSupervisorTreatments(user)) {
    const error = new Error("Sem permissão para visualizar tratativas de documentos.");
    error.statusCode = 403;
    throw error;
  }
...
async function reviewSubmission({ id, status, motivo, user }) {
  const submission = await getSubmissionWithFiles({ id, user });
  if (!canApprove(user, { regional: submission.regional, empresaId: submission.empresaId })) {
...
async function uploadDocument({ empresaId, file, tipo, user }) {
  ...
  try {
    ...
    const uploadError = new Error(`Falha ao enviar arquivo para o Google Drive: ${error.message}`);
    uploadError.statusCode = 502;
    throw uploadError;
  }

  if (!uploaded?.id) {
    const error = new Error("Google Drive não retornou o ID do arquivo enviado.");
  ...
  try {
    ...
    const metadataError = new Error(`Arquivo enviado ao Drive, mas falhou ao salvar metadados no banco: ${error.message}`);
...
async function listDocuments({ status, empresaId, limit, offset, user }) {
  ...
  if (role === "supervisor") filters.regional = text(user?.regional || user?.profile?.regional);
  if (role === "lider_empresa") {
    filters.empresaId = text(user?.empresaId || user?.empresa_id || user?.profile?.empresaId || user?.profile?.empresa_id || empresaId);
  }
...
async function downloadSubmissionZip({ id, user }) {
  ...
  for (const file of approvedFiles) {
    ...
    if (!file.driveFileId) {
      const error = new Error(`Documento aprovado sem arquivo no Google Drive: ${label}.`);
      error.statusCode = 502;
      throw error;
    }
    ...
    try {
      ...
      if (isDriveNotFoundError(error)) {
        ...
        const missingError = new Error(`O documento "${label}" estava aprovado, mas não existe mais no Google Drive. Ele foi marcado para reenvio e o mês voltou para pendente.`);
        missingError.statusCode = 409;
        throw missingError;
      }
      const downloadError = new Error(`Não foi possível baixar o documento "${label}" do Google Drive: ${error.message}`);
    ...
    zip.append(buffer, { name: `${baseName}${count ? ` (${count + 1})` : ""}.pdf` });
  }
...
async function updateApproval({ id, status, motivo, user }) {
  ...
  if (file.submissionId) {
    ...
    const nextStatus = computeSubmissionStatus(files);
    const updatedSubmission = await repository.updateSubmissionOnly(file.submissionId, {
      status: nextStatus,
...
async function purgeDocumentHistory({ deleteDriveFiles = false, resetFolders = false, confirmation = "", user }) {
  requireAdminUser(user);
  if (text(confirmation).toUpperCase() !== "ZERAR DOCUMENTOS") {
    const error = new Error("Digite ZERAR DOCUMENTOS para confirmar a limpeza.");
    error.statusCode = 400;
    throw error;
  }
