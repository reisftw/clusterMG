...
async function getDriveClient() {
  ...
  if (!keyFile) throw new Error("GOOGLE_APPLICATION_CREDENTIALS nao configurado.");
...
async function getGoogleDriveConfig() {
  ...
  return {
    folderId: normalizeDriveFolderId(stored.folderId || process.env.GOOGLE_DRIVE_FOLDER_ID),
    oauthClientId: cleanText(stored.oauthClientId || process.env.GOOGLE_OAUTH_CLIENT_ID),
    oauthClientSecret: cleanText(stored.oauthClientSecret || process.env.GOOGLE_OAUTH_CLIENT_SECRET),
    oauthRedirectUri: cleanText(stored.oauthRedirectUri || process.env.GOOGLE_OAUTH_REDIRECT_URI || getDefaultOAuthRedirectUri()),
    refreshToken: cleanText(stored.refreshToken || process.env.GOOGLE_DRIVE_REFRESH_TOKEN),
...
async function getRootFolderId() {
  ...
  if (!config.folderId) throw new Error("GOOGLE_DRIVE_FOLDER_ID nao configurado.");
...
async function getOAuthAuthUrl() {
  const client = await createOAuthClient();
  if (!client) {
    const error = new Error("Configure GOOGLE_OAUTH_CLIENT_ID e GOOGLE_OAUTH_CLIENT_SECRET.");
...
async function connectOAuth(code, user = {}) {
  ...
  if (!tokens.refresh_token) {
    const error = new Error("Google nao retornou refresh_token. Revogue o acesso do app na conta Google e conecte novamente.");
...
async function saveDriveConfig(config = {}, user = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(config, "oauthClientId")) {
    next.oauthClientId = cleanText(config.oauthClientId);
  }
  if (Object.prototype.hasOwnProperty.call(config, "oauthRedirectUri")) {
    next.oauthRedirectUri = cleanText(config.oauthRedirectUri) || getDefaultOAuthRedirectUri();
  }
  if (Object.prototype.hasOwnProperty.call(config, "oauthClientSecret")) {
  ...
  await saveStoredOAuthConfig({
    ...
    updatedConfigByName: user?.profile?.nome || user?.nome || user?.email || null,
  });
  resetDriveClient();
  return getDriveStatus();
}
...
async function getDriveStatus() {
  ...
  return {
    ...
    serviceAccountConfigured: Boolean(process.env.GOOGLE_APPLICATION_CREDENTIALS),
  };
}
...
async function getFileMetadata(fileId, fields = "id,name,mimeType,size,createdTime,modifiedTime,parents") {
  const drive = await getDriveClient();
