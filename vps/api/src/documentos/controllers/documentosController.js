...
async function listFields(req, res, next) {
	try {
		res.json({
			items: await service.listRequiredFields({
...
async function listInvoiceFields(req, res, next) {
	try {
		res.json({
			items: await service.listInvoiceFields({
				includeInactive: req.query.includeInactive === "true",
			}),
		});
	} catch (error) {
...
async function saveBillingConfig(req, res, next) {
	try {
		res.json({
			ok: true,
			config: await service.saveBillingConfig(req.body || {}, req.user),
		});
	} catch (error) {
		next(error);
	}
...
async function purgeHistory(req, res, next) {
	try {
		res.json(
			await service.purgeDocumentHistory({
...
async function getSubmission(req, res, next) {
	try {
		res.json(
			await service.getSubmissionWithFiles({
				id: req.params.id,
				user: req.user,
			}),
		);
...
async function downloadSubmissionZip(req, res, next) {
	try {
		...
		const zipError = new Error(
			`Falha ao gerar ZIP: ${error?.message || "erro desconhecido"}`,
		);
...
async function browseCompanyFolder(req, res, next) {
	try {
		res.json(
			await service.listCompanyDriveFolder({
				empresaId: req.params.empresaId,
...
async function downloadCompanyDriveFile(req, res, next) {
	try {
		...
		res.setHeader(
			"Content-Disposition",
			`${disposition}; filename*=UTF-8''${encodeURIComponent(file.nome)}`,
		);
		stream.on("error", next);
		stream.pipe(res);
	} catch (error) {
...
async function download(req, res, next) {
	try {
		...
		res.setHeader(
			"Content-Disposition",
			`attachment; filename="${encodeURIComponent(file.nome)}"`,
		);
		stream.pipe(res);
	} catch (error) {
		next(error);
