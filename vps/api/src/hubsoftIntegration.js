...
function normalizeConfig(config = {}) {
	return {
		...
		syncMode:
			cleanText(config.syncMode || "preview") === "production"
				? "production"
				: "preview",
		...
		syncFontes: Array.isArray(config.syncFontes)
			? config.syncFontes
					.map(cleanText)
					.filter((item) => ["sempre", "onnet"].includes(item))
			: ["sempre", "onnet"],
...
function assertAuthConfig(config = {}) {
	...
	if (missing.length) {
		const error = new Error(
			`Configure Hubsoft antes de validar: ${missing.join(", ")}.`,
		);
...
function buildHubsoftError(prefix, response, payload) {
	...
	const error = new Error(
		`${prefix}: Hubsoft HTTP ${response.status}${detail ? ` - ${detail}` : ""}`,
	);
...
async function hubsoftRequest(
	path,
	{ method = "GET", query = {}, body = null, retry = true } = {},
) {
...
async function searchOrdensServico(params = {}) {
	...
	const termoBusca = cleanText(
		params.termo_busca || params.termoBusca || params.termo,
	);
	if (!termoBusca && params.allowEmptyTerm !== true) {
		const error = new Error(
			"Informe o termo de busca para consultar O.S. no Hubsoft.",
		);
		error.statusCode = 400;
		throw error;
	}
...
function collectArrays(source, arrays = []) {
	...
	if (Array.isArray(source)) {
		if (
			source.length &&
			source.some((item) => item && typeof item === "object")
		)
			arrays.push(source);
...
function normalizeHubsoftStatus(value) {
	...
	if (raw.includes("aguardando") && raw.includes("agendamento"))
...
function parseHubsoftAddress(value) {
	...
	const endereco =
		streetPieces.length > 1 ? streetPieces.slice(0, -1).join(", ") : streetPart;
	return {
		raw: text,
...
function normalizeHubsoftOrderRow(order = {}) {
	const cliente =
		order.cliente ||
		order.cliente_servico?.cliente ||
		order.atendimento?.cliente ||
		{};
	const servico = order.cliente_servico || order.servico || {};
	const endereco =
		order.endereco ||
		order.endereco_instalacao ||
		servico.endereco ||
		cliente.endereco ||
		{};
	const clienteText =
		typeof cliente === "string"
			? parseHubsoftLabeledText(cliente)
			: { id: "", label: "" };
	const servicoText =
		typeof servico === "string"
			? parseHubsoftLabeledText(servico)
			: { id: "", label: "" };
	const enderecoText =
		typeof endereco === "string" ? parseHubsoftAddress(endereco) : null;
	const cidade =
		readPath(order, [
			"cidade",
			"cidade.nome",
			"endereco.cidade",
			"endereco_instalacao.cidade",
		]) ||
		readPath(servico, ["cidade", "cidade.nome"]) ||
		readPath(cliente, ["cidade", "cidade.nome"]) ||
		enderecoText?.cidade;
	const telefone =
		readPath(order, ["telefone", "telefone_primario", "celular"]) ||
	...
	return {
		...
		status: normalizeHubsoftStatus(
			readPath(order, [
				"status",
				"status.descricao",
				"ordem_servico_status.descricao",
			]),
		),
		cidade,
		regional: readPath(order, [
			"regional",
			"regional.nome",
			"regiao",
			"regiao.nome",
		...
		codigo_cliente:
			readPath(order, ["codigo_cliente"]) ||
			readPath(cliente, ["codigo_cliente", "codigo", "id_cliente", "id"]) ||
			clienteText.id,
		nome_razaosocial:
			readPath(order, ["nome_razaosocial", "cliente_nome"]) ||
			readPath(cliente, ["nome_razaosocial", "nome", "razao_social"]) ||
			clienteText.label,
		id_cliente_servico:
			readPath(order, ["id_cliente_servico"]) ||
			readPath(servico, ["id_cliente_servico", "id"]) ||
			servicoText.id,
		servico:
			readPath(order, ["plano", "plano.descricao"]) ||
			readPath(servico, ["servico", "plano", "plano.descricao", "descricao"]) ||
			servicoText.label,
		data_cadastro: readPath(order, [
			"data_cadastro",
			"data_abertura",
		...
		telefone_secundario: readPath(cliente, [
			"telefone_secundario",
			"telefone2",
		]),
		telefone_terciario: readPath(cliente, ["telefone_terciario", "telefone3"]),
		mac_addr:
			readPath(order, ["mac_addr", "mac", "mac_address"]) ||
			readPath(servico, ["mac_addr", "mac", "mac_address"]),
		phy_addr:
			readPath(order, ["phy_addr", "phy", "phy_address"]) ||
			readPath(servico, ["phy_addr", "phy", "phy_address"]),
		tecnico: readPath(order, [
			"tecnico",
			"tecnico.nome",
			"usuario.nome",
			"responsavel.nome",
...
async function fetchSyncRows(config, jobId) {
	const statuses = config.syncStatuses.length
		? config.syncStatuses
		: ["pendente", "aguardando_agendamento"];
...
async function getSyncJob(jobId) {
	const doc = await documents.getDocument(
		`${SYNC_JOB_COLLECTION}/${cleanText(jobId)}`,
	);
...
async function startSyncJob(options = {}, user = {}) {
	...
	setImmediate(() => {
		const promise = runSyncJob(id, options, user)
			.catch((error) =>
				console.error(`[hubsoftIntegration] Falha no job ${id}:`, error),
			)
...
async function runSyncJob(jobId, options = {}, user = {}) {
	...
	try {
		const config = await readConfig({ sanitized: false });
		const mode =
			cleanText(options.mode || config.syncMode || "preview") === "production"
				? "production"
				: "preview";
		...
		await updateSyncJob(jobId, {
			stage:
				mode === "production"
					? "Preparando persistência do mapa"
					: "Prévia gerada",
		...
		if (mode === "production") {
			const payload = {
				...
				fontes: config.syncFontes.length
					? config.syncFontes
					: ["sempre", "onnet"],
			};
