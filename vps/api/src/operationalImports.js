...
function normalizeMac(value) {
  const mac = String(value || "").replace(/[^a-fA-F0-9]/g, "").toUpperCase();
...
function getMapaFonteLabel(fontes = []) {
  ...
  if (normalized.includes("sempre") && normalized.includes("onnet")) return "SEMPRE + ONNET";
...
function formatDatePartsPtBr(year, month, day, hour = 0, minute = 0, second = 0) {
  ...
  function addRegional(data = {}) {
    ...
    (data.cidades || []).forEach((city) => {
      ...
      cityMap[key] = {
        ...
        agente: current.agente === true || city?.agente === true || kind.includes("agente"),
      };
    });
  }
...
async function loadMatchIgnoredTypes() {
  ...
  const extras = Array.isArray(config?.data?.tiposIgnoradosAdicionais)
    ...
    : [];
  const unique = [...new Set(extras.map((item) => String(item || "").trim()).filter(Boolean))];
  return { adicionais: extras, todos: unique };
}

async function buildOrdersFromRows(rows = [], fontesSelecionadas = ["sempre"], { forMatch = false } = {}) {
  const cityMap = await loadCityMap();
  const ignoredTypes = forMatch ? await loadMatchIgnoredTypes() : { adicionais: [], todos: [] };
  ...
  rows.forEach((row) => {
    const numOs = getRowValue(row, ["numero_ordem_servico", "num_o_s", "num_os", "numero_os"]);
    ...
    if (forMatch) {
      ...
      const shouldIgnore = ignoredTypes.todos.some((item) => normalizeText(item) === normalizedType);
      if (shouldIgnore) {
        ignoradasPorTipo += 1;
    ...
    const dataCadastro = normalizeMapaDateValue(
      getRowValue(row, ["data_cadastro", "data_abertura", "data_abertura_os", "abertura"]),
    );
    ...
    const macAddr = getRowValue(row, ["mac_addr", "mac addr", "macaddr", "mac", "mac_address"]);
    const phyAddr = getRowValue(row, ["phy_addr", "phy addr", "phyaddr", "phy", "phy_address"]);
    ...
    ordensNovas[id] = {
      ...
      codigo_cliente: getRowValue(row, ["codigo_cliente", "codigo"])
        ...
        : "",
      nome_cliente: getRowValue(row, ["nome_razaosocial", "cliente", "nome_cliente"])
        ? String(getRowValue(row, ["nome_razaosocial", "cliente", "nome_cliente"])).trim()
        : "",
      id_cliente_servico: getRowValue(row, ["id_cliente_servico", "cliente_servico", "id_servico"])
        ? String(getRowValue(row, ["id_cliente_servico", "cliente_servico", "id_servico"])).trim()
        : "",
...
function buildMapaSnapshot(ordens = []) {
  ...
  const kpis = { total: ordens.length, pendente: 0, aguardando: 0, totalRegional: 0, totalAgente: 0 };
  ...
  ordens.forEach((order) => {
    ...
    if (order.agente) {
      ...
      if (!agentes[city]) agentes[city] = { pendente: {}, aguardando: {}, regional: order.regional || "" };
      agentes[city][statusKey][type] = (agentes[city][statusKey][type] || 0) + 1;
    ...
    if (!regionais[regional][city]) regionais[regional][city] = { pendente: {}, aguardando: {} };
    regionais[regional][city][statusKey][type] = (regionais[regional][city][statusKey][type] || 0) + 1;
    rankingRegionais[city] = (rankingRegionais[city] || 0) + 1;
  });

  const totalCity = (cityData) =>
    Object.values(cityData.pendente || {}).reduce((sum, value) => sum + value, 0) +
    Object.values(cityData.aguardando || {}).reduce((sum, value) => sum + value, 0);
  ...
  return {
    ...
    chartRegionais: Object.entries(regionais)
      .map(([regional, cities]) => ({
        regional,
        total: Object.values(cities).reduce((sum, cityData) => sum + totalCity(cityData), 0),
      }))
...
function buildMatchData(ordens = []) {
        ...
        const cidades = Object.entries(cities)
          .map(([city, list]) => {
            ...
            return {
              ...
              totalRetiradasRelacionadas: matches.reduce((sum, item) => sum + item.totalRelacionadas, 0),
              isAgente,
            };
          })
  ...
  const totalRelated = (section) =>
    section.reduce(
      (sum, group) =>
        sum + (group.cidades || []).reduce((citySum, city) => citySum + Number(city.totalRetiradasRelacionadas || 0), 0),
      0,
    );
  ...
  return {
    ...
    resumo: {
      ...
      totalRetiradasRelacionadas: totalRelated(regionais) + totalRelated(agentes),
      ignoradasSemRegional,
...
async function saveImportRun(type, payload = {}, uid = null) {
  ...
  await documents.upsertDocument({
    ...
    documentId: id,
    parentPath: null,
    data: { ...payload, tipo: type, createdBy: uid, createdAt: new Date().toISOString() },
  });
  return id;
}
...
function broadcastImportResult(type, result = {}) {
  ...
  const payload = {
    ...result,
    source,
    sourceLabel: source === "match" ? "MATCH" : source === "mapa" ? "Mapa" : "Operacional",
    message:
      result?.message ||
      (source === "match"
...
async function getMensageriaQueueKeys() {
  const items = await documents.listAllDocuments("mensageria_fila").catch(() => []);
  return new Set(
    items
      .map((item) => {
...
function buildMensageriaQueueItemFromOrder(order = {}, id = "", config = {}) {
  ...
  const cliente = clienteRaw
    ...
    .replace(/\s+/g, " ")
    .trim();
  const telefone = String(readAnyField(order, ["telefone", "celular", "fone", "whatsapp", "contato"]));
  const endereco = String(readAnyField(order, ["endereco_resumo", "endereco", "logradouro"]));
  const codigoCliente = String(readAnyField(order, ["codigo_cliente", "codigo"])) ||
    (clienteRaw.match(/^\s*\((\d+)\)/)?.[1] || "");
  return {
    cliente: cliente || "Cliente nao informado",
    codigo_cliente: codigoCliente,
    contrato: String(readAnyField(order, ["contrato", "codigo_contrato", "id_contrato"])) || id,
    os: String(readAnyField(order, ["num_os", "os", "numero_os", "numero_ordem_servico"])) || id,
    ...
    data_abertura_os: String(readAnyField(order, ["data_abertura", "data_abertura_os", "abertura", "data"])),
...
async function enqueueNewMapOrdersForMensageria(newEntries = [], user = {}) {
  ...
  if (!config.autoEnqueueMapDiff) return { created: 0, skipped: newEntries.length, disabled: true };
  ...
  for (const [id, order] of newEntries) {
    ...
    if (allowedCities.size && !allowedCities.has(normalizeCityKey(item.cidade))) {
      skipped += 1;
      continue;
    }
...
async function persistMapaImport(payload = {}, user = {}, context = {}) {
  ...
  await context.update?.({
    ...
    percent: 35,
    processedRows: Array.isArray(payload.rows) ? payload.rows.length : Object.keys(incoming).length,
  });
  const current = await getCollectionMap("ordens_abertas");
  const currentBySource = Object.fromEntries(
    Object.entries(current).filter(([, order]) => fontesSet.has(normalizeMapaFonte(order?.fonte || order?.empresa))),
  );
  const newMapEntries = Object.entries(incoming).filter(([id]) => !currentBySource[id]);
  const deleted = await documents.deleteDocumentsByCollectionAndSources("ordens_abertas", fontes);
  await context.update?.({
  ...
  const finalOrders = Object.entries(finalMap).map(([id, data]) => ({ id, ...data }));
  const totalGeral = finalOrders.length;
  const totalSempre = finalOrders.filter((order) => normalizeMapaFonte(order.fonte || order.empresa) === "sempre").length;
  const totalOnnet = finalOrders.filter((order) => normalizeMapaFonte(order.fonte || order.empresa) === "onnet").length;
  const meta = {
    ...
    novasSalvas: Object.keys(incoming).filter((id) => !currentBySource[id]).length,
    removidas: deleted,
  ...
  const importRunId = await saveImportRun("mapa", { total: Object.keys(incoming).length, totalGeral, fontes }, user.uid);
  const mensageriaDiff = await enqueueNewMapOrdersForMensageria(newMapEntries, user);
  await context.update?.({
  ...
  const agendamentosMapa = await reconcileAppointmentsWithMapa({
    ...
    console.error("[operationalImports] Falha ao conferir agendamentos no mapa:", error);
    return { ok: false, error: error?.message || "Falha ao conferir agendamentos no mapa." };
  });
  await context.update?.({
  ...
  await publishAcompanhamentoUpdate("mapa", {
    ...
    message: `Mapa atualizado: ${totalGeral} O.S em aberto.`,
    summary: { totalGeral, totalSempre, totalOnnet, fontes, mensageriaDiff, agendamentosMapa },
  });
  ...
  setImmediate(() => {
    try {
      ...
      sempreIntegration
        ...
        .catch((error) => console.error("[operationalImports] Falha ao atualizar equipamentos do mapa:", error));
    } catch (error) {
      console.error("[operationalImports] Falha ao iniciar atualizacao de equipamentos do mapa:", error);
    }
  });
...
async function persistMatchImport(payload = {}, user = {}, context = {}) {
  ...
  const deleted = await documents.deleteDocumentsByCollectionAndSources("match_os_abertas", fontes);
  await context.update?.({
  ...
  const meta = {
    ...
    tiposIgnoradosAplicados: prepared.tiposIgnoradosAplicados || DEFAULT_MATCH_IGNORED_TYPES,
  ...
  const agentesData = {
    ...
    resumo: {
      ...matchData.resumo,
      totalRegionais: 0,
      totalCidades: matchData.agentes.reduce((sum, item) => sum + item.totalCidades, 0),
      totalMatches: matchData.agentes.reduce((sum, item) => sum + item.totalMatches, 0),
    },
  };
  await context.update?.({
  ...
  const importRunId = await saveImportRun("match", { total: Object.keys(incoming).length, totalGeral, fontes }, user.uid);
  await context.update?.({
    stage: "Publicando atualizacao do acompanhamento",
    percent: 94,
  });
...
function shouldKeepMonth(data) {
  return hasMonthMovement(data) || hasMonthMovement(data?.onnet) || hasMonthMovement(data?.onnetSempre);
}
...
function dedupeAgentCities(cities = []) {
  ...
  return [...map.values()].sort((a, b) => String(a.cidade || "").localeCompare(String(b.cidade || ""), "pt-BR"));
}
...
async function persistMetasImport(payload = {}, user = {}) {
  ...
  for (const month of MONTHORDER) {
    ...
    if (!cities.length) {
      ...
      const totalRealizado = normalizedCities.reduce((sum, city) => sum + Number(city.realizado || 0), 0);
      const totalMeta = normalizedCities.reduce((sum, city) => sum + Number(city.meta80 || 0), 0);
      const totalCancelamentos = normalizedCities.reduce((sum, city) => sum + Number(city.cancelamentos || 0), 0);
      ...
      const totalDaily = Array.from({ length: dayCount }, (_, index) =>
        normalizedCities.reduce((sum, city) => sum + Number(city.daily?.[index] || 0), 0),
      );
      ...
      await documents.upsertDocument({
        ...
        data: {
          ...
          cidadesRanking: [...normalizedCities].sort((a, b) => b.realizado - a.realizado),
          dayCount,
          totalCancelamentos,
          totalMeta,
          totalRealizado,
...
async function saveMetasForceTaskConfig(rawConfig = {}, user = {}) {
  ...
  if (config.ativa && (!config.inicio || !config.fim)) {
    const error = new Error("Informe data inicial e data final para ativar a forca tarefa.");
...
async function saveMatchConfig(payload = {}, user = {}) {
  const extras = [...new Set(
    (Array.isArray(payload.tiposIgnoradosAdicionais) ? payload.tiposIgnoradosAdicionais : [])
  ...
  await documents.upsertDocument({
    ...
    documentId: "match_os",
    parentPath: null,
    data: { ...current, tiposIgnoradosAdicionais: extras, updatedAt: new Date().toISOString(), updatedBy: user.uid || null },
  });
  await publishAcompanhamentoUpdate("match", {
    updatedBy: user.uid || null,
