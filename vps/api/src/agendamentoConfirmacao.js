...
function isOpenAppointment(data = {}) {
  ...
  return !["concluido", "entregue", "cancelado", "nao recolhido", "não recolhido"].includes(status);
}

function getAppointmentClient(data = {}) {
  return String(data.cliente_nome || data.nome_cliente || data.cliente || data.nome || "").trim();
}

function getAppointmentCode(data = {}) {
  return String(data.codigo_cliente || data.codigo || data.cod_cliente || data.contrato || "").trim();
}
...
function getAppointmentAgent(data = {}) {
  return String(data.agendado_por_nome || data.agendado_por || data.usuario_nome || data.criado_por_nome || "").trim();
}
...
function buildEvolutionConfig(config = {}) {
  return {
    ...
    evolutionSendTextPath: config.evolutionSendTextPath || DEFAULT_CONFIG.evolutionSendTextPath,
    evolutionWebhookUrl: config.evolutionWebhookUrl || DEFAULT_CONFIG.evolutionWebhookUrl,
  };
}
...
function assertEvolutionConfigured(config = {}) {
  ...
  if (missing.length) {
    const error = new Error(`Configure ${missing.join(", ")} da confirmação antes de enviar.`);
...
async function getEvolutionStatus() {
  ...
  return {
    ok: true,
    configured: Boolean(evolutionConfig.evolutionBaseUrl && evolutionConfig.evolutionApiKey && evolutionConfig.evolutionInstance),
...
async function connectEvolutionInstance() {
  ...
  const result = await evolutionMessaging.createOrConnectInstance(evolutionConfig, { persist: false });
  await saveConfig({ evolutionEnabled: true, evolutionWebhookConfiguredAt: nowIso() });
  return result;
}
...
async function configureEvolutionWebhook(webhookUrl = "") {
  const config = await getConfig();
  const nextConfig = webhookUrl ? await saveConfig({ evolutionWebhookUrl: webhookUrl }) : config;
  assertEvolutionConfigured(buildEvolutionConfig(nextConfig));
  const result = await evolutionMessaging.configureWebhook(buildEvolutionConfig(nextConfig), { persist: false });
...
async function getPreview({ dateKey = "", daysAhead = 1 } = {}) {
  const targetDateKey = normalizeAppointmentDateKey(dateKey) ||
    getLocalDateKey(new Date(Date.now() + Number(daysAhead || 1) * 24 * 60 * 60 * 1000));
  const [appointments, regionais] = await Promise.all([listTodayAppointments(targetDateKey), listRegionais()]);
  const grouped = new Map();
  appointments.forEach((row) => {
    const regional = findRegional(regionais, row);
    const regionalName = regional?.nome || getAppointmentRegional(row.data) || "Sem regional";
  ...
  })).sort((a, b) => String(a.regional || "").localeCompare(String(b.regional || "")));
...
function findRegional(regionais = [], appointment = {}) {
  const regionalName = normalizeText(getAppointmentRegional(appointment.data || appointment));
  const cityName = normalizeText(getAppointmentCity(appointment.data || appointment));
  return regionais.find((regional) => normalizeText(regional.nome) === regionalName) ||
    regionais.find((regional) =>
      (Array.isArray(regional.cidades) ? regional.cidades : []).some((city) => normalizeText(city.nome || city) === cityName),
    ) ||
    null;
}

function buildEscalationSteps(regional = {}) {
  const backoffices = Array.isArray(regional.backoffices) ? regional.backoffices : regional.backoffice?.nome ? [regional.backoffice] : [];
  return [
    { key: "backoffice_01", label: "BackOffice 01", pessoa: backoffices[0] || {} },
    { key: "backoffice_02", label: "BackOffice 02", pessoa: backoffices[1] || {} },
    { key: "lider_tecnico", label: "Líder Técnico", pessoa: regional.lider || {} },
    { key: "supervisor", label: "Supervisor", pessoa: regional.supervisor || {} },
  ].map((step) => ({
    ...step,
...
async function upsertTrack(id, data = {}) {
  ...
  broadcastRealtime("mensageria", { collectionPath: TRACK_COLLECTION, documentId: id });
}

async function getTrack(id) {
  const row = await documents.getDocument(`${TRACK_COLLECTION}/${id}`).catch(() => null);
...
async function saveRegionalBackoffice(regionalName = "", pessoa = {}) {
  ...
  const regional = regionais.find((item) => normalizeText(item.nome) === normalizeText(regionalName));
  if (!regional) return { updated: false, reason: "Regional não localizada." };

  const backoffices = Array.isArray(regional.backoffices) ? [...regional.backoffices] : [];
  if (!backoffices.length && regional.backoffice?.nome) backoffices.push(regional.backoffice);
  const normalizedPhone = normalizePhone(pessoa.telefone);
  const existingIndex = backoffices.findIndex((item) =>
    normalizePhone(item?.telefone) === normalizedPhone || normalizeText(item?.nome) === normalizeText(pessoa.nome),
  );
  const nextPessoa = {
  ...
  if (existingIndex >= 0) {
    backoffices[existingIndex] = { ...backoffices[existingIndex], ...nextPessoa };
  } else if (!backoffices[0]?.nome || !normalizePhone(backoffices[0]?.telefone)) {
...
async function assignManualResponsible(trackId, payload = {}, user = {}) {
  ...
  const regionalUpdate = await saveRegionalBackoffice(track.regional, { nome, telefone, email });
  const regional = regionalUpdate.regional || {};
  const steps = buildEscalationSteps(regional);
  const manualStep = { key: "backoffice_01", label: "BackOffice 01", nome, telefone, email };
...
async function sendTrackStep(track, stepIndex = 0, config = null) {
  ...
  const appointments = (Array.isArray(track.appointments) ? track.appointments : [])
  ...
  if (!expectedDateKey || !appointments.length) {
    const nextTrack = {
      ...
      appointments,
      updatedAt: nowIso(),
      invalidReason: "Nenhum agendamento do envio possui data válida igual ao dia da rotina.",
    };
    await upsertTrack(track.id, nextTrack);
    await createLog({
      trackId: track.id,
  ...
  const listaClientes = appointments.map((item, index) => buildClientLine(item, index)).join("\n");
  const first = appointments[0] || {};
  const previousStep = stepIndex > 0 ? track.steps[stepIndex - 1] : null;
  const template = stepIndex > 0
  ...
  const message = render(template, {
    ...
    responsavel_anterior: previousStep ? `${previousStep.label} ${previousStep.nome}` : "",
  ...
  const response = await evolutionMessaging.sendWhatsAppMessage(messagingConfig, step.telefone, message, {
    telefone: step.telefone,
    cliente: step.nome,
  });
  const nextCheckAt = new Date(Date.now() + Math.max(1, Number(currentConfig.escalationMinutes || 60)) * 60 * 1000).toISOString();
  const nextTrack = {
    ...track,
    appointments,
...
async function ensureMorningTracks({ force = false } = {}) {
  ...
  if (!config.enabled && !force) return { ok: true, skipped: "Automação desativada." };
  if (!force && localMinutes() < timeToMinutes(config.dailySendTime || "08:00")) {
    return { ok: true, skipped: "Antes do horário da rotina." };
  }

  const [appointments, regionais] = await Promise.all([listTodayAppointments(dateKey), listRegionais()]);
  const grouped = new Map();
  appointments.forEach((row) => {
    const regional = findRegional(regionais, row);
    const regionalName = regional?.nome || getAppointmentRegional(row.data) || "Sem regional";
    const key = normalizeText(regionalName) || "sem_regional";
    if (!grouped.has(key)) grouped.set(key, { regional, regionalName, rows: [] });
  ...
  for (const group of grouped.values()) {
    ...
    if (steps.length) {
      ...
      await createLog({ trackId, type: "no_responsible", origin: "rotina_08h", regional: group.regionalName });
    }
  }
...
async function ensureSameDayTracks() {
  const config = await getConfig();
  if (!config.enabled || !config.sendSameDayNewAppointments) return { ok: true, skipped: "Automação desativada." };
  const dateKey = getLocalDateKey();
  const [appointments, regionais] = await Promise.all([listTodayAppointments(dateKey), listRegionais()]);
  let created = 0;
  let sent = 0;
...
async function processEscalations() {
  ...
  for (const row of rows) {
    ...
    if (!track.nextCheckAt || new Date(track.nextCheckAt).getTime() > Date.now()) continue;
    const nextStep = Number(track.currentStepIndex || 0) + 1;
    if ((track.steps || [])[nextStep]) {
      await sendTrackStep(track, nextStep, config);
      escalated += 1;
    } else {
      const nextTrack = { ...track, status: "sem_resposta_final", updatedAt: nowIso() };
      await upsertTrack(track.id, nextTrack);
      await createLog({ trackId: track.id, type: "no_final_response", origin: track.origin, regional: track.regional });
...
async function listTracks({ limit = 50, offset = 0 } = {}) {
  const rows = await documents.listDocuments({ collectionPath: TRACK_COLLECTION, limit, offset });
  const items = rows.map((row) => ({ id: row.documentId, ...(row.data || {}) }))
    .sort((a, b) => String(b.updatedAt || b.createdAt || "").localeCompare(String(a.updatedAt || a.createdAt || "")));
  return { items };
}

async function listLogs({ limit = 50, offset = 0 } = {}) {
  const rows = await documents.listDocuments({ collectionPath: LOG_COLLECTION, limit, offset });
  const items = rows.map((row) => ({ id: row.documentId, ...(row.data || {}) }))
...
async function getLastMessageSentAt() {
  ...
  return rows
    ...
    .filter((log) => ["message_sent", "escalation_sent", "test_sent"].includes(log.type))
...
function getNextDailyRunAt(config = {}) {
  ...
  if (todayRunAt && new Date(todayRunAt).getTime() > Date.now()) return todayRunAt;
...
async function getReport() {
  ...
  const sent = logs.filter((log) => ["message_sent", "escalation_sent"].includes(log.type));
  ...
  [...sent, ...replied].forEach((log) => {
    ...
    if (["message_sent", "escalation_sent"].includes(log.type)) byResponsible[name].enviados += 1;
    if (log.type === "responsible_confirmed") byResponsible[name].confirmados += 1;
  });
  return {
    ok: true,
    totals: {
      ...
      escalonamentos: logs.filter((log) => log.type === "escalation_sent").length,
      semRespostaFinal: logs.filter((log) => log.type === "no_final_response").length,
    },
    responsaveis: Object.values(byResponsible).sort((a, b) => b.enviados - a.enviados),
  };
}

async function sendTestMessage(payload = {}, user = {}) {
  const config = await getConfig();
  const phone = normalizePhone(payload.telefone || payload.phone || payload.number);
  if (!phone) {
  ...
  const listaClientes = sampleAppointments.map((item, index) => buildClientLine(item, index)).join("\n");
  const first = sampleAppointments[0];
  const template = testType === "same_day"
  ...
  assertEvolutionConfigured(messagingConfig);
  const response = await evolutionMessaging.sendWhatsAppMessage(messagingConfig, phone, message, {
    telefone: phone,
...
async function registerIncomingResponse(payload = {}) {
  const config = await getConfig();
  if (!config.enabled) return { ok: true, ignored: true, reason: "Automação desativada." };
  const text = extractIncomingText(payload);
  const phone = extractIncomingPhone(payload);
  if (!text || !phone) return { ok: true, ignored: true, reason: "Sem texto ou telefone." };
  const accepted = (Array.isArray(config.acceptedReplies) ? config.acceptedReplies : DEFAULT_CONFIG.acceptedReplies)
    .map(normalizeText);
  if (!accepted.includes(normalizeText(text))) return { ok: true, ignored: true, reason: "Resposta não confirmadora." };

  const rows = await listAll(TRACK_COLLECTION);
  const trackRow = rows
    .map((row) => ({ id: row.documentId, ...(row.data || {}) }))
    .filter((track) => track.status === "aguardando_resposta")
    .find((track) => (track.steps || []).some((step) => normalizePhone(step.telefone) === phone));
  if (!trackRow) return { ok: true, ignored: true, reason: "Nenhuma trilha ativa para o telefone." };
  const step = (trackRow.steps || []).find((item) => normalizePhone(item.telefone) === phone) || {};
  const nextTrack = {
    ...trackRow,
    status: "confirmado",
    confirmedAt: nowIso(),
