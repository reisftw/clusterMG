...
function normalizeConfig(config = {}) {
  return {
    ...
    statusEndpoint: normalizePath(config.statusEndpoint, DEFAULT_STATUS_ENDPOINT),
...
function assertBaseConfig(config = {}) {
  ...
  if (missing.length) {
    const error = new Error(`Configure Cvortex antes de validar: ${missing.join(", ")}.`);
...
async function testConnection() {
  ...
  const endpoint = buildUrl(config, config.statusEndpoint || DEFAULT_STATUS_ENDPOINT);
  ...
  if (!response.ok) {
    const message = payload?.message || payload?.error || payload?.raw || `Cvortex HTTP ${response.status}`;
    await saveConfig({ ...config, lastValidatedAt: checkedAt, lastError: message });
...
async function sendMessage(numberRaw, messageRaw, item = {}, user = {}) {
  ...
  const endpoint = buildUrl(config, config.sendEndpoint || DEFAULT_SEND_ENDPOINT);
  const body = {
    phone,
    to: phone,
    number: phone,
...
async function sendTextMessage(number, text, item = {}, user = {}) {
  const { response, result, phone, endpoint } = await sendMessage(number, text, item, user);
  if (!response.ok) {
    const messageError = result?.message || result?.error || result?.raw || `Cvortex HTTP ${response.status}`;
    const error = new Error(`Falha ao enviar mensagem Cvortex: ${messageError}`);
    error.statusCode = response.status >= 500 ? 502 : 400;
    throw error;
  }
  return {
...
async function sendTestMessage(payload = {}, user = {}) {
  ...
  const { response, result, phone, endpoint } = await sendMessage(phoneRaw, messageRaw, {
  ...
  if (!response.ok) {
    const messageError = result?.message || result?.error || result?.raw || `Cvortex HTTP ${response.status}`;
    await saveConfig({ ...config, lastTestSentAt: now, lastTestMessage: messageError });
...
async function associateCvortex(user = {}) {
  ...
  const messaging = await documents.getDocument(MESSAGING_CONFIG_PATH).catch(() => null);
  await documents.upsertDocument({
...
async function registerWebhook(payload = {}, meta = {}) {
  ...
  const message = cleanText(pickFirst(payload, [
    "message",
    "mensagem",
    "text",
    "body",
