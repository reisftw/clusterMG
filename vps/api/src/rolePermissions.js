...
const DEFAULT_ROLE_PERMISSIONS = Object.freeze({
  ...
  lider_empresa: ["view_dashboard", "view_empresas_tecnicos", "view_documentos", "view_insumos_requisicoes", "tecnicos.auditoria_bolsa.view"],
  agente_autorizado: ["view_empresas_tecnicos", "view_documentos", "view_insumos_requisicoes"],
  backoffice: ["view_dashboard", "view_acerto_estoque", "manage_acerto_estoque", "view_insumos_requisicoes", "tecnicos.auditoria_bolsa.view", "tecnicos.auditoria_bolsa.manage"],
...
async function getRolePermissions(role) {
  ...
  try {
    ...
    if (result.rows.length) return normalizePermissions(result.rows.map((row) => row.permission));
  } catch (error) {
    if (error?.code !== "42P01") throw error;
  }
...
async function saveRole({ id, name, description = "", active = true, permissions = [] }) {
  ...
  const cleanPermissions = normalizePermissions(permissions)
    .filter((permission) => permission !== "*" && (!catalogIds.size || catalogIds.has(permission)));
  await db.query("begin");
  try {
    const current = await db.query("select system_role from app_roles where id = $1", [roleId]);
    await db.query(
