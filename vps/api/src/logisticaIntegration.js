...
const DEFAULT_PROVIDERS = [
  ...
  { id: "lalamove", nome: "Lalamove", tipo: "api", ativo: false, environment: "sandbox", market: "BR", serviceType: "LALAGO", language: "pt_BR" },
  { id: "uber_direct", nome: "Uber Direct", tipo: "api", ativo: false },
  { id: "loggi", nome: "Loggi", tipo: "api", ativo: false },
  { id: "ifood_sob_demanda", nome: "iFood Sob Demanda", tipo: "api", ativo: false },
];
...
function normalizeProvider(provider = {}, currentProvider = {}) {
  const id = cleanText(provider.id || currentProvider.id || provider.nome).toLowerCase().replace(/[^a-z0-9_]+/g, "_");
  const next = {
    ...currentProvider,
    ...provider,
    id,
    ...
    credentialRef: cleanText(provider.credentialRef || currentProvider.credentialRef),
  ...
  if (provider.apiSecret !== undefined && cleanText(provider.apiSecret)) next.apiSecret = cleanText(provider.apiSecret);
  if (provider.market !== undefined) next.market = cleanText(provider.market || "BR").toUpperCase();
  if (provider.environment !== undefined) next.environment = cleanText(provider.environment || "sandbox");
  if (provider.serviceType !== undefined) next.serviceType = normalizeLalamoveServiceType(provider.serviceType || "LALAGO");
  if (provider.language !== undefined) next.language = cleanText(provider.language || "pt_BR");
  return next;
}
...
async function readConfig({ sanitized = true } = {}) {
  ...
  const currentProviders = new Map((stored.providers || []).map((provider) => [provider.id, provider]));
  const providers = DEFAULT_PROVIDERS.map((provider) =>
    normalizeProvider(currentProviders.get(provider.id) || provider, provider),
  );
  const extraProviders = (stored.providers || []).filter((provider) => !providers.some((item) => item.id === provider.id));
  const config = {
    ...DEFAULT_CONFIG,
    ...stored,
    providers: [...providers, ...extraProviders.map((provider) => normalizeProvider(provider))],
  };
  ...
  return {
    ...
    providers: config.providers.map((provider) => {
      ...
      return {
        ...safeProvider,
        apiSecret: "",
        apiSecretConfigured: Boolean(apiSecret || getProviderEnv(provider.id).apiSecret),
      };
    }),
  };
}
...
async function saveConfig(payload = {}) {
  ...
  const providers = (payload.providers || current.providers || DEFAULT_PROVIDERS).map((provider) =>
...
function getLalamoveCredentials(provider = {}) {
  ...
  const environment = cleanText(env.environment || provider.environment || "sandbox");
  return {
    ...
    baseUrl: cleanText(
      env.baseUrl ||
        provider.baseUrl ||
        (environment === "production" ? "https://rest.lalamove.com" : "https://rest.sandbox.lalamove.com"),
    ).replace(/\/+$/, ""),
    market: cleanText(env.market || provider.market || "BR").toUpperCase(),
    serviceType: normalizeLalamoveServiceType(env.serviceType || provider.serviceType || "LALAGO"),
...
function normalizeLalamoveServiceType(value) {
  ...
  const allowed = new Set(["CAR", "HATCHBACK", "LALAGO", "LALAGOFOUR", "TRUCK330", "TRUCK_6H", "UV_4H", "UV_FIORINO", "VAN"]);
...
function normalizeLalamoveQuote(data = {}) {
  ...
  const price = quote.priceBreakdown?.total || quote.priceBreakdown?.subTotal || quote.priceBreakdown?.base || "";
  const currency = quote.priceBreakdown?.currency || quote.priceBreakdown?.totalCurrency || "";
  const distance = quote.distance?.value ? `${quote.distance.value} ${quote.distance.unit || "m"}` : "";
  return {
...
function buildLalamoveError(data, responseText, status) {
  const errors = Array.isArray(data?.errors)
    ? data.errors.map((item) => item.message || item.detail || item.code).filter(Boolean)
    : [];
  const details = [
    data?.message,
    data?.error,
...
async function geocodeAddress(payload = {}) {
  ...
  if (!query) {
    const error = new Error("Informe um endereço para buscar latitude e longitude.");
    error.statusCode = 400;
    throw error;
  }

  const baseUrl = cleanText(process.env.LOGISTICA_GEOCODER_URL || "https://nominatim.openstreetmap.org/search");
  ...
  const response = await fetch(url.toString(), {
    headers: {
      ...
      "User-Agent": cleanText(process.env.LOGISTICA_GEOCODER_USER_AGENT || "retiradas.tech logística/1.0"),
    },
  });
  ...
  if (!response.ok) {
    const error = new Error(`Falha ao consultar coordenadas: HTTP ${response.status}.`);
...
async function requestLalamoveQuotation(payload = {}) {
  ...
  if (!credentials.apiKey || !credentials.apiSecret) {
    const error = new Error("Configure API Key e API Secret da Lalamove em Logística > Configuração.");
    error.statusCode = 400;
    throw error;
  }

  const pickupAddress = cleanText(payload.pickup?.address || payload.enderecoColeta);
  const dropoffAddress = cleanText(payload.dropoff?.address || payload.pontoEndereco);
  if (!pickupAddress || !dropoffAddress) {
    const error = new Error("Informe endereço de coleta e ponto estratégico para cotar na Lalamove.");
    error.statusCode = 400;
    throw error;
  }
  ...
  if (missingCoordinates.length) {
    const error = new Error(`Para testar a Lalamove, preencha latitude e longitude de ${missingCoordinates.join(" e ")}.`);
    error.statusCode = 400;
    throw error;
  }
  ...
  const response = await fetch(`${credentials.baseUrl}${path}`, {
    method,
    headers: {
      Authorization: signLalamoveRequest({ ...credentials, method, path, body }),
  ...
  if (!response.ok) {
    const error = new Error(buildLalamoveError(data, responseText, response.status));
