...
const CATEGORY_DEFINITIONS = Object.freeze([
  {
    ...
    tipo: "metragem",
    limite: 1000,
    terms: ["cabo", "drop", "bobina", "fibra", "cordao", "cordão", "patch cord", "utp", "lan", "metro"],
  },
  {
    id: "cameras",
    label: "Câmeras",
    tipo: "unidade",
    limite: 20,
    terms: ["camera", "câmera", "camera de video", "câmera de vídeo", "video wi-fi", "vídeo wi-fi", "im5", "imxc", "tapo", "tc60"],
  },
  {
    id: "ont_onu",
    label: "ONT e ONU",
  ...
  },
  {
    ...
    tipo: "unidade",
    limite: 20,
    terms: ["roteador", "router", "wifi", "wi-fi", "tplink", "tp-link", "archer"],
  },
  {
    id: "insumos",
    label: "Insumos",
...
const DEFAULT_CONFIG = Object.freeze({
  ...
  categoryThresholds: CATEGORY_DEFINITIONS.map(({ id, label, tipo, limite }) => ({ id, label, tipo, limite })),
  itemThresholds: [],
  lastDailyRunDate: "",
});
...
function getCategoryDefinition(id) {
  return CATEGORY_DEFINITIONS.find((category) => category.id === id) || CATEGORY_DEFINITIONS[CATEGORY_DEFINITIONS.length - 1];
}
...
function categorizeStockItem(name, unit = "") {
  ...
  return CATEGORY_DEFINITIONS.find((category) => category.terms.some((term) => key.includes(normalizeText(term)))) || getCategoryDefinition("insumos");
}
...
function normalizeCategoryThresholds(value) {
  ...
  return CATEGORY_DEFINITIONS.map((category) => {
    ...
    return {
      ...
      limite: Math.max(0, toNumber(saved.limite ?? saved.quantidade ?? category.limite)),
    };
  });
}
...
function getUserCompanyScope(user = {}) {
  const profile = getProfile(user);
  return {
    id: cleanText(profile.empresaId || profile.empresa_id || user.empresaId || user.empresa_id),
    nome: normalizeText(profile.empresaNome || profile.empresa_nome || user.empresaNome || user.empresa_nome),
  };
}
...
function canSeeCompany(user, empresa) {
  ...
  if (companyScope.id || companyScope.nome || ["lider_empresa", "agente_autorizado"].includes(role)) {
    return Boolean(
      (companyScope.id && companyScope.id === empresa.id) ||
        (companyScope.nome && companyScope.nome === normalizeText(empresa.nome)),
    );
  }

  const regional = getUserRegional(user);
  if (regional || ["supervisor", "backoffice", "backoffice_retirada"].includes(role)) {
...
function normalizeTecnico(empresa, tecnico) {
  const id = cleanText(tecnico.id) || normalizeId(`${empresa.id}-${tecnico.nome}-${tecnico.emailHubsoft || tecnico.email}`);
  return {
    id,
    ...
    emailHubsoft: cleanText(tecnico.emailHubsoft || tecnico.email_hubsoft || tecnico.emailHubSoft || tecnico.hubsoftEmail || tecnico.email),
...
async function listAuthorizedTechnicians(user) {
  ...
  const empresas = docs.map(normalizeEmpresa).filter((empresa) => canSeeCompany(user, empresa));
  return empresas.map((empresa) => ({
    ...empresa,
    tecnicos: empresa.tecnicos
      .map((tecnico) => normalizeTecnico(empresa, tecnico))
      .filter((tecnico) => tecnico.nome && normalizeText(tecnico.status) !== "inativo"),
  }));
}
...
async function readConfig({ sanitized = true } = {}) {
  ...
  config.categoryThresholds = normalizeCategoryThresholds(config.categoryThresholds);
  if (sanitized) {
...
async function saveConfig(payload = {}, user = {}) {
  ...
  const next = {
    ...
    alertsEnabled: payload.alertsEnabled !== undefined ? Boolean(payload.alertsEnabled) : current.alertsEnabled,
    dailyRunTime: cleanText(payload.dailyRunTime || current.dailyRunTime || DEFAULT_CONFIG.dailyRunTime).slice(0, 5),
    thresholdTotalUnits: Math.max(0, toNumber(payload.thresholdTotalUnits ?? current.thresholdTotalUnits)),
    thresholdTotalMeters: Math.max(0, toNumber(payload.thresholdTotalMeters ?? current.thresholdTotalMeters)),
    categoryThresholds: normalizeCategoryThresholds(payload.categoryThresholds ?? current.categoryThresholds),
    lastDailyRunDate: cleanText(payload.lastDailyRunDate || current.lastDailyRunDate),
    lastReportRefreshAt: cleanText(payload.lastReportRefreshAt || current.lastReportRefreshAt),
    lastReportRefreshStart: cleanText(payload.lastReportRefreshStart || current.lastReportRefreshStart),
    lastReportRefreshEnd: cleanText(payload.lastReportRefreshEnd || current.lastReportRefreshEnd),
    lastReportRefreshTotal: Math.max(0, toNumber(payload.lastReportRefreshTotal ?? current.lastReportRefreshTotal)),
    itemThresholds: Array.isArray(payload.itemThresholds)
      ? payload.itemThresholds.map((item) => ({
...
function pickQuantityValue(item, preferredKeys) {
  ...
  const preferred = preferredKeys
    .map((key, index) => [key, key.split(".").reduce((acc, part) => acc?.[part], item), index])
  ...
  const flattenedCandidates = Object.entries(flattened)
    ...
    .filter(([key, value]) => {
      ...
      if (!/(disponivel|disponivel|saldo|quantidade|qtde|qtd|balance|amount|metragem|metros)/.test(normalizedKey)) return false;
      return cleanText(value) && Number.isFinite(toNumber(value));
    })
    .sort(([leftKey, leftValue, leftIndex], [rightKey, rightValue, rightIndex]) => {
      const score = (key) => {
        const normalizedKey = normalizeText(key);
        ...
        if (normalizedKey.includes("qtd") || normalizedKey.includes("qtde")) return 3;
        return 4;
      };
      const leftPositive = toNumber(leftValue) > 0 ? 0 : 1;
      const rightPositive = toNumber(rightValue) > 0 ? 0 : 1;
      return leftPositive - rightPositive || score(leftKey) - score(rightKey) || leftIndex - rightIndex;
    });
...
async function findTecnicoSeniorId(email) {
  ...
  const payload = await sempreIntegration.requestSempreRaw(`/mesclar/usuario?${params.toString()}`, {
    timeoutMs: SEMPRE_STOCK_TIMEOUT_MS,
  });
  const users = extractArray(payload, ["usuarios", "users", "data"]);
  const exact = users.find((item) => normalizeText(item.email) === normalizeText(email)) || users[0];
  const seniorId = cleanText(pickValue(exact, ["senior_id", "id_senior", "seniorId", "codigo_senior", "id", "id_usuario", "usuario_id", "codigo", "value"]));
  if (!seniorId) {
    const error = new Error(`Nenhum usuário encontrado na API Sempre para ${email}.`);
...
function isMetricItem(name, unit) {
  const key = normalizeText(`${name} ${unit}`);
  return /\b(m|mt|mts|metro|metros)\b/.test(key) || key.includes("cabo") || key.includes("drop") || key.includes("fibra");
}
...
function normalizeStockItem(item) {
  const name = cleanText(pickValue(item, [
    "produto.nome",
    "produto.descricao",
    "material.nome",
    "material.descricao",
  ...
  const rawUnit = cleanText(pickValue(item, [
    "unidade.sigla",
    "unidade.nome",
    "unidade_medida",
    "unidade",
  ...
  const quantity = directQuantity !== null ? directQuantity : toNumber(pickQuantityValue(item, [
    "disponivel",
    "disponiveis",
    "disponível",
    "saldo_disponivel",
...
function summarizeCategories(items = []) {
  const grouped = new Map(CATEGORY_DEFINITIONS.map((category) => [
    category.id,
    {
      id: category.id,
      label: category.label,
...
function summarizeItems(items = []) {
  ...
  for (const item of items.map(normalizeStockItem)) {
    const current = grouped.get(item.key) || { ...item, quantidade: 0, raw: undefined };
...
function countRawPositiveItems(items = []) {
  return items.filter((item) => toNumber(pickDirectAvailableQuantity(item) ?? pickQuantityValue(item, [
    "disponivel",
    "disponiveis",
    "disponível",
    "saldo_disponivel",
...
function getPeriodRange(periodo = "mensal", dataInicio = "", dataFim = "") {
  ...
  if (customStart || customEnd) {
    const resolvedStart = customStart || new Date(now.getFullYear(), now.getMonth(), 1);
...
function addGroupedValue(map, key, base, item, quantityField = "consumidoDesdeAnterior") {
...
function addClientConsumption(row = {}, item = {}) {
  const clienteNome = cleanText(item.clienteNome || item.destino || "Cliente não informado");
  const clienteCodigo = cleanText(item.clienteCodigo || item.codigoCliente || "");
  ...
  current.movimentacoes = [...(current.movimentacoes || []), {
    id: item.id,
    data: item.data,
    nome: item.nome,
  ...
  row.clientes = clientes.sort((a, b) => (b.unidades + b.metros) - (a.unidades + a.metros));
}
...
function parseObservation(value) {
  ...
  return {
    ...
    movimentoEstoqueId: cleanText(text.match(/movimento_estoque_id:\s*([^|\s]+)/i)?.[1]),
  };
}
...
function buildPeriodDays(start, end) {
  ...
  while (cursor <= limit) {
    days.push(getBrazilDateKey(cursor));
...
function classifyMovement(note = {}, parsed = {}) {
  const movimento = normalizeText(note.tipo_operacao?.tipo_movimento || note.tipo_operacao?.descricao || parsed.operacao);
  ...
  if ((movimento.includes("comodato") || operacao.includes("comodato")) && origem.includes("tecnico") && destino.includes("cliente")) {
    return "consumo";
  }
  if (destino.includes("tecnico") && (origem.includes("estoque") || origem.includes("local de estoque"))) {
    return "entrada_bolsa";
  }
  if (movimento.includes("transferencia") || operacao.includes("usuario")) {
    return "transferencia";
  }
  if (movimento.includes("devolucao") || operacao.includes("retirada") || operacao.includes("retorno ao estoque")) {
    return tipo.includes("entrada") || destino.includes("estoque") ? "devolucao_estoque" : "devolucao";
  }
...
function normalizeMovementItem(note = {}, item = {}, parsed = {}, tecnico = null) {
  const productName = cleanText(item.produto?.descricao || item.produto_descricao || item.descricao || item.nome || "Item sem nome");
  const unit = cleanText(item.produto?.unidade_medida?.sigla || item.unidade_medida?.sigla || item.unidade || "");
  ...
  return {
    ...
    movimentoEstoqueId: cleanText(note.externo_id || parsed.movimentoEstoqueId).replace(/^movimento_estoque_id=/, ""),
    ...
    tecnicoNome: tecnico?.nome || cleanText(parsed.origem).replace(/^técnico:\s*/i, ""),
    tecnicoEmail: tecnico?.emailHubsoft || recebedorEmail,
    empresaId: tecnico?.empresaId || "",
    empresaNome: tecnico?.empresaNome || cleanText(note.empresa?.nome_razaosocial),
    regional: tecnico?.regional || "",
    usuarioRecebedorEmail: recebedorEmail,
    usuarioCadastroEmail: cadastroEmail,
    clienteNome: cleanText(parceiro.nome_razaosocial || parsed.destino).replace(/^cliente:\s*/i, ""),
...
async function fetchSempreNotesMovements({ start, end, tecnicoByEmail }) {
  ...
  while (page <= maxPages) {
    ...
    let payload = null;
    try {
      payload = await sempreIntegration.requestSempreRaw(`/nota?${params.toString()}`, {
        timeoutMs: SEMPRE_STOCK_TIMEOUT_MS,
      });
    } catch (error) {
    ...
    for (const note of notes) {
      ...
      if (Number.isNaN(noteDate.getTime()) || noteDate < start || noteDate > end) continue;
      const email = normalizeText(note.usuario_recebedor?.email || note.usuario_cadastro?.email);
      const tecnico = tecnicoByEmail.get(email) || null;
      if (!tecnico) continue;
      const parsed = parseObservation(note.observacao);
      for (const item of Array.isArray(note.itens) ? note.itens : []) {
        const movement = normalizeMovementItem(note, item, parsed, tecnico);
        if (movement.quantidade > 0) movements.push(movement);
      }
    }
  ...
  return movements.sort((a, b) => String(b.data || "").localeCompare(String(a.data || "")));
}

async function saveMovementCache(movements = [], { user = {}, start, end } = {}) {
  const cachedAt = nowIso();
  let saved = 0;
  for (const movement of movements) {
    const documentId = normalizeId(movement.id || `${movement.numero}-${movement.nome}-${movement.tecnicoEmail}-${movement.data}`);
    await documents.upsertDocument({
...
async function refreshMovementCache({ user = {}, start, end } = {}) {
  ...
  const movements = await fetchSempreNotesMovements({ start, end, tecnicoByEmail });
...
async function refreshReportMovements({ user = {}, query = {} } = {}) {
  ...
  const { start, end } = getPeriodRange(filters.periodo, filters.dataInicio, filters.dataFim);
...
async function runReportRefreshJob(jobId, user = {}, query = {}) {
  ...
  try {
    ...
    await updateRefreshJob(jobId, {
      ...
      percent: 100,
      error: error?.message || "Falha ao atualizar movimentações do Playground.",
      finishedAt: nowIso(),
    });
    throw error;
  }
...
async function startReportRefreshJob(user = {}, query = {}) {
  ...
  setImmediate(() => {
    runReportRefreshJob(job.id, user, query).catch((error) => {
      console.error(`[tecnicosBolsaAuditoria] Falha no job de relatório ${job.id}:`, error);
    });
  });
...
async function loadCachedMovements({ start, end, tecnicoByEmail }) {
  ...
  return docs
    ...
    .filter((item) => {
      const email = normalizeText(item.tecnicoEmail || item.usuarioRecebedorEmail || item.usuarioCadastroEmail);
      return !email || tecnicoByEmail.has(email);
    })
    .sort((a, b) => String(b.data || "").localeCompare(String(a.data || "")));
}

function summarizeMovementHistory(movements = [], { start, end, estimatedByDay = new Map() } = {}) {
  const consumptions = movements.filter((item) => item.tipoMovimentacao === "consumo");
  const byCategory = new Map();
  const byItem = new Map();
  const byDay = new Map(buildPeriodDays(start, end).map((date) => [
    date,
    {
      date,
      movimentacoes: 0,
  ...
  for (const item of consumptions) {
    const categoryRow = addGroupedValue(byCategory, item.categoria || "insumos", {
    ...
    addGroupedValue(byItem, item.nome || item.id, {
  ...
  const sortRows = (rows) => rows.sort((a, b) => (b.unidades + b.metros) - (a.unidades + a.metros));
  ...
  const dias = [...byDay.values()]
    .map((day) => ({
      ...day,
      bate: Math.abs(toNumber(day.unidades) - toNumber(day.estimadoUnidades)) < 0.001 &&
...
async function fetchTecnicoStock(tecnico) {
  ...
  const payload = await sempreIntegration.requestSempreRaw(`/v2/mesclar/estoque?tipo_vinculo=tecnico&vinculo_id=${encodeURIComponent(seniorId)}`, {
    timeoutMs: SEMPRE_STOCK_TIMEOUT_MS,
  });
  const stockItems = extractArray(payload, ["estoques", "estoque", "materiais", "produtos", "items", "data"]);
...
function buildAlerts(snapshot, config) {
  ...
  for (const threshold of normalizeCategoryThresholds(config.categoryThresholds)) {
    const category = categories.find((item) => item.id === threshold.id);
    const amount = threshold.tipo === "metragem" ? toNumber(category?.metros) : toNumber(category?.unidades);
    if (threshold.limite && amount > threshold.limite) {
      const suffix = threshold.tipo === "metragem" ? "m" : "un";
      alerts.push(`${threshold.label} acima do limite (${amount} ${suffix} > ${threshold.limite} ${suffix}).`);
    }
  }
  for (const threshold of config.itemThresholds || []) {
    const item = snapshot.items.find((current) => normalizeText(current.nome).includes(normalizeText(threshold.nome)));
    if (item && item.quantidade > threshold.quantidade) {
      alerts.push(`${item.nome} acima do limite (${item.quantidade} ${item.unidade} > ${threshold.quantidade}).`);
    }
  }
  return alerts;
}
...
async function sendAlertIfNeeded(snapshot, alerts, config) {
  ...
  if (!config.alertsEnabled) return { sent: false, reason: "Alertas desativados." };
  ...
  await emailService.sendMail({
    ...
    `,
    meta: { type: "tecnicos_bolsa_alerta", tecnicoId: snapshot.tecnicoId, empresaId: snapshot.empresaId },
  });
  return { sent: true, reason: "Enviado." };
}

async function refreshTecnico(tecnico, user = {}) {
  const previous = await documents.getDocument(`${SNAPSHOT_COLLECTION}/${tecnico.snapshotId}`).catch(() => null);
  const config = await readConfig({ sanitized: false });
  const updatedAt = nowIso();

  try {
    const result = await fetchTecnicoStock(tecnico);
    const itemsWithDiff = calculateDiff(previous?.data?.items || [], result.items);
    const snapshot = {
    ...
    const delivery = await sendAlertIfNeeded(snapshot, alerts, config).catch((error) => ({
      sent: false,
    ...
    await appendLog({
      ...
      items: itemsWithDiff.map(({ key, nome, unidade, tipo, categoria, categoriaLabel, quantidade, quantidadeAnterior, diferenca, consumidoDesdeAnterior, reposicionadoDesdeAnterior }) => ({
        key,
        nome,
        unidade,
        tipo,
...
async function getSnapshotMap() {
  ...
  return new Map(docs.map((doc) => [doc.documentId, { id: doc.documentId, ...(doc.data || {}) }]));
}
...
async function getDashboard(user) {
  ...
  const movements = await loadCachedMovements({ start, end, tecnicoByEmail }).catch(() => []);
  const movementsBySnapshotId = new Map();
  const estimatedBySnapshotId = new Map();
  for (const movement of movements) {
    const email = normalizeText(movement.tecnicoEmail || movement.usuarioRecebedorEmail || movement.usuarioCadastroEmail);
    const tecnico = tecnicoByEmail.get(email);
    if (!tecnico) continue;
    const current = movementsBySnapshotId.get(tecnico.snapshotId) || [];
    current.push(movement);
    movementsBySnapshotId.set(tecnico.snapshotId, current);
  }
  ...
  for (const log of logs) {
    const tecnico = tecnicos.find((item) => item.id === log.tecnicoId);
    if (!tecnico) continue;
    const date = getBrazilDateKey(log.createdAt);
    if (!date) continue;
    const byDay = estimatedBySnapshotId.get(tecnico.snapshotId) || new Map();
    ...
    byDay.set(date, day);
    estimatedBySnapshotId.set(tecnico.snapshotId, byDay);
  }
  const enriched = empresas.map((empresa) => ({
    ...empresa,
    tecnicos: empresa.tecnicos.map((tecnico) => {
      const snapshot = snapshots.get(tecnico.snapshotId) || null;
      const monthlyHistory = summarizeMovementHistory(movementsBySnapshotId.get(tecnico.snapshotId) || [], {
        start,
        end,
        estimatedByDay: estimatedBySnapshotId.get(tecnico.snapshotId) || new Map(),
      });
      return {
        ...tecnico,
    ...
    summary: enriched.reduce(
      (acc, empresa) => {
        ...
        for (const tecnico of empresa.tecnicos) {
          ...
          if (tecnico.snapshot?.alerts?.length) acc.alertas += tecnico.snapshot.alerts.length;
        }
        return acc;
      },
...
async function runRefreshAllJob(jobId, user = {}) {
  ...
  try {
    ...
    await updateRefreshJob(jobId, {
      total,
      processed: 0,
      stage: total ? "Consultando bolsas na API Sempre" : "Nenhum técnico encontrado",
      percent: total ? 5 : 100,
    });
    ...
    for (let index = 0; index < tecnicos.length; index += 1) {
      ...
      await updateRefreshJob(jobId, {
        ...
        percent: Math.min(95, Math.round((index / Math.max(total, 1)) * 90) + 5),
      });
      ...
      await updateRefreshJob(jobId, {
        ...
        percent: Math.min(98, Math.round(((index + 1) / Math.max(total, 1)) * 90) + 5),
      });
    }
    ...
    try {
      ...
      reportCache = { ok: false, error: error?.message || "Falha ao atualizar movimentações do Playground." };
      await appendLog({
        type: "movement_cache",
        status: "erro",
    ...
    const result = { ok: true, total: results.length, success, errors, reportCache, results };
    await updateRefreshJob(jobId, {
      status: "completed",
      stage: errors ? `Atualização finalizada com ${errors} erro(s)` : "Atualização finalizada",
      percent: 100,
      processed: total,
      success,
      errors,
...
async function refreshBySnapshotId(snapshotId, user = {}) {
  const empresas = await listAuthorizedTechnicians(user);
  const tecnico = empresas.flatMap((empresa) => empresa.tecnicos).find((item) => item.snapshotId === snapshotId || item.id === snapshotId);
  if (!tecnico) {
...
async function getReports({ user, query = {} } = {}) {
  ...
  const allowedTecnicos = empresas.flatMap((empresa) => empresa.tecnicos);
  const allowedEmpresaIds = new Set(empresas.map((empresa) => empresa.id));
  const allowedTecnicoIds = new Set(allowedTecnicos.map((tecnico) => tecnico.id));
  const tecnicoByEmail = new Map(
    allowedTecnicos
      .filter((tecnico) => tecnico.emailHubsoft)
      .map((tecnico) => [normalizeText(tecnico.emailHubsoft), tecnico]),
  ...
  const logs = (await documents.listAllDocuments(LOG_COLLECTION))
    .map((doc) => ({ id: doc.documentId, ...(doc.data || {}) }))
    .filter((log) => log.status === "ok")
    .filter((log) => allowedEmpresaIds.has(log.empresaId) || allowedTecnicoIds.has(log.tecnicoId))
    .filter((log) => !filters.regional || normalizeText(log.regional) === filters.regional)
  ...
  for (const log of logs) {
    for (const item of Array.isArray(log.items) ? log.items : []) {
      ...
      const category = getCategoryDefinition(item.categoria);
      addGroupedValue(byEmpresa, log.empresaId || "sem_empresa", {
        id: log.empresaId || "sem_empresa",
        nome: log.empresaNome || "Sem empresa",
        regional: log.regional || "",
      }, item);
      addGroupedValue(byTecnico, log.tecnicoId || "sem_tecnico", {
        id: log.tecnicoId || "sem_tecnico",
        nome: log.tecnicoNome || "Sem técnico",
        empresaNome: log.empresaNome || "",
        regional: log.regional || "",
      }, item);
      addGroupedValue(byCategoria, category.id, {
        id: category.id,
        nome: category.label,
      }, item);
      addGroupedValue(byItem, item.key || normalizeId(item.nome), {
  ...
  const movimentos = (await loadCachedMovements({ start, end, tecnicoByEmail }))
    .filter((item) => !filters.regional || normalizeText(item.regional) === filters.regional)
    .filter((item) => !filters.empresaId || item.empresaId === filters.empresaId)
    .filter((item) => !filters.tecnicoId || item.tecnicoId === filters.tecnicoId);
  const consumosConfirmados = movimentos.filter((item) => item.tipoMovimentacao === "consumo");
  for (const item of consumosConfirmados) {
    addGroupedValue(movimentosByEmpresa, item.empresaId || "sem_empresa", {
      id: item.empresaId || "sem_empresa",
      nome: item.empresaNome || "Sem empresa",
      regional: item.regional || "",
    }, item, "quantidade");
    addGroupedValue(movimentosByTecnico, item.tecnicoId || "sem_tecnico", {
      id: item.tecnicoId || "sem_tecnico",
      nome: item.tecnicoNome || "Sem técnico",
      empresaNome: item.empresaNome || "",
      regional: item.regional || "",
    }, item, "quantidade");
    addGroupedValue(movimentosByCategoria, item.categoria, {
      id: item.categoria,
      nome: item.categoriaLabel,
    }, item, "quantidade");
    addGroupedValue(movimentosByItem, item.nome || item.id, {
      id: item.nome || item.id,
      nome: item.nome || "Item",
      categoria: item.categoriaLabel,
      unidade: item.unidade || "",
  ...
  return {
    ...
    periodo: { inicio: start.toISOString(), fim: end.toISOString() },
    options: {
      regionais: [...new Set(empresas.map((empresa) => empresa.regional).filter(Boolean))].sort((a, b) => a.localeCompare(b, "pt-BR")),
      empresas: empresas.map((empresa) => ({ id: empresa.id, nome: empresa.nome, regional: empresa.regional })),
    ...
    summary: {
      ...
      unidades: itensMovimentados.reduce((acc, item) => acc + toNumber(item.unidades), 0),
      metros: itensMovimentados.reduce((acc, item) => acc + toNumber(item.metros), 0),
      movimentacoes: movimentos.length,
      consumosConfirmados: consumosConfirmados.length,
      unidadesMovimentadas: itensMovimentados.reduce((acc, item) => acc + toNumber(item.unidades), 0),
      metrosMovimentados: itensMovimentados.reduce((acc, item) => acc + toNumber(item.metros), 0),
      consumoEstimadoUnidades: itensEstimados.reduce((acc, item) => acc + toNumber(item.unidades), 0),
      consumoEstimadoMetros: itensEstimados.reduce((acc, item) => acc + toNumber(item.metros), 0),
    },
    porEmpresa,
    porTecnico,
    porCategoria,
...
async function sendReportByEmail({ to, pdfBase64, fileName, filters = {} } = {}, user = {}) {
...
async function runDailyIfDue(user = {}) {
  const config = await readConfig({ sanitized: false });
  if (!config.enabled) return { ok: true, skipped: true, reason: "Rotina desativada." };
  const formatter = new Intl.DateTimeFormat("en-CA", {
  ...
  if (config.lastDailyRunDate === today || currentTime < config.dailyRunTime) {
    return { ok: true, skipped: true, reason: "Fora da janela ou rotina já executada hoje." };
  }
  const result = await startRefreshAllJob(user);
  await saveConfig({ ...config, lastDailyRunDate: today }, user);
  await appendLog({ type: "daily_run", status: "ok", total: result.total, userName: getProfile(user).nome || "Rotina" });
  return result;
}
