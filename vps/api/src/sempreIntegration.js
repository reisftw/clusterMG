...
async function findSempreIntegration() {
	const direct = await documents.getDocument(
		`integracoes_api/${SEMPRE_INTEGRATION_ID}`,
	);
	...
	const found = integrations.find((item) => {
		...
		const haystack =
			`${data.name || ""} ${data.provider || ""} ${data.baseUrl || ""}`.toLowerCase();
		return (
			haystack.includes("sempre") ||
			haystack.includes("playground.sempre.net.br")
		);
	});
...
async function getSempreConfig() {
	...
	return {
		...
		baseUrl: String(
			integration?.baseUrl ||
				process.env.SEMPRE_API_BASE_URL ||
				DEFAULT_BASE_URL,
		).replace(/\/+$/, ""),
		token,
		...
		sistemaId: Number(
			integration?.sistemaId || process.env.SEMPRE_API_SISTEMA_ID || 6,
		),
		timeoutMs: Number(
			integration?.timeoutMs ||
				process.env.SEMPRE_API_TIMEOUT_MS ||
				DEFAULT_TIMEOUT_MS,
		),
	};
}
...
function requireSempreToken(config) {
	...
	if (!config.token) {
		const error = new Error(
			"Token da API Sempre nao configurado na Central de Integracoes.",
		);
		error.statusCode = 503;
		throw error;
	}
	...
	try {
		const response = await fetch(`${config.baseUrl}${path}`, {
			...
			headers: {
				...
				...(config.token
					? { authorization: normalizeBearer(config.token) }
					: {}),
		...
		if (!response.ok) {
			const error = new Error(
				data?.message || data?.error || `Erro Sempre HTTP ${response.status}.`,
			);
...
async function ensureSempreToken(config) {
	requireSempreToken({
		...config,
		token: config.token || config.refreshToken || config.loginPassword,
	});
	...
	const error = new Error(
		"Token da API Sempre expirado. Cadastre novo token ou e-mail/senha na Central de Integracoes.",
	);
...
async function requestSempre(path, options = {}) {
	...
	try {
		...
		return rawSempreRequest(
			{ ...config, token: nextToken },
			path,
			requestOptions(nextToken),
		);
	}
}
...
function classifyEquipmentLink({ order = {}, lookup = {} }) {
	...
	const linkedName =
		primary.vinculadoEm ||
		primary.origemStatus ||
		stockLocal.display ||
		stockLocal.descricao ||
		"";
	const linkedText = normalizeText(linkedName);
	const stockText = normalizeText(
		`${primary.estoqueLocal || ""} ${stockLocal.descricao || ""} ${stockLocal.prefixo || ""} ${stockLocal.display || ""}`,
	);
	const statusText = normalizeText(
		`${primary.status || ""} ${primary.vinculadoTipo || ""} ${primary.origemStatus || ""}`,
	);

	if (
		clientName &&
		linkedText &&
		(linkedText.includes(clientName) || clientName.includes(linkedText))
	) {
		return {
			status: "com_cliente",
			label: "Com cliente",
	...
	if (linkedName) {
		return {
			...
			responsibleName: linkedName,
			responsibleKind:
				hasEmailLikeValue(linkedName) ||
				!isSamePersonName(linkedName, order.nomeCliente)
					? "tecnico"
					: "cliente",
	...
	return {
		...
		responsibleKind: "",
		reason:
			"A API retornou equipamento, mas sem nome/local suficiente para confirmar.",
	};
}
...
function chooseWorstClassification(results = []) {
	...
	return (
		results.reduce((best, item) => {
			...
			return (priority[current.status] || 0) > (priority[best.status] || 0)
				? current
				: best;
		}, null) || {
			status: "indefinido",
...
async function consultEquipment(macRaw) {
	...
	if (!isValidEquipmentMac(mac)) {
		const error = new Error(
			"MAC invalido. Informe 12 caracteres hexadecimais.",
		);
	...
	const equipment = Array.isArray(data)
		? data.map(simplifyEquipmentResult)
		: [];
	...
	if (primary?.estoqueLocalId) {
		...
		const stockData = await requestSempre(
			`/estoque_local?${params.toString()}`,
		);
		stockLocal =
			Array.isArray(stockData?.data) && stockData.data[0]
...
function buildReturnedLogs({
	...
	for (const item of previousSnapshot.items || []) {
		...
		for (const mac of item.macs || []) {
			if (!isValidEquipmentMac(mac) || currentMacs.has(mac) || seen.has(mac))
				continue;
			seen.add(mac);
			logs.push({
				id: `devolucao_${mac}`,
...
async function listTreatmentMap() {
	const docs = await documents.listAllDocuments(
		MAP_EQUIPMENT_TREATMENT_COLLECTION,
	);
	const map = new Map();
...
async function buildAcertoIndex() {
	...
	for (const doc of docs) {
		...
		const createdAt = new Date(
			data.createdAt || data.dataAcerto || 0,
		).getTime();
...
function itemMatchesFilters(item = {}, filters = {}) {
	...
	if (status && status !== "todos" && item.classificationStatus !== status)
		return false;
	if (filters.cidade && !textIncludes(item.cidade, filters.cidade))
		return false;
	if (
		filters.tecnico &&
		!textIncludes(
			`${item.tecnico || ""} ${item.responsibleName || ""}`,
			filters.tecnico,
		)
	)
		return false;
	if (filters.estoque) {
		const haystack = (item.results || [])
			.map(
				(result) =>
					`${result.stockLocal?.descricao || ""} ${result.stockLocal?.display || ""} ${result.primary?.estoqueLocal || ""}`,
			)
			.join(" ");
	...
	if (filters.empresa) {
		const haystack = (item.results || [])
			.map(
				(result) =>
					`${result.stockLocal?.empresa?.nome || ""} ${result.primary?.sistema || ""}`,
			)
			.join(" ");
...
function buildFilterOptions(items = []) {
	...
	return {
		...
		estoques: collect((item) => {
			...
			return (
				result.stockLocal?.descricao ||
				result.stockLocal?.display ||
				result.primary?.estoqueLocal ||
				""
			);
		}),
...
function buildTecnicoGroups(items = []) {
	...
	for (const item of items.filter(
		(entry) =>
			entry.classificationStatus !== "com_cliente" &&
			entry.classificationStatus !== "nao_encontrado",
	)) {
		const key = item.tecnico || "Sem tecnico";
		if (!grouped.has(key))
			grouped.set(key, { tecnico: key, total: 0, items: [] });
...
function buildBagGroups(items = []) {
	...
	for (const item of items.filter(isRealResponsibleForBag)) {
		...
		holder.status[item.classificationStatus] =
			(holder.status[item.classificationStatus] || 0) + 1;
	...
	return {
		equipamentosPorResponsavel: [...holders.values()].sort(
			(a, b) => b.totalEquipamentos - a.totalEquipamentos,
		),
		bolsaPorTipo: [...holdersByType.values()]
			.map((group) => ({
				...group,
				responsaveis: [...group.responsaveis.values()].sort(
					(a, b) => b.totalEquipamentos - a.totalEquipamentos,
				),
			}))
...
async function enrichSnapshotForResponse(snapshot = {}) {
	...
	const items = (snapshot.items || []).map((item) => {
		...
		const acertosResponsavel =
			acertoIndex.get(normalizeText(item.responsibleName)) || [];
		const tratativa = treatmentMap.get(treatmentId) || {
			id: treatmentId,
			status:
				item.classificationStatus === "com_cliente" ? "resolvido" : "pendente",
			observacao: "",
			responsavel: "",
			updatedAt: null,
			updatedByName: "",
	...
	const itemByTreatmentId = new Map(
		items.map((item) => [item.treatmentId, item]),
	);
	const enrichItemList = (list = []) =>
		list.map(
			(item) =>
				itemByTreatmentId.get(item.treatmentId || buildTreatmentId(item)) ||
				item,
		);
	...
	const equipamentosPorResponsavel = (
		snapshot.equipamentosPorResponsavel || []
	).map((holder) => ({
		...holder,
	...
	const openDivergences = items.filter(
		(item) =>
			item.classificationStatus !== "com_cliente" &&
	...
	return {
		...
		alertas: {
			...
			naoEncontrados: openDivergences.filter(
				(item) => item.classificationStatus === "nao_encontrado",
			).length,
			outroVinculo: openDivergences.filter(
				(item) => item.classificationStatus === "outro_vinculo",
			).length,
			emEstoque: openDivergences.filter(
				(item) => item.classificationStatus === "em_estoque",
			).length,
...
async function buildMapEquipmentsSnapshot({
	trigger = "manual",
	forceFull = false,
} = {}) {
	...
	const previousSnapshotDoc = await documents.getDocument(
		MAP_EQUIPMENT_SNAPSHOT_PATH,
	);
	const previousSnapshot = previousSnapshotDoc?.data || {};
	const previousLookupByMac = forceFull
		? new Map()
		: buildPreviousLookupMap(previousSnapshot);
	const orders = await documents.listAllDocuments("ordens_abertas");
	const eligibleOrders = orders
		.map((item) => ({ item, data: item.data || {} }))
		.filter(({ data }) =>
			isRetiradaEquipmentType(
				pickFirst(data, ["tipo", "servico_tipo", "servico", "tipo_os"]),
			),
		);
	...
	const withMac = eligibleOrders
		.map(({ item, data }) => {
			...
			const order = {
				...
				numero:
					pickFirst(data, ["num_os", "numero_os", "os", "numero", "id_os"]) ||
					item.documentId,
				codigoCliente: pickFirst(data, [
					"codigo_cliente",
					"cod_cliente",
					"cliente_id",
					"contrato",
				]),
				nomeCliente: pickFirst(data, [
					"nome_cliente",
					"cliente",
					"assinante",
					"nome",
	...
	const macsToConsult = uniqueMacs.filter(
		(mac) => !previousLookupByMac.has(mac),
	);
	let processedMacs = uniqueMacs.length - macsToConsult.length;

	const updateProgress = async (stage, force = false) => {
		if (!force && processedMacs % 5 !== 0 && processedMacs < uniqueMacs.length)
			return;
	...
	for (const mac of uniqueMacs) {
		if (previousLookupByMac.has(mac))
			lookupByMac.set(mac, previousLookupByMac.get(mac));
	}
	...
	const items = withMac.map(({ order, macs }) => {
		...
		const responsibleIsClient = isSamePersonName(
			classification.responsibleName,
			order.nomeCliente,
		);
		return {
			...order,
			...
			responsibleKind: responsibleIsClient
				? "cliente"
				: classification.responsibleKind || "",
			responsibleIsClient,
		};
	});
	...
	const summary = items.reduce(
		(acc, item) => {
			acc.total += 1;
			acc[item.classificationStatus] =
				(acc[item.classificationStatus] || 0) + 1;
	...
	const returnedLogs = buildReturnedLogs({
		previousSnapshot,
		currentMacs: currentMacSet,
		generatedAt,
	});
	await saveReturnedLogs(returnedLogs);
	const previousLogs = Array.isArray(previousSnapshot.logsRecent)
		? previousSnapshot.logsRecent
		: [];
...
async function listMapEquipments({
	limit = 20,
	page = 1,
	refresh = false,
	filters = {},
	...
	if (refresh && !mapEquipmentRefreshPromise) {
		refreshMapEquipmentsSnapshot({ trigger: "manual-refresh" }).catch((error) =>
			console.error(
				"[sempreIntegration] Falha ao atualizar equipamentos do mapa:",
				error,
			),
		);
	}
	...
	if (!snapshotDoc?.data) {
		if (!mapEquipmentRefreshPromise) {
			refreshMapEquipmentsSnapshot({ trigger: "first-load" }).catch((error) =>
				console.error(
					"[sempreIntegration] Falha ao gerar primeiro snapshot de equipamentos:",
					error,
				),
			);
		}
	...
	const notLocatedItems = filteredItems.filter(
		(item) => item.classificationStatus === "nao_encontrado",
	);
	...
	const paginatedHolders = bagGroups.equipamentosPorResponsavel.map(
		(holder) => ({
			...holder,
			page: paginatedItems.page,
			limit: paginatedItems.limit,
	...
	return {
		...
		refreshing:
			Boolean(mapEquipmentRefreshPromise) || currentStatus?.running === true,
	};
}
...
async function saveEquipmentTreatment(payload = {}, user = {}) {
	...
	const data = {
		...
		observacao: String(payload.observacao || "")
			...
			.slice(0, 2000),
		responsavel: String(
			payload.responsavel || user?.profile?.nome || user?.nome || "",
		).trim(),
		item: payload.item || null,
		macs: Array.isArray(payload.macs)
			? payload.macs.map(normalizeMac).filter(isValidEquipmentMac)
			: [],
		updatedAt: now,
		updatedBy: user?.uid || "",
