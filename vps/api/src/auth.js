...
const REFRESH_TOKEN_TTL_SECONDS = Number(
	process.env.REFRESH_TOKEN_TTL_SECONDS || DEFAULT_REFRESH_TOKEN_TTL_SECONDS,
);
...
async function createSession({ uid, email, role, sessionVersion }) {
	...
	await db.query(
		`insert into app_sessions (jti, uid, expires_at, refresh_token_hash, refresh_expires_at)
     values ($1, $2, $3, $4, $5)`,
		[
			jti,
			uid,
	...
	await db.query(
      ...
      where (refresh_expires_at is not null and refresh_expires_at < now() - interval '7 days')
         or (refresh_expires_at is null and expires_at < now() - interval '7 days')
...
async function rotateRefreshSession(refreshToken) {
	const refreshTokenHash = hashOpaqueToken(refreshToken);
	const result = await db.query(
		`select s.jti, s.uid, au.email, au.role, au.session_version, au.disabled
       from app_sessions s
       join app_users au on au.uid = s.uid
	...
	if (!session || session.disabled) throw new Error("Refresh token invalido.");
	const user = await getLocalUserByUid(session.uid);
