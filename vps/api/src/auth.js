...
async function getGoogleOAuthConfig() {
  ...
  const clientId = String(stored.clientId || process.env.GOOGLE_OAUTH_CLIENT_ID || "").trim();
  return {
    enabled: parseBoolean(stored.enabled, parseBoolean(process.env.GOOGLE_OAUTH_ENABLED, false)),
    clientId,
    autoProvision: parseBoolean(stored.autoProvision, parseBoolean(process.env.GOOGLE_OAUTH_AUTO_PROVISION, false)),
    defaultRole: GOOGLE_OAUTH_DEFAULT_AUTO_ROLE,
    allowedDomains: String(stored.allowedDomains || process.env.GOOGLE_OAUTH_ALLOWED_DOMAINS || "")
      .split(",")
...
async function getOktaOAuthConfig() {
  ...
  const stored = result.rows[0]?.data || {};
  const issuer = normalizeIssuerUrl(stored.issuer || process.env.OKTA_OAUTH_ISSUER);
  const clientId = String(stored.clientId || process.env.OKTA_OAUTH_CLIENT_ID || "").trim();
  return {
    enabled: parseBoolean(stored.enabled, parseBoolean(process.env.OKTA_OAUTH_ENABLED, false)),
    issuer,
    clientId,
    redirectUri: String(stored.redirectUri || process.env.OKTA_OAUTH_REDIRECT_URI || "").trim(),
    autoProvision: parseBoolean(stored.autoProvision, parseBoolean(process.env.OKTA_OAUTH_AUTO_PROVISION, false)),
    defaultRole: OKTA_OAUTH_DEFAULT_AUTO_ROLE,
    allowedDomains: String(stored.allowedDomains || process.env.OKTA_OAUTH_ALLOWED_DOMAINS || "")
      .split(",")
      .map((item) => item.trim().toLowerCase())
      .filter(Boolean),
  };
...
async function syncProfileDocument(user, extra = {}) {
  ...
  const data = {
    ...
    criado_por_oauth: Boolean(extra.oauthAutoProvisioned ?? current?.criado_por_oauth),
...
async function loginWithPassword(email, password, { req = null } = {}) {
  const { user, profile } = await preparePasswordLogin(email, password, { req });
...
async function touchUserLogin(user, { req = null, loginProvider = "local" } = {}) {
...
async function preparePasswordLogin(email, password, { req = null } = {}) {
  ...
  const touchedUser = await touchUserLogin(user, { req, loginProvider: "local" });
...
async function startEmailMfaLogin(email, password, { ttlMinutes = 10, req = null } = {}) {
  const user = await verifyPasswordCredentials(email, password);
  const code = createEmailMfaCode();
  const challengeId = crypto.randomUUID();
  const expiresAt = new Date(Date.now() + Math.max(3, Math.min(30, Number(ttlMinutes || 10))) * 60 * 1000).toISOString();
...
async function verifyEmailMfaLogin(challengeId, code, { req = null } = {}) {
  ...
  const validCode = timingSafeEqualText(challenge.code_hash, hashOpaqueToken(String(code || "").replace(/\D/g, "")));
  if (!validCode) {
    await db.query(
  ...
  await db.query("update email_mfa_challenges set used_at = now() where id = $1", [challenge.id]);
...
async function fetchOktaJwks(issuer) {
  ...
  if (!response.ok) {
    throw new Error(`Não foi possível carregar as chaves públicas da Okta (${response.status}).`);
  }
...
async function verifyOktaIdToken(idToken, config, expectedNonce = "") {
  ...
  if (payload.aud !== config.clientId) throw new Error("Audience Okta inválido.");
  if (!payload.exp || Number(payload.exp) < now) throw new Error("Token Okta expirado.");
  if (payload.nbf && Number(payload.nbf) > now + 60) throw new Error("Token Okta ainda não é válido.");
  if (expectedNonce && payload.nonce !== expectedNonce) throw new Error("Nonce Okta inválido.");
...
async function loginWithOktaIdToken(idToken, { nonce = "", req = null } = {}) {
  ...
  const payload = await verifyOktaIdToken(String(idToken || ""), config, String(nonce || ""));
  ...
  if (!user && config.autoProvision && config.allowedDomains.length) {
    user = await createOktaProvisionedUser({
      email,
      nome: payload.name || [payload.given_name, payload.family_name].filter(Boolean).join(" ") || email,
      req,
    });
  }
