...
function createAgendamentoConfirmacaoAdminController({
	...
	async function getConfig(_req, res, next) {
		try {
			res.json({
				ok: true,
				config: await agendamentoConfirmacao.getConfig(),
				worker: await agendamentoConfirmacao.getStatus(),
			});
		} catch (error) {
			next(error);
		}
	...
	async function saveConfig(req, res, next) {
		try {
			res.json({
				ok: true,
				config: await agendamentoConfirmacao.saveConfig(req.body || {}),
			});
		} catch (error) {
			next(error);
		}
	...
	async function runOnce(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.processOnce({
					forceMorning: req.body?.forceMorning === true,
				}),
			);
		} catch (error) {
			next(error);
	...
	async function sendTestMessage(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.sendTestMessage(
					req.body || {},
					getProfile(req),
				),
			);
	...
	async function configureEvolutionWebhook(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.configureEvolutionWebhook(
					req.body?.webhookUrl || "",
				),
			);
		} catch (error) {
	...
	async function assignManualResponsible(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.assignManualResponsible(
					req.params.id,
					req.body || {},
					getProfile(req),
				),
	...
	async function listTracks(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.listTracks({
	...
	async function listLogs(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.listLogs({
					limit: req.query.limit,
					offset: req.query.offset,
				}),
			);
	...
	async function getPreview(req, res, next) {
		try {
			res.json(
				await agendamentoConfirmacao.getPreview({
					dateKey: req.query.dateKey,
					daysAhead: req.query.daysAhead,
				}),
			);
		} catch (error) {
			next(error);
