...
async function handleSchedule(payload = {}, user = {}) {
  ...
  const blockRecord = await documents.getDocument(`agendamento_esteira_blocos/${blockId}`);
  if (!blockRecord?.data) {
  ...
  const pending = updatedCustomers.filter((item) => item.status === "pendente").length;
  const scheduled = updatedCustomers.filter((item) => item.status === "agendado").length;
  ...
  const catalogRecord = await documents.getDocument("agendamento_esteira_catalogo/ativo");
  if (catalogRecord?.data) {
    const summaries = Array.isArray(catalogRecord.data.blocos)
      ? catalogRecord.data.blocos.map((item) =>
          item.id === blockId
            ? { ...item, pendentes: pending, agendados: scheduled, atualizado_em: now }
            : item,
        )
      : [];
    await documents.upsertDocument({
  ...
  await documents.upsertDocument({
    ...
    data: {
      ...
      agendamentos_por_usuario: {
        ...
        [userKey]: Number(metricData.agendamentos_por_usuario?.[userKey] || 0) + 1,
      },
      agendamentos_por_cidade: {
        ...(metricData.agendamentos_por_cidade || {}),
        [cityKey]: Number(metricData.agendamentos_por_cidade?.[cityKey] || 0) + 1,
      },
      usuarios: {
        ...(metricData.usuarios || {}),
        [userKey]: actorName,
