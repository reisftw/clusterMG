...
function userKey(user = {}) {
  return String(user.uid || user.id || user.email || "anonimo").trim() || "anonimo";
}

function userRole(user = {}) {
  return String(user.role || user.profile?.role || "").trim().toLowerCase();
}
...
function mapNotification(row, user) {
  const data = row?.data || {};
  const readBy = data.readBy && typeof data.readBy === "object" ? data.readBy : {};
...
async function createNotification({
  ...
  const id = dedupeKey ? String(dedupeKey).replace(/[^\w.-]/g, "_") : `${Date.now().toString(36)}-${crypto.randomUUID()}`;
  ...
  broadcastRealtime("notifications", { action: "created", id, type, targetPath, createdAt });
  return data;
}
...
function filterMappedNotifications(items, { user, type, severity, unread } = {}) {
...
async function listNotifications({ user, limit, offset, type, severity, unread } = {}) {
  ...
  const items = filterMappedNotifications(rawRows, { user, type, severity, unread }).slice(0, normalizeLimit(limit));
  const unreadRows = await listRawNotifications({ limit: 300, offset: 0 });
  const unreadCount = filterMappedNotifications(unreadRows, { user }).filter((item) => !item.read).length;
...
async function getPreferences({ user } = {}) {
  ...
  return {
    ...
    quietHours: data.quietHours || { enabled: false, start: "22:00", end: "07:00" },
  };
}
...
async function savePreferences({ user, preferences = {} } = {}) {
  ...
  await db.query(
    ...
    [`${PREFERENCES_COLLECTION}/${id}`, PREFERENCES_COLLECTION, id, JSON.stringify(data)],
  );
  return data;
}
...
async function createPendingDocumentNotificationForRow(row, user = {}) {
  ...
  if (role === "admin") {
    ...
    targets = { roles: ["supervisor"], regionais: row.regional ? [row.regional] : [] };
  } else if (role === "lider_empresa") {
    dedupeKey = `documentos_pendentes_empresa_${row.id}`;
    targets = { roles: ["lider_empresa"], empresas: row.empresa_id ? [row.empresa_id] : [] };
...
async function syncPendingDocumentNotifications({ user } = {}) {
  try {
    ...
    for (const row of rows) {
      const notification = await createPendingDocumentNotificationForRow(row, user || {});
      if (notification) created += 1;
    }
    return created;
  } catch (error) {
    console.error("[notifications] Falha ao sincronizar documentos pendentes:", error?.message || error);
    return 0;
  }
}
...
async function createCriticalServiceAlerts({ user } = {}) {
  ...
  for (const service of offlineServices) {
    ...
    const notification = await createNotification({
      ...
      message: service.reason || "Serviço indisponível. Verifique o painel de APIs.",
