...
async function recordRuntimeEvent(type, details = {}) {
  ...
  const id = `${createdAt.replace(/[^0-9]/g, "")}_${process.pid}_${type}`;
  try {
    await db.query(
    ...
    console.error("[apiStatus] Falha ao registrar evento de runtime:", error?.message || error);
  }
}
...
async function recordServiceEvent(service, previousStatus = "") {
  ...
  const id = `${createdAt.replace(/[^0-9]/g, "")}_${service.id}_${service.status}`;
  const data = {
    id,
  ...
  await db.query(
    ...
    [`${SERVICE_EVENTS_COLLECTION}/${id}`, SERVICE_EVENTS_COLLECTION, id, JSON.stringify(data)],
  );
  return data;
}
...
async function syncServiceEvents(services = []) {
  try {
    ...
    console.error("[apiStatus] Falha ao sincronizar eventos de serviço:", error?.message || error);
    return [];
  }
}
...
async function countTable(tableName) {
  const allowedTables = new Set(["app_users", "app_sessions", "app_documents", "static_snapshots"]);
  if (!allowedTables.has(tableName)) {
    throw new Error(`Tabela nao permitida para contagem: ${tableName}`);
  }
  const result = await db.query(`select count(*)::bigint as total from ${tableName}`);
  return Number(result.rows[0]?.total || 0);
}

async function getDocumentData(path) {
  const result = await db.query(`select data from app_documents where path = $1`, [path]);
...
async function checkEvolutionStatus() {
  ...
  return {
    ...
    state: payload?.instance?.state || payload?.state || payload?.connectionState || "desconhecido",
  };
}
...
async function getApiStatus() {
  ...
  return {
    ...
    runtime: {
      ...
      lastRestartAt: runtimeEvents.find((event) => event.type === "startup")?.createdAt || API_PROCESS_STARTED_AT.toISOString(),
      recentIssues: runtimeEvents.filter((event) =>
        ["uncaughtException", "unhandledRejection", "shutdown"].includes(event.type),
      ).slice(0, 10),
    },
    summary: {
      online,
