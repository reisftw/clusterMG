...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (item === "--password") {
      ...
      args.onlyEmail = String(argv[index + 1] || "").trim().toLowerCase();
      index += 1;
    }
  }
...
function normalizeRole(role) {
  return String(role || "tecnico").trim().toLowerCase();
}
...
async function main() {
  ...
  try {
    ...
    if (!args.resetExisting) {
      console.log("[seed-local-auth] Senhas existentes foram preservadas. Use --reset-existing para redefinir todos.");
    }
  } catch (error) {
