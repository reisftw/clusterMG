...
- Fluxo de trabalho local entre os apps do monorepo (padrão fixado em 18/09/2026):
  ...
  - Sempre trabalhar local primeiro (editar, testar, buildar) e só depois subir para a VPS via **deploy manual por SSH** (chave `~/.ssh/retiradas_github_actions_deploy`, usuário `root@operacao.retiradas.tech` para o ROT — confirmar host/usuário equivalente de cada app antes de deployar). Não depender do CI para publicar essas mudanças.
